...
custo = true, -- se os teleport ir�o custa (true or false)
...
if config.need_level == true and player:getLevel() < a.level then
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� precisa do level ".. a.level .." para ir at� essa hunt.")
...
if not a then
	player:popupFYI(
					"[+] As cidades que voc� pode ir s�o: promo, edoras, belfallas, ashenport, bree, riv \z
					dol, esg, zargond, moria, mordor, forod, condado, dunedain, anfallas, minas. \n\n\z
					
					As hunts que voc� pode ir s�o: \n\n\z
...
if config.custo == true and player:getMoney() < a.price then
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� precisa de ".. a.price .." money para ir at� essa hunt.")
	player:getPosition():sendMagicEffect(CONST_ME_POFF)
	return false
end
