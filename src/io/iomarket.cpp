...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action,
  ...
  query << "SELECT `id`, `amount`, `price`, `created`, `anonymous`, (SELECT "
           "`name` FROM `players` WHERE `id` = `player_id`) AS `player_name` "
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action,
                                       uint32_t playerId) {
  MarketOfferList offerList;
  ...
  std::ostringstream query;
  query << "SELECT `id`, `amount`, `price`, `created`, `itemtype` FROM "
           "`market_offers` WHERE `player_id` = "
...
HistoryMarketOfferList IOMarket::getOwnHistory(MarketAction_t action,
                                               uint32_t playerId) {
  HistoryMarketOfferList offerList;

  std::ostringstream query;
  query << "SELECT `itemtype`, `amount`, `price`, `expires_at`, `state` FROM "
           "`market_history` WHERE `player_id` = "
        << playerId << " AND `sale` = " << action;
...
void IOMarket::processExpiredOffers(DBResult_ptr result, bool) {
  ...
  do {
    if (!IOMarket::moveOfferToHistory(result->getNumber<uint32_t>("id"),
                                      OFFERSTATE_EXPIRED)) {
      continue;
    }
    ...
    if (result->getNumber<uint16_t>("sale") == 1) {
      const ItemType &itemType =
          Item::items[result->getNumber<uint16_t>("itemtype")];
      if (itemType.id == 0) {
        continue;
      }
      ...
      if (itemType.stackable) {
        ...
        while (tmpAmount > 0) {
          ...
          if (Item *item = Item::CreateItem(itemType.id, stackCount);
              g_game().internalAddItem(player->getInbox(), item,
                                       INDEX_WHEREEVER,
        ...
        for (uint16_t i = 0; i < amount; ++i) {
          ...
          if (g_game().internalAddItem(player->getInbox(), item,
                                       INDEX_WHEREEVER,
                                       FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
            delete item;
            break;
...
void IOMarket::checkExpiredOffers() {
  const time_t lastExpireDate =
      time(nullptr) - g_configManager().getNumber(MARKET_OFFER_DURATION);

  std::ostringstream query;
  query << "SELECT `id`, `amount`, `price`, `itemtype`, `player_id`, `sale` "
  ...
  int32_t checkExpiredMarketOffersEachMinutes =
      g_configManager().getNumber(CHECK_EXPIRED_MARKET_OFFERS_EACH_MINUTES);
  if (checkExpiredMarketOffersEachMinutes <= 0) {
    return;
  }

  g_scheduler().addEvent(
      createSchedulerTask(checkExpiredMarketOffersEachMinutes * 60 * 1000,
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp,
                                          uint16_t counter) {
  MarketOfferEx offer;

  const int32_t created =
      timestamp - g_configManager().getNumber(MARKET_OFFER_DURATION);

  std::ostringstream query;
  query
      << "SELECT `id`, `sale`, `itemtype`, `amount`, `created`, `price`, "
         "`player_id`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = "
         "`player_id`) AS `player_name` FROM `market_offers` WHERE `created` = "
...
void IOMarket::createOffer(uint32_t playerId, MarketAction_t action,
                           uint32_t itemId, uint16_t amount, uint32_t price,
                           bool anonymous) {
  std::ostringstream query;
  query << "INSERT INTO `market_offers` (`player_id`, `sale`, `itemtype`, "
           "`amount`, `price`, `created`, `anonymous`) VALUES ("
        << playerId << ',' << action << ',' << itemId << ',' << amount << ','
        << price << ',' << time(nullptr) << ',' << anonymous << ')';
...
void IOMarket::appendHistory(uint32_t playerId, MarketAction_t type,
                             uint16_t itemId, uint16_t amount, uint32_t price,
                             time_t timestamp, MarketOfferState_t state) {
  std::ostringstream query;
  query << "INSERT INTO `market_history` (`player_id`, `sale`, `itemtype`, "
           "`amount`, `price`, `expires_at`, `inserted`, `state`) VALUES ("
        << playerId << ',' << type << ',' << itemId << ',' << amount << ','
        << price << ',' << timestamp << ',' << time(nullptr) << ',' << state
        << ')';
...
bool IOMarket::moveOfferToHistory(uint32_t offerId, MarketOfferState_t state) {
  ...
  std::ostringstream query;
  query << "SELECT `player_id`, `sale`, `itemtype`, `amount`, `price`, "
           "`created` FROM `market_offers` WHERE `id` = "
        << offerId;
  ...
  appendHistory(
      ...
      result->getNumber<uint32_t>("price"), time(nullptr), state);
  return true;
}

void IOMarket::updateStatistics() {
  std::ostringstream query;
  query << "SELECT `sale` AS `sale`, `itemtype` AS `itemtype`, COUNT(`price`) "
           "AS `num`, MIN(`price`) AS `min`, MAX(`price`) AS `max`, "
           "SUM(`price`) AS `sum` FROM `market_history` WHERE `state` = "
