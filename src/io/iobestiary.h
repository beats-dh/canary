...
public:
  Charm() = default;
  Charm(std::string initname, charmRune_t initcharmRune_t,
        std::string initdescription, charm_t inittype, uint16_t initpoints,
        int32_t initbinary)
      : name(initname), id(initcharmRune_t), description(initdescription),
        type(inittype), points(initpoints), binary(initbinary) {}
...
public:
  ...
  bool parseCharmCombat(Charm *charm, Player *player, Creature *target,
                        int32_t realDamage);
  void addCharmPoints(Player *player, uint16_t amount, bool negative = false);
  void sendBuyCharmRune(Player *player, charmRune_t runeID, uint8_t action,
                        uint16_t raceid);
  ...
  std::map<uint16_t, uint32_t> getBestiaryKillCountByMonsterIDs(
  ...
  findRaceByName(const std::string &race, bool Onlystring = true,
