...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result) {
	...
	if (!player->setVocation(result->getNumber<uint16_t>("vocation"))) {
		SPDLOG_ERROR("Player {} has vocation id {} whitch doesn't exist", player->name, result->getNumber<uint16_t>("vocation"));
		return false;
	}
	...
	if (!town) {
		SPDLOG_ERROR("Player {} has town id {} whitch doesn't exist", player->name, result->getNumber<uint16_t>("town_id"));
		return false;
	}
