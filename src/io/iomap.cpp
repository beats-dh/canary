...
Tile *IOMap::createTile(Item *&ground, Item *item, uint16_t x, uint16_t y,
                        uint8_t z) {
  if (!ground) {
...
bool IOMap::loadMap(Map *map, const std::string &fileName) {
  ...
  if (headerVersion <= 0) {
    // In otbm version 1 the count variable after splashes/fluidcontainers and
    // stackables are saved as attributes instead, this solves alot of problems
    // with items that is changed (stackable/charges/fluidcontainer/splash)
    // during an update.
    setLastErrorString("This map need to be upgraded by using the latest map "
                       "editor version to be able to load correctly.");
    return false;
  }
...
bool IOMap::parseMapDataAttributes(OTB::Loader &loader,
                                   const OTB::Node &mapNode, Map &map,
  ...
  while (propStream.read<uint8_t>(attribute)) {
    ...
    case OTBM_ATTR_EXT_SPAWN_MONSTER_FILE:
      if (!propStream.readString(tmp)) {
        setLastErrorString("Invalid monster spawn tag. Make sure you are using "
                           "the correct version of the map editor.");
        return false;
      }
    ...
    case OTBM_ATTR_EXT_SPAWN_NPC_FILE:
      if (!propStream.readString(tmp)) {
        setLastErrorString("Invalid npc spawn tag. Make sure you are using the "
                           "correct version of the map editor.");
        return false;
      }
...
bool IOMap::parseTileArea(OTB::Loader &loader, const OTB::Node &tileAreaNode,
                          Map &map) {
  PropStream propStream;
  ...
  for (auto &tileNode : tileAreaNode.children) {
    ...
    while (propStream.read<uint8_t>(attribute)) {
      ...
      case OTBM_ATTR_ITEM: {
        ...
        if (Teleport *teleport = item->getTeleport()) {
          ...
          uint64_t teleportPosition =
              (static_cast<uint64_t>(x) << 24) | (y << 8) | z;
          uint64_t destinationPosition =
              (static_cast<uint64_t>(destPos.x) << 24) | (destPos.y << 8) |
              destPos.z;
    ...
    for (auto &itemNode : tileNode.children) {
      ...
      if (!item->unserializeItemNode(loader, itemNode, stream)) {
        std::ostringstream ss;
        ss << "[x:" << x << ", y:" << y << ", z:" << z
           << "] Failed to load item " << item->getID() << '.';
        setLastErrorString(ss.str());
        delete item;
        return false;
      }
...
bool IOMap::parseTowns(OTB::Loader &loader, const OTB::Node &townsNode,
                       Map &map) {
...
bool IOMap::parseWaypoints(OTB::Loader &loader, const OTB::Node &waypointsNode,
                           Map &map) {
  PropStream propStream;
  for (auto &node : waypointsNode.children) {
    ...
    map.waypoints[name] =
        Position(waypoint_coords.x, waypoint_coords.y, waypoint_coords.z);
  }
  return true;
}
