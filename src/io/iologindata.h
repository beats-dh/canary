...
public:
  ...
  static void setAccountType(uint32_t accountId,
  ...
  static bool getGuidByNameEx(uint32_t &guid, bool &specialVip,
                              std::string &name);
  ...
  static void addVIPEntry(uint32_t accountId, uint32_t guid,
                          ...
                          bool notify);
  static void editVIPEntry(uint32_t accountId, uint32_t guid,
                           const std::string &description, uint32_t icon,
                           bool notify);
...
private:
  ...
  static bool saveItems(const Player *player, const ItemBlockList &itemList,
                        DBInsert &query_insert, PropWriteStream &stream);
};
