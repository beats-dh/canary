...
bool IOBestiary::parseCharmCombat(Charm *charm, Player *player,
                                  Creature *target, int32_t realDamage) {
...
Charm *IOBestiary::getBestiaryCharm(charmRune_t activeCharm,
                                    bool force /*= false*/) {
...
std::map<uint16_t, std::string> IOBestiary::findRaceByName(
    const std::string &race, bool Onlystring /*= true*/,
...
uint8_t IOBestiary::getKillStatus(MonsterType *mtype,
                                  uint32_t killAmount) const {
...
void IOBestiary::setCharmRuneCreature(Player *player, Charm *charm,
                                      uint16_t raceid) {
  if (!player || !charm) {
    return;
  }
...
uint16_t IOBestiary::getBestiaryRaceUnlocked(Player *player,
                                             BestiaryType_t race) const {
  if (!player) {
    return 0;
  }
  ...
  for (auto it : besty_l) {
    ...
    if (mtype && mtype->info.bestiaryRace == race &&
        player->getBestiaryKillCount(mtype->info.raceid) > 0) {
      count++;
    }
  }
  return count;
...
void IOBestiary::addCharmPoints(Player *player, uint16_t amount,
                                bool negative /*= false*/) {
  if (!player) {
    return;
  }
...
void IOBestiary::addBestiaryKill(Player *player, MonsterType *mtype,
  ...
  if ((curCount == 0) || // Initial kill stage
      (curCount < mtype->info.bestiaryFirstUnlock &&
       (curCount + amount) >=
           mtype->info.bestiaryFirstUnlock) || // First kill stage reached
      (curCount < mtype->info.bestiarySecondUnlock &&
       (curCount + amount) >=
           mtype->info.bestiarySecondUnlock) || // Second kill stage reached
      (curCount < mtype->info.bestiaryToUnlock &&
       (curCount + amount) >=
           mtype->info.bestiaryToUnlock)) { // Final kill stage reached
...
void IOBestiary::sendBuyCharmRune(Player *player, charmRune_t runeID,
                                  uint8_t action, uint16_t raceid) {
  ...
  if (action == 0) {
    ...
    ss << "You successfully unlocked '" << charm->name << "' for "
    ...
    player->sendFYIBox("Creature has been set! You are Premium player, so you "
                       "benefit from up to 6 runes! Charm Expansion allow you "
                       "to set creatures to all runes at once!");
  } else if (action == 2) {
...
std::map<uint8_t, int16_t>
IOBestiary::getMonsterElements(MonsterType *mtype) const {
...
std::map<uint16_t, uint32_t> IOBestiary::getBestiaryKillCountByMonsterIDs(
    Player *player, std::map<uint16_t, std::string> mtype_list) const {
