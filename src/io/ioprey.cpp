...
// Prey class
PreySlot::PreySlot(PreySlot_t id) : id(id) {
  eraseBonus();
  reloadBonusValue();
  reloadBonusType();
  freeRerollTimeStamp =
      OTSYS_TIME() + g_configManager().getNumber(PREY_FREE_REROLL_TIME) * 1000;
}
...
void PreySlot::reloadBonusType() {
  if (bonusRarity == 10) {
    ...
    while (bonus_tmp == bonus) {
      bonus = static_cast<PreyBonus_t>(
          uniform_random(PreyBonus_First, PreyBonus_Last));
    }
    return;
  }

  bonus =
      static_cast<PreyBonus_t>(uniform_random(PreyBonus_First, PreyBonus_Last));
}

void PreySlot::reloadBonusValue() {
  auto minBonusPercent = static_cast<uint16_t>(
      g_configManager().getNumber(PREY_BONUS_PERCENT_MIN));
  auto maxBonusPercent = static_cast<uint16_t>(
      g_configManager().getNumber(PREY_BONUS_PERCENT_MAX));
  auto stagePercent = static_cast<uint16_t>(
      std::floor((maxBonusPercent - minBonusPercent) / 8));
  if (bonusRarity >= 9) {
    bonusRarity = 10;
  } else {
...
void PreySlot::reloadMonsterGrid(std::vector<uint16_t> blackList,
                                 uint32_t level) {
  raceIdList.clear();
  ...
  // Disabling prey system if the server have less then 36 registered monsters
  // on bestiary because:
  ...
  while (raceIdList.size() < 9) {
    uint16_t raceId =
        (*(std::next(bestiary.begin(), uniform_random(0, maxIndex)))).first;
    tries++;
...
// Task hunting class
TaskHuntingSlot::TaskHuntingSlot(PreySlot_t id) : id(id) {
  freeRerollTimeStamp =
      OTSYS_TIME() +
      g_configManager().getNumber(TASK_HUNTING_FREE_REROLL_TIME) * 1000;
}

void TaskHuntingSlot::reloadMonsterGrid(std::vector<uint16_t> blackList,
                                        uint32_t level) {
  raceIdList.clear();
  ...
  // Disabling task hunting system if the server have less then 36 registered
...
void IOPrey::CheckPlayerPreys(Player *player, uint8_t amount) const {
  ...
  for (uint8_t slotId = PreySlot_First; slotId <= PreySlot_Last; slotId++) {
    if (PreySlot *slot =
            player->getPreySlotById(static_cast<PreySlot_t>(slotId));
        slot && slot->isOccupied()) {
      if (slot->bonusTimeLeft <= amount) {
        if (slot->option == PreyOption_AutomaticReroll) {
          if (player->usePreyCards(static_cast<uint16_t>(
                  g_configManager().getNumber(PREY_BONUS_REROLL_PRICE)))) {
            slot->reloadBonusValue();
            slot->reloadBonusType();
            slot->bonusTimeLeft = static_cast<uint16_t>(
                g_configManager().getNumber(PREY_BONUS_TIME));
            player->sendTextMessage(
                MESSAGE_STATUS,
                "Your prey bonus type and time has been succesfully reseted.");
            player->reloadPreySlot(static_cast<PreySlot_t>(slotId));
            continue;
          }

          player->sendTextMessage(MESSAGE_STATUS,
                                  "You don't have enought prey cards to enable "
                                  "automatic reroll when your slot expire.");
          ...
          if (player->usePreyCards(static_cast<uint16_t>(
            ...
            player->sendTextMessage(
                MESSAGE_STATUS,
                "Your prey bonus time has been succesfully reseted.");
            player->reloadPreySlot(static_cast<PreySlot_t>(slotId));
            continue;
          }

          player->sendTextMessage(MESSAGE_STATUS,
                                  "You don't have enought prey cards to lock "
                                  "monster and bonus when the slot expire.");
        } else {
          player->sendTextMessage(MESSAGE_STATUS,
                                  "Your prey bonus has expired.");
        }
...
void IOPrey::ParsePreyAction(Player *player, PreySlot_t slotId,
                             PreyAction_t action, PreyOption_t option,
                             int8_t index, uint16_t raceId) const {
  PreySlot *slot = player->getPreySlotById(slotId);
  if (!slot || slot->state == PreyDataState_Locked) {
    player->sendMessageDialog(
        "To unlock this prey slot first you must buy it on store.");
    return;
  }

  if (action == PreyAction_ListReroll) {
    if (slot->freeRerollTimeStamp > OTSYS_TIME() &&
        !g_game().removeMoney(player, player->getPreyRerollPrice(), 0, true)) {
      player->sendMessageDialog(
          "You don't have enought money to reroll the prey slot.");
      return;
    } else if (slot->freeRerollTimeStamp <= OTSYS_TIME()) {
      slot->freeRerollTimeStamp =
          OTSYS_TIME() +
          g_configManager().getNumber(PREY_FREE_REROLL_TIME) * 1000;
    }
    ...
    if (!player->usePreyCards(static_cast<uint16_t>(
            g_configManager().getNumber(PREY_SELECTION_LIST_PRICE)))) {
      player->sendMessageDialog("You don't have enought prey cards to choose a "
    ...
    if (slot->isOccupied()) {
      player->sendMessageDialog(
          "You already have an active monster on this prey slot.");
      return;
    } else if (!slot->canSelect() ||
               slot->state != PreyDataState_ListSelection) {
      player->sendMessageDialog(
          "There was an error while processing your action. Please try "
          "reopening the prey window.");
      return;
    } else if (player->getPreyWithMonster(raceId)) {
      player->sendMessageDialog(
          "This creature is already selected on another slot.");
      return;
    }
    ...
    slot->bonusTimeLeft =
        static_cast<uint16_t>(g_configManager().getNumber(PREY_BONUS_TIME));
  } else if (action == PreyAction_BonusReroll) {
    if (!slot->isOccupied()) {
      player->sendMessageDialog(
          "You don't have any active monster on this prey slot.");
      return;
    } else if (!player->usePreyCards(static_cast<uint16_t>(
                   g_configManager().getNumber(PREY_BONUS_REROLL_PRICE)))) {
      player->sendMessageDialog("You don't have enought prey cards to reroll "
                                "this prey slot bonus type.");
      return;
    }
    ...
    if (slot->isOccupied()) {
      player->sendMessageDialog(
          "You already have an active monster on this prey slot.");
      return;
    } else if (!slot->canSelect() || index == -1 ||
               (index + 1) > slot->raceIdList.size()) {
      player->sendMessageDialog(
          "There was an error while processing your action. Please try "
          "reopening the prey window.");
      return;
    ...
    if (option == PreyOption_AutomaticReroll &&
        player->getPreyCards() <
            static_cast<uint64_t>(
                g_configManager().getNumber(PREY_BONUS_REROLL_PRICE))) {
      player->sendMessageDialog("You don't have enought prey cards to enable "
                                "automatic reroll when your slot expire.");
      return;
    } else if (option == PreyOption_Locked &&
               player->getPreyCards() <
                   static_cast<uint64_t>(g_configManager().getNumber(
                       PREY_SELECTION_LIST_PRICE))) {
      player->sendMessageDialog("You don't have enought prey cards to lock "
                                "monster and bonus when the slot expire.");
      return;
    }
...
void IOPrey::ParseTaskHuntingAction(Player *player, PreySlot_t slotId,
                                    PreyTaskAction_t action, bool upgrade,
  ...
  if (!slot || slot->state == PreyTaskDataState_Locked) {
    player->sendMessageDialog(
        "To unlock this task hunting slot first you must buy it on store.");
    return;
  }
  ...
  if (action == PreyTaskAction_ListReroll) {
    if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
      ...
      ss << "You need to wait "
         << ((slot->disabledUntilTimeStamp - OTSYS_TIME()) / 60000)
         << " minutes to select a new creature on task.";
      player->sendMessageDialog(ss.str());
      return;
    } else if (slot->freeRerollTimeStamp > OTSYS_TIME() &&
               !g_game().removeMoney(
                   player, player->getTaskHuntingRerollPrice(), 0, true)) {
      player->sendMessageDialog(
          "You don't have enought money to reroll the task hunting slot.");
      return;
    } else if (slot->freeRerollTimeStamp <= OTSYS_TIME()) {
      slot->freeRerollTimeStamp =
          OTSYS_TIME() +
    ...
    if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
      ...
      player->sendMessageDialog(ss.str());
      return;
    } else if (!player->usePreyCards(
                   static_cast<uint16_t>(g_configManager().getNumber(
                       TASK_HUNTING_SELECTION_LIST_PRICE)))) {
      player->sendMessageDialog("You don't have enought prey cards to choose a "
                                "creature on list for you task hunting slot.");
      return;
    }
    ...
    if (slot->disabledUntilTimeStamp >= OTSYS_TIME()) {
      ...
      player->sendMessageDialog(
          "There was an error while processing your action. Please try "
          "reopening the task window.");
      return;
    } else if (slot->isOccupied()) {
      player->sendMessageDialog(
          "You already have an active monster on this task hunting slot.");
      return;
    } else if (slot->state == PreyTaskDataState_Selection &&
               !slot->isCreatureOnList(raceId)) {
      player->sendMessageDialog(
          "There was an error while processing your action. Please try "
          "reopening the task window.");
      return;
    ...
    if (!g_game().removeMoney(player, player->getTaskHuntingRerollPrice(), 0,
                              true)) {
      player->sendMessageDialog(
          "You don't have enought money to cancel your current task hunting.");
      return;
    }
    ...
    if (!slot->isOccupied()) {
      player->sendMessageDialog(
          "You cannot claim your task reward with an empty task hunting slot.");
      return;
    }
    ...
    if (const TaskHuntingOption *option = GetTaskRewardOption(slot)) {
      ...
      if (slot->upgrade && slot->currentKills >= option->secondKills) {
        ...
        player->sendMessageDialog(
            "There was an error while processing you task hunting reward. "
            "Please try reopening the window.");
        return;
      }
      ...
      slot->disabledUntilTimeStamp =
          OTSYS_TIME() +
          g_configManager().getNumber(TASK_HUNTING_LIMIT_EXHAUST) * 1000;
    }
  } else {
    SPDLOG_WARN("[IOPrey::ParseTaskHuntingAction] - Unknown task action: {}",
                action);
    return;
  }
...
void IOPrey::InitializeTaskHuntOptions() {
  ...
  uint8_t killStage =
      25; // Kill stage is the multiplier for kills and rewards on task hunting.
  ...
  for (uint8_t difficulty = PreyTaskDifficult_First;
       difficulty <= PreyTaskDifficult_Last;
       ++difficulty) { // Difficulties of creatures on bestiary.
    auto reward = static_cast<uint16_t>(std::round((10 * kills) / killStage));
    for (uint8_t star = 1; star <= limitOfStars;
         ++star) { // Amount of task stars on task hunting.
      ...
      reward = static_cast<uint16_t>(
          std::round((reward * (115 + (difficulty * limitOfStars))) / 100));
    }
  ...
  std::for_each(taskOption.begin(), taskOption.end(),
                [&msg](const TaskHuntingOption *option) {
...
TaskHuntingOption *
IOPrey::GetTaskRewardOption(const TaskHuntingSlot *slot) const {
  if (!slot) {
    return nullptr;
  }
  ...
  if (auto it =
          std::find_if(taskOption.begin(), taskOption.end(),
                       [difficult, slot](const TaskHuntingOption *optionIt) {
                         return optionIt->difficult == difficult &&
                                optionIt->rarity == slot->rarity;
                       });
      it != taskOption.end()) {
    return *it;
  }
