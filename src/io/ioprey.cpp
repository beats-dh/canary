...
void PreySlot::reloadMonsterGrid(std::vector<uint16_t> blackList, uint32_t level) {
	...
	while (raceIdList.size() < 9) {
		...
		const std::shared_ptr<MonsterType> mtype = g_monsters().getMonsterTypeByRaceId(raceId);
...
void IOPrey::InitializeTaskHuntOptions() {
	...
	std::for_each(bestiaryList.begin(), bestiaryList.end(), [&msg](auto &mType) {
		const std::shared_ptr<MonsterType> mtype = g_monsters().getMonsterType(mType.second);
		if (!mtype) {
			return;
		}
...
TaskHuntingOption* IOPrey::GetTaskRewardOption(const TaskHuntingSlot* slot) const {
	...
	const std::shared_ptr<MonsterType> mtype = g_monsters().getMonsterTypeByRaceId(slot->selectedRaceId);
	if (!mtype) {
		return nullptr;
	}
