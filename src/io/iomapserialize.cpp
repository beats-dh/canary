...
void IOMapSerialize::loadHouseItems(Map *map) {
  ...
  DBResult_ptr result =
      Database::getInstance().storeQuery("SELECT `data` FROM `tile_store`");
  if (!result) {
    return;
  }
  ...
  SPDLOG_INFO("Loaded house items in {} seconds",
...
bool IOMapSerialize::saveHouseItems() {
  ...
  SPDLOG_INFO("Saved house items in {} seconds",
              (OTSYS_TIME() - start) / (1000.));
  return success;
}

bool IOMapSerialize::loadContainer(PropStream &propStream,
                                   Container *container) {
  while (container->serializationCount > 0) {
    if (!loadItem(propStream, container)) {
      SPDLOG_WARN("Deserialization error for container item: {}",
                  container->getID());
      return false;
    }
...
void IOMapSerialize::saveItem(PropWriteStream &stream, const Item *item) {
  ...
  if (container) {
    ...
    for (auto it = container->getReversedItems(),
              end = container->getReversedEnd();
         it != end; ++it) {
      saveItem(stream, *it);
    }
  }
...
void IOMapSerialize::saveTile(PropWriteStream &stream, const Tile *tile) {
  ...
  for (Item *item : *tileItems) {
    ...
    if (!(it.moveable || it.isCarpet() || item->getDoor() ||
          (item->getContainer() && !item->getContainer()->empty()) ||
          it.canWriteText || item->getBed())) {
      continue;
    }
...
bool IOMapSerialize::loadHouseInfo() {
  ...
  DBResult_ptr result =
      db.storeQuery("SELECT `id`, `owner`, `paid`, `warnings` FROM `houses`");
  if (!result) {
    return false;
  }
  ...
  do {
    House *house =
        g_game().map.houses.getHouse(result->getNumber<uint32_t>("id"));
    if (house) {
  ...
  result =
      db.storeQuery("SELECT `house_id`, `listid`, `list` FROM `house_lists`");
  if (result) {
    do {
      House *house =
          g_game().map.houses.getHouse(result->getNumber<uint32_t>("house_id"));
      if (house) {
        house->setAccessList(result->getNumber<uint32_t>("listid"),
...
bool IOMapSerialize::saveHouseInfo() {
  ...
  for (const auto &[key, house] : g_game().map.houses.getHouses()) {
    ...
    if (result) {
      ...
      query << "UPDATE `houses` SET `owner` = " << house->getOwner()
            ...
            << ", `town_id` = " << house->getTownId()
            << ", `rent` = " << house->getRent()
            << ", `size` = " << house->getTiles().size()
            << ", `beds` = " << house->getBedCount()
            << " WHERE `id` = " << house->getId();
    } else {
      query.str(std::string());
      query << "INSERT INTO `houses` (`id`, `owner`, `paid`, `warnings`, "
               "`name`, `town_id`, `rent`, `size`, `beds`) VALUES ("
  ...
  for (const auto &[key, house] : g_game().map.houses.getHouses()) {
    ...
    if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
      query << house->getId() << ',' << SUBOWNER_LIST << ','
            << db.escapeString(listText);
      if (!stmt.addRow(query)) {
        return false;
      }
    ...
    for (Door *door : house->getDoors()) {
      if (door->getAccessList(listText) && !listText.empty()) {
        query << house->getId() << ',' << door->getDoorId() << ','
              << db.escapeString(listText);
        if (!stmt.addRow(query)) {
          return false;
        }
