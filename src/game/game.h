...
public:
  ...
  /**
   * Load the main map
   * \param filename Is the map custom name (Example: "map".otbm, not is
   * necessary add extension .otbm) \returns true if the custom map was loaded
   * successfully
   */
  ...
  bool placeCreature(Creature *creature, const Position &pos,
                     bool extendedPos = false, bool force = false);
  ...
  uint16_t getItemsPriceCount() const { return itemsSaleCount; }
  ...
  ItemClassification *getItemsClassification(uint8_t id, bool create) {
    auto it =
        std::find_if(itemsClassifications.begin(), itemsClassifications.end(),
                     [id](ItemClassification *it) { return it->id == id; });
  ...
  ReturnValue internalMoveCreature(Creature *creature, Direction direction,
                                   uint32_t flags = 0);
  ReturnValue internalMoveCreature(Creature &creature, Tile &toTile,
                                   uint32_t flags = 0);
  ...
  ReturnValue
  internalQuickLootItem(Player *player, Item *item,
                        ObjectCategory_t category = OBJECTCATEGORY_DEFAULT);

  ObjectCategory_t getObjectCategory(const Item *item);

  uint64_t getItemMarketPrice(std::map<uint16_t, uint32_t> const &itemMap,
                              bool buyPrice) const;
  ...
  void playerPreyAction(uint32_t playerId, uint8_t slot, uint8_t action,
                        uint8_t option, int8_t index, uint16_t raceId);
  void playerTaskHuntingAction(uint32_t playerId, uint8_t slot, uint8_t action,
                               bool upgrade, uint16_t raceId);
  ...
  void
  playerCyclopediaCharacterInfo(Player *player, uint32_t characterID,
                                CyclopediaCharacterInfoType_t characterInfoType,
                                uint16_t entriesPerPage, uint16_t page);

  void playerHighscores(Player *player, HighscoreType_t type, uint8_t category,
                        uint32_t vocation, const std::string &worldName,
                        uint16_t page, uint8_t entriesPerPage);
  ...
  void playerMoveThing(uint32_t playerId, const Position &fromPos,
  ...
  void playerMoveCreature(Player *playerId, Creature *movingCreature,
                          const Position &movingCreatureOrigPos, Tile *toTile);
  void playerMoveItemByPlayerID(uint32_t playerId, const Position &fromPos,
                                uint16_t itemId, uint8_t fromStackPos,
                                const Position &toPos, uint8_t count);
  void playerMoveItem(Player *player, const Position &fromPos, uint16_t itemId,
                      uint8_t fromStackPos, const Position &toPos,
                      uint8_t count, Item *item, Cylinder *toCylinder);
  ...
  void playerStowItem(uint32_t playerId, const Position &pos, uint16_t itemId,
                      uint8_t stackpos, uint8_t count, bool allItems);
  void playerStashWithdraw(uint32_t playerId, uint16_t itemId, uint32_t count,
                           uint8_t stackpos);
  void playerCloseNpcChannel(uint32_t playerId);
  void playerReceivePing(uint32_t playerId);
  void playerReceivePingBack(uint32_t playerId);
  void playerAutoWalk(uint32_t playerId,
                      const std::forward_list<Direction> &listDir);
  void playerStopAutoWalk(uint32_t playerId);
  void playerUseItemEx(uint32_t playerId, const Position &fromPos,
  ...
  void playerRotateItem(uint32_t playerId, const Position &pos,
                        uint8_t stackPos, const uint16_t itemId);
  void playerConfigureShowOffSocket(uint32_t playerId, const Position &pos,
                                    uint8_t stackPos, const uint16_t itemId);
  void playerSetShowOffSocket(uint32_t playerId, Outfit_t &outfit,
                              const Position &pos, uint8_t stackPos,
                              const uint16_t itemId, uint8_t podiumVisible,
                              uint8_t direction);
  void playerWrapableItem(uint32_t playerId, const Position &pos,
                          uint8_t stackPos, const uint16_t itemId);
  void playerWriteItem(uint32_t playerId, uint32_t windowTextId,
                       const std::string &text);
  void playerBrowseField(uint32_t playerId, const Position &pos);
  void playerSeekInContainer(uint32_t playerId, uint8_t containerId,
                             uint16_t index);
  void playerUpdateHouseWindow(uint32_t playerId, uint8_t listId,
  ...
  void playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer,
                         uint8_t index);
  ...
  void playerSetFightModes(uint32_t playerId, FightMode_t fightMode,
                           bool chaseMode, bool secureMode);
  void playerLookAt(uint32_t playerId, uint16_t itemId, const Position &pos,
                    uint8_t stackPos);
  void playerLookInBattleList(uint32_t playerId, uint32_t creatureId);
  void playerQuickLoot(uint32_t playerId, const Position &pos, uint16_t itemId,
                       uint8_t stackPos, Item *defaultItem = nullptr,
                       bool lootAllCorpses = false, bool autoLoot = false);
  void playerLootAllCorpses(Player *player, const Position &pos,
                            bool lootAllCorpses);
  ...
  void playerRequestDepotSearchItem(uint32_t playerId, uint16_t itemId,
                                    uint8_t tier);
  void playerRequestDepotSearchRetrieve(uint32_t playerId, uint16_t itemId,
                                        uint8_t tier, uint8_t type);
  void playerRequestOpenContainerFromDepotSearch(uint32_t playerId,
                                                 const Position &pos);
  void playerMoveThingFromDepotSearch(Player *player, uint16_t itemId,
                                      uint8_t tier, uint8_t count,
  ...
  void playerRequestEditVip(uint32_t playerId, uint32_t guid,
                            const std::string &description, uint32_t icon,
                            bool notify);
  void playerApplyImbuement(uint32_t playerId, uint16_t imbuementid,
  ...
  void playerEnableSharedPartyExperience(uint32_t playerId,
  ...
  void playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t itemId,
                               uint16_t amount, uint32_t price, bool anonymous);
  void playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp,
                               uint16_t counter);
  void playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp,
                               uint16_t counter, uint16_t amount);
  void playerStoreOpen(uint32_t playerId, uint8_t serviceType);
  void playerShowStoreCategoryOffers(uint32_t playerId,
                                     StoreCategory *category);
  void playerBuyStoreOffer(uint32_t playerId, uint32_t offerId,
                           uint8_t productType,
                           const std::string &additionalInfo = "");
  void playerCoinTransfer(uint32_t playerId, const std::string &receiverName,
                          uint32_t amount);
  void playerStoreTransactionHistory(uint32_t playerId, uint32_t page);

  void parsePlayerExtendedOpcode(uint32_t playerId, uint8_t opcode,
                                 const std::string &buffer);

  std::vector<Item *> getMarketItemList(uint16_t wareId,
  ...
  void setBoostedName(std::string name) { boostedCreature = name; }

  std::string getBoostedMonsterName() const { return boostedCreature; }
  ...
  bool combatBlockHit(CombatDamage &damage, Creature *attacker,
                      Creature *target, bool checkDefense, bool checkArmor,
                      bool field);

  void combatGetTypeInfo(CombatType_t combatType, Creature *target,
                         TextColor_t &color, uint8_t &effect);

  bool combatChangeHealth(Creature *attacker, Creature *target,
  ...
  const std::map<uint16_t, uint32_t> &getItemsPrice() const {
  ...
  void internalRemoveItems(std::vector<Item *> itemList, uint32_t amount,
                           bool stackable);
  ...
  phmap::flat_hash_set<Tile *> getTilesToClean() const { return tilesToClean; }
  ...
  void playerInspectItem(Player *player, uint16_t itemId, uint8_t itemCount,
                         bool cyclopedia);
  ...
  std::vector<Charm *> getCharmList() { return CharmList; }
  ...
  bool isMagicEffectRegistered(uint8_t type) const {
    return std::find(registeredMagicEffects.begin(),
                     registeredMagicEffects.end(),
                     type) != registeredMagicEffects.end();
  }

  bool isDistanceEffectRegistered(uint8_t type) const {
    return std::find(registeredDistanceEffects.begin(),
                     registeredDistanceEffects.end(),
                     type) != registeredDistanceEffects.end();
  }

  bool isLookTypeRegistered(uint16_t type) const {
    return std::find(registeredLookTypes.begin(), registeredLookTypes.end(),
                     type) != registeredLookTypes.end();
  }
...
private:
  ...
  bool playerSaySpell(Player *player, SpeakClasses type,
                      const std::string &text);
  void playerWhisper(Player *player, const std::string &text);
  bool playerYell(Player *player, const std::string &text);
  bool playerSpeakTo(Player *player, SpeakClasses type,
                     const std::string &receiver, const std::string &text);
   ...
   * This basically works so that the item is created after the map is loaded,
   * because the scripts are loaded before the map is loaded, we will use this
   * table to create items that don't exist in the map natively through each
   * script
   */
  ...
  ModalWindow offlineTrainingWindow{std::numeric_limits<uint32_t>::max(),
                                    "Choose a Skill", "Please choose a skill:"};
  ...
  // (1440 total light of tibian day)/(3600 real seconds each tibian day) * 10
  // seconds event interval
  int32_t lightHourDelta =
      (LIGHT_DAY_LENGTH * (EVENT_LIGHTINTERVAL_MS / 1000)) / DAY_LENGTH_SECONDS;
