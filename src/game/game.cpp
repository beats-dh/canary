...
Game::Game() {
  offlineTrainingWindow.choices.emplace_back("Sword Fighting and Shielding",
                                             SKILL_SWORD);
  offlineTrainingWindow.choices.emplace_back("Axe Fighting and Shielding",
                                             SKILL_AXE);
  offlineTrainingWindow.choices.emplace_back("Club Fighting and Shielding",
                                             SKILL_CLUB);
  offlineTrainingWindow.choices.emplace_back("Distance Fighting and Shielding",
                                             SKILL_DISTANCE);
  offlineTrainingWindow.choices.emplace_back("Magic Level and Shielding",
                                             SKILL_MAGLEVEL);
...
void Game::loadBoostedCreature() {
  ...
  if (date == today) {
    ...
    const MonsterType *monsterType =
        g_monsters().getMonsterTypeByRaceId(newrace);
    ...
    if (monsterType) {
      query << "`looktype` = "
            << static_cast<int>(monsterType->info.outfit.lookType) << ",";
      query << "`lookfeet` = "
            << static_cast<int>(monsterType->info.outfit.lookFeet) << ",";
      query << "`looklegs` = "
            << static_cast<int>(monsterType->info.outfit.lookLegs) << ",";
      query << "`lookhead` = "
            << static_cast<int>(monsterType->info.outfit.lookHead) << ",";
      query << "`lookbody` = "
            << static_cast<int>(monsterType->info.outfit.lookBody) << ",";
      query << "`lookaddons` = "
            << static_cast<int>(monsterType->info.outfit.lookAddons) << ",";
      query << "`lookmount` = "
            << static_cast<int>(monsterType->info.outfit.lookMount) << ",";
    }
...
void Game::start(ServiceManager *manager) {
  ...
  g_scheduler().addEvent(createSchedulerTask(
      EVENT_LIGHTINTERVAL_MS, std::bind(&Game::checkLight, this)));
  g_scheduler().addEvent(
      createSchedulerTask(EVENT_CREATURE_THINK_INTERVAL,
                          std::bind(&Game::checkCreatures, this, 0)));
  g_scheduler().addEvent(createSchedulerTask(
      EVENT_IMBUEMENT_INTERVAL, std::bind(&Game::checkImbuements, this)));
}
  ...
  if (itemType.weaponType == WEAPON_AMMO) {
    ...
    if (quiverThing && backpackThing) {
      ...
      if (quiver && quiver->isQuiver() && backpack) {
        ...
        if (item) {
          ret = internalMoveItem(item->getParent(), quiver->getContainer(), 0,
                                 item, item->getItemCount(), nullptr);
        } else {
    ...
    } else if (hasBitSet(SLOTP_LEFT, slotP) &&
               !hasBitSet(SLOTP_TWO_HAND, slotP)) {
    ...
    if (hasBitSet(SLOTP_TWO_HAND, slotP)) {
      ...
      if (leftthing) {
        ...
        if (slotLeft_item) {
          ...
          ret = internalMoveItem(slotLeft_item->getParent(), player, 0,
                                 slotLeft_item, slotLeft_item->getItemCount(),
                                 nullptr);
        }
      }
      ...
      if (rightthing) {
        ...
        if (slotRight_Item) {
          if (newitemType.weaponType != WEAPON_DISTANCE ||
              !slotRight_Item->isQuiver())
            ret = internalMoveItem(slotRight_Item->getParent(), player, 0,
                                   slotRight_Item,
                                   slotRight_Item->getItemCount(), nullptr);
        } else {
          return;
        }
      }
      if (!removed) {
        ret = internalMoveItem(item->getParent(), player, CONST_SLOT_LEFT, item,
      ...
      if (ringthing) {
        ...
        if (slotRing_Item) {
          ret = internalMoveItem(slotRing_Item->getParent(), player, 0,
                                 slotRing_Item, slotRing_Item->getItemCount(),
                                 nullptr);
      ...
      if (!removed) {
        ret = internalMoveItem(item->getParent(), player, CONST_SLOT_RING, item,
                               item->getItemCount(), nullptr);
      }
      ...
      if (rightthing) {
        ...
        if (slotRight_Item) {
          ...
          ret = internalMoveItem(slotRight_Item->getParent(), player, 0,
                                 slotRight_Item, slotRight_Item->getItemCount(),
                                 nullptr);
        } else {
          return;
        }
      ...
      if (leftthing) {
        ...
        if (slotLeft_item) {
          ...
          if (hasBitSet(SLOTP_TWO_HAND, it.slotPosition)) {
            if (newitemType.type != ITEM_TYPE_QUIVER ||
                slotLeft_item->getWeaponType() != WEAPON_DISTANCE)
              ret = internalMoveItem(slotLeft_item->getParent(), player, 0,
                                     slotLeft_item,
                                     slotLeft_item->getItemCount(), nullptr);
          }
        } else {
          return;
        }
      ...
      if (!removed) {
        ret = internalMoveItem(item->getParent(), player, slotP, item,
                               item->getItemCount(), nullptr);
      }
    } else if (slotP) {
      if (newitemType.stackable) {
        ...
        if (ammothing) {
          ...
          if (ammoItem == nullptr) {
            SPDLOG_DEBUG("[Game::onPressHotkeyEquip] - Player {} ammoItem is "
                         "nullptr {}.",
                         player->getName());
            return;
          }
          ...
          if (ammoItem->getID() == item->getID()) {
            if (item->getDuration() > 0 || ammoItem->getItemCount() == 100 ||
                ammoItem->getItemCount() ==
                    player->getItemTypeCount(ammoItem->getID())) {
              ret = internalQuickLootItem(player, ammoItem, category);
              if (ret != RETURNVALUE_NOERROR) {
                ret =
                    internalMoveItem(ammoItem->getParent(), player, 0, ammoItem,
                                     ammoItem->getItemCount(), nullptr);
              }
              if (ret != RETURNVALUE_NOERROR) {
            ...
            ret = internalQuickLootItem(player, ammoItem, category);
            if (ret != RETURNVALUE_NOERROR) {
              ret = internalMoveItem(ammoItem->getParent(), player, 0, ammoItem,
                                     ammoItem->getItemCount(), nullptr);
            }
          }
        }
        if (ReturnValue returnQueryAdd =
                player->queryAdd(slotP, *item, item->getItemCount(), 0);
        ...
        if (item->getItemCount() < 100 &&
          ...
          while (player->getItemTypeCount(item->getID())) {
            ...
            if (playerItem == nullptr) {
              SPDLOG_DEBUG("[Game::onPressHotkeyEquip] - Player {} has no "
                           "backpack. Item is nullptr.",
                           player->getName());
              break;
            }
            Container *mainBP = playerItem->getContainer();
            if (mainBP == nullptr) {
              SPDLOG_DEBUG("[Game::onPressHotkeyEquip] - Player {} backpack is "
                           "not a container. Container is nullptr.",
                           player->getName());
              break;
            }
            ...
            if (removeItem == nullptr) {
              SPDLOG_DEBUG(
                  "[Game::onPressHotkeyEquip] - Player {} item is nullptr.",
                  player->getName());
              break;
            }
        ...
        if (slotthing) {
          ...
          if (slotItem == nullptr) {
            SPDLOG_DEBUG(
                "[Game::onPressHotkeyEquip] - Player {} slotitem is nullptr.",
                player->getName());
            return;
          }

          ret = internalMoveItem(slotItem->getParent(), player, 0, slotItem,
...
bool Game::loadItemsPrice() {
  ...
  do {
    ...
    query2 << "SELECT `price` FROM `market_offers` WHERE `itemtype` = "
           << itemId << " ORDER BY `price` DESC LIMIT 1";
...
bool Game::loadCustomMap(const std::string &filename) {
  Monster::despawnRange = g_configManager().getNumber(DEFAULT_DESPAWNRANGE);
  Monster::despawnRadius = g_configManager().getNumber(DEFAULT_DESPAWNRADIUS);
  return map.loadMapCustom("data/world/custom/" + filename + ".otbm", true,
                           true, true);
}

void Game::loadMap(const std::string &path) { map.loadMap(path); }
...
Thing *Game::internalGetThing(Player *player, const Position &pos,
  ...
  if (pos.y & 0x40) {
    ...
    uint8_t slot = pos.z;
    return parentContainer->getItemByIndex(player->getContainerIndex(fromCid) +
                                           slot);
...
bool Game::placeCreature(Creature *creature, const Position &pos,
    ...
    if (Player *player = spectator->getPlayer()) {
      oldStackPosVector.push_back(
          player->canSeeCreature(creature)
              ? tile->getStackposOfCreature(player, creature)
              : -1);
    }
  }
...
void Game::playerTeleport(uint32_t playerId, const Position &newPosition) {
  ...
  ReturnValue returnValue =
      g_game().internalTeleport(player, newPosition, false);
...
void Game::playerInspectItem(Player *player, const Position &pos) {
  ...
  player->sendItemInspection(
      item->getID(), static_cast<uint8_t>(item->getItemCount()), item, false);
}

void Game::playerInspectItem(Player *player, uint16_t itemId, uint8_t itemCount,
                             bool cyclopedia) {
...
FILELOADER_ERRORS Game::loadAppearanceProtobuf(const std::string &file) {
  ...
  if (!fileStream.is_open()) {
    SPDLOG_ERROR("[Game::loadAppearanceProtobuf] - Failed to load {}, file "
                 "cannot be oppened",
                 file);
    fileStream.close();
  ...
  if (!appearances.ParseFromIstream(&fileStream)) {
    SPDLOG_ERROR("[Game::loadAppearanceProtobuf] - Failed to parse binary file "
                 "{}, file is invalid",
                 file);
    fileStream.close();
    return ERROR_NOT_OPEN;
  ...
  if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS)) {
    ...
    for (uint32_t it = 0; it < appearances.effect_size(); it++) {
      registeredMagicEffects.push_back(
          static_cast<uint8_t>(appearances.effect(it).id()));
    }

    // Registering missile effects
    for (uint32_t it = 0; it < appearances.missile_size(); it++) {
      registeredDistanceEffects.push_back(
          static_cast<uint8_t>(appearances.missile(it).id()));
    }

    // Registering outfits
    for (uint32_t it = 0; it < appearances.outfit_size(); it++) {
      registeredLookTypes.push_back(
          static_cast<uint16_t>(appearances.outfit(it).id()));
    }
  }
...
void Game::playerMoveThing(uint32_t playerId, const Position &fromPos,
                           uint16_t itemId, uint8_t fromStackPos,
  ...
  if (fromPos.x == 0xFFFF) {
    if (fromPos.y & 0x40) {
      fromIndex = fromPos.z;
               ...
               !player->isDepotSearchOpenOnItem(itemId)) {
      // '0x20' -> From depot.
      // '0x21' -> From inbox.
  ...
  Thing *thing =
      internalGetThing(player, fromPos, fromIndex, itemId, STACKPOS_MOVE);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (Creature *movingCreature = thing->getCreature()) {
    ...
    if (Position::areInRange<1, 1, 0>(movingCreature->getPosition(),
      ...
      playerMoveCreature(player, movingCreature, movingCreature->getPosition(),
                         tile);
    }
    ...
    playerMoveItem(player, fromPos, itemId, fromStackPos, toPos, count,
                   thing->getItem(), toCylinder);
  }
}
...
void Game::playerMoveCreature(Player *player, Creature *movingCreature,
  ...
  if (!player->canDoAction()) {
    ...
    SchedulerTask *task = createSchedulerTask(
        delay, std::bind(&Game::playerMoveCreatureByID, this, player->getID(),
                         movingCreature->getID(), movingCreatureOrigPos,
                         toTile->getPosition()));
  ...
  if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos,
                                     player->getPosition())) {
    // need to walk to the creature first before moving it
    std::forward_list<Direction> listDir;
    if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
      g_dispatcher().addTask(createTask(
          std::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));

      SchedulerTask *task = createSchedulerTask(
          600, std::bind(&Game::playerMoveCreatureByID, this, player->getID(),
                         movingCreature->getID(), movingCreatureOrigPos,
                         toTile->getPosition()));
  ...
  if ((Position::getDistanceX(movingCreaturePos, toPos) >
       movingCreature->getThrowRange()) ||
      (Position::getDistanceY(movingCreaturePos, toPos) >
       movingCreature->getThrowRange()) ||
      (Position::getDistanceZ(movingCreaturePos, toPos) * 4 >
       movingCreature->getThrowRange())) {
  ...
  if (player != movingCreature) {
    if (toTile->hasFlag(TILESTATE_BLOCKPATH)) {
      ...
      return;
               ...
               (movingCreature->getZone() == ZONE_NOPVP &&
  ...
  if (!g_events().eventPlayerOnMoveCreature(player, movingCreature,
                                            movingCreaturePos, toPos)) {
    return;
  }
...
ReturnValue Game::internalMoveCreature(Creature *creature, Direction direction,
  ...
  if (player && !diagonalMovement) {
    ...
    if (currentPos.z != 8 && creature->getTile()->hasHeight(3)) {
      Tile *tmpTile =
          map.getTile(currentPos.x, currentPos.y, currentPos.getZ() - 1);
      if (tmpTile == nullptr || (tmpTile->getGround() == nullptr &&
                                 !tmpTile->hasFlag(TILESTATE_BLOCKSOLID))) {
        tmpTile = map.getTile(destPos.x, destPos.y, destPos.getZ() - 1);
        if (tmpTile && tmpTile->getGround() &&
            !tmpTile->hasFlag(TILESTATE_BLOCKSOLID)) {
...
ReturnValue Game::internalMoveCreature(Creature &creature, Tile &toTile,
                                       uint32_t flags /*= 0*/) {
  ...
  while ((subCylinder = toCylinder->queryDestination(index, creature, &toItem,
                                                     flags)) != toCylinder) {
...
void Game::playerMoveItemByPlayerID(uint32_t playerId, const Position &fromPos,
                                    uint16_t itemId, uint8_t fromStackPos,
                                    const Position &toPos, uint8_t count) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
  ...
  playerMoveItem(player, fromPos, itemId, fromStackPos, toPos, count, nullptr,
                 nullptr);
}

void Game::playerMoveItem(Player *player, const Position &fromPos,
                          uint16_t itemId, uint8_t fromStackPos,
                          const Position &toPos, uint8_t count, Item *item,
                          Cylinder *toCylinder) {
  if (!player->canDoAction()) {
    uint32_t delay = player->getNextActionTime();
    SchedulerTask *task = createSchedulerTask(
        delay, std::bind(&Game::playerMoveItemByPlayerID, this, player->getID(),
                         fromPos, itemId, fromStackPos, toPos, count));
  ...
  if (playerPos.z != mapFromPos.z) {
    player->sendCancelMessage(playerPos.z > mapFromPos.z
  ...
  if (!Position::areInRange<1, 1>(playerPos, mapFromPos)) {
    ...
    if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true)) {
      ...
      SchedulerTask *task = createSchedulerTask(
          400, std::bind(&Game::playerMoveItemByPlayerID, this, player->getID(),
                         fromPos, itemId, fromStackPos, toPos, count));
  ...
  if (item->isHangable() &&
      toCylinderTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
    ...
    if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos)) {
      ...
      if (fromPos.x != 0xFFFF &&
        ...
        ReturnValue ret = internalMoveItem(
            fromCylinder, player, INDEX_WHEREEVER, item, count, &moveItem);
        if (ret != RETURNVALUE_NOERROR) {
          player->sendCancelMessage(ret);
          return;
        }
      ...
      if (player->getPathTo(walkPos, listDir, 0, 0, true, true)) {
        ...
        SchedulerTask *task = createSchedulerTask(
            400,
            std::bind(&Game::playerMoveItemByPlayerID, this, player->getID(),
                      itemPos, itemId, itemStackPos, toPos, count));
        player->setNextWalkActionTask(task);
      } else {
        player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
      }
  ...
  if ((Position::getDistanceX(playerPos, mapToPos) > item->getThrowRange()) ||
      ...
      (Position::getDistanceZ(mapFromPos, mapToPos) * 4 >
       item->getThrowRange())) {
    player->sendCancelMessage(RETURNVALUE_DESTINATIONOUTOFREACH);
    return;
  }
  ...
  if (item->isWrapable()) {
    ...
    HouseTile *fromHouseTile =
        dynamic_cast<HouseTile *>(map.getTile(mapFromPos));
    if (fromHouseTile &&
        (!toHouseTile || toHouseTile->getHouse()->getId() !=
                             fromHouseTile->getHouse()->getId())) {
      player->sendCancelMessage("You can't move this item outside a house.");
      return;
    }
  }
  ReturnValue ret = internalMoveItem(fromCylinder, toCylinder, toIndex, item,
                                     count, nullptr, 0, player);
  if (ret != RETURNVALUE_NOERROR) {
    player->sendCancelMessage(ret);
  } else if (toCylinder->getContainer() && fromCylinder->getContainer() &&
...
ReturnValue Game::internalMoveItem(Cylinder *fromCylinder, Cylinder *toCylinder,
  ...
  while ((subCylinder = toCylinder->queryDestination(index, *item, &toItem,
                                                     flags)) != toCylinder) {
    toCylinder = subCylinder;
    flags = 0;
  ...
  //  Cip's client never sends the count of stackables when using "Move up" menu
  //  option
  if (item->isStackable() && count == 255 &&
      fromCylinder->getParent() == toCylinder) {
  ...
  if (ret == RETURNVALUE_NEEDEXCHANGE) {
    ...
    ret = fromCylinder->queryAdd(fromCylinder->getThingIndex(item), *toItem,
                                 toItem->getItemCount(), 0);
    if (ret == RETURNVALUE_NOERROR) {
      // check how much we can move
      uint32_t maxExchangeQueryCount = 0;
      ReturnValue retExchangeMaxCount = fromCylinder->queryMaxCount(
          INDEX_WHEREEVER, *toItem, toItem->getItemCount(),
          maxExchangeQueryCount, 0);

      if (retExchangeMaxCount != RETURNVALUE_NOERROR &&
          maxExchangeQueryCount == 0) {
        return retExchangeMaxCount;
      }

      if (toCylinder->queryRemove(*toItem, toItem->getItemCount(), flags,
                                  actor) == RETURNVALUE_NOERROR) {
        int32_t oldToItemIndex = toCylinder->getThingIndex(toItem);
        toCylinder->removeThing(toItem, toItem->getItemCount());
        fromCylinder->addThing(toItem);

        if (oldToItemIndex != -1) {
          toCylinder->postRemoveNotification(toItem, fromCylinder,
                                             oldToItemIndex);
        }
  ...
  ReturnValue retMaxCount =
      toCylinder->queryMaxCount(index, *item, count, maxQueryCount, flags);
  ...
  if (item->isStackable()) {
    ...
    if (toItem && item->equals(toItem)) {
      ...
      toCylinder->updateThing(toItem, toItem->getID(),
                              toItem->getItemCount() + n);
      updateItem = toItem;
    } else {
      n = 0;
    }
  ...
  if (updateItem) {
    ...
    if (updateItemIndex != -1) {
      toCylinder->postAddNotification(updateItem, fromCylinder,
                                      updateItemIndex);
    }
  ...
  if (quiver && quiver->isQuiver() && quiver->getHoldingPlayer() &&
  ...
  if (fromCylinder && actor && toCylinder) {
    if (!fromCylinder->getContainer() || !actor->getPlayer() ||
        !toCylinder->getContainer()) {
      return ret;
    }
    ...
    if (Player *player = actor->getPlayer()) {
      ...
      // To-Do: Set 'item->getTier()' here on the '0' values when tier system is
      // ready.
      if (player->isDepotSearchOpenOnItem(item->getID()) &&
          ((fromCylinder->getItem() &&
            fromCylinder->getItem()->isInsideDepot(true)) ||
           (toCylinder->getItem() &&
            toCylinder->getItem()->isInsideDepot(true)))) {
      ...
      if (it.isCorpse && toCylinder->getContainer()->getTopParent() == player &&
          item->getIsLootTrackeable()) {
...
ReturnValue Game::internalAddItem(Cylinder *toCylinder, Item *item,
                                  int32_t index, uint32_t flags, bool test,
  ...
  // check if we can add this item
  ReturnValue ret =
      toCylinder->queryAdd(index, *item, item->getItemCount(), flags);
  if (ret != RETURNVALUE_NOERROR) {
    return ret;
  }

  /*
  Check if we can move add the whole amount, we do this by checking against the
  original cylinder, since the queryDestination can return a cylinder that might
  only hold a part of the full amount.
  */
  uint32_t maxQueryCount = 0;
  ret = destCylinder->queryMaxCount(INDEX_WHEREEVER, *item,
                                    item->getItemCount(), maxQueryCount, flags);

  if (ret != RETURNVALUE_NOERROR && toCylinder->getItem() &&
      toCylinder->getItem()->getID() != ITEM_REWARD_CONTAINER) {
    return ret;
  }
  ...
  if (item->isStackable() && item->equals(toItem)) {
    ...
    if (count > 0) {
      if (item->getItemCount() != count) {
        ...
        if (internalAddItem(destCylinder, remainderItem, INDEX_WHEREEVER, flags,
                            false) != RETURNVALUE_NOERROR) {
...
ReturnValue Game::internalRemoveItem(Item *item, int32_t count /*= -1*/,
  ...
  // check if we can remove this item
  ReturnValue ret =
      cylinder->queryRemove(*item, count, flags | FLAG_IGNORENOTMOVEABLE);
  if (ret != RETURNVALUE_NOERROR) {
    return ret;
  }
  if (!item->canRemove()) {
...
ReturnValue
Game::internalPlayerAddItem(Player *player, Item *item,
                            bool dropOnMap /*= true*/,
                            Slots_t slot /*= CONST_SLOT_WHEREEVER*/) {
  uint32_t remainderCount = 0;
  ReturnValue ret = internalAddItem(player, item, static_cast<int32_t>(slot), 0,
                                    false, remainderCount);
  if (remainderCount != 0) {
    Item *remainderItem = Item::CreateItem(item->getID(), remainderCount);
    ReturnValue remaindRet = internalAddItem(player->getTile(), remainderItem,
                                             INDEX_WHEREEVER, FLAG_NOLIMIT);
  ...
  if (ret != RETURNVALUE_NOERROR && dropOnMap) {
    ret =
        internalAddItem(player->getTile(), item, INDEX_WHEREEVER, FLAG_NOLIMIT);
  }
...
Item *Game::findItemOfType(Cylinder *cylinder, uint16_t itemId,
  ...
  for (size_t i = cylinder->getFirstIndex(), j = cylinder->getLastIndex();
       i < j; ++i) {
    ...
    if (item->getID() == itemId &&
        (subType == -1 || subType == item->getSubType())) {
      return item;
    }
...
bool Game::removeMoney(Cylinder *cylinder, uint64_t money,
                       uint32_t flags /*= 0*/, bool useBalance /*= false*/) {
  if (cylinder == nullptr) {
    return false;
  }
  if (money == 0) {
  ...
  for (const auto &moneyEntry : moneyMap) {
    ...
    if (moneyEntry.first < money) {
      ...
      const uint32_t removeCount =
          std::ceil(money / static_cast<double>(worth));
...
void Game::addMoney(Cylinder *cylinder, uint64_t money,
                    uint32_t flags /*= 0*/) {
  if (money == 0) {
    return;
  }
  ...
  while (crystalCoins > 0) {
    ...
    ReturnValue ret =
        internalAddItem(cylinder, remaindItem, INDEX_WHEREEVER, flags);
    if (ret != RETURNVALUE_NOERROR) {
      internalAddItem(cylinder->getTile(), remaindItem, INDEX_WHEREEVER,
                      FLAG_NOLIMIT);
    }
...
Item *Game::transformItem(Item *item, uint16_t newId,
                          int32_t newCount /*= -1*/) {
  if (item->getID() == newId &&
      (newCount == -1 ||
       (newCount == item->getSubType() &&
  ...
  if (item->isAlwaysOnTop() != (newType.alwaysOnTopOrder != 0)) {
    // This only occurs when you transform items on tiles from a downItem to a
    // topItem (or vice versa) Remove the old, and add the new
    ...
    newParent->postAddNotification(item, cylinder,
                                   newParent->getThingIndex(item));
    item->startDecaying();
  ...
  if (curType.type == newType.type) {
    ...
    if (newCount == 0 &&
        (item->isStackable() || item->hasAttribute(ITEM_ATTRIBUTE_CHARGES))) {
      if (item->isStackable()) {
        internalRemoveItem(item);
        return nullptr;
      } else {
        ...
        if (newItemId < 0) {
          ...
          // Replacing the the old item with the new while maintaining the old
          // position
...
void Game::internalQuickLootCorpse(Player *player, Container *corpse) {
  ...
  bool ignoreListItems =
      (player->quickLootFilter == QUICKLOOTFILTER_SKIPPEDLOOT);
  ...
  std::stringstream ss;
  if (totalLootedGold != 0 || missedAnyGold || totalLootedItems != 0 ||
      missedAnyItem) {
    ...
    if (lootedAllGold) {
      ...
      if (totalLootedGold != 0) {
        ss << ", but you only looted " << totalLootedGold
           << " of the dropped gold";
  ...
  if (shouldNotifyCapacity) {
    ss.str(std::string());
    ss << "Attention! The loot you are trying to pick up is too heavy for you "
          "to carry.";
  } else if (shouldNotifyNotEnoughRoom != OBJECTCATEGORY_NONE) {
    ss.str(std::string());
    ss << "Attention! The container assigned to category "
       << getObjectCategoryName(shouldNotifyNotEnoughRoom) << " is full.";
  } else {
    return;
  }
...
ReturnValue Game::internalQuickLootItem(
    ...
    if (player->quickLootFallbackToMainContainer) {
      ...
      if (fallbackItem) {
        ...
        if (mainBackpack && !fallbackConsumed) {
          ...
          player->sendInventoryItem(
              CONST_SLOT_BACKPACK,
              player->getInventoryItem(CONST_SLOT_BACKPACK));
        }
      }
  ...
  do {
    ...
    if (item->getParent()) { // Stash retrive dont have parent cylinder.
      ret = internalMoveItem(item->getParent(), lootContainer, INDEX_WHEREEVER,
                             item, item->getItemCount(), &moveItem, 0, player);
    } else {
    ...
    if (!obtainedNewContainer && lastSubContainer &&
        lastSubContainer->size() > 0) {
      Item *cur =
          lastSubContainer->getItemByIndex(lastSubContainer->size() - 1);
      Container *subContainer = cur ? cur->getContainer() : nullptr;
    ...
    // consumed all sub-container & there is simply no more containers to
    // iterate over. check if fallback should be used and if not, then break
...
uint64_t Game::getItemMarketPrice(std::map<uint16_t, uint32_t> const &itemMap,
                                  bool buyPrice) const {
  uint64_t total = 0;
...
void Game::playerEquipItem(uint32_t playerId, uint16_t itemId) {
  ...
  if (slotItem && slotItem->getID() == it.id &&
    ...
    internalMoveItem(slotItem->getParent(), player, CONST_SLOT_WHEREEVER,
                     slotItem, slotItem->getItemCount(), nullptr);
  } else if (equipItem) {
    internalMoveItem(equipItem->getParent(), player, slot, equipItem,
                     equipItem->getItemCount(), nullptr);
  }
}
...
bool Game::playerBroadcastMessage(Player *player,
...
void Game::playerOpenChannel(uint32_t playerId, uint16_t channelId) {
  ...
  player->sendChannel(channel->getId(), channel->getName(), users,
                      invitedUsers);
}
...
void Game::playerUseItemEx(uint32_t playerId, const Position &fromPos,
                           uint8_t fromStackPos, uint16_t fromItemId,
  ...
  Thing *thing = internalGetThing(player, fromPos, fromStackPos, fromItemId,
                                  STACKPOS_FIND_THING);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  ...
  if (ret != RETURNVALUE_NOERROR) {
    if (ret == RETURNVALUE_TOOFARAWAY) {
      ...
      if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
        ...
        SchedulerTask *task = createSchedulerTask(
            400,
            std::bind(&Game::playerUseItemEx, this, playerId, itemPos,
                      itemStackPos, fromItemId, toPos, toStackPos, toItemId));
        if (it.isRune() || it.type == ITEM_TYPE_POTION) {
  ...
  if (!canDoAction) {
    ...
    SchedulerTask *task = createSchedulerTask(
        delay,
        std::bind(&Game::playerUseItemEx, this, playerId, fromPos, fromStackPos,
                  fromItemId, toPos, toStackPos, toItemId));
    if (it.isRune() || it.type == ITEM_TYPE_POTION) {
      player->setNextPotionActionTask(task);
    } else {
      player->setNextActionTask(task);
  ...
  if (player->isDepotSearchOpenOnItem(fromItemId)) {
    if (item->isInsideDepot(true)) {
      ...
      if (Thing *targetThing = internalGetThing(player, toPos, toStackPos,
                                                toItemId, STACKPOS_FIND_THING);
          targetThing && targetThing->getItem() &&
          targetThing->getItem()->isInsideDepot(true)) {
        mustReloadDepotSearch = true;
      }
    }
  }
...
void Game::playerUseItem(uint32_t playerId, const Position &pos,
                         uint8_t stackPos, uint8_t index, uint16_t itemId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  Thing *thing =
      internalGetThing(player, pos, stackPos, itemId, STACKPOS_FIND_THING);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (ret != RETURNVALUE_NOERROR) {
    if (ret == RETURNVALUE_TOOFARAWAY) {
      ...
      if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
        ...
        SchedulerTask *task = createSchedulerTask(
            400, std::bind(&Game::playerUseItem, this, playerId, pos, stackPos,
                           index, itemId));
        if (it.isRune() || it.type == ITEM_TYPE_POTION) {
          player->setNextPotionActionTask(task);
        } else {
  ...
  if (!canDoAction) {
    ...
    SchedulerTask *task = createSchedulerTask(
        delay, std::bind(&Game::playerUseItem, this, playerId, pos, stackPos,
                         index, itemId));
    if (it.isRune() || it.type == ITEM_TYPE_POTION) {
      player->setNextPotionActionTask(task);
    } else {
...
void Game::playerUseWithCreature(uint32_t playerId, const Position &fromPos,
                                 uint8_t fromStackPos, uint32_t creatureId,
                                 uint16_t itemId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (!Position::areInRange<7, 5, 0>(creature->getPosition(),
                                     player->getPosition())) {
    return;
  }
  ...
  Thing *thing = internalGetThing(player, fromPos, fromStackPos, itemId,
                                  STACKPOS_FIND_THING);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  ...
  if (ret != RETURNVALUE_NOERROR) {
    if (ret == RETURNVALUE_TOOFARAWAY) {
      ...
      if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
        ...
        SchedulerTask *task = createSchedulerTask(
            400, std::bind(&Game::playerUseWithCreature, this, playerId,
                           itemPos, itemStackPos, creatureId, itemId));
        if (it.isRune() || it.type == ITEM_TYPE_POTION) {
          player->setNextPotionActionTask(task);
        } else {
          player->setNextWalkActionTask(task);
  ...
  if (!canDoAction) {
    ...
    SchedulerTask *task = createSchedulerTask(
        delay, std::bind(&Game::playerUseWithCreature, this, playerId, fromPos,
                         fromStackPos, creatureId, itemId));
  ...
  g_actions().useItemEx(player, fromPos, creature->getPosition(),
                        creature->getParent()->getThingIndex(creature), item,
                        isHotkey, creature);
}
...
void Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid) {
  ...
  Container *parentContainer =
      dynamic_cast<Container *>(container->getRealParent());
  if (!parentContainer) {
    ...
    if (it == browseFields.end()) {
      ...
      g_scheduler().addEvent(
          createSchedulerTask(30000, std::bind(&Game::decreaseBrowseFieldRef,
  ...
  if (parentContainer->hasPagination() && parentContainer->hasParent()) {
    uint16_t indexContainer =
        std::floor(parentContainer->getThingIndex(container) /
                   parentContainer->capacity()) *
        parentContainer->capacity();
...
void Game::playerRotateItem(uint32_t playerId, const Position &pos,
                            uint8_t stackPos, const uint16_t itemId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  Thing *thing =
      internalGetThing(player, pos, stackPos, itemId, STACKPOS_TOPDOWN_ITEM);
  if (!thing) {
    return;
  }

  Item *item = thing->getItem();
  if (!item || item->getID() != itemId || !item->isRotatable() ||
  ...
  if (pos.x != 0xFFFF &&
    ...
    if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
      ...
      SchedulerTask *task =
          createSchedulerTask(400, std::bind(&Game::playerRotateItem, this,
...
void Game::playerConfigureShowOffSocket(uint32_t playerId, const Position &pos,
                                        uint8_t stackPos,
  ...
  Item *item = thing->getItem();
  if (!item || item->getID() != itemId || !item->isPodium() ||
      item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID)) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (!Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
    ...
    if (player->getPathTo(pos, listDir, 0, 1, true, false)) {
      g_dispatcher().addTask(createTask(
          std::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
      SchedulerTask *task = createSchedulerTask(
          400, std::bind(&Game::playerBrowseField, this, playerId, pos));
      player->setNextWalkActionTask(task);
...
void Game::playerSetShowOffSocket(uint32_t playerId, Outfit_t &outfit,
                                  ...
                                  const uint16_t itemId, uint8_t podiumVisible,
                                  uint8_t direction) {
  Player *player = getPlayerByID(playerId);
  if (!player || pos.x == 0xFFFF) {
    return;
  ...
  std::string
      key; // Too lazy to fix it :) let's just use temporary key variable
  if (outfit.lookType != 0) {
    key = "LookType";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookType));
    key = "LookHead";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookHead));
    key = "LookBody";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookBody));
    key = "LookLegs";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookLegs));
    key = "LookFeet";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookFeet));
    key = "LookAddons";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookAddons));
  } else {
  ...
  if (outfit.lookMount != 0) {
    key = "LookMount";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookMount));
    key = "LookMountHead";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookMountHead));
    key = "LookMountBody";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookMountBody));
    key = "LookMountLegs";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookMountLegs));
    key = "LookMountFeet";
    item->setCustomAttribute(key, static_cast<int64_t>(outfit.lookMountFeet));
  } else {
    item->removeCustomAttribute("LookMount");
  }

  key = "PodiumVisible";
  item->setCustomAttribute(key, static_cast<int64_t>(podiumVisible));
  key = "LookDirection";
  item->setCustomAttribute(key, static_cast<int64_t>(direction));
...
void Game::playerWrapableItem(uint32_t playerId, const Position &pos,
                              uint8_t stackPos, const uint16_t itemId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (!house) {
    player->sendCancelMessage(
        "You don't own a house, you need own a house to use this.");
    return;
  }
  ...
  if (!item || item->getID() != itemId ||
      item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) ||
      (!item->isWrapable() && item->getID() != ITEM_DECORATION_KIT)) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (pos.x != 0xFFFF &&
    ...
    if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
      ...
      SchedulerTask *task =
          createSchedulerTask(400, std::bind(&Game::playerWrapableItem, this,
                                             playerId, pos, stackPos, itemId));
      player->setNextWalkActionTask(task);
    } else {
      player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
  ...
  if ((item->getHoldingPlayer() && item->getID() == ITEM_DECORATION_KIT) ||
      (tile->hasFlag(TILESTATE_IMMOVABLEBLOCKSOLID) &&
       !item->hasProperty(CONST_PROP_IMMOVABLEBLOCKSOLID))) {
  ...
  const ItemAttributes::CustomAttribute *attr =
      item->getCustomAttribute("unWrapId");
  uint16_t unWrapId = 0;
  if (attr != nullptr) {
  ...
  if (item->isWrapable() && item->getID() != ITEM_DECORATION_KIT) {
    ...
    item->setSpecialDescription("Unwrap it in your own house to create a <" +
                                itemName + ">.");
    if (hiddenCharges > 0) {
...
void Game::playerWriteItem(uint32_t playerId, uint32_t windowTextId,
  ...
  if (!Position::areInRange<1, 1, 0>(writeItem->getPosition(),
                                     player->getPosition())) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }

  for (auto creatureEvent :
       player->getCreatureEvents(CREATURE_EVENT_TEXTEDIT)) {
...
void Game::playerBrowseField(uint32_t playerId, const Position &pos) {
  ...
  if (playerPos.z != pos.z) {
    player->sendCancelMessage(playerPos.z > pos.z
                                  ? RETURNVALUE_FIRSTGOUPSTAIRS
                                  : RETURNVALUE_FIRSTGODOWNSTAIRS);
    return;
  }
...
void Game::playerStowItem(uint32_t playerId, const Position &pos,
                          uint16_t itemId, uint8_t stackpos, uint8_t count,
                          bool allItems) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerStashWithdraw(uint32_t playerId, uint16_t itemId,
  ...
  Container *stashContainer =
      player->getLootContainer(OBJECTCATEGORY_STASHRETRIEVE);
  ...
  uint32_t NoWithdrawCount =
      (noSlotsWith < noCapWith ? noCapWith : noSlotsWith);
  const char *NoWithdrawMsg = (noSlotsWith < noCapWith ? "capacity" : "slots");

  if (WithdrawCount != count) {
    ss << "Retrieved " << WithdrawCount << "x " << it.name << ".\n";
    ss << NoWithdrawCount
       << "x are impossible to retrieve due to insufficient inventory "
       << NoWithdrawMsg << ".";
  } else {
    ss << "Retrieved " << WithdrawCount << "x " << it.name << '.';
  }
...
void Game::playerSeekInContainer(uint32_t playerId, uint8_t containerId,
                                 uint16_t index) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerUpdateHouseWindow(uint32_t playerId, uint8_t listId,
...
void Game::playerRequestTrade(uint32_t playerId, const Position &pos,
                              uint8_t stackPos, uint32_t tradePlayerId,
                              uint16_t itemId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (tradeItem->getID() != itemId || !tradeItem->isPickupable() ||
  ...
  if (g_configManager().getBoolean(ONLY_INVITED_CAN_MOVE_HOUSE_ITEMS)) {
    if (HouseTile *houseTile =
            dynamic_cast<HouseTile *>(tradeItem->getTile())) {
  ...
  if (!Position::areInRange<1, 1>(tradeItemPosition, playerPosition)) {
    ...
    if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
      ...
      SchedulerTask *task = createSchedulerTask(
          400, std::bind(&Game::playerRequestTrade, this, playerId, pos,
                         stackPos, tradePlayerId, itemId));
      player->setNextWalkActionTask(task);
    } else {
      player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
    }
  ...
  if (tradeItemContainer) {
    for (const auto &it : tradeItems) {
      Item *item = it.first;
      if (tradeItem == item) {
        player->sendTextMessage(MESSAGE_TRADE,
                                "This item is already being traded.");
        return;
      }
  ...
  if (tradeContainer && tradeContainer->getItemHoldingCount() + 1 > 100) {
    player->sendTextMessage(MESSAGE_TRADE,
                            "You can not trade more than 100 items.");
    return;
  }
...
bool Game::internalStartTrade(Player *player, Player *tradePartner,
                              Item *tradeItem) {
  if (player->tradeState != TRADE_NONE &&
      !(player->tradeState == TRADE_ACKNOWLEDGE &&
        player->tradePartner == tradePartner)) {
    player->sendCancelMessage(RETURNVALUE_YOUAREALREADYTRADING);
    return false;
  } else if (tradePartner->tradeState != TRADE_NONE &&
  ...
  if (tradePartner->tradeState == TRADE_NONE) {
    ...
    player->sendTradeItemRequest(tradePartner->getName(), counterOfferItem,
                                 false);
...
void Game::playerAcceptTrade(uint32_t playerId) {
  ...
  if (tradePartner->getTradeState() == TRADE_ACCEPT) {
    ...
    if (!g_events().eventPlayerOnTradeAccept(player, tradePartner, tradeItem1,
                                             tradeItem2)) {
    ...
    ReturnValue ret1 =
        internalAddItem(tradePartner, tradeItem1, INDEX_WHEREEVER, 0, true);
    ReturnValue ret2 =
        internalAddItem(player, tradeItem2, INDEX_WHEREEVER, 0, true);
    if (ret1 == RETURNVALUE_NOERROR && ret2 == RETURNVALUE_NOERROR) {
      ...
      if (ret1 == RETURNVALUE_NOERROR && ret2 == RETURNVALUE_NOERROR) {
        ...
        ret1 = internalMoveItem(cylinder1, tradePartner, INDEX_WHEREEVER,
                                tradeItem1, count1, nullptr,
                                FLAG_IGNOREAUTOSTACK, nullptr, tradeItem2);
        if (ret1 == RETURNVALUE_NOERROR) {
          internalMoveItem(cylinder2, player, INDEX_WHEREEVER, tradeItem2,
                           count2, nullptr, FLAG_IGNOREAUTOSTACK);
...
void Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer,
                             uint8_t index) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (index == 0) {
    g_events().eventPlayerOnLookInTrade(player, tradePartner, tradeItem,
                                        lookDistance);
    return;
  }
  ...
  while (i < containers.size()) {
    ...
    for (Item *item : container->getItemList()) {
      ...
      if (--index == 0) {
        g_events().eventPlayerOnLookInTrade(player, tradePartner, item,
                                            lookDistance);
        return;
      }
    }
...
void Game::playerBuyItem(uint32_t playerId, uint16_t itemId, uint8_t count,
                         uint8_t amount, bool ignoreCap /* = false*/,
  ...
  merchant->onPlayerBuyItem(player, it.id, count, amount, ignoreCap,
                            inBackpacks);
  player->updateNpcExhausted();
}

void Game::playerSellItem(uint32_t playerId, uint16_t itemId, uint8_t count,
                          uint8_t amount, bool ignoreEquipped) {
  if (amount == 0 || amount > 100) {
    return;
  }
...
void Game::playerLookAt(uint32_t playerId, uint16_t itemId, const Position &pos,
                        uint8_t stackPos) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (thing != player) {
    lookDistance =
        std::max<int32_t>(Position::getDistanceX(playerPos, thingPos),
...
void Game::playerLookInBattleList(uint32_t playerId, uint32_t creatureId) {
  ...
  if (creature != player) {
    const Position &playerPos = player->getPosition();
    lookDistance =
        std::max<int32_t>(Position::getDistanceX(playerPos, creaturePos),
...
void Game::playerQuickLoot(uint32_t playerId, const Position &pos,
                           uint16_t itemId, uint8_t stackPos, Item *defaultItem,
                           bool lootAllCorpses, bool autoLoot) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  if (!player->canDoAction()) {
    ...
    SchedulerTask *task = createSchedulerTask(
        delay,
        std::bind(&Game::playerQuickLoot, this, player->getID(), pos, itemId,
                  stackPos, defaultItem, lootAllCorpses, autoLoot));
    player->setNextActionTask(task);
    return;
  }
  ...
  if (!autoLoot && pos.x != 0xffff) {
    if (!Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
      ...
      if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
        ...
        SchedulerTask *task = createSchedulerTask(
            0,
            std::bind(&Game::playerQuickLoot, this, player->getID(), pos,
  ...
  if (!corpse || corpse->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) ||
      corpse->hasAttribute(ITEM_ATTRIBUTE_ACTIONID)) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (pos.x == 0xffff && !browseField) {
    ...
    std::stringstream ss;
    if (ret == RETURNVALUE_NOTENOUGHCAPACITY) {
      ss << "Attention! The loot you are trying to pick up is too heavy for "
            "you to carry.";
    } else if (ret == RETURNVALUE_CONTAINERNOTENOUGHROOM) {
      ss << "Attention! The container for " << getObjectCategoryName(category)
         << " is full.";
    } else {
      if (ret == RETURNVALUE_NOERROR) {
        player->sendLootStats(item, item->getItemCount());
...
void Game::playerLootAllCorpses(Player *player, const Position &pos,
                                bool lootAllCorpses) {
  if (lootAllCorpses) {
    ...
    for (Item *tileItem : *itemVector) {
      ...
      if (!tileCorpse || !tileCorpse->isCorpse() ||
          tileCorpse->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) ||
          tileCorpse->hasAttribute(ITEM_ATTRIBUTE_ACTIONID)) {
        continue;
      }
      ...
      if (!tileCorpse->isRewardCorpse() && tileCorpse->getCorpseOwner() != 0 &&
        ...
        player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
        SPDLOG_DEBUG("Player {} cannot loot corpse from id {} in position {}",
                     player->getName(), tileItem->getID(),
...
void Game::playerSetLootContainer(uint32_t playerId, ObjectCategory_t category,
                                  const Position &pos, uint16_t itemId,
                                  uint8_t stackPos) {
  Player *player = getPlayerByID(playerId);
  if (!player || pos.x != 0xffff) {
    return;
  ...
  Thing *thing =
      internalGetThing(player, pos, stackPos, itemId, STACKPOS_USEITEM);
  if (!thing) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (!container || (container->getID() == ITEM_GOLD_POUCH &&
                     category != OBJECTCATEGORY_GOLD)) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
  ...
  if (container->getHoldingPlayer() != player) {
    player->sendCancelMessage(
        "You must be holding the container to set it as a loot container.");
    return;
  }
  ...
  if (parent) {
    parent->updateThing(container, container->getID(),
  ...
  if (previousContainer != nullptr) {
    ...
    if (parent) {
      parent->updateThing(previousContainer, previousContainer->getID(),
                          previousContainer->getItemCount());
    }
  }
}

void Game::playerClearLootContainer(uint32_t playerId,
...
void Game::playerOpenLootContainer(uint32_t playerId,
                                   ObjectCategory_t category) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  player->sendContainer(static_cast<uint8_t>(container->getID()), container,
                        container->hasParent(), 0);
}
...
void Game::playerQuickLootBlackWhitelist(uint32_t playerId,
                                         QuickLootFilter_t filter,
...
void Game::playerRequestDepotSearchItem(uint32_t playerId, uint16_t itemId,
                                        uint8_t tier) {
  Player *player = getPlayerByID(playerId);
  if (!player || !player->isDepotSearchOpen() ||
      player->isDepotSearchExhausted()) {
...
void Game::playerRequestDepotSearchRetrieve(uint32_t playerId, uint16_t itemId,
                                            uint8_t tier, uint8_t type) {
  Player *player = getPlayerByID(playerId);
...
void Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId) {
  ...
  g_dispatcher().addTask(
      createTask(std::bind(&Game::updateCreatureWalk, this, player->getID())));
}
...
void Game::playerSetFightModes(uint32_t playerId, FightMode_t fightMode,
  ...
  if (!vipPlayer) {
    ...
    if (!IOLoginData::getGuidByNameEx(guid, specialVip, formattedName)) {
      player->sendTextMessage(MESSAGE_FAILURE,
                              "A player with this name does not exist.");
      return;
    }
    ...
    if (vipPlayer->hasFlag(PlayerFlag_SpecialVIP) &&
        !player->hasFlag(PlayerFlag_SpecialVIP)) {
      player->sendTextMessage(MESSAGE_FAILURE, "You can not add this player.");
      return;
    }

    if (!vipPlayer->isInGhostMode() || player->isAccessPlayer()) {
      player->addVIP(vipPlayer->getGUID(), vipPlayer->getName(),
                     vipPlayer->statusVipList);
    } else {
      player->addVIP(vipPlayer->getGUID(), vipPlayer->getName(),
                     VIPSTATUS_OFFLINE);
    }
...
void Game::playerRequestEditVip(uint32_t playerId, uint32_t guid,
                                const std::string &description, uint32_t icon,
                                bool notify) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
...
void Game::playerApplyImbuement(uint32_t playerId, uint16_t imbuementid,
                                uint8_t slot, bool protectionCharm) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  if (item->getTopParent() != player) {
    SPDLOG_ERROR("[Game::playerApplyImbuement] - An error occurred while "
                 "player with name {} try to apply imbuement",
                 player->getName());
    player->sendImbuementResult(
        "An error has occurred, reopen the imbuement window. If the problem "
...
bool Game::playerSaySpell(Player *player, SpeakClasses type,
                          const std::string &text) {
  if (player->walkExhausted()) {
    return true;
  }
  ...
  TalkActionResult_t result =
      g_talkActions().playerSaySpell(player, type, words);
...
bool Game::playerSpeakTo(Player *player, SpeakClasses type,
                         const std::string &receiver, const std::string &text) {
...
bool Game::isSightClear(const Position &fromPos, const Position &toPos,
                        bool floorCheck) const {
...
bool Game::internalCreatureSay(Creature *creature, SpeakClasses type,
                               const std::string &text, bool ghostMode,
...
void Game::addCreatureCheck(Creature *creature) {
  ...
  checkCreatureLists[uniform_random(0, EVENT_CREATURECOUNT - 1)].push_back(
      creature);
  creature->incrementReferenceCounter();
}
...
void Game::checkCreatures(size_t index) {
  g_scheduler().addEvent(
      createSchedulerTask(EVENT_CHECK_CREATURE_INTERVAL,
...
void Game::internalCreatureChangeOutfit(Creature *creature,
...
bool Game::combatBlockHit(CombatDamage &damage, Creature *attacker,
                          Creature *target, bool checkDefense, bool checkArmor,
                          bool field) {
  ...
  static const auto sendBlockEffect = [this](BlockType_t blockType,
  ...
  if (damage.primary.type != COMBAT_NONE) {
    ...
    if (attacker && target->getMonster()) {
      uint32_t primaryReflect =
          target->getMonster()->getReflectValue(damage.primary.type);
      if (primaryReflect > 0) {
        damageReflected.primary.type = damage.primary.type;
        damageReflected.primary.value =
            std::ceil((damage.primary.value) * (primaryReflect / 100.));
    ...
    if (attacker && target->getMonster()) {
      uint32_t primaryHealing =
          target->getMonster()->getHealingCombatValue(damage.primary.type);
      if (primaryHealing > 0) {
        damageHeal.primary.value =
            std::ceil((damage.primary.value) * (primaryHealing / 100.));
        canHeal = true;
      }
    }
    primaryBlockType =
        target->blockHit(attacker, damage.primary.type, damage.primary.value,
                         checkDefense, checkArmor, field);

    damage.primary.value = -damage.primary.value;
    sendBlockEffect(primaryBlockType, damage.primary.type,
                    target->getPosition());
  } else {
  ...
  if (damage.secondary.type != COMBAT_NONE) {
    ...
    if (attacker && target->getMonster()) {
      uint32_t secondaryReflect =
          target->getMonster()->getReflectValue(damage.secondary.type);
      if (secondaryReflect > 0) {
        if (!canReflect) {
          damageReflected.primary.type = damage.secondary.type;
          damageReflected.primary.value =
              std::ceil((damage.secondary.value) * (secondaryReflect / 100.));
          damageReflected.extension = true;
          damageReflected.exString = "(damage reflection)";
          canReflect = true;
        } else {
          damageReflected.secondary.type = damage.secondary.type;
          damageReflected.secondary.value =
              std::ceil((damage.secondary.value) * (secondaryReflect / 100.));
        }
      }
    }
    ...
    if (attacker && target->getMonster()) {
      uint32_t secondaryHealing =
          target->getMonster()->getHealingCombatValue(damage.secondary.type);
      if (secondaryHealing > 0) {
        ;
        damageHeal.primary.value +=
            std::ceil((damage.secondary.value) * (secondaryHealing / 100.));
        canHeal = true;
      }
    }
    secondaryBlockType =
        target->blockHit(attacker, damage.secondary.type,
                         damage.secondary.value, false, false, field);

    damage.secondary.value = -damage.secondary.value;
    sendBlockEffect(secondaryBlockType, damage.secondary.type,
                    target->getPosition());
  } else {
...
void Game::combatGetTypeInfo(CombatType_t combatType, Creature *target,
                             TextColor_t &color, uint8_t &effect) {
  switch (combatType) {
  case COMBAT_PHYSICALDAMAGE: {
    Item *splash = nullptr;
...
bool Game::combatChangeHealth(Creature *attacker, Creature *target,
                              CombatDamage &damage, bool isEvent /*= false*/) {
  using namespace std;
  ...
  if (damage.primary.value > 0) {
    ...
    if (damage.origin != ORIGIN_NONE) {
      const auto &events =
          target->getCreatureEvents(CREATURE_EVENT_HEALTHCHANGE);
      if (!events.empty()) {
    ...
    if (realHealthChange > 0 && !target->isInGhostMode()) {
      ...
      // Party hunt analyzer
      if (Party *party =
              attackerPlayer ? attackerPlayer->getParty() : nullptr) {
      ...
      ss << realHealthChange
         << (realHealthChange != 1 ? " hitpoints." : " hitpoint.");
      ...
      for (Creature *spectator : spectators) {
        ...
        if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
          ss.str({});
          ss << "You heal " << target->getNameDescription() << " for "
             << damageString;
          ...
          if (spectatorMessage.empty()) {
            ...
            if (!attacker) {
              ...
              if (attacker == target) {
                ss << (targetPlayer
                           ? (targetPlayer->getSex() == PLAYERSEX_FEMALE
                                  ? "herself"
                                  : "himself")
                           : "itself");
              } else {
    ...
    if (attackerPlayer && targetMonster) {
      const PreySlot *slot =
          attackerPlayer->getPreyWithMonster(targetMonster->getRaceId());
      if (slot && slot->isOccupied() && slot->bonus == PreyBonus_Damage &&
        ...
        damage.secondary.value += static_cast<int32_t>(
            std::ceil((damage.secondary.value * slot->bonusPercentage) / 100));
      }
    } else if (attackerMonster && targetPlayer) {
      const PreySlot *slot =
          targetPlayer->getPreyWithMonster(attackerMonster->getRaceId());
      if (slot && slot->isOccupied() && slot->bonus == PreyBonus_Defense &&
          slot->bonusTimeLeft > 0) {
        damage.primary.value -= static_cast<int32_t>(
            std::ceil((damage.primary.value * slot->bonusPercentage) / 100));
        damage.secondary.value -= static_cast<int32_t>(
    ...
    if (!isEvent) {
      g_events().eventCreatureOnDrainHealth(
          target, attacker, damage.primary.type, damage.primary.value,
          damage.secondary.type, damage.secondary.value, message.primary.color,
          message.secondary.color);
    }
    if (damage.origin != ORIGIN_NONE && attacker &&
    ...
    if (damage.origin != ORIGIN_NONE && target &&
        damage.primary.type != COMBAT_HEALING) {
    ...
    if (!damage.extension && attackerMonster && targetPlayer) {
      ...
      if (charmRune_t activeCharm = g_iobestiary().getCharmFromTarget(
              targetPlayer, g_monsters().getMonsterTypeByRaceId(
                                attackerMonster->getRaceId()));
          activeCharm != CHARM_NONE && activeCharm != CHARM_CLEANSE) {
        if (Charm *charm = g_iobestiary().getBestiaryCharm(activeCharm);
            charm->type == CHARM_DEFENSIVE &&
            charm->chance > normal_random(0, 100) &&
            g_iobestiary().parseCharmCombat(
                charm, targetPlayer, attacker,
                (damage.primary.value + damage.secondary.value))) {
    ...
    if (target->hasCondition(CONDITION_MANASHIELD) &&
      ...
      if (manaDamage != 0) {
        if (damage.origin != ORIGIN_NONE) {
          const auto &events =
              target->getCreatureEvents(CREATURE_EVENT_MANACHANGE);
          if (!events.empty()) {
            for (CreatureEvent *creatureEvent : events) {
        ...
        if (damage.primary.value < 0) {
          damage.secondary.value = std::max<int32_t>(
              0, damage.secondary.value + damage.primary.value);
    ...
    if (damage.primary.value >= targetHealth) {
      ...
      damage.secondary.value = std::min<int32_t>(
          damage.secondary.value, targetHealth - damage.primary.value);
    }
    ...
    if (realDamage == 0) {
      ...
      for (CreatureEvent *creatureEvent :
           target->getCreatureEvents(CREATURE_EVENT_PREPAREDEATH)) {
    ...
    if (realDamage > 0 && targetMonster) {
      if (attackerPlayer && attackerPlayer->getPlayer()) {
        attackerPlayer->updateImpactTracker(damage.secondary.type,
    ...
    if (attackerPlayer) {
      // life leech
      uint16_t lifeChance =
          attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_CHANCE);
      uint16_t lifeSkill =
          attackerPlayer->getSkillLevel(SKILL_LIFE_LEECH_AMOUNT);
      if (normal_random(0, 100) < lifeChance) {
        // Vampiric charm rune
        if (targetMonster) {
          if (uint16_t playerCharmRaceidVamp =
                  attackerPlayer->parseRacebyCharm(CHARM_VAMP, false, 0);
              playerCharmRaceidVamp != 0 &&
              playerCharmRaceidVamp == targetMonster->getRaceId()) {
            if (const Charm *lifec =
                    g_iobestiary().getBestiaryCharm(CHARM_VAMP)) {
        ...
        tmpDamage.primary.value = std::round(realDamage * (lifeSkill / 100.) *
                                             (0.2 * affected + 0.9)) /
                                  affected;
      ...
      // mana leech
      uint16_t manaChance =
          attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_CHANCE);
      uint16_t manaSkill =
          attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_AMOUNT);
      if (normal_random(0, 100) < manaChance) {
        // Void charm rune
        if (targetMonster) {
          if (uint16_t playerCharmRaceidVoid =
                  attackerPlayer->parseRacebyCharm(CHARM_VOID, false, 0);
              playerCharmRaceidVoid != 0 &&
              playerCharmRaceidVoid == targetMonster->getRace()) {
            if (const Charm *voidc =
                    g_iobestiary().getBestiaryCharm(CHARM_VOID)) {
        ...
        tmpDamage.primary.value = std::round(realDamage * (manaSkill / 100.) *
                                             (0.1 * affected + 0.9)) /
                                  affected;
      ...
      if (!damage.extension && targetMonster) {
        if (charmRune_t activeCharm = g_iobestiary().getCharmFromTarget(
                attackerPlayer, g_monsters().getMonsterTypeByRaceId(
                                    targetMonster->getRaceId()));
            activeCharm != CHARM_NONE) {
          if (Charm *charm = g_iobestiary().getBestiaryCharm(activeCharm);
              charm->type == CHARM_OFFENSIVE &&
              (charm->chance >= normal_random(0, 100))) {
            g_iobestiary().parseCharmCombat(charm, attackerPlayer, target,
                                            realDamage);
          }
        }
      }
    ...
    if (message.primary.value) {
      combatGetTypeInfo(damage.primary.type, target, message.primary.color,
                        hitEffect);
    ...
    if (message.secondary.value) {
      combatGetTypeInfo(damage.secondary.type, target, message.secondary.color,
                        hitEffect);
      if (hitEffect != CONST_ME_NONE) {
        addMagicEffect(spectators, targetPos, hitEffect);
      }
    ...
    if (message.primary.color != TEXTCOLOR_NONE ||
        message.secondary.color != TEXTCOLOR_NONE) {
      if (attackerPlayer) {
        attackerPlayer->updateImpactTracker(damage.primary.type,
      ...
      if (targetPlayer) {
        ...
        targetPlayer->updateInputAnalyzer(damage.primary.type,
                                          damage.primary.value, cause);
        if (attackerPlayer) {
          if (damage.secondary.type != COMBAT_NONE) {
            attackerPlayer->updateInputAnalyzer(damage.secondary.type,
                                                damage.secondary.value, cause);
          }
        }
      }
      std::stringstream ss;
      ...
      for (Creature *spectator : spectators) {
        ...
        if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
          ss.str({});
          ss << ucfirst(target->getNameDescription()) << " loses "
             << damageString << " due to your attack.";
          if (damage.extension) {
            ss << " " << damage.exString;
          }
          message.type = MESSAGE_DAMAGE_DEALT;
          ...
          if (spectatorMessage.empty()) {
            ...
            if (attacker) {
              ...
              if (attacker == target) {
                if (targetPlayer) {
                  ss << (targetPlayer->getSex() == PLAYERSEX_FEMALE
                             ? "her own attack"
                             : "his own attack");
                } else {
                  ss << "its own attack";
...
bool Game::combatChangeMana(Creature *attacker, Creature *target,
  ...
  if (manaChange > 0) {
    ...
    if (realManaChange > 0 && !target->isInGhostMode()) {
      ...
      if (!attacker) {
        ...
        if (attacker == target) {
          spectatorMessage +=
              (targetPlayer
                   ? (targetPlayer->getSex() == PLAYERSEX_FEMALE ? "herself"
                                                                 : "himself")
                   : "itself");
        } else {
      ...
      for (Creature *spectator : spectators) {
        ...
        if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
          message.type = MESSAGE_HEALED;
          message.text = "You restored " + target->getNameDescription() +
                         " for " + damageString;
        } else if (tmpPlayer == targetPlayer) {
          message.type = MESSAGE_HEALED;
          if (!attacker) {
            ...
            message.text = "You were restored by " +
                           attacker->getNameDescription() + " for " +
                           damageString;
          }
        } else {
          message.type = MESSAGE_HEALED_OTHERS;
    ...
    BlockType_t blockType =
        target->blockHit(attacker, COMBAT_MANADRAIN, manaLoss);
    ...
    if (targetPlayer && attacker && attacker->getMonster()) {
      ...
      if (mType) {
        charmRune_t activeCharm =
            g_iobestiary().getCharmFromTarget(targetPlayer, mType);
        if (activeCharm != CHARM_NONE && activeCharm != CHARM_CLEANSE) {
          ...
          if (charm && charm->type == CHARM_DEFENSIVE &&
              (charm->chance > normal_random(0, 100))) {
            if (g_iobestiary().parseCharmCombat(charm, targetPlayer, attacker,
                                                manaChange)) {
              return false; // Dodge charm
            }
          }
      ...
      if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
        ...
        if (spectatorMessage.empty()) {
          ...
          if (attacker) {
            ss << " due to ";
            if (attacker == target) {
              ss << (targetPlayer ? (targetPlayer->getSex() == PLAYERSEX_FEMALE
                                         ? "her own attack"
                                         : "his own attack")
                                  : "its own attack");
            } else {
...
void Game::addCreatureHealth(const SpectatorHashSet &spectators,
                             const Creature *target) {
  uint8_t healthPercent =
      std::ceil((static_cast<double>(target->getHealth()) /
                 std::max<int32_t>(target->getMaxHealth(), 1)) *
                100);
...
void Game::addPlayerMana(const Player *target) {
  if (Party *party = target->getParty()) {
    uint8_t manaPercent =
        std::ceil((static_cast<double>(target->getMana()) /
                   std::max<int32_t>(target->getMaxMana(), 1)) *
                  100);
...
void Game::addMagicEffect(const SpectatorHashSet &spectators,
...
void Game::addDistanceEffect(const SpectatorHashSet &spectators,
                             const Position &fromPos, const Position &toPos,
                             uint8_t effect) {
  for (Creature *spectator : spectators) {
    if (Player *tmpPlayer = spectator->getPlayer()) {
...
void Game::checkLight() {
  ...
  if (currentLightState != lightState) {
    ...
    for (auto &[key, it] :
         g_globalEvents().getEventMap(GLOBALEVENT_PERIODCHANGE)) {
...
void Game::shutdown() {
  ...
  webhook_send_message("Server is shutting down", "Shutting down...",
                       WEBHOOK_COLOR_OFFLINE, url);
...
void Game::broadcastMessage(const std::string &text,
                            MessageClasses type) const {
...
void Game::updateCreatureWalkthrough(const Creature *creature) {
  ...
  for (Creature *spectator : spectators) {
    Player *tmpPlayer = spectator->getPlayer();
    tmpPlayer->sendCreatureWalkthrough(creature,
...
void Game::updatePremium(account::Account &account) {
  ...
  if (rem_days != 0) {
    if (last_day == 0) {
      ...
      if (days > 0) {
        if (days >= rem_days) {
          if (!account.SetPremiumRemaningDays(0) ||
              !account.SetPremiumLastDay(0)) {
            account.GetEmail(&email);
            SPDLOG_ERROR(
                "Failed to set account premium days, account email: {}", email);
          }
        } else {
...
void Game::loadMotdNum() {
  ...
  DBResult_ptr result = db.storeQuery(
      "SELECT `value` FROM `server_config` WHERE `config` = 'motd_num'");
  if (result) {
    motdNum = result->getNumber<uint32_t>("value");
  } else {
    db.executeQuery("INSERT INTO `server_config` (`config`, `value`) VALUES "
                    "('motd_num', '0')");
  }

  result = db.storeQuery(
      "SELECT `value` FROM `server_config` WHERE `config` = 'motd_hash'");
  if (result) {
...
void Game::saveMotdNum() const {
  ...
  std::ostringstream query;
  query << "UPDATE `server_config` SET `value` = '" << motdNum
        << "' WHERE `config` = 'motd_num'";
  db.executeQuery(query.str());

  query.str(std::string());
  query << "UPDATE `server_config` SET `value` = '"
        << transformToSHA1(g_configManager().getString(MOTD))
...
void Game::playerJoinParty(uint32_t playerId, uint32_t leaderId) {
  ...
  if (player->getParty()) {
    player->sendTextMessage(MESSAGE_PARTY_MANAGEMENT,
...
void Game::playerEnableSharedPartyExperience(uint32_t playerId,
  ...
  if (!party || (player->hasCondition(CONDITION_INFIGHT) && playerTile &&
...
void Game::sendGuildMotd(uint32_t playerId) {
  ...
  if (guild) {
    player->sendChannelMessage("Message of the Day", guild->getMotd(),
...
void Game::playerCyclopediaCharacterInfo(
    Player *player, uint32_t characterID,
    CyclopediaCharacterInfoType_t characterInfoType, uint16_t entriesPerPage,
    uint16_t page) {
  uint32_t playerGUID = player->getGUID();
  if (characterID != playerGUID) {
    // For now allow viewing only our character since we don't have tournaments
    // supported
  ...
  case CYCLOPEDIA_CHARACTERINFO_RECENTDEATHS: {
    ...
    query << "SELECT `time`, `level`, `killed_by`, `mostdamage_by`, (select "
             "count(*) FROM `player_deaths` WHERE `player_id` = "
          << playerGUID
          << ") as `entries` FROM `player_deaths` WHERE `player_id` = "
          << playerGUID << " ORDER BY `time` DESC LIMIT " << offset << ", "
          << entriesPerPage;

    uint32_t playerID = player->getID();
    std::function<void(DBResult_ptr, bool)> callback =
        [playerID, page, entriesPerPage](DBResult_ptr result, bool) {
          ...
          do {
            ...
            cause << "Died at Level " << result->getNumber<uint32_t>("level")
                  << " by";
            if (!cause1.empty()) {
            ...
            cause << '.';
            entries.emplace_back(std::move(cause.str()),
                                 result->getNumber<uint32_t>("time"));
          } while (result->next());
          player->sendCyclopediaCharacterRecentDeaths(
              page, static_cast<uint16_t>(pages), entries);
        };
  ...
  case CYCLOPEDIA_CHARACTERINFO_RECENTPVPKILLS: {
    ...
    std::ostringstream query;
    uint32_t offset = static_cast<uint32_t>(page - 1) * entriesPerPage;
    query << "SELECT `d`.`time`, `d`.`killed_by`, `d`.`mostdamage_by`, "
             "`d`.`unjustified`, `d`.`mostdamage_unjustified`, `p`.`name`, "
             "(select count(*) FROM `player_deaths` WHERE ((`killed_by` = "
          << escapedName
          << " AND `is_player` = 1) OR (`mostdamage_by` = " << escapedName
          << " AND `mostdamage_is_player` = 1))) as `entries` FROM "
             "`player_deaths` AS `d` INNER JOIN `players` AS `p` ON "
             "`d`.`player_id` = `p`.`id` WHERE ((`d`.`killed_by` = "
          << escapedName
          << " AND `d`.`is_player` = 1) OR (`d`.`mostdamage_by` = "
          << escapedName
          << " AND `d`.`mostdamage_is_player` = 1)) ORDER BY `time` DESC LIMIT "
    ...
    std::function<void(DBResult_ptr, bool)> callback =
        [playerID, page, entriesPerPage](DBResult_ptr result, bool) {
          ...
          do {
            ...
            uint8_t status =
                CYCLOPEDIA_CHARACTERINFO_RECENTKILLSTATUS_JUSTIFIED;
            ...
            entries.emplace_back(std::move(description.str()),
                                 result->getNumber<uint32_t>("time"), status);
          } while (result->next());
          player->sendCyclopediaCharacterRecentPvPKills(
              page, static_cast<uint16_t>(pages), entries);
        };
    g_databaseTasks().addTask(query.str(), callback, true);
...
void Game::playerHighscores(Player *player, HighscoreType_t type,
                            uint8_t category, uint32_t vocation,
                            const std::string &, uint16_t page,
  ...
  if (type == HIGHSCORE_GETENTRIES) {
    uint32_t startPage = (static_cast<uint32_t>(page - 1) *
                          static_cast<uint32_t>(entriesPerPage));
    uint32_t endPage = startPage + static_cast<uint32_t>(entriesPerPage);
    query << "SELECT *, @row AS `entries`, " << page
          << " AS `page` FROM (SELECT *, (@row := @row + 1) AS `rn` FROM "
             "(SELECT `id`, `name`, `level`, `vocation`, `"
          << categoryName << "` AS `points`, @curRank := IF(@prevRank = `"
          << categoryName << "`, @curRank, IF(@prevRank := `" << categoryName
          << "`, @curRank + 1, @curRank + 1)) AS `rank` FROM `players` `p`, "
             "(SELECT @curRank := 0, @prevRank := NULL, @row := 0) `r` WHERE "
             "`group_id` < "
          << static_cast<int>(account::GROUP_TYPE_GAMEMASTER) << " ORDER BY `"
          << categoryName << "` DESC) `t`";
    ...
    query << "SELECT *, @row AS `entries`, (@ourRow DIV " << entriesStr
          << ") + 1 AS `page` FROM (SELECT *, (@row := @row + 1) AS `rn`, "
             "@ourRow := IF(`id` = "
          << player->getGUID()
          << ", @row - 1, @ourRow) AS `rw` FROM (SELECT `id`, `name`, `level`, "
             "`vocation`, `"
          << categoryName << "` AS `points`, @curRank := IF(@prevRank = `"
          << categoryName << "`, @curRank, IF(@prevRank := `" << categoryName
          << "`, @curRank + 1, @curRank + 1)) AS `rank` FROM `players` `p`, "
             "(SELECT @curRank := 0, @prevRank := NULL, @row := 0, @ourRow := "
             "0) `r` WHERE `group_id` < "
          << static_cast<int>(account::GROUP_TYPE_GAMEMASTER) << " ORDER BY `"
          << categoryName << "` DESC) `t`";
    if (vocation != 0xFFFFFFFF) {
    ...
    query << ") `T` WHERE `rn` > ((@ourRow DIV " << entriesStr << ") * "
          << entriesStr << ") AND `rn` <= (((@ourRow DIV " << entriesStr
          << ") * " << entriesStr << ") + " << entriesStr << ")";
  }

  uint32_t playerID = player->getID();
  std::function<void(DBResult_ptr, bool)> callback =
      [playerID, category, vocation, entriesPerPage](DBResult_ptr result,
                                                     bool) {
        Player *player = g_game().getPlayerByID(playerID);
        if (!player) {
          return;
        ...
        do {
          ...
          const Vocation *voc = g_vocations().getVocation(
              result->getNumber<uint16_t>("vocation"));
          if (voc) {
            characterVocation = voc->getClientId();
          } else {
            characterVocation = 0;
          }
          characters.emplace_back(std::move(result->getString("name")),
                                  result->getNumber<uint64_t>("points"),
                                  result->getNumber<uint32_t>("id"),
                                  result->getNumber<uint32_t>("rank"),
                                  result->getNumber<uint16_t>("level"),
                                  characterVocation);
        } while (result->next());
        player->sendHighscores(characters, category, vocation, page,
...
void Game::playerTournamentLeaderboard(uint32_t playerId,
                                       uint8_t leaderboardType) {
  Player *player = getPlayerByID(playerId);
...
void Game::playerReportRuleViolationReport(uint32_t playerId,
  ...
  g_events().eventPlayerOnReportRuleViolation(
      player, targetName, reportType, reportReason, comment, translation);
}
...
void Game::playerDebugAssert(uint32_t playerId, const std::string &assertLine,
  ...
  if (file) {
    ...
    fprintf(file, "%s\n%s\n%s\n%s\n", assertLine.c_str(), date.c_str(),
            description.c_str(), comment.c_str());
    fclose(file);
  }
}

void Game::playerPreyAction(uint32_t playerId, uint8_t slot, uint8_t action,
                            uint8_t option, int8_t index, uint16_t raceId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  }
  ...
  g_ioprey().ParsePreyAction(player, static_cast<PreySlot_t>(slot),
                             static_cast<PreyAction_t>(action),
                             static_cast<PreyOption_t>(option), index, raceId);
}

void Game::playerTaskHuntingAction(uint32_t playerId, uint8_t slot,
                                   uint8_t action, bool upgrade,
                                   uint16_t raceId) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  g_ioprey().ParseTaskHuntingAction(player, static_cast<PreySlot_t>(slot),
                                    static_cast<PreyTaskAction_t>(action),
                                    upgrade, raceId);
}
...
void Game::playerNpcGreet(uint32_t playerId, uint32_t npcId) {
  ...
  if (npc->getSpeechBubble() == SPEECHBUBBLE_TRADE) {
    internalCreatureSay(player, TALKTYPE_PRIVATE_PN, "trade", false,
                        &spectators);
  } else {
    internalCreatureSay(player, TALKTYPE_PRIVATE_PN, "sail", false,
                        &spectators);
  }
}
...
void Game::playerBrowseMarket(uint32_t playerId, uint16_t itemId) {
  ...
  const MarketOfferList &buyOffers =
      IOMarket::getActiveOffers(MARKETACTION_BUY, it.id);
  const MarketOfferList &sellOffers =
      IOMarket::getActiveOffers(MARKETACTION_SELL, it.id);
...
void Game::playerBrowseMarketOwnOffers(uint32_t playerId) {
  ...
  const MarketOfferList &buyOffers =
      IOMarket::getOwnOffers(MARKETACTION_BUY, player->getGUID());
  const MarketOfferList &sellOffers =
      IOMarket::getOwnOffers(MARKETACTION_SELL, player->getGUID());
...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type,
                                   uint16_t itemId, uint16_t amount,
  ...
  if (g_configManager().getBoolean(MARKET_PREMIUM) && !player->isPremium()) {
    player->sendTextMessage(
        MESSAGE_MARKET,
        "Only premium accounts may create offers for that object.");
    return;
  }
  ...
  const uint32_t maxOfferCount =
      g_configManager().getNumber(MAX_MARKET_OFFERS_AT_A_TIME_PER_PLAYER);
  if (maxOfferCount != 0 &&
  ...
  if (type == MARKETACTION_SELL) {
    ...
    if (depotLocker == nullptr) {
      SPDLOG_ERROR(
          "[Game::playerCreateMarketOffer] - Sell depot chest is nullptr");
      return;
    }
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      std::vector<Item *> itemVector =
          getMarketItemList(it.wareId, amount, depotLocker);
      if (itemVector.empty() && amount > 0) {
        SPDLOG_ERROR(
            "[Game::playerCreateMarketOffer] - Sell item list is empty");
        return;
      }

      if (stashminus > 0) {
        player->withdrawItem(it.wareId,
                             (amount > stashminus ? stashminus : amount));
      }
      ...
      for (Item *item : itemVector) {
        ...
        uint16_t removeCount =
            std::min<uint16_t>(tmpAmount, item->getItemCount());
  ...
  IOMarket::createOffer(player->getGUID(), static_cast<MarketAction_t>(type),
                        it.id, amount, price, anonymous);
...
void Game::playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp,
                                   uint16_t counter) {
  Player *player = getPlayerByID(playerId);
  if (!player) {
    return;
  ...
  if (offer.type == MARKETACTION_BUY) {
    player->setBankBalance(player->getBankBalance() +
                           static_cast<uint64_t>(offer.price) * offer.amount);
    // Send market window again for update stats
    player->sendMarketEnter(player->getLastDepotId());
  } else {
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      while (tmpAmount > 0) {
        ...
        if (internalAddItem(player->getInbox(), item, INDEX_WHEREEVER,
...
void Game::playerAcceptMarketOffer(uint32_t playerId, uint32_t timestamp,
                                   uint16_t counter, uint16_t amount) {
  ...
  // The player has an offer to by something and someone is going to sell to
  // item type so the market action is 'buy' as who created the offer is buying.
  if (offer.type == MARKETACTION_BUY) {
    DepotLocker *depotLocker = player->getDepotLocker(player->getLastDepotId());
    if (depotLocker == nullptr) {
      SPDLOG_ERROR(
          "[Game::playerCreateMarketOffer] - Buy depot chest is nullptr");
      return;
    }
    ...
    if (player == buyerPlayer ||
        player->getAccount() == buyerPlayer->getAccount()) {
      player->sendTextMessage(MESSAGE_MARKET,
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      if (stashCount > 0) {
        if (removeAmount > stashCount &&
            player->withdrawItem(it.wareId, stashCount)) {
      ...
      if (removeAmount > 0) {
        ...
        if (itemVector.empty()) {
          SPDLOG_ERROR(
              "[Game::playerCreateMarketOffer] - Buy item list is empty");
          return;
        }
    ...
    if (it.id == ITEM_STORE_COIN) {
      ...
      while (tmpAmount > 0) {
        ...
        Item *item = Item::CreateItem(it.id, stackCount);
        if (internalAddItem(buyerPlayer->getInbox(), item, INDEX_WHEREEVER,
                            FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
          delete item;
          break;
        }
  ...
  const int32_t marketOfferDuration =
      g_configManager().getNumber(MARKET_OFFER_DURATION);

  IOMarket::appendHistory(
      player->getGUID(),
      (offer.type == MARKETACTION_BUY ? MARKETACTION_SELL : MARKETACTION_BUY),
      offer.itemId, amount, offer.price, time(nullptr), OFFERSTATE_ACCEPTEDEX);

  IOMarket::appendHistory(offer.playerId, offer.type, offer.itemId, amount,
                          offer.price, time(nullptr), OFFERSTATE_ACCEPTED);
...
void Game::playerShowStoreCategoryOffers(uint32_t playerId,
...
void Game::playerBuyStoreOffer(uint32_t playerId, uint32_t offerId,
  ...
  if (player) {
    ...
    if (offer == nullptr || offer->type == DISABLED) {
      player->sendStoreError(STORE_ERROR_NETWORK,
                             "The offer is either fake or corrupt.");
      return;
    }
    ...
    {
      player->sendStoreError(STORE_ERROR_PURCHASE,
    ...
    if (offer->type == ITEM || offer->type == STACKABLE_ITEM ||
        offer->type == WRAP_ITEM) {
      const ItemOffer *tmp = (ItemOffer *)offer;

      message << "You have purchased " << tmp->count << "x " << offer->name
              << " for " << offer->price << " coins.";

      Thing *thing = player->getThing(CONST_SLOT_STORE_INBOX);
      if (thing == nullptr) {
        player->sendStoreError(
            STORE_ERROR_NETWORK,
            "We cannot locate your store inbox, try again after relog and if "
            "this error persists, contact the system administrator.");
        return;
      }

      Container *inbox =
          thing->getItem()
              ->getContainer(); // TODO: Not the right way to get the storeInbox
      if (!inbox) {
        player->sendStoreError(
            STORE_ERROR_NETWORK,
            "We cannot locate your store inbox, try again after relog and if "
      ...
      uint32_t requiredSlots =
          (tmp->type == ITEM || tmp->type == WRAP_ITEM) ? tmp->count
      ...
      if (freeSlots < requiredSlots) {
        player->sendStoreError(STORE_ERROR_PURCHASE,
                               "Insuficient free slots in your store inbox.");
        return;
      } else if (player->getFreeCapacity() < capNeeded) {
        player->sendStoreError(STORE_ERROR_PURCHASE,
                               "Not enough cap to carry.");
        return;
      } else {
        ...
        while (pendingCount > 0) {
          ...
          if (offer->type == WRAP_ITEM) {
            item = Item::CreateItem(ITEM_DECORATION_KIT,
                                    std::min<uint16_t>(packSize, pendingCount));
            item->setActionId(tmp->productId);
            item->setSpecialDescription(
                "Unwrap it in your own house to create a <" +
                Item::items[tmp->productId].name + ">.");
          } else {
            item = Item::CreateItem(tmp->productId,
                                    std::min<uint16_t>(packSize, pendingCount));
          }

          if (internalAddItem(inbox, item, INDEX_WHEREEVER, FLAG_NOLIMIT) !=
              RETURNVALUE_NOERROR) {
            delete item;
            player->sendStoreError(
                STORE_ERROR_PURCHASE,
                "We couldn't deliver all the items.\nOnly the delivered ones "
                "were charged from you account");
            account.AddCoins(
                (offer->price * (tmp->count - pendingCount) / tmp->count));
            account.RegisterCoinsTransaction(
                account::COIN_REMOVE,
                offer->price +
                    (offer->price * (tmp->count - pendingCount)) / tmp->count,
                offer->name);
      ...
      uint16_t looktype = (player->getSex() == PLAYERSEX_MALE)
                              ? outfitOffer->maleLookType
                              : outfitOffer->femaleLookType;
      ...
      if (!player->canWear(looktype, addons)) {
        ...
        player->sendStoreError(
            STORE_ERROR_NETWORK,
            "This outfit seems not to suit you well, we are sorry for that!");
        return;
      }
      ...
      if (player->hasMount(mount)) {
        ...
        if (!player->tameMount(mount->id)) {
          ...
          player->sendStoreError(
              STORE_ERROR_PURCHASE,
              "An error ocurred processing your purchase. Try again later.");
          return;
        } else {
          account.RegisterCoinsTransaction(account::COIN_REMOVE, offer->price,
                                           offer->name);
          message << "You've successfully bought the " << mount->name
                  << " Mount.";
          account.GetCoins(&coins);
          player->sendStorePurchaseSuccessful(message.str(), coins);
          return;
      ...
      if (productType == SIMPLE) { // client didn't sent the new name yet,
        ...
        if (db.storeQuery(query.str())) { // name already in use
          ...
          case INVALID_LENGTH:
            responseMessage = "Your new name must be more than 3 and less than "
                              "14 characters long.";
            break;
          case INVALID_TOKEN_LENGTH:
            responseMessage = "Every words of your new name must be at least 2 "
                              "characters long.";
            break;
          ...
          case INVALID:
            ...
            break;
          case VALID:
            responseMessage = "You have successfully changed you name, you "
                              "must relog to see changes.";
            break;
          }
          ...
          if (nameValidation != VALID) { // invalid name typed
            ...
            if (g_monsters().getMonsterType(newName)) {
              ...
              if (db.executeQuery(query.str())) {
                ...
                account.GetCoins(&coins);
                message << "You have successfully changed you name, you must "
                           "relog to see the changes.";
                player->sendStorePurchaseSuccessful(message.str(), coins);
                return;
              } else {
                message << "An error ocurred processing your request, no "
                           "changes were made.";
                player->sendStoreError(STORE_ERROR_PURCHASE, message.str());
                return;
              }
      ...
      for (auto outfit :
           player->outfits) { // adding all outfits of the oposite sex.
        const Outfit *opositeSexOutfit =
            Outfits::getInstance().getOpositeSexOutfitByLookType(
                playerSex, outfit.lookType);
      ...
      // TODO: add the other sex equivalent outfits player already have in the
      // current sex.
      account.LoadAccountDB(player->getAccount());
      account.RemoveCoins(offer->price);
      account.RegisterCoinsTransaction(account::COIN_REMOVE, offer->price,
      ...
      if (player->isPremium() && !player->isPromoted()) {
        uint16_t promotedId =
            g_vocations().getPromotedVocation(player->getVocation()->getId());

        if (promotedId == VOCATION_NONE ||
            promotedId == player->getVocation()->getId()) {
          player->sendStoreError(STORE_ERROR_PURCHASE,
                                 "Your character cannot be promoted.");
          return;
        } else {
          account.LoadAccountDB(player->getAccount());
          account.RemoveCoins(offer->price);
      ...
      message << "You've successfully bought " << premiumTimeOffer->days
      ...
      if (player->canLogout()) {
        ...
        if (tpOffer->position.x == 0 || tpOffer->position.y == 0 ||
            tpOffer->position.z == 0) { // temple teleport
        ...
        if (returnValue != RETURNVALUE_NOERROR) {
          player->sendStoreError(
              STORE_ERROR_PURCHASE,
              "Your character cannot be teleported there at the moment.");
          return;
        } else {
          account.LoadAccountDB(player->getAccount());
          account.RemoveCoins(offer->price);
          ...
          player->sendStorePurchaseSuccessful(
              "You've successfully been teleported.", coins);
          return;
        }
      } else {
        player->sendStoreError(STORE_ERROR_PURCHASE,
                               "Your character has some teleportation block at "
                               "the moment and cannot be teleported.");
        return;
      }
      ...
      for (uint8_t bless : blessingOffer->blessings) {
        if (player->hasBlessing(bless)) { // player already has this bless
          ...
          message << ((blessingOffer->blessings.size() > 1)
                          ? "one or more of these blessings."
                          : "this bless.");
      ...
      // TODO: BOOST_XP and BOOST_STAMINA (the support systems are not yet
      // implemented)
      player->sendStoreError(STORE_ERROR_INFORMATION,
                             "JLCVP: NOT YET IMPLEMENTED!");
      return;
    }
  }
}
...
void Game::playerCoinTransfer(uint32_t playerId,
  ...
  if (!sender) {
    ...
    if (sender->getAccount() ==
        receiver->getAccount()) { // sender and receiver are the same
      ...
      receiver_account.RegisterCoinsTransaction(account::COIN_REMOVE, amount,
                                                message.str());

      sender_account.GetCoins(&sender_coins);
      message.str("");
      message << "You have successfully transfered " << amount << " coins to "
              << receiverName << ".";
...
void Game::playerStoreTransactionHistory(uint32_t playerId, uint32_t page) {
  Player *player = getPlayerByID(playerId);
  if (player) {
    HistoryStoreOfferList list =
        IOGameStore::getHistoryEntries(player->getAccount(), page);
    if (!list.empty()) {
      player->sendStoreTrasactionHistory(list, page,
                                         GameStore::HISTORY_ENTRIES_PER_PAGE);
    } else {
      player->sendStoreError(STORE_ERROR_HISTORY,
...
void Game::parsePlayerExtendedOpcode(uint32_t playerId, uint8_t opcode,
  ...
  for (CreatureEvent *creatureEvent :
       player->getCreatureEvents(CREATURE_EVENT_EXTENDED_OPCODE)) {
    creatureEvent->executeExtendedOpcode(player, opcode, buffer);
  }
}

std::vector<Item *> Game::getMarketItemList(uint16_t wareId,
                                            uint16_t sufficientCount,
  ...
  do {
    ...
    for (Item *item : container->getItemList()) {
      ...
      if (itemContainer && (!itemType.isContainer() ||
                            itemContainer->capacity() != itemType.maxItems)) {
        continue;
      }
...
void Game::forceRemoveCondition(uint32_t creatureId,
...
void Game::playerAnswerModalWindow(uint32_t playerId, uint32_t modalWindowId,
  ...
  if (modalWindowId == std::numeric_limits<uint32_t>::max()) {
    if (button == 1) {
      if (choice == SKILL_SWORD || choice == SKILL_AXE ||
          choice == SKILL_CLUB || choice == SKILL_DISTANCE ||
      ...
      player->sendTextMessage(MESSAGE_EVENT_ADVANCE,
                              "Offline training aborted.");
    }

    player->setBedItem(nullptr);
  } else {
    for (auto creatureEvent :
         player->getCreatureEvents(CREATURE_EVENT_MODALWINDOW)) {
...
void Game::internalRemoveItems(std::vector<Item *> itemList, uint32_t amount,
                               bool stackable) {
  if (stackable) {
    for (Item *item : itemList) {
...
bool Game::reload(ReloadTypes_t reloadType) {
  ...
  case RELOAD_TYPE_SCRIPTS: {
    ...
    // lean up the monsters interface, ensuring that after reloading the scripts
    // there is no use of any deallocated memory
    g_scripts().loadScripts("monster", false, true);
    // Reload npc scripts
    g_scripts().loadScripts("npc", false, true);
    // Reload npclib
...
void Game::createLuaItemsOnMap() {
  for (auto const [position, itemId] : mapLuaItemsStored) {
    ...
    if (!item) {
      SPDLOG_WARN("[Game::createLuaItemsOnMap] - Cannot create item with id {}",
                  itemId);
      continue;
    }
    ...
    if (position.x != 0) {
      ...
      if (!tile) {
        SPDLOG_WARN("[Game::createLuaItemsOnMap] - Tile is wrong or not found "
                    "position: {}",
                    position.toString());
        delete item;
        continue;
      ...
      if (g_game().findItemOfType(tile, itemId, false, -1)) {
        SPDLOG_WARN("[Game::createLuaItemsOnMap] - Cannot create item with id "
                    "{} on position {}, item already exists",
