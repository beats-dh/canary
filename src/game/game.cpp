			...
			if (newitemType.stackable) {
				...
				if (ammothing) {
					...
					if (ammoItem->getID() == item->getID()) {
						if (item->getDuration() > 0 || ammoItem->getItemCount() == 100 || ammoItem->getItemCount() == player->getItemTypeCount(ammoItem->getID())) {
				...
				if (item->getItemCount() < 100 && item->getItemCount() < player->getItemTypeCount(item->getID(), -1) && item->getDuration() <= 0) {
					uint16_t count = 0;
...
void Game::playerMoveCreature(Player* player, Creature* movingCreature,
	...
	if (!player->canDoAction()) {
		uint32_t delay = 600;
		SchedulerTask* task = createSchedulerTask(
			delay, std::bind(&Game::playerMoveCreatureByID, this, player->getID(), movingCreature->getID(), movingCreatureOrigPos, toTile->getPosition()));
	...
	if (!Position::areInRange<1, 1, 0>(movingCreatureOrigPos,
		...
		if (player->getPathTo(movingCreatureOrigPos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(
				600, std::bind(&Game::playerMoveCreatureByID, this, player->getID(), movingCreature->getID(), movingCreatureOrigPos, toTile->getPosition()));
	...
	if (!isFamiliar && ((!movingCreature->isPushable() && !player->hasFlag(PlayerFlag_CanPushAllCreatures)) || (movingCreature->isInGhostMode() && !player->isAccessPlayer()))) {
...
void Game::playerMoveItem(Player* player, const Position& fromPos,
	...
	if (!player->canDoAction()) {
		...
		SchedulerTask* task = createSchedulerTask(
			delay, std::bind(&Game::playerMoveItemByPlayerID, this, player->getID(), fromPos, itemId, fromStackPos, toPos, count));
	...
	if (!Position::areInRange<1, 1>(playerPos, mapFromPos)) {
		...
		if (player->getPathTo(item->getPosition(), listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(
				400, std::bind(&Game::playerMoveItemByPlayerID, this, player->getID(), fromPos, itemId, fromStackPos, toPos, count));
	...
	if (toCylinder->getContainer() != NULL && toCylinder->getItem()->getID() == ITEM_LOCKER && toPos.getZ() == ITEM_SUPPLY_STASH_INDEX) {
...
ReturnValue Game::internalMoveItem(Cylinder* fromCylinder, Cylinder* toCylinder,
	...
	if (quiver && quiver->isQuiver() && quiver->getHoldingPlayer() && quiver->getHoldingPlayer()->getThing(CONST_SLOT_RIGHT) == quiver) {
...
void Game::playerUseWithCreature(uint32_t playerId, const Position& fromPos,
	...
	if (ret != RETURNVALUE_NOERROR) {
		if (ret == RETURNVALUE_TOOFARAWAY) {
			...
			if (player->getPathTo(walkToPos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(
					400, std::bind(&Game::playerUseWithCreature, this, playerId, itemPos, itemStackPos, creatureId, itemId));
				if (it.isRune() || it.type == ITEM_TYPE_POTION) {
					player->setNextPotionActionTask(task);
				} else {
					player->setNextWalkActionTask(task);
	...
	if (!canDoAction) {
		...
		SchedulerTask* task = createSchedulerTask(
			delay, std::bind(&Game::playerUseWithCreature, this, playerId, fromPos, fromStackPos, creatureId, itemId));
...
void Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid) {
	...
	if (!parentContainer) {
		...
		if (it == browseFields.end()) {
			...
			g_scheduler().addEvent(
				createSchedulerTask(30000, std::bind(&Game::decreaseBrowseFieldRef, this, tile->getPosition())));
		} else {
...
void Game::playerRequestTrade(uint32_t playerId, const Position& pos,
	...
	if (!Position::areInRange<1, 1>(tradeItemPosition, playerPosition)) {
		...
		if (player->getPathTo(pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(
				400, std::bind(&Game::playerRequestTrade, this, playerId, pos, stackPos, tradePlayerId, itemId));
			player->setNextWalkActionTask(task);
		} else {
			player->sendCancelMessage(RETURNVALUE_THEREISNOWAY);
		}
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker,
	...
	if (damage.secondary.type != COMBAT_NONE) {
		...
		secondaryBlockType = target->blockHit(attacker, damage.secondary.type,
	...
	if (damage.primary.value > 0) {
		...
		if (!damage.extension && attackerMonster && targetPlayer) {
			...
			if (charmRune_t activeCharm = g_iobestiary().getCharmFromTarget(
					targetPlayer, g_monsters().getMonsterTypeByRaceId(attackerMonster->getRaceId()));
		...
		if (attackerPlayer) {
			...
			if (!damage.extension && targetMonster) {
				if (charmRune_t activeCharm = g_iobestiary().getCharmFromTarget(
						attackerPlayer, g_monsters().getMonsterTypeByRaceId(targetMonster->getRaceId()));
	...
	if (player) {
		...
		if (offer->type == ITEM || offer->type == STACKABLE_ITEM || offer->type == WRAP_ITEM) {
				...
				while (pendingCount > 0) {
					...
					if (offer->type == WRAP_ITEM) {
						...
						item->setSpecialDescription(
							"Unwrap it in your own house to create a <" + Item::items[tmp->productId].name + ">.");
					} else {
			...
			for (auto outfit :
				...
				const Outfit* opositeSexOutfit = Outfits::getInstance().getOpositeSexOutfitByLookType(
