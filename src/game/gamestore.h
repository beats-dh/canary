...
public:
	...
	static void startup() { HISTORY_ENTRIES_PER_PAGE = 16; }
	...
	uint16_t getCategoryCount() { return (uint16_t)storeCategoryOffers.size(); }
...
class IOGameStore {
public:
	static HistoryStoreOfferList getHistoryEntries(uint32_t account_id,
		uint32_t page);
};
