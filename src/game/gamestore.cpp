...
bool GameStore::loadFromXml() {
  if (isLoaded()) {
    ...
    if (!result) {
      printXMLError("Error - GameStore::loadFromXml", "data/XML/gamestore.xml",
                    result);
      return false;
    }

    for (auto categoryNode :
         doc.child("gamestore").children()) { // category iterator
      ...
      if (!cat->name.length()) {
        printXMLError(
            "Error parsing XML category name  - GameStore::loadFromXml",
            "data/XML/gamestore.xml", result);
        return false;
      }

      std::string state = categoryNode.attribute("state").as_string("normal");
      if (boost::iequals(state,
                         "normal")) { // reading state (defaults to normal)
      ...
      cat->icons = getIconsVector(
          categoryNode.attribute("icons").as_string("default.png"));
      ...
      for (auto offerNode : categoryNode.children()) {
        ...
        if (boost::iequals(type, "namechange")) {
          ...
          tmp->type = OUTFIT;
          tmp->maleLookType =
              (uint16_t)offerNode.attribute("malelooktype").as_uint();
          tmp->femaleLookType =
              (uint16_t)offerNode.attribute("femalelooktype").as_uint();
          tmp->addonNumber = (uint8_t)offerNode.attribute("addon").as_uint(0);
          if (!tmp->femaleLookType || !tmp->maleLookType ||
              tmp->addonNumber > 3) {
            printXMLError(
                "Error parsing XML outfit offer  - GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          } else {
            offer = tmp;
          ...
          tmp->femaleLookType =
              (uint16_t)offerNode.attribute("femalelooktype").as_uint();
          tmp->addonNumber = (uint8_t)offerNode.attribute("addon").as_uint(0);
          if (!tmp->femaleLookType || !tmp->maleLookType || !tmp->addonNumber ||
              tmp->addonNumber > 3) {
            printXMLError(
                "Error parsing XML addon offer - GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          } else {
            offer = tmp;
          ...
          if (!tmp->mountId) {
            printXMLError("Error parsing XML mountID number not specified for "
                          "an mount offer - GameStore::loadFromXml",
                          "data/XML/gamestore.xml", result);
            return false;
          } else {
            offer = tmp;
          ...
          if (!tmp->productId || !tmp->count) {
            printXMLError(
                "Error parsing XML Item Offer - GameStore::loadFromXml",
                "data/XML/gamestore.xml", result);
            return false;
          } else {
            offer = tmp;
          ...
          if (!tmp->productId || !tmp->count) {
            printXMLError("Error parsing XML Stackable Item Offer - "
          ...
          tmp->count = (uint16_t)offerNode.attribute("count").as_uint();
          if (!tmp->productId || !tmp->count) {
            printXMLError("Error parsing XML Wrappable Item Offer - "
                          "GameStore::loadFromXml",
                          "data/XML/gamestore.xml", result);
            return false;
          } else {
          ...
          tmp->blessings =
              getIntVector(offerNode.attribute("blessnumber").as_string());
          tmp->type = BLESSING;
        ...
        if (!offer) {
          printXMLError(
              "Error parsing XML invalid offer type - GameStore::loadFromXml",
              "data/XML/gamestore.xml", result);
          return false;
        } else {
          ...
          offer->icons = getIconsVector(
              offerNode.attribute("icons").as_string("default.png"));

          std::string offerstate =
              categoryNode.attribute("state").as_string("normal");

          if (boost::iequals(offerstate,
                             "normal")) { // reading state (defaults to normal)
            ...
            //  TODO: Solve the client crash with sale offers. Probably we need
            //  to add the previous price to show the strikethrough text
            //  indicating a sale.
            offer->state = StoreState_t::NORMAL;
          ...
          if (!offer->name.length() || !offer->price) {
            printXMLError("Error parsing XML - "
...
HistoryStoreOfferList IOGameStore::getHistoryEntries(uint32_t account_id,
                                                     uint32_t page) {
  HistoryStoreOfferList historyStoreOfferList;

  std::ostringstream query;

  query << "SELECT `description`,`mode`,`coin_amount`,`time` FROM "
           "`store_history` WHERE `account_id` = "
        << account_id << " ORDER BY `time` DESC LIMIT "
