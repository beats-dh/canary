...
bool EventsScheduler::loadScheduleEventFromXml() const {
  ...
  if (pugi::xml_parse_result result = doc.load_file("data/XML/events.xml");
      !result) {
    printXMLError("Error - EventsScheduler::loadScheduleEventFromXml",
                  "data/XML/events.xml", result);
    consoleHandlerExit();
    return false;
  }
  ...
  int daysMath = ((timePtr->tm_year + 1900) * 365) +
                 ((timePtr->tm_mon + 1) * 30) + (timePtr->tm_mday);
  ...
  for (auto schedNode : doc.child("events").children()) {
    ...
    if ((attr = schedNode.attribute("script")) &&
        (!(g_scripts().loadEventSchedulerScripts(attr.as_string())))) {
    ...
    for (auto schedENode : schedNode.children()) {
      if ((schedENode.attribute("exprate"))) {
        uint16_t exprate =
            pugi::cast<uint16_t>(schedENode.attribute("exprate").value());
      ...
      if ((schedENode.attribute("lootrate"))) {
        uint16_t lootrate =
            pugi::cast<uint16_t>(schedENode.attribute("lootrate").value());
      ...
      if ((schedENode.attribute("spawnrate"))) {
        uint32_t spawnrate =
            pugi::cast<uint32_t>(schedENode.attribute("spawnrate").value());
      ...
      if ((schedENode.attribute("skillrate"))) {
        uint16_t skillrate =
            pugi::cast<uint16_t>(schedENode.attribute("skillrate").value());
