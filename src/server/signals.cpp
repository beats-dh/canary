...
Signals::Signals(boost::asio::io_service& service)
	...
	set.add(SIGHUP);
#else
	// This must be a blocking call as Windows calls it in a new thread and
	// terminates the process when the handler returns (or after 5 seconds,
	// whichever is earlier). On Windows it is called in a new thread.
