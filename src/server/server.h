...
template <typename ProtocolType> class Service final : public ServiceBase {
public:
...
public:
  ...
  static void openAcceptor(std::weak_ptr<ServicePort> weak_service,
                           uint16_t port);
...
public:
  ...
  template <typename ProtocolType> bool add(uint16_t port);
...
template <typename ProtocolType> bool ServiceManager::add(uint16_t port) {
  if (port == 0) {
  ...
  if (foundServicePort == acceptors.end()) {
    ...
    if (service_port->is_single_socket() || ProtocolType::SERVER_SENDS_FIRST) {
      SPDLOG_ERROR("[ServiceManager::add] - "
                   ...
                   ProtocolType::protocol_name(),
                   service_port->get_protocol_names(), port);
      return false;
    }
  }
