...
void NetworkMessage::addString(const std::string& value) {
	...
	if (!canAdd(stringLen + 2)) {
		SPDLOG_ERROR(
			"[NetworkMessage::addString] - NetworkMessage size is wrong: {}",
			stringLen);
		return;
	}
	if (stringLen > NETWORKMESSAGE_MAXSIZE) {
		SPDLOG_ERROR("[NetworkMessage::addString] - Exceded NetworkMessage max "
					 "size: {}, actually size: {}",
			NETWORKMESSAGE_MAXSIZE, stringLen);
		return;
	}
...
void NetworkMessage::addBytes(const char* bytes, size_t size) {
	...
	if (!canAdd(size)) {
		SPDLOG_ERROR(
			"[NetworkMessage::addBytes] - NetworkMessage size is wrong: {}", size);
		return;
	}
	if (size > NETWORKMESSAGE_MAXSIZE) {
		SPDLOG_ERROR("[NetworkMessage::addBytes] - Exceded NetworkMessage max "
					 "size: {}, actually size: {}",
			NETWORKMESSAGE_MAXSIZE, size);
		return;
	}
