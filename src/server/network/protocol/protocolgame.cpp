...
void ProtocolGame::logout(bool displayEffect, bool forced)
{
	...
	if (player->getProtocolVersion() >= 1272) {
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage &msg)
{
	...
	bool allowClientOld = g_config.getBoolean(ALLOW_CLIENT_OLD);
	if (clientVersion != g_config.getNumber(CLIENT_VERSION) && (allowClientOld && version != 1100))
	{
		std::ostringstream ss;
		ss << "Only clients with protocol " << g_config.getString(CLIENT_VERSION_STR);
		if (allowClientOld)
			ss << " and 10.00";
			ss << " allowed!";
			disconnectClient(ss.str());
