...
void ProtocolGame::addGameTask(Callable function, Args &&...args) {
  g_dispatcher().addTask(
      createTask(std::bind(function, &g_game(), std::forward<Args>(args)...)));
}

template <typename Callable, typename... Args>
void ProtocolGame::addGameTaskTimed(uint32_t delay, Callable function,
                                    Args &&...args) {
  g_dispatcher().addTask(createTask(
      delay, std::bind(function, &g_game(), std::forward<Args>(args)...)));
}
...
void ProtocolGame::AddItem(NetworkMessage &msg, const Item *item) {
  ...
  if (it.stackable) {
    ...
    // Quiver ammo count
    if (container && item->isQuiver() &&
        player->getThing(CONST_SLOT_RIGHT) == item) {
  ...
  if (it.isPodium) {
    const ItemAttributes::CustomAttribute *podiumVisible =
        item->getCustomAttribute("PodiumVisible");
    const ItemAttributes::CustomAttribute *lookType =
        item->getCustomAttribute("LookType");
    const ItemAttributes::CustomAttribute *lookMount =
        item->getCustomAttribute("LookMount");
    const ItemAttributes::CustomAttribute *lookDirection =
        item->getCustomAttribute("LookDirection");

    if (lookType) {
      uint16_t look =
          static_cast<uint16_t>(boost::get<int64_t>(lookType->value));
      msg.add<uint16_t>(look);

      if (look != 0) {
        const ItemAttributes::CustomAttribute *lookHead =
            item->getCustomAttribute("LookHead");
        const ItemAttributes::CustomAttribute *lookBody =
            item->getCustomAttribute("LookBody");
        const ItemAttributes::CustomAttribute *lookLegs =
            item->getCustomAttribute("LookLegs");
        const ItemAttributes::CustomAttribute *lookFeet =
            item->getCustomAttribute("LookFeet");

        msg.addByte(lookHead ? static_cast<uint8_t>(
                                   boost::get<int64_t>(lookHead->value))
                             : 0);
        msg.addByte(lookBody ? static_cast<uint8_t>(
                                   boost::get<int64_t>(lookBody->value))
                             : 0);
        msg.addByte(lookLegs ? static_cast<uint8_t>(
                                   boost::get<int64_t>(lookLegs->value))
                             : 0);
        msg.addByte(lookFeet ? static_cast<uint8_t>(
                                   boost::get<int64_t>(lookFeet->value))
                             : 0);

        const ItemAttributes::CustomAttribute *lookAddons =
            item->getCustomAttribute("LookAddons");
        msg.addByte(lookAddons ? static_cast<uint8_t>(
                                     boost::get<int64_t>(lookAddons->value))
                               : 0);
      }
    } else {
      msg.add<uint16_t>(0);
    ...
    if (lookMount) {
      uint16_t look =
          static_cast<uint16_t>(boost::get<int64_t>(lookMount->value));
      msg.add<uint16_t>(look);

      if (look != 0) {
        const ItemAttributes::CustomAttribute *lookHead =
            item->getCustomAttribute("LookMountHead");
        const ItemAttributes::CustomAttribute *lookBody =
            item->getCustomAttribute("LookMountBody");
        const ItemAttributes::CustomAttribute *lookLegs =
            item->getCustomAttribute("LookMountLegs");
        const ItemAttributes::CustomAttribute *lookFeet =
            item->getCustomAttribute("LookMountFeet");
    ...
    msg.addByte(lookDirection ? static_cast<uint8_t>(
                                    boost::get<int64_t>(lookDirection->value))
                              : 2);
    msg.addByte(podiumVisible ? static_cast<uint8_t>(
                                    boost::get<int64_t>(podiumVisible->value))
                              : 0x01);
  }
  if (it.upgradeClassification > 0) {
    msg.addByte(0);
...
void ProtocolGame::release() {
  ...
  OutputMessagePool::getInstance().removeProtocolFromAutosend(
      shared_from_this());
  Protocol::release();
}
  ...
  if (!foundPlayer || g_configManager().getBoolean(ALLOW_CLONES)) {
    ...
    if (g_configManager().getBoolean(ONLY_PREMIUM_ACCOUNT) &&
      ...
      disconnectClient("Your premium time for this account is out.\n\nTo play "
                       "please buy additional premium time from our website");
      return;
    }
    ...
    if (!player->hasFlag(PlayerFlag_CannotBeBanned)) {
      ...
      if (IOBan::isAccountBanned(accountId, banInfo)) {
        ...
        if (banInfo.expiresAt > 0) {
          ss << "Your account has been banned until "
             << formatDateShort(banInfo.expiresAt) << " by " << banInfo.bannedBy
             << ".\n\nReason specified:\n"
             << banInfo.reason;
        } else {
          ss << "Your account has been permanently banned by "
             << banInfo.bannedBy << ".\n\nReason specified:\n"
             << banInfo.reason;
        }
    ...
    if (!g_game().placeCreature(player, player->getLoginPosition()) &&
        !g_game().placeCreature(player, player->getTemplePosition(), false,
                                true)) {
      disconnectClient(
    ...
    player->lastLoginSaved =
        std::max<time_t>(time(nullptr), player->lastLoginSaved + 1);
    acceptPackets = true;
  } else {
    if (eventConnect != 0 ||
        !g_configManager().getBoolean(REPLACE_KICK_ON_LOGIN)) {
    ...
    if (foundPlayer->client) {
      ...
      eventConnect = g_scheduler().addEvent(createSchedulerTask(
          1000, std::bind(&ProtocolGame::connect, getThis(),
                          foundPlayer->getID(), operatingSystem)));
    } else {
...
void ProtocolGame::connect(uint32_t playerId,
  ...
  if (isConnectionExpired()) {
    // ProtocolGame::release() has been called at this point and the Connection
    // object no longer exists, so we return to prevent leakage of the Player.
    return;
  }
...
void ProtocolGame::logout(bool displayEffect, bool forced) {
  ...
  displayEffect = displayEffect && !player->isRemoved() &&
                  player->getHealth() > 0 && !player->isInGhostMode();
  if (displayEffect) {
    g_game().addMagicEffect(player->getPosition(), CONST_ME_POFF);
  }

  sendSessionEndInformation(forced ? SESSION_END_FORCECLOSE
                                   : SESSION_END_LOGOUT);
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage &msg) {
  ...
  OperatingSystem_t operatingSystem =
      static_cast<OperatingSystem_t>(msg.get<uint16_t>());
  ...
  std::array<uint32_t, 4> key = {msg.get<uint32_t>(), msg.get<uint32_t>(),
                                 msg.get<uint32_t>(), msg.get<uint32_t>()};
  ...
  if (clientVersion != CLIENT_VERSION) {
    std::ostringstream ss;
    ss << "Only clients with protocol " << CLIENT_VERSION_UPPER << "."
       << CLIENT_VERSION_LOWER << " allowed!";
    disconnectClient(ss.str());
    return;
  }
  ...
  uint32_t accountId;
  if (!IOLoginData::gameWorldAuthentication(email, password, characterName,
                                            &accountId)) {
  ...
  g_dispatcher().addTask(
      createTask(std::bind(&ProtocolGame::login, getThis(), characterName,
...
void ProtocolGame::onConnect() {
  ...
  output->add<uint32_t>(
      adlerChecksum(output->getOutputBuffer() + sizeof(uint32_t), 8));
...
void ProtocolGame::parsePacket(NetworkMessage &msg) {
  ...
  if (player->isDead() || player->getHealth() <= 0) {
    ...
    if (recvbyte == 0x0F) {
      ...
      if (player->getLevel() <
          g_configManager().getNumber(ADVENTURERSBLESSING_LEVEL)) {
    ...
    if (recvbyte != 0x1D && recvbyte != 0x1E) {
      ...
      g_scheduler().addEvent(createSchedulerTask(
          500, std::bind(&ProtocolGame::sendPing, getThis())));
      g_scheduler().addEvent(createSchedulerTask(
          1000, std::bind(&ProtocolGame::sendPingBack, getThis())));
      return;
    }
  }
  ...
  if (player && recvbyte != 0xD3) {
    g_dispatcher().addTask(
        createTask(std::bind(&Modules::executeOnRecvbyte, &g_modules(),
                             player->getID(), msg, recvbyte)));
  }

  g_dispatcher().addTask(createTask(std::bind(
      &ProtocolGame::parsePacketFromDispatcher, getThis(), msg, recvbyte)));
}

void ProtocolGame::parsePacketFromDispatcher(NetworkMessage msg,
                                             uint8_t recvbyte) {
  if (!acceptPackets || g_game().getGameState() == GAME_STATE_SHUTDOWN) {
    return;
  }
  ...
  case 0x14:
    g_dispatcher().addTask(
        createTask(std::bind(&ProtocolGame::logout, getThis(), true, false)));
    break;
  case 0x1D:
  ...
  case 0x6D:
    ...
    break;
  case 0x6F:
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerTurn,
                     player->getID(), DIRECTION_NORTH);
    break;
  case 0x70:
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerTurn,
                     player->getID(), DIRECTION_EAST);
    break;
  case 0x71:
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerTurn,
                     player->getID(), DIRECTION_SOUTH);
    break;
  case 0x72:
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerTurn,
                     player->getID(), DIRECTION_WEST);
    break;
  case 0x73:
    parseTeleport(msg);
    break;
  ...
  case 0xD2:
    ...
    break;
  // g_dispatcher().addTask(createTask(std::bind(&Modules::executeOnRecvbyte,
  // g_modules, player, msg, recvbyte)));
  case 0xD3:
    g_dispatcher().addTask(
        createTask(std::bind(&ProtocolGame::parseSetOutfit, getThis(), msg)));
    break;
  case 0xD4:
  ...
  case 0xF0:
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerShowQuestLog,
                     player->getID());
    break;
  case 0xF1:
    parseQuestLine(msg);
...
void ProtocolGame::GetFloorDescription(NetworkMessage &msg, int32_t x,
                                       int32_t y, int32_t z, int32_t width,
                                       int32_t height, int32_t offset,
                                       int32_t &skip) {
  for (int32_t nx = 0; nx < width; nx++) {
    for (int32_t ny = 0; ny < height; ny++) {
      const Tile *tile = g_game().map.getTile(
          static_cast<uint16_t>(x + nx + offset),
...
void ProtocolGame::checkCreatureAsKnown(uint32_t id, bool &known,
                                        uint32_t &removedKnown) {
  ...
  if (knownCreatureSet.size() > 1300) {
    ...
    for (auto it = knownCreatureSet.begin(), end = knownCreatureSet.end();
         it != end; ++it) {
      if (*it == id) {
        continue;
      }
      ...
      if (const Player * checkPlayer;
        ...
        if (player->getParty() != checkPlayer->getParty() &&
            !canSee(creature)) {
          removedKnown = *it;
...
bool ProtocolGame::canSee(int32_t x, int32_t y, int32_t z) const {
  ...
  // negative offset means that the action taken place is on a lower floor than
  // ourself
...
void ProtocolGame::parseAutoWalk(NetworkMessage &msg) {
  ...
  if (numdirs == 0 ||
      (msg.getBufferPosition() + numdirs) != (msg.getLength() + 8)) {
    return;
  }
...
void ProtocolGame::parseSetOutfit(NetworkMessage &msg) {
  ...
  if (msg.getBufferPosition() == startBufferPosition) {
    ...
    if (outfitType == 0) {
      ...
      // This value probably has something to do with try outfit variable inside
      // outfit window dialog if try outfit is set to 2 it expects uint32_t
      // value after mounted and disable mounts from outfit window dialog
      ...
      newOutfit.lookMountLegs = std::min<uint8_t>(132, msg.getByte());
      newOutfit.lookMountFeet = std::min<uint8_t>(132, msg.getByte());
      uint8_t direction = std::max<uint8_t>(
          DIRECTION_NORTH, std::min<uint8_t>(DIRECTION_WEST, msg.getByte()));
      uint8_t podiumVisible = msg.getByte();
      g_game().playerSetShowOffSocket(player->getID(), newOutfit, pos, stackpos,
                                      itemId, podiumVisible, direction);
    }
  }
}
...
void ProtocolGame::parseApplyImbuement(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerApplyImbuement, player->getID(), imbuementId, slot,
              protectionCharm);
}
...
void ProtocolGame::parseUseItem(NetworkMessage &msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseItem,
                   player->getID(), pos, stackpos, index, itemId);
}
...
void ProtocolGame::parseUseItemEx(NetworkMessage &msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseItemEx,
                   player->getID(), fromPos, fromStackPos, fromItemId, toPos,
                   toStackPos, toItemId);
}
...
void ProtocolGame::parseUseWithCreature(NetworkMessage &msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerUseWithCreature,
                   player->getID(), fromPos, fromStackPos, creatureId, itemId);
}
...
void ProtocolGame::parseThrow(NetworkMessage &msg) {
  ...
  if (toPos != fromPos) {
    addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerMoveThing,
                     player->getID(), fromPos, itemId, fromStackpos, toPos,
                     count);
  }
}
...
void ProtocolGame::parseLookAt(NetworkMessage &msg) {
  ...
  uint16_t itemId = msg.get<uint16_t>();
  uint8_t stackpos = msg.getByte();
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerLookAt,
                   player->getID(), itemId, pos, stackpos);
}
...
void ProtocolGame::parseQuickLoot(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerQuickLoot, player->getID(), pos, itemId, stackpos,
              nullptr, lootAllCorpses, autoLoot);
}
...
void ProtocolGame::parseLootContainer(NetworkMessage &msg) {
  ...
  if (action == 0) {
    ...
    uint16_t itemId = msg.get<uint16_t>();
    uint8_t stackpos = msg.getByte();
    addGameTask(&Game::playerSetLootContainer, player->getID(), category, pos,
                itemId, stackpos);
  } else if (action == 1) {
    ObjectCategory_t category = (ObjectCategory_t)msg.getByte();
    ...
    addGameTask(&Game::playerSetQuickLootFallback, player->getID(),
                useMainAsFallback);
  }
}
...
void ProtocolGame::parseQuickLootBlackWhitelist(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerQuickLootBlackWhitelist, player->getID(), filter,
              listedItems);
}
...
void ProtocolGame::parseSay(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerSay, player->getID(), channelId, type, receiver,
              text);
}

void ProtocolGame::parseFightModes(NetworkMessage &msg) {
  uint8_t rawFightMode =
      msg.getByte(); // 1 - offensive, 2 - balanced, 3 - defensive
  uint8_t rawChaseMode =
      msg.getByte(); // 0 - stand while fightning, 1 - chase opponent
  uint8_t rawSecureMode =
      msg.getByte(); // 0 - can't attack unmarked, 1 - can attack unmarked
  ...
  addGameTask(&Game::playerSetFightModes, player->getID(), fightMode,
              rawChaseMode != 0, rawSecureMode != 0);
}
...
void ProtocolGame::parseHouseWindow(NetworkMessage &msg) {
  ...
  const std::string text = msg.getString();
  addGameTask(&Game::playerUpdateHouseWindow, player->getID(), doorId, id,
              text);
}
...
void ProtocolGame::parseLookInShop(NetworkMessage &msg) {
  ...
  uint8_t count = msg.getByte();
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerLookInShop,
                   player->getID(), id, count);
}
...
void ProtocolGame::parsePlayerBuyOnShop(NetworkMessage &msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerBuyItem,
                   player->getID(), id, count, amount, ignoreCap, inBackpacks);
}
...
void ProtocolGame::parsePlayerSellOnShop(NetworkMessage &msg) {
  ...
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerSellItem,
                   player->getID(), id, count, amount, ignoreEquipped);
}
...
void ProtocolGame::parseRequestTrade(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerRequestTrade, player->getID(), pos, stackpos,
              playerId, itemId);
}
...
void ProtocolGame::parseLookInTrade(NetworkMessage &msg) {
  ...
  uint8_t index = msg.getByte();
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerLookInTrade,
                   player->getID(), counterOffer, index);
}
...
void ProtocolGame::parseEditVip(NetworkMessage &msg) {
  ...
  uint32_t icon =
      std::min<uint32_t>(10, msg.get<uint32_t>()); // 10 is max icon in 9.63
  bool notify = msg.getByte() != 0;
  addGameTask(&Game::playerRequestEditVip, player->getID(), guid, description,
              icon, notify);
}
...
void ProtocolGame::parseWrapableItem(NetworkMessage &msg) {
  ...
  uint16_t itemId = msg.get<uint16_t>();
  uint8_t stackpos = msg.getByte();
  addGameTaskTimed(DISPATCHER_TASK_EXPIRATION, &Game::playerWrapableItem,
                   player->getID(), pos, stackpos, itemId);
}
...
void ProtocolGame::parseInspectionObject(NetworkMessage &msg) {
  ...
  } else if (inspectionType == INSPECT_NPCTRADE ||
             inspectionType == INSPECT_CYCLOPEDIA) {
    uint16_t itemId = msg.get<uint16_t>();
    uint16_t itemCount = msg.getByte();
    g_game().playerInspectItem(player, itemId, static_cast<int8_t>(itemCount),
                               (inspectionType == INSPECT_CYCLOPEDIA));
  }
}

void ProtocolGame::sendSessionEndInformation(
    SessionEndInformations information) {
  auto output = OutputMessagePool::getOutputMessage();
  output->addByte(0x18);
...
void ProtocolGame::sendItemInspection(uint16_t itemId, uint8_t itemCount,
...
void ProtocolGame::parseCyclopediaCharacterInfo(NetworkMessage &msg) {
  ...
  if (characterInfoType == CYCLOPEDIA_CHARACTERINFO_RECENTDEATHS ||
      characterInfoType == CYCLOPEDIA_CHARACTERINFO_RECENTPVPKILLS) {
    entriesPerPage =
        std::min<uint16_t>(30, std::max<uint16_t>(5, msg.get<uint16_t>()));
    page = std::max<uint16_t>(1, msg.get<uint16_t>());
  }
  if (characterID == 0) {
    characterID = player->getGUID();
  }
  g_game().playerCyclopediaCharacterInfo(player, characterID, characterInfoType,
                                         entriesPerPage, page);
}
...
void ProtocolGame::parseHighscores(NetworkMessage &msg) {
  ...
  uint8_t entriesPerPage =
      std::min<uint8_t>(30, std::max<uint8_t>(5, msg.getByte()));
  g_game().playerHighscores(player, type, category, vocation, worldName, page,
                            entriesPerPage);
}
...
void ProtocolGame::parseTaskHuntingAction(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerTaskHuntingAction, player->getID(), slot, action,
              upgrade, raceId);
}
...
void ProtocolGame::sendHighscores(
    const std::vector<HighscoreCharacter> &characters, uint8_t categoryId,
    uint32_t vocationId, uint16_t page, uint16_t pages) {
  NetworkMessage msg;
  msg.addByte(0xB1);
  ...
  msg.addByte(sizeof(highscoreCategories) /
              sizeof(HighscoreCategory)); // Category Count
  ...
  for (const HighscoreCharacter &character : characters) {
    ...
    msg.addByte(
        (player->getGUID() == character.id)); // Player Indicator Boolean
...
void ProtocolGame::parseTournamentLeaderboard(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerTournamentLeaderboard, player->getID(),
              ledaerboardType);
}
...
void ProtocolGame::parseRuleViolationReport(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerReportRuleViolationReport, player->getID(),
              targetName, reportType, reportReason, comment, translation);
}
...
void ProtocolGame::parseBestiarysendRaces() {
  ...
  for (uint8_t i = BESTY_RACE_FIRST; i <= BESTY_RACE_LAST; i++) {
    ...
    uint16_t unlockedCount = g_iobestiary().getBestiaryRaceUnlocked(
        player, static_cast<BestiaryType_t>(i));
...
void ProtocolGame::parseBestiarysendMonsterData(NetworkMessage &msg) {
  ...
  if (currentLevel > 2) {
    std::map<uint8_t, int16_t> elements =
        g_iobestiary().getMonsterElements(mtype);

    newmsg.addByte(elements.size());
    for (auto it = std::begin(elements), end = std::end(elements); it != end;
         it++) {
...
void ProtocolGame::sendTeamFinderList() {
  ...
  msg.addByte(0x2D);
  msg.addByte(
      0x00); // Bool value, with 'true' the player exceed packets for second.
  ...
  for (auto it : teamFinder) {
    ...
    msg.add<uint16_t>(std::max<uint16_t>(
        (teamAssemble->teamSlots - teamAssemble->freeSlots), membersSize));
    // The leader does not count on this math, he is included inside the
    // 'freeSlots'.
...
void ProtocolGame::parsePartyAnalyzerAction(NetworkMessage &msg) const {
  ...
  if (!party || !party->getLeader() ||
      party->getLeader()->getID() != player->getID()) {
    return;
  }

  PartyAnalyzerAction_t action =
      static_cast<PartyAnalyzerAction_t>(msg.getByte());
...
void ProtocolGame::parseLeaderFinderWindow(NetworkMessage &msg) {
  ...
  case 2: {
    ...
    case 3: {
      member->sendTextMessage(MESSAGE_STATUS,
                              "Your team finder request was accepted.");
      break;
    }
    case 4: {
      member->sendTextMessage(MESSAGE_STATUS,
                              "Your team finder request was denied.");
      break;
    }
...
void ProtocolGame::parseMemberFinderWindow(NetworkMessage &msg) {
  ...
  if (action == 0) {
    ...
    if (action == 1) {
      leader->sendTextMessage(MESSAGE_STATUS,
                              "There is a new request to join your team.");
      teamAssemble->membersMap.insert({player->getGUID(), 1});
    } else {
      for (auto itt = teamAssemble->membersMap.begin(),
                end = teamAssemble->membersMap.end();
           itt != end; ++itt) {
        if (itt->first == player->getGUID()) {
...
void ProtocolGame::refreshBestiaryTracker(
    std::list<MonsterType *> trackerList) {
  NetworkMessage msg;
  msg.addByte(0xB9);
...
void ProtocolGame::BestiarysendCharms() {
  ...
  for (Charm *c_type : charmList) {
    ...
    if (g_iobestiary().hasCharmUnlockedRuneBit(c_type,
                                               player->getUnlockedRunesBit())) {
      msg.addByte(1);
  ...
  std::list<uint16_t> finishedMonsters =
      g_iobestiary().getBestiaryFinished(player);
  std::list<charmRune_t> usedRunes =
      g_iobestiary().getCharmUsedRuneBitAll(player);
...
void ProtocolGame::parseBestiarysendCreatures(NetworkMessage &msg) {
  ...
  if (search == 1) {
    ...
    std::map<uint16_t, std::string> mtype_list = g_game().getBestiaryList();
    for (uint16_t monsterCount = 1; monsterCount <= monsterAmount;
         monsterCount++) {
      uint16_t raceid = msg.get<uint16_t>();
  ...
  std::map<uint16_t, uint32_t> creaturesKilled =
      g_iobestiary().getBestiaryKillCountByMonsterIDs(player, race);
...
void ProtocolGame::parseDebugAssert(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerDebugAssert, player->getID(), assertLine, date,
              description, comment);
}
...
void ProtocolGame::parsePreyAction(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerPreyAction, player->getID(), slot, action, option,
              index, raceId);
}
...
void ProtocolGame::parseEnableSharedPartyExperience(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerEnableSharedPartyExperience, player->getID(),
              sharedExpActive);
}
...
void ProtocolGame::parseStoreRequestOffers(NetworkMessage &message) {
  ...
  if (index >= 0) {
    ...
    SPDLOG_WARN("[ProtocolGame::parseStoreRequestOffers] - "
                "Requested category: {} doesn't exists",
                categoryName);
  }
}
...
void ProtocolGame::parseStoreBuyOffer(NetworkMessage &message) {
  ...
  uint8_t productType =
      message.getByte(); // used only in return of a namechange offer request
  std::string additionalInfo;
  if (productType == ADDITIONALINFO) {
    additionalInfo = message.getString();
  }
  addGameTaskTimed(350, &Game::playerBuyStoreOffer, player->getID(), offerId,
                   productType, additionalInfo);
}

void ProtocolGame::parseStoreOpenTransactionHistory(NetworkMessage &msg) {
  uint8_t entriesPerPage = msg.getByte();
  if (entriesPerPage > 0 &&
      entriesPerPage != GameStore::HISTORY_ENTRIES_PER_PAGE) {
    GameStore::HISTORY_ENTRIES_PER_PAGE = entriesPerPage;
  }

  addGameTaskTimed(2000, &Game::playerStoreTransactionHistory, player->getID(),
                   1);
}
...
void ProtocolGame::parseCoinTransfer(NetworkMessage &msg) {
  ...
  if (amount > 0) {
    addGameTaskTimed(350, &Game::playerCoinTransfer, player->getID(),
                     receiverName, amount);
  }
...
void ProtocolGame::parseMarketCreateOffer(NetworkMessage &msg) {
  ...
  if (amount > 0 && price > 0) {
    addGameTask(&Game::playerCreateMarketOffer, player->getID(), type, itemId,
                amount, price, anonymous);
  }
}
...
void ProtocolGame::parseMarketCancelOffer(NetworkMessage &msg) {
  ...
  if (counter > 0) {
    addGameTask(&Game::playerCancelMarketOffer, player->getID(), timestamp,
                counter);
  }
...
void ProtocolGame::parseMarketAcceptOffer(NetworkMessage &msg) {
  ...
  if (amount > 0 && counter > 0) {
    addGameTask(&Game::playerAcceptMarketOffer, player->getID(), timestamp,
                counter, amount);
  }
...
void ProtocolGame::parseModalWindowAnswer(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerAnswerModalWindow, player->getID(), id, button,
              choice);
}
...
void ProtocolGame::parseSeekInContainer(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerSeekInContainer, player->getID(), containerId,
              index);
}
...
void ProtocolGame::sendChannelEvent(uint16_t channelId,
...
void ProtocolGame::sendCreatureOutfit(const Creature *creature,
...
void ProtocolGame::sendCreatureWalkthrough(const Creature *creature,
                                           bool walkthrough) {
  if (!canSee(creature)) {
    return;
  }
...
void ProtocolGame::sendCreatureType(const Creature *creature,
...
void ProtocolGame::sendCreatureSquare(const Creature *creature,
                                      SquareColor_t color) {
  if (!canSee(creature)) {
    return;
  }
...
void ProtocolGame::sendAddMarker(const Position &pos, uint8_t markType,
...
void ProtocolGame::sendCyclopediaCharacterNoData(
    CyclopediaCharacterInfoType_t characterInfoType, uint8_t errorCode) {
  NetworkMessage msg;
  msg.addByte(0xDA);
...
void ProtocolGame::sendCyclopediaCharacterGeneralStats() {
  ...
  // canBuyXpBoost
  msg.addByte(0x00);
  msg.add<uint16_t>(std::min<int32_t>(player->getHealth(),
                                      std::numeric_limits<uint16_t>::max()));
  msg.add<uint16_t>(std::min<int32_t>(player->getMaxHealth(),
                                      std::numeric_limits<uint16_t>::max()));
  msg.add<uint16_t>(std::min<int32_t>(player->getMana(),
                                      std::numeric_limits<uint16_t>::max()));
  msg.add<uint16_t>(std::min<int32_t>(player->getMaxMana(),
                                      std::numeric_limits<uint16_t>::max()));
  msg.addByte(player->getSoul());
  msg.add<uint16_t>(player->getStaminaMinutes());

  Condition *condition =
      player->getCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT);
  ...
  for (uint8_t i = SKILL_FIRST; i < SKILL_CRITICAL_HIT_CHANCE; ++i) {
    ...
    msg.add<uint16_t>(std::min<int32_t>(player->getSkillLevel(i),
                                        std::numeric_limits<uint16_t>::max()));
...
void ProtocolGame::sendCyclopediaCharacterCombatStats() {
  ...
  if (weapon) {
    ...
    if (it.weaponType == WEAPON_WAND) {
      ...
      int32_t maxDamage = static_cast<int32_t>(
          Weapons::getMaxWeaponDamage(player->getLevel(), attackSkill,
                                      attackValue, attackFactor, true) *
      ...
      if (it.abilities && it.abilities->elementType != COMBAT_NONE) {
        msg.addByte(static_cast<uint32_t>(it.abilities->elementDamage) * 100 /
                    attackValue);
    ...
    int32_t maxDamage = Weapons::getMaxWeaponDamage(
        player->getLevel(), attackSkill, attackValue, attackFactor, true);
    msg.add<uint16_t>(maxDamage >> 1);
    msg.addByte(CIPBIA_ELEMENTAL_PHYSICAL);
    msg.addByte(0);
    msg.addByte(CIPBIA_ELEMENTAL_UNDEFINED);
  ...
  static const Cipbia_Elementals_t cipbiaCombats[] = {
      CIPBIA_ELEMENTAL_PHYSICAL,  CIPBIA_ELEMENTAL_ENERGY,
      CIPBIA_ELEMENTAL_EARTH,     CIPBIA_ELEMENTAL_FIRE,
      CIPBIA_ELEMENTAL_UNDEFINED, CIPBIA_ELEMENTAL_LIFEDRAIN,
      CIPBIA_ELEMENTAL_UNDEFINED, CIPBIA_ELEMENTAL_HEALING,
      CIPBIA_ELEMENTAL_DROWN,     CIPBIA_ELEMENTAL_ICE,
      CIPBIA_ELEMENTAL_HOLY,      CIPBIA_ELEMENTAL_DEATH};
  for (size_t i = 0; i < COMBAT_COUNT; ++i) {
    if (absorbs[i] != 0) {
...
void ProtocolGame::sendCyclopediaCharacterRecentDeaths(
    ...
    const std::vector<RecentDeathEntry> &entries) {
  NetworkMessage msg;
  msg.addByte(0xDA);
...
void ProtocolGame::sendCyclopediaCharacterOutfitsMounts() {
  ...
  msg.skipBytes(2);
  const auto &familiars =
      Familiars::getInstance().getFamiliars(player->getVocationId());
...
void ProtocolGame::sendCyclopediaCharacterInspection() {
  ...
  msg.skipBytes(1);
  for (std::underlying_type<Slots_t>::type slot = CONST_SLOT_FIRST;
       slot <= CONST_SLOT_LAST; slot++) {
    ...
    if (inventoryItem) {
      ...
      auto descriptions = Item::getDescriptions(
          Item::items[inventoryItem->getID()], inventoryItem);
      msg.addByte(descriptions.size());
      for (const auto &description : descriptions) {
        msg.addString(description.first);
        msg.addString(description.second);
  ...
  const Outfit *outfit = Outfits::getInstance().getOutfitByLookType(
      player->getSex(), player->getDefaultOutfit().lookType);
  if (outfit) {
    msg.addString(outfit->name);
  } else {
...
void ProtocolGame::sendBlessStatus() {
  ...
  NetworkMessage msg;
  // uint8_t maxClientBlessings = (player->operatingSystem ==
  // CLIENTOS_NEW_WINDOWS) ? 8 : 6; (compartability for the client 10) Ignore
  // ToF (bless 1)
  ...
  bool glow = g_configManager().getBoolean(INVENTORY_GLOW) ||
              player->getLevel() <
                  g_configManager().getNumber(ADVENTURERSBLESSING_LEVEL);
...
void ProtocolGame::sendPremiumTrigger() {
  if (!g_configManager().getBoolean(FREE_PREMIUM)) {
    ...
    for (uint16_t i = 0; i <= 15; i++) {
      // PREMIUM_TRIGGER_TRAIN_OFFLINE = false, PREMIUM_TRIGGER_XP_BOOST =
      // false, PREMIUM_TRIGGER_MARKET = false, PREMIUM_TRIGGER_VIP_LIST =
      // false, PREMIUM_TRIGGER_DEPOT_SPACE = false,
      // PREMIUM_TRIGGER_INVITE_PRIVCHAT = false
      msg.addByte(0x01);
    }
    writeToOutputBuffer(msg);
  }
...
void ProtocolGame::sendTextMessage(const TextMessage &message) {
  if (message.type == MESSAGE_NONE) {
    SPDLOG_ERROR("[ProtocolGame::sendTextMessage] - Message type is wrong, "
                 "missing or invalid for player with name {}, on position {}",
                 player->getName(), player->getPosition().toString());
    player->sendTextMessage(MESSAGE_ADMINISTRADOR,
...
void ProtocolGame::sendCreatePrivateChannel(uint16_t channelId,
...
void ProtocolGame::sendChannel(uint16_t channelId,
                               const std::string &channelName,
                               const UsersMap *channelUsers,
...
void ProtocolGame::sendChannelMessage(const std::string &author,
...
void ProtocolGame::sendUnjustifiedPoints(const uint8_t &dayProgress,
                                         const uint8_t &dayLeft,
                                         const uint8_t &weekProgress,
                                         const uint8_t &weekLeft,
                                         const uint8_t &monthProgress,
                                         const uint8_t &monthLeft,
                                         const uint8_t &skullDuration) {
  NetworkMessage msg;
  msg.addByte(0xB7);
...
void ProtocolGame::sendContainer(uint8_t cid, const Container *container,
                                 bool hasParent, uint16_t firstIndex) {
  NetworkMessage msg;
  msg.addByte(0x6E);
  ...
  // Depot search
  msg.addByte(
      (player->isDepotSearchAvailable() && container->isInsideDepot(true))
          ? 0x01
          : 0x00);
  ...
  if (firstIndex >= containerSize) {
    ...
    for (ItemDeque::const_iterator it = itemList.begin() + firstIndex,
                                   end = itemList.end();
         i < maxItemsToSend && it != end; ++it, ++i) {
      AddItem(msg, *it);
    }
  }
  writeToOutputBuffer(msg);
...
void ProtocolGame::sendShop(Npc *npc) {
  ...
  uint16_t itemsToSend =
      std::min<size_t>(shoplist.size(), std::numeric_limits<uint16_t>::max());
...
void ProtocolGame::sendResourcesBalance(uint64_t money /*= 0*/,
                                        uint64_t bank /*= 0*/,
                                        uint64_t preyCards /*= 0*/,
                                        uint64_t taskHunting /*= 0*/) {
...
void ProtocolGame::sendSaleItemList(
    const std::vector<ShopBlock> &shopVector,
    const std::map<uint32_t, uint32_t> &inventoryMap) {
  // Since we already have full inventory map we shouldn't call getMoney here -
  // it is simply wasting cpu power
  ...
  uint16_t currency = player->getShopOwner()
                          ? player->getShopOwner()->getCurrency()
                          : ITEM_GOLD_COIN;
  msg.addByte(0xEE);
  ...
  for (ShopBlock shopBlock : shopVector) {
    ...
    if (it != inventoryMap.end()) {
      ...
      msg.addByte(
          std::min<uint32_t>(it->second, std::numeric_limits<uint8_t>::max()));
      if (++itemsToSend >= 0xFF) {
        break;
      }
    }
...
void ProtocolGame::sendMarketEnter(uint32_t depotId) {
  ...
  msg.addByte(0xF6);
  msg.addByte(
      std::min<uint32_t>(IOMarket::getPlayerOfferCount(player->getGUID()),
  ...
  do {
    ...
    for (Item *item : container->getItemList()) {
      ...
      if (c &&
          (!itemType.isContainer() || c->capacity() != itemType.maxItems)) {
        continue;
      }
  ...
  uint16_t itemsToSend =
      std::min<size_t>(depotItems.size(), std::numeric_limits<uint16_t>::max());
  msg.add<uint16_t>(itemsToSend);

  uint16_t i = 0;
  for (std::map<uint16_t, uint32_t>::const_iterator it = depotItems.begin();
       i < itemsToSend; ++it, ++i) {
  ...
  sendResourcesBalance(player->getMoney(), player->getBankBalance(),
                       player->getPreyCards(), player->getTaskHuntingPoints());
}
...
void ProtocolGame::sendMarketBrowseItem(uint16_t itemId,
...
void ProtocolGame::sendMarketBrowseOwnOffers(
    const MarketOfferList &buyOffers, const MarketOfferList &sellOffers) {
  NetworkMessage msg;
  msg.addByte(0xF9);
...
void ProtocolGame::sendMarketBrowseOwnHistory(
  ...
  uint32_t buyOffersToSend = std::min<uint32_t>(
      buyOffers.size(), 810 + std::max<int32_t>(0, 810 - sellOffers.size()));
  uint32_t sellOffersToSend = std::min<uint32_t>(
      sellOffers.size(), 810 + std::max<int32_t>(0, 810 - buyOffers.size()));
...
void ProtocolGame::sendForgingData() {
  ...
  std::vector<ItemClassification *> classifications =
      g_game().getItemsClassifications();
...
void ProtocolGame::sendMarketDetail(uint16_t itemId) {
  ...
  if (it.defense != 0) {
    if (it.extraDefense != 0) {
      std::ostringstream ss;
      ss << it.defense << ' ' << std::showpos << it.extraDefense
         << std::noshowpos;
      msg.addString(ss.str());
    } else {
  ...
  if (it.abilities) {
    ...
    for (uint8_t i = SKILL_FIRST; i <= SKILL_FISHING; i++) {
      ...
      ss << getSkillName(i) << ' ' << std::showpos << it.abilities->skills[i]
         << std::noshowpos;
    }
    ...
    if (it.abilities->stats[STAT_MAGICPOINTS] != 0) {
      ...
      ss << "magic level " << std::showpos
         << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
    }
    ...
    if (it.abilities->speed != 0) {
      ...
      ss << "speed " << std::showpos << (it.abilities->speed >> 1)
         << std::noshowpos;
    }
  ...
  MarketStatistics *statistics =
      IOMarket::getInstance().getPurchaseStatistics(itemId);
  if (statistics) {
    msg.addByte(0x01);
  ...
  if (statistics) {
    msg.addByte(0x01);
    msg.add<uint32_t>(statistics->numTransactions);
    msg.add<uint64_t>(std::min<uint64_t>(std::numeric_limits<uint32_t>::max(),
...
void ProtocolGame::sendTradeItemRequest(const std::string &traderName,
...
void ProtocolGame::sendCreatureTurn(const Creature *creature,
                                    uint32_t stackPos) {
  if (!canSee(creature)) {
    return;
  }
...
void ProtocolGame::sendDistanceShoot(const Position &from, const Position &to,
                                     uint8_t type) {
  NetworkMessage msg;
  msg.addByte(0x83);
  ...
  msg.addByte(type);
  msg.addByte(static_cast<uint8_t>(static_cast<int8_t>(
      static_cast<int32_t>(to.x) - static_cast<int32_t>(from.x))));
  msg.addByte(static_cast<uint8_t>(static_cast<int8_t>(
      static_cast<int32_t>(to.y) - static_cast<int32_t>(from.y))));
...
void ProtocolGame::sendPartyCreatureHealth(const Creature *target,
...
void ProtocolGame::sendPartyPlayerMana(const Player *target,
                                       uint8_t manaPercent) {
  uint32_t cid = target->getID();
  if (knownCreatureSet.find(cid) == knownCreatureSet.end()) {
    sendPartyCreatureUpdate(target);
...
void ProtocolGame::sendPartyCreatureShowStatus(const Creature *target,
                                               bool showStatus) {
  uint32_t cid = target->getID();
  if (knownCreatureSet.find(cid) == knownCreatureSet.end()) {
    sendPartyCreatureUpdate(target);
...
void ProtocolGame::sendAddTileItem(const Position &pos, uint32_t stackpos,
                                   const Item *item) {
  if (!canSee(pos)) {
    return;
  }
...
void ProtocolGame::sendUpdateTileItem(const Position &pos, uint32_t stackpos,
                                      const Item *item) {
  if (!canSee(pos)) {
    return;
  }
...
void ProtocolGame::sendAddCreature(const Creature *creature,
  ...
  msg.add<uint16_t>(
      static_cast<uint16_t>(g_configManager().getNumber(STORE_COIN_PACKET)));
  ...
  for (int i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; ++i) {
    sendInventoryItem(static_cast<Slots_t>(i),
                      player->getInventoryItem(static_cast<Slots_t>(i)));
  }
  ...
  const std::forward_list<VIPEntry> &vipEntries =
      IOLoginData::getVIPEntries(player->getAccount());
  ...
  if (player->isAccessPlayer()) {
    for (const VIPEntry &entry : vipEntries) {
      ...
      sendVIP(entry.guid, entry.name, entry.description, entry.icon,
              entry.notify, vipStatus);
    }
  } else {
    for (const VIPEntry &entry : vipEntries) {
      VipStatus_t vipStatus;
  ...
  if (slotItem) {
    ...
    Container *hasQuickLootContainer =
        player->getLootContainer(OBJECTCATEGORY_DEFAULT);
    if (mainBackpack && !hasQuickLootContainer) {
      player->setLootContainer(OBJECTCATEGORY_DEFAULT, mainBackpack);
      sendInventoryItem(CONST_SLOT_BACKPACK,
...
void ProtocolGame::sendMoveCreature(const Creature *creature,
                                    const Position &newPos, int32_t newStackPos,
                                    const Position &oldPos, int32_t oldStackPos,
                                    bool teleport) {
  if (creature == player) {
    if (oldStackPos >= 10) {
...
void ProtocolGame::sendAddContainerItem(uint8_t cid, uint16_t slot,
                                        const Item *item) {
  NetworkMessage msg;
  msg.addByte(0x70);
  msg.addByte(cid);
...
void ProtocolGame::sendUpdateContainerItem(uint8_t cid, uint16_t slot,
                                           const Item *item) {
  NetworkMessage msg;
  msg.addByte(0x71);
  msg.addByte(cid);
...
void ProtocolGame::sendTextWindow(uint32_t windowTextId, uint32_t itemId,
                                  const std::string &text) {
  NetworkMessage msg;
  msg.addByte(0x96);
  msg.add<uint32_t>(windowTextId);
...
void ProtocolGame::sendHouseWindow(uint32_t windowTextId,
                                   const std::string &text) {
  NetworkMessage msg;
  msg.addByte(0x97);
  msg.addByte(0x00);
...
void ProtocolGame::sendOutfitWindow() {
  ...
  const Mount *currentMount =
      g_game().mounts.getMountByID(player->getCurrentMount());
  if (currentMount) {
...
void ProtocolGame::sendPodiumWindow(const Item *podium,
                                    const Position &position, uint16_t itemId,
                                    uint8_t stackpos) {
  NetworkMessage msg;
  msg.addByte(0xC8);

  const ItemAttributes::CustomAttribute *podiumVisible =
      podium->getCustomAttribute("PodiumVisible");
  const ItemAttributes::CustomAttribute *lookType =
      podium->getCustomAttribute("LookType");
  const ItemAttributes::CustomAttribute *lookMount =
      podium->getCustomAttribute("LookMount");
  const ItemAttributes::CustomAttribute *lookDirection =
      podium->getCustomAttribute("LookDirection");
  ...
  if (lookType) {
    ...
    if (outfited) {
      const ItemAttributes::CustomAttribute *lookHead =
          podium->getCustomAttribute("LookHead");
      const ItemAttributes::CustomAttribute *lookBody =
          podium->getCustomAttribute("LookBody");
      const ItemAttributes::CustomAttribute *lookLegs =
          podium->getCustomAttribute("LookLegs");
      const ItemAttributes::CustomAttribute *lookFeet =
          podium->getCustomAttribute("LookFeet");

      msg.addByte(
          lookHead ? static_cast<uint8_t>(boost::get<int64_t>(lookHead->value))
                   : 0);
      msg.addByte(
          lookBody ? static_cast<uint8_t>(boost::get<int64_t>(lookBody->value))
                   : 0);
      msg.addByte(
          lookLegs ? static_cast<uint8_t>(boost::get<int64_t>(lookLegs->value))
                   : 0);
      msg.addByte(
          lookFeet ? static_cast<uint8_t>(boost::get<int64_t>(lookFeet->value))
                   : 0);

      const ItemAttributes::CustomAttribute *lookAddons =
          podium->getCustomAttribute("LookAddons");
      msg.addByte(lookAddons ? static_cast<uint8_t>(
                                   boost::get<int64_t>(lookAddons->value))
                             : 0);
    }
  ...
  if (lookMount) {
    ...
    if (mounted) {
      const ItemAttributes::CustomAttribute *lookHead =
          podium->getCustomAttribute("LookMountHead");
      const ItemAttributes::CustomAttribute *lookBody =
          podium->getCustomAttribute("LookMountBody");
      const ItemAttributes::CustomAttribute *lookLegs =
          podium->getCustomAttribute("LookMountLegs");
      const ItemAttributes::CustomAttribute *lookFeet =
          podium->getCustomAttribute("LookMountFeet");
...
void ProtocolGame::sendVIP(uint32_t guid, const std::string &name,
...
void ProtocolGame::sendOpenStore(uint8_t) {
  ...
  // add categories
  auto categoriesCount =
      static_cast<uint16_t>(g_game().gameStore.getCategoryOffers().size());
...
void ProtocolGame::sendStoreCategoryOffers(StoreCategory *category) {
  ...
  for (BaseOffer *offer : category->offers) {
    ...
    if (offer->type == Offer_t::ITEM ||
        offer->type == Offer_t::STACKABLE_ITEM) {
    ...
    if (offer->type == OUTFIT || offer->type == OUTFIT_ADDON) {
      ...
      if (player->canWear(looktype,
                          addons)) { // player can wear the offer already
        disabled = 1;
        if (addons ==
            0) { // addons == 0 //oufit-only offer and player already has it
          disabledReason << "You already have this outfit.";
        } else {
          disabledReason << "You already have this outfit/addon.";
        }
      } else {
        if (outfitOffer->type == OUTFIT_ADDON &&
            !player->canWear(
                looktype,
                0)) { // addon offer and player doesnt have the base outfit
          disabled = 1;
          disabledReason
              << "You don't have the outfit, you can't buy the addon.";
        }
      }
      ...
      if (player->isPromoted() ||
          !player->isPremium()) { // TODO: add support to multiple promotion
                                  // levels
        disabled = 1;
...
void ProtocolGame::sendStoreError(GameStoreError_t error,
...
void ProtocolGame::sendStorePurchaseSuccessful(const std::string &message,
                                               const uint32_t coinBalance) {
  NetworkMessage msg;
  ...
  msg.addString(message);
  msg.add<uint32_t>(
      coinBalance); // dont know why the client needs it duplicated. But ok...
  msg.add<uint32_t>(coinBalance);

  writeToOutputBuffer(msg);
}

void ProtocolGame::sendStoreRequestAdditionalInfo(
    uint32_t offerId, ClientOffer_t clientOfferType) {
  NetworkMessage msg;
...
void ProtocolGame::sendPreyData(const PreySlot *slot) {
  ...
  if (slot->state == PreyDataState_Locked) {
    ...
    if (const MonsterType *mtype =
            g_monsters().getMonsterTypeByRaceId(slot->selectedRaceId)) {
    ...
    std::for_each(slot->raceIdList.begin(), slot->raceIdList.end(),
                  [&msg](uint16_t raceId) {
                    if (const MonsterType *mtype =
                            g_monsters().getMonsterTypeByRaceId(raceId)) {
                      msg.addString(mtype->name);
                      const Outfit_t outfit = mtype->info.outfit;
                      msg.add<uint16_t>(outfit.lookType);
                      if (outfit.lookType == 0) {
                      ...
                      SPDLOG_WARN("[ProtocolGame::sendPreyData] - Unknown "
                                  "monster type raceid: {}",
                                  raceId);
                      return;
                    }
    ...
    const std::map<uint16_t, std::string> bestiaryList =
        g_game().getBestiaryList();
    msg.add<uint16_t>(static_cast<uint16_t>(bestiaryList.size()));
    std::for_each(bestiaryList.begin(), bestiaryList.end(),
                  [&msg](auto &mType) { msg.add<uint16_t>(mType.first); });
  } else {
    SPDLOG_WARN("[ProtocolGame::sendPreyData] - Unknown prey state: {}",
                slot->state);
    return;
  }

  msg.add<uint32_t>(std::max<uint32_t>(
      static_cast<uint32_t>(
          ((slot->freeRerollTimeStamp - OTSYS_TIME()) / 1000)),
      0));
...
void ProtocolGame::sendPreyPrices() {
  ...
  msg.addByte(static_cast<uint8_t>(
      g_configManager().getNumber(PREY_BONUS_REROLL_PRICE)));
  msg.addByte(static_cast<uint8_t>(
      g_configManager().getNumber(PREY_SELECTION_LIST_PRICE)));
  msg.add<uint32_t>(player->getTaskHuntingRerollPrice());
  msg.add<uint32_t>(player->getTaskHuntingRerollPrice());
  msg.addByte(static_cast<uint8_t>(
      g_configManager().getNumber(TASK_HUNTING_SELECTION_LIST_PRICE)));
  msg.addByte(static_cast<uint8_t>(
      g_configManager().getNumber(TASK_HUNTING_BONUS_REROLL_PRICE)));

  writeToOutputBuffer(msg);
}

void ProtocolGame::sendStoreTrasactionHistory(HistoryStoreOfferList &list,
                                              uint32_t page,
                                              uint8_t entriesPerPage) {
  NetworkMessage msg;
...
////////////// Add common messages
void ProtocolGame::AddCreature(NetworkMessage &msg, const Creature *creature,
...
void ProtocolGame::AddPlayerStats(NetworkMessage &msg) {
  ...
  msg.add<uint16_t>(
      player->getStaminaXpBoost()); // stamina multiplier (100 = 1.0x)
...
void ProtocolGame::addImbuementInfo(NetworkMessage &msg,
                                    uint16_t imbuementId) const {
  Imbuement *imbuement = g_imbuements().getImbuement(imbuementId);
  const BaseImbuement *baseImbuement =
      g_imbuements().getBaseByID(imbuement->getBaseID());
  const CategoryImbuement *categoryImbuement =
      g_imbuements().getCategoryByID(imbuement->getCategory());
...
void ProtocolGame::openImbuementWindow(Item *item) {
  ...
  // Send imbuement time
  for (uint8_t slotid = 0;
       slotid < static_cast<uint8_t>(item->getImbuementSlot()); slotid++) {
    ...
    msg.add<uint32_t>(g_imbuements()
                          .getBaseByID(imbuementInfo.imbuement->getBaseID())
                          ->removeCost);
  }

  std::vector<Imbuement *> imbuements =
      g_imbuements().getImbuements(player, item);
  ...
  for (const Imbuement *imbuement : imbuements) {
    ...
    for (const auto &itm : items) {
      if (!needItems.count(itm.first)) {
        ...
        uint32_t stashCount =
            player->getStashItemCount(Item::items[itm.first].id);
        if (stashCount > 0) {
...
void ProtocolGame::AddCreatureLight(NetworkMessage &msg,
...
// tile
void ProtocolGame::RemoveTileThing(NetworkMessage &msg, const Position &pos,
                                   uint32_t stackpos) {
  if (stackpos >= 10) {
    return;
  }
...
void ProtocolGame::sendKillTrackerUpdate(Container *corpse,
                                         const std::string &name,
                                         const Outfit_t creatureOutfit) {
...
void ProtocolGame::sendUpdateInputAnalyzer(CombatType_t type, int32_t amount,
                                           std::string target) {
  NetworkMessage msg;
  msg.addByte(0xCC);
  ...
  if (slot->state == PreyTaskDataState_Locked) {
    ...
    std::for_each(bestiaryList.begin(), bestiaryList.end(),
                  [&msg, user](auto &mType) {
                    ...
                    msg.addByte(user->isCreatureUnlockedOnTaskHunting(
                                    g_monsters().getMonsterType(mType.second))
                                    ? 0x01
                                    : 0x00);
                  });
  } else if (slot->state == PreyTaskDataState_Active) {
    if (const TaskHuntingOption *option =
            g_ioprey().GetTaskRewardOption(slot)) {
      ...
      SPDLOG_WARN("[ProtocolGame::sendTaskHuntingData] - Unknown slot option "
                  "{} on player {}",
                  slot->id, player->getName());
      return;
    }
    ...
    if (const TaskHuntingOption *option =
      ...
      if (slot->upgrade) {
        ...
        msg.add<uint16_t>(option->secondKills);
        msg.add<uint16_t>(
            std::min<uint16_t>(slot->currentKills, option->secondKills));
      } else {
        msg.addByte(0x00);
        msg.add<uint16_t>(option->firstKills);
        msg.add<uint16_t>(
            std::min<uint16_t>(slot->currentKills, option->firstKills));
      }
      msg.addByte(slot->rarity);
    } else {
      SPDLOG_WARN("[ProtocolGame::sendTaskHuntingData] - Unknown slot option "
    ...
    SPDLOG_WARN(
        "[ProtocolGame::sendTaskHuntingData] - Unknown task hunting state: {}",
        slot->state);
    return;
  }
...
void ProtocolGame::MoveUpCreature(NetworkMessage &msg, const Creature *creature,
  ...
  // underground, going one floor up (still underground)
    ...
    int32_t skip = -1;
    GetFloorDescription(msg, oldPos.x - 8, oldPos.y - 6, oldPos.getZ() - 3, 18,
                        14, 3, skip);
...
void ProtocolGame::MoveDownCreature(NetworkMessage &msg,
                                    const Creature *creature,
                                    const Position &newPos,
                                    const Position &oldPos) {
  if (creature != player) {
  ...
  if (newPos.z == 8) {
    ...
    GetFloorDescription(msg, oldPos.x - 8, oldPos.y - 6, newPos.z, 18, 14, -1,
                        skip);
    GetFloorDescription(msg, oldPos.x - 8, oldPos.y - 6, newPos.z + 1, 18, 14,
                        -2, skip);
    GetFloorDescription(msg, oldPos.x - 8, oldPos.y - 6, newPos.z + 2, 18, 14,
                        -3, skip);
...
void ProtocolGame::AddShopItem(NetworkMessage &msg,
                               const ShopBlock &shopBlock) {
  // Sends the item information empty if the player doesn't have the storage to
  // buy/sell a certain item
  int32_t storageValue;
  player->getStorageValue(shopBlock.itemStorageKey, storageValue);
  if (shopBlock.itemStorageKey != 0 &&
      storageValue < shopBlock.itemStorageValue) {
  ...
  // If not send "itemName" variable from the npc shop, will registered the name
  // that is in items.xml
  ...
  msg.add<uint32_t>(
      shopBlock.itemBuyPrice == 4294967295 ? 0 : shopBlock.itemBuyPrice);
  msg.add<uint32_t>(
      shopBlock.itemSellPrice == 4294967295 ? 0 : shopBlock.itemSellPrice);
}
...
void ProtocolGame::parseExtendedOpcode(NetworkMessage &msg) {
  ...
  addGameTask(&Game::parsePlayerExtendedOpcode, player->getID(), opcode,
              buffer);
}
...
void ProtocolGame::reloadCreature(const Creature *creature) {
  ...
  uint32_t stackpos =
      creature->getTile()->getClientIndexOfCreature(player, creature);
  ...
  phmap::flat_hash_set<uint32_t>::iterator it = std::find(
      knownCreatureSet.begin(), knownCreatureSet.end(), creature->getID());
...
void ProtocolGame::sendOpenStash() {
  ...
  msg.add<uint16_t>(static_cast<uint16_t>(
      g_configManager().getNumber(STASH_ITEMS) - getStashSize(list)));
  writeToOutputBuffer(msg);
}
...
void ProtocolGame::parseStashWithdraw(NetworkMessage &msg) {
  ...
  Supply_Stash_Actions_t action =
      static_cast<Supply_Stash_Actions_t>(msg.getByte());
  switch (action) {
  case SUPPLY_STASH_ACTION_STOW_ITEM: {
    ...
    addGameTask(&Game::playerStowItem, player->getID(), pos, itemId, stackpos,
                count, false);
    break;
  }
  ...
  case SUPPLY_STASH_ACTION_WITHDRAW: {
    ...
    uint8_t stackpos = msg.getByte();
    addGameTask(&Game::playerStashWithdraw, player->getID(), itemId, count,
                stackpos);
    break;
  }
  default:
...
void ProtocolGame::sendDepotItems(const ItemsTierCountList &itemMap,
                                  uint16_t count) {
  NetworkMessage msg;
  msg.addByte(0x94);
...
void ProtocolGame::sendDepotSearchResultDetail(uint16_t itemId, uint8_t tier,
                                               uint32_t depotCount,
...
void ProtocolGame::parseDepotSearchItemRequest(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerRequestDepotSearchItem, player->getID(), itemId,
              itemTier);
}
...
void ProtocolGame::parseRetrieveDepotSearch(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerRequestDepotSearchRetrieve, player->getID(), itemId,
              itemTier, type);
}
...
void ProtocolGame::parseOpenParentContainer(NetworkMessage &msg) {
  ...
  addGameTask(&Game::playerRequestOpenContainerFromDepotSearch, player->getID(),
              pos);
}
