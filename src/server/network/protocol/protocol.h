...
public:
  ...
  bool isConnectionExpired() const { return connectionPtr.expired(); }

  Connection_ptr getConnection() const { return connectionPtr.lock(); }
  ...
  void send(OutputMessage_ptr msg) const {
    if (auto connection = getConnection(); connection != nullptr) {
      connection->send(msg);
    }
  }
...
protected:
  ...
  void setChecksumMethod(ChecksumMethods_t method) { checksumMethod = method; }
...
private:
  ...
  std::underlying_type_t<ChecksumMethods_t> checksumMethod =
