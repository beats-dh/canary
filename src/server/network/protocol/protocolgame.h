...
public:
	...
	void login(const std::string& name, uint32_t accnumber,
...
private:
	...
	// Unjust Panel
	void sendUnjustifiedPoints(const uint8_t& dayProgress, const uint8_t& dayLeft,
	...
	void sendResourcesBalance(uint64_t money = 0, uint64_t bank = 0,
		uint64_t preyCards = 0, uint64_t taskHunting = 0);
	void sendResourceBalance(Resource_t resourceType, uint64_t value);
	void sendSaleItemList(const std::vector<ShopBlock>& shopVector,
	...
	void sendTextWindow(uint32_t windowTextId, Item* item, uint16_t maxlen,
		bool canWrite);
	...
	void sendVIP(uint32_t guid, const std::string& name,
		const std::string& description, uint32_t icon, bool notify,
		VipStatus_t status);
	...
	void sendAddCreature(const Creature* creature, const Position& pos,
		int32_t stackpos, bool isLogin);
	void sendMoveCreature(const Creature* creature, const Position& newPos,
	...
	// analyzers
	void sendKillTrackerUpdate(Container* corpse, const std::string& name,
	...
	void sendUpdateInputAnalyzer(CombatType_t type, int32_t amount,
		std::string target);
	...
	void GetMapDescription(int32_t x, int32_t y, int32_t z, int32_t width,
		int32_t height, NetworkMessage& msg);

	void AddCreature(NetworkMessage& msg, const Creature* creature, bool known,
		uint32_t remove);
	...
	// tiles
	static void RemoveTileThing(NetworkMessage& msg, const Position& pos,
		uint32_t stackpos);
