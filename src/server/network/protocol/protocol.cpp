...
void Protocol::onSendMessage(const OutputMessage_ptr &msg) {
  if (!rawMessages) {
    ...
    if (checksumMethod == CHECKSUM_METHOD_NONE) {
      ...
      msg->addCryptoHeader(
          true, adlerChecksum(msg->getOutputBuffer(), msg->getLength()));
    } else if (checksumMethod == CHECKSUM_METHOD_SEQUENCE) {
      msg->addCryptoHeader(true,
                           sendMessageChecksum | (++serverSequenceNumber));
...
bool Protocol::onRecvMessage(NetworkMessage &msg) {
  if (checksumMethod != CHECKSUM_METHOD_NONE) {
    ...
    if (checksumMethod == CHECKSUM_METHOD_SEQUENCE) {
      if (recvChecksum == 0) {
        // checksum 0 indicate that the packet should be connection ping - 0x1C
        // packet header since we don't need that packet skip it
        return false;
      }
      ...
      uint32_t checksum;
      if (int32_t len = msg.getLength() - msg.getBufferPosition(); len > 0) {
        checksum =
            adlerChecksum(msg.getBuffer() + msg.getBufferPosition(), len);
      } else {
        checksum = 0;
      }
...
void Protocol::XTEA_encrypt(OutputMessage &msg) const {
  ...
  while (readPos < messageLength) {
    ...
    for (int32_t i = 0; i < 32; ++i) {
      vData[0] += ((vData[1] << 4 ^ vData[1] >> 5) + vData[1]) ^
                  precachedControlSum[i][0];
      vData[1] += ((vData[0] << 4 ^ vData[0] >> 5) + vData[0]) ^
                  precachedControlSum[i][1];
    }
...
bool Protocol::XTEA_decrypt(NetworkMessage &msg) const {
  uint16_t msgLength =
      msg.getLength() - (checksumMethod == CHECKSUM_METHOD_NONE ? 2 : 6);
  if ((msgLength & 7) != 0) {
    return false;
  }
...
void Protocol::enableCompression() {
  if (!compreesionEnabled) {
    ...
    if (compressionLevel != 0) {
      ...
      if (deflateInit2(defStream.get(), compressionLevel, Z_DEFLATED, -15, 9,
                       Z_DEFAULT_STRATEGY) != Z_OK) {
        defStream.reset();
        SPDLOG_ERROR(
            "[Protocol::enableCompression()] - Zlib deflateInit2 error: {}",
            (defStream->msg ? defStream->msg : " unknown error"));
      } else {
...
bool Protocol::compression(OutputMessage &msg) const {
  ...
  if (outputMessageSize > NETWORKMESSAGE_MAXSIZE) {
    SPDLOG_ERROR("[NetworkMessage::compression] - Exceded NetworkMessage max "
                 "size: {}, actually size: {}",
                 NETWORKMESSAGE_MAXSIZE, outputMessageSize);
    return false;
  }
