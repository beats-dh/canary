...
void ProtocolStatus::onRecvFirstMessage(NetworkMessage &msg) {
  ...
  if (ip != 0x0100007F) {
    ...
    if (ipStr != g_configManager().getString(IP)) {
      ...
      if (it != ipConnectMap.end() &&
          (OTSYS_TIME() <
           (it->second + g_configManager().getNumber(STATUSQUERY_TIMEOUT)))) {
        disconnect();
        return;
      }
    }
  ...
  case 0x01: {
    uint16_t requestedInfo =
        msg.get<uint16_t>(); // only a Byte is necessary, though we could add
                             // new info here
    ...
    g_dispatcher().addTask(createTask(
        std::bind(&ProtocolStatus::sendInfo,
                  std::static_pointer_cast<ProtocolStatus>(shared_from_this()),
...
void ProtocolStatus::sendStatusString() {
  ...
  serverinfo.append_attribute("servername") =
      g_configManager().getString(SERVER_NAME).c_str();
  serverinfo.append_attribute("port") =
      std::to_string(g_configManager().getNumber(LOGIN_PORT)).c_str();
  serverinfo.append_attribute("location") =
      g_configManager().getString(LOCATION).c_str();
  ...
  serverinfo.append_attribute("client") =
      (std::to_string(CLIENT_VERSION_UPPER) + "." +
       std::to_string(CLIENT_VERSION_LOWER))
          .c_str();

  pugi::xml_node owner = tsqp.append_child("owner");
  owner.append_attribute("name") =
      g_configManager().getString(OWNER_NAME).c_str();
  owner.append_attribute("email") =
      g_configManager().getString(OWNER_EMAIL).c_str();
  ...
  players.append_attribute("max") =
      std::to_string(g_configManager().getNumber(MAX_PLAYERS)).c_str();
  players.append_attribute("peak") =
      std::to_string(g_game().getPlayersRecord()).c_str();

  pugi::xml_node monsters = tsqp.append_child("monsters");
  monsters.append_attribute("total") =
      std::to_string(g_game().getMonstersOnline()).c_str();

  pugi::xml_node npcs = tsqp.append_child("npcs");
  npcs.append_attribute("total") =
      std::to_string(g_game().getNpcsOnline()).c_str();

  pugi::xml_node rates = tsqp.append_child("rates");
  rates.append_attribute("experience") =
      std::to_string(g_configManager().getNumber(RATE_EXPERIENCE)).c_str();
  rates.append_attribute("skill") =
      std::to_string(g_configManager().getNumber(RATE_SKILL)).c_str();
  rates.append_attribute("loot") =
      std::to_string(g_configManager().getNumber(RATE_LOOT)).c_str();
  rates.append_attribute("magic") =
      std::to_string(g_configManager().getNumber(RATE_MAGIC)).c_str();
  rates.append_attribute("spawn") =
      std::to_string(g_configManager().getNumber(RATE_SPAWN)).c_str();

  pugi::xml_node map = tsqp.append_child("map");
  map.append_attribute("name") = g_configManager().getString(MAP_NAME).c_str();
  map.append_attribute("author") =
      g_configManager().getString(MAP_AUTHOR).c_str();
...
void ProtocolStatus::sendInfo(uint16_t requestedInfo,
  ...
  if (requestedInfo & REQUEST_SERVER_SOFTWARE_INFO) {
    ...
    output->addString(std::to_string(CLIENT_VERSION_UPPER) + "." +
                      std::to_string(CLIENT_VERSION_LOWER));
  }
  send(output);
  disconnect();
