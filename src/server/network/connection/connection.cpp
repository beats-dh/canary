...
Connection_ptr
ConnectionManager::createConnection(boost::asio::io_service &io_service,
...
// Connection
// Constructor
Connection::Connection(boost::asio::io_service &initIoService,
                       ConstServicePort_ptr initservicePort)
    : readTimer(initIoService), writeTimer(initIoService),
      service_port(std::move(initservicePort)), socket(initIoService) {
...
void Connection::accept(Protocol_ptr protocolPtr) {
  ...
  g_dispatcher().addTask(
      createSchedulerTask(1000, std::bind(&Protocol::onConnect, protocolPtr)));
...
void Connection::accept(bool toggleParseHeader /* = true */) {
  try {
    readTimer.expires_from_now(
        boost::posix_time::seconds(CONNECTION_READ_TIMEOUT));
    readTimer.async_wait(std::bind(
        &Connection::handleTimeout,
        std::weak_ptr<Connection>(shared_from_this()), std::placeholders::_1));

    // If toggleParseHeader is true, execute the parseHeader, if not, execute
    // parseProxyIdentification
    if (toggleParseHeader) {
      // Read size of the first packet
      boost::asio::async_read(
          socket, boost::asio::buffer(msg.getBuffer(), HEADER_LENGTH),
      ...
      boost::asio::async_read(
          socket, boost::asio::buffer(msg.getBuffer(), HEADER_LENGTH),
          std::bind(&Connection::parseProxyIdentification, shared_from_this(),
                    std::placeholders::_1));
    }
  } catch (const boost::system::system_error &e) {
...
void Connection::parseProxyIdentification(
  ...
  if (connectionState == CONNECTION_STATE_IDENTIFYING) {
      ...
      if (remainder > 0) {
        ...
        try {
          ...
          readTimer.async_wait(
              std::bind(&Connection::handleTimeout,
                        std::weak_ptr<Connection>(shared_from_this()),
                        std::placeholders::_1));

          // Read the remainder of proxy identification
          boost::asio::async_read(
              socket, boost::asio::buffer(msg.getBuffer(), remainder),
              std::bind(&Connection::parseProxyIdentification,
                        shared_from_this(), std::placeholders::_1));
        } catch (const boost::system::system_error &e) {
          SPDLOG_ERROR("Connection::parseProxyIdentification] - error: {}",
                       e.what());
          close(FORCE_CLOSE);
        }
        return;
    ...
    if (strncasecmp(charData, &serverName[2], remainder) == 0) {
      ...
      SPDLOG_ERROR("Connection::parseProxyIdentification] Invalid Client "
                   "Login! Server Name mismatch!");
      close(FORCE_CLOSE);
      return;
    }
  }
...
void Connection::parseHeader(const boost::system::error_code &error) {
  ...
  uint32_t timePassed =
      std::max<uint32_t>(1, (time(nullptr) - timeConnected) + 1);
  if ((++packetsSent / timePassed) >
      static_cast<uint32_t>(
          g_configManager().getNumber(MAX_PACKETS_PER_SECOND))) {
    SPDLOG_WARN("{} disconnected for exceeding packet per second limit.",
...
void Connection::parsePacket(const boost::system::error_code &error) {
  ...
  if (!receivedFirst) {
    ...
    if (!protocol) {
      ...
      uint32_t checksum;
      if (int32_t len =
              msg.getLength() - msg.getBufferPosition() - CHECKSUM_LENGTH;
          len > 0) {
        checksum = adlerChecksum(
            msg.getBuffer() + msg.getBufferPosition() + CHECKSUM_LENGTH, len);
      } else {
        checksum = 0;
      }
      ...
      protocol = service_port->make_protocol(recvChecksum == checksum, msg,
                                             shared_from_this());
      if (!protocol) {
        close(FORCE_CLOSE);
        return;
      ...
      // It is rather hard to detect if we have checksum or sequence method here
      // so let's skip checksum check it doesn't generate any problem because
      // olders protocol don't use 'server sends first' feature
      msg.get<uint32_t>();
      // Skip protocol ID
      msg.skipBytes(1);
    }
...
void Connection::send(const OutputMessage_ptr &outputMessage) {
  ...
  if (noPendingWrite) {
    ...
    try {
      ...
      boost::asio::post(
          socket.get_executor(),
          std::bind(&Connection::internalWorker, shared_from_this()));
#else
...
void Connection::internalSend(const OutputMessage_ptr &outputMessage) {
  try {
    writeTimer.expires_from_now(
        boost::posix_time::seconds(CONNECTION_WRITE_TIMEOUT));
    writeTimer.async_wait(std::bind(
        &Connection::handleTimeout,
        std::weak_ptr<Connection>(shared_from_this()), std::placeholders::_1));

    boost::asio::async_write(
        socket,
        boost::asio::buffer(outputMessage->getOutputBuffer(),
                            outputMessage->getLength()),
        std::bind(&Connection::onWriteOperation, shared_from_this(),
                  std::placeholders::_1));
  } catch (const boost::system::system_error &e) {
