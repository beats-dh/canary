...
static int webhook_send_message_(const char *url, const char *payload,
                                 std::string *response_body);
static std::string get_payload(std::string title, std::string message,
                               int color);
...
void webhook_send_message(std::string title, std::string message, int color,
  ...
  std::string response_body = "";
  int response_code =
      webhook_send_message_(url.c_str(), payload.c_str(), &response_body);
...
static std::string get_payload(std::string title, std::string message,
  ...
  footer_text << g_configManager().getString(IP) << ":"
              << g_configManager().getNumber(GAME_PORT) << " | " << time_buf
              << " UTC";
...
static int webhook_send_message_(const char *url, const char *payload,
  ...
  curl_easy_setopt(curl, CURLOPT_USERAGENT,
                   "canary (https://github.com/Hydractify/canary)");
