...
Condition *Condition::createCondition(ConditionId_t id, ConditionType_t type,
                                      int32_t ticks, int32_t param /* = 0*/,
                                      bool buff /* = false*/,
                                      uint32_t subId /* = 0*/) {
  switch (type) {
  ...
  case CONDITION_LIGHT:
    return new ConditionLight(id, type, ticks, buff, subId, param & 0xFF,
                              (param & 0xFF00) >> 8);
...
Condition *Condition::createCondition(PropStream &propStream) {
  ...
  return createCondition(static_cast<ConditionId_t>(id),
                         static_cast<ConditionType_t>(type), ticks, 0,
                         buff != 0, subId);
}
...
void ConditionAttributes::addCondition(Creature *creature,
                                       const Condition *addCondition) {
  if (updateCondition(addCondition)) {
    setTicks(addCondition->getTicks());

    const ConditionAttributes &conditionAttrs =
        static_cast<const ConditionAttributes &>(*addCondition);
...
bool ConditionAttributes::unserializeProp(ConditionAttr_t attr,
...
void ConditionAttributes::updatePercentStats(Player *player) {
  for (int32_t i = STAT_FIRST; i <= STAT_LAST; ++i) {
    ...
    case STAT_MAXHITPOINTS:
      stats[i] = static_cast<int32_t>(player->getMaxHealth() *
                                      ((statsPercent[i] - 100) / 100.f));
      break;

    case STAT_MAXMANAPOINTS:
      stats[i] = static_cast<int32_t>(player->getMaxMana() *
                                      ((statsPercent[i] - 100) / 100.f));
      break;

    case STAT_MAGICPOINTS:
      stats[i] = static_cast<int32_t>(player->getBaseMagicLevel() *
                                      ((statsPercent[i] - 100) / 100.f));
      break;

    case STAT_CAPACITY:
      stats[i] = static_cast<int32_t>(player->getCapacity() *
                                      (statsPercent[i] / 100.f));
      break;
    }
  }
}
...
void ConditionAttributes::updatePercentSkills(Player *player) {
  for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) {
    ...
    skills[i] = static_cast<int32_t>(unmodifiedSkill *
...
void ConditionAttributes::updatePercentBuffs(Creature *creature) {
  for (int32_t i = BUFF_FIRST; i <= BUFF_LAST; ++i) {
    ...
    buffs[i] =
        static_cast<int32_t>(actualBuff * ((buffsPercent[i] - 100) / 100.f));
  }
}
...
bool ConditionAttributes::executeCondition(Creature *creature,
                                           int32_t interval) {
...
void ConditionRegeneration::addCondition(Creature *creature,
                                         const Condition *addCondition) {
  if (updateCondition(addCondition)) {
    setTicks(addCondition->getTicks());

    const ConditionRegeneration &conditionRegen =
        static_cast<const ConditionRegeneration &>(*addCondition);
...
bool ConditionRegeneration::unserializeProp(ConditionAttr_t attr,
                                            PropStream &propStream) {
...
bool ConditionRegeneration::executeCondition(Creature *creature,
                                             int32_t interval) {
  ...
  if (creature->getZone() != ZONE_PROTECTION ||
      PlayerdailyStreak >= DAILY_REWARD_HP_REGENERATION) {
    if (internalHealthTicks >= getHealthTicks(creature)) {
      internalHealthTicks = 0;

      int32_t realHealthGain = creature->getHealth();
      if (creature->getZone() == ZONE_PROTECTION &&
          PlayerdailyStreak >= DAILY_REWARD_DOUBLE_HP_REGENERATION) {
        creature->changeHealth(healthGain *
                               2); // Double regen from daily reward
      } else {
        creature->changeHealth(healthGain);
      }
      ...
      if (isBuff && realHealthGain > 0) {
        if (player) {
          std::string healString =
              std::to_string(realHealthGain) +
              (realHealthGain != 1 ? " hitpoints." : " hitpoint.");

          TextMessage message(MESSAGE_HEALED,
                              "You were healed for " + healString);
          ...
          g_game().map.getSpectators(spectators, player->getPosition(), false,
                                     true);
  ...
  if (creature->getZone() != ZONE_PROTECTION ||
      PlayerdailyStreak >= DAILY_REWARD_MP_REGENERATION) {
    if (internalManaTicks >= getManaTicks(creature)) {
      internalManaTicks = 0;
      if (creature->getZone() == ZONE_PROTECTION &&
          PlayerdailyStreak >= DAILY_REWARD_DOUBLE_MP_REGENERATION) {
...
void ConditionManaShield::addCondition(Creature *creature,
                                       const Condition *addCondition) {
  endCondition(creature);
  setTicks(addCondition->getTicks());

  const ConditionManaShield &conditionManaShield =
      static_cast<const ConditionManaShield &>(*addCondition);
...
bool ConditionManaShield::unserializeProp(ConditionAttr_t attr,
                                          PropStream &propStream) {
...
void ConditionSoul::addCondition(Creature *, const Condition *addCondition) {
  if (updateCondition(addCondition)) {
    ...
    const ConditionSoul &conditionSoul =
        static_cast<const ConditionSoul &>(*addCondition);
...
bool ConditionSoul::unserializeProp(ConditionAttr_t attr,
                                    PropStream &propStream) {
...
bool ConditionDamage::unserializeProp(ConditionAttr_t attr,
                                      PropStream &propStream) {
...
bool ConditionDamage::updateCondition(const Condition *addCondition) {
  const ConditionDamage &conditionDamage =
      static_cast<const ConditionDamage &>(*addCondition);
...
bool ConditionDamage::doDamage(Creature *creature, int32_t healthChange) {
  ...
  if (field && creature->getPlayer() && attacker && attacker->getPlayer()) {
    damage.primary.value =
        static_cast<int32_t>(std::round(damage.primary.value / 2.));
  }

  if (!creature->isAttackable() ||
      Combat::canDoCombat(attacker, creature) != RETURNVALUE_NOERROR) {
  ...
  if (g_game().combatBlockHit(damage, attacker, creature, false, false,
                              field)) {
    return false;
  }
...
void ConditionDamage::addCondition(Creature *creature,
                                   const Condition *addCondition) {
...
void ConditionDamage::generateDamageList(int32_t amount, int32_t start,
                                         std::list<int32_t> &list) {
...
void ConditionSpeed::setFormulaVars(float NewMina, float NewMinb, float NewMaxa,
                                    float NewMaxb) {
  this->mina = NewMina;
  this->minb = NewMinb;
  this->maxa = NewMaxa;
...
void ConditionSpeed::getFormulaValues(int32_t var, int32_t &min,
                                      int32_t &max) const {
  min = (var * mina) + minb;
  max = (var * maxa) + maxb;
}
...
bool ConditionSpeed::unserializeProp(ConditionAttr_t attr,
                                     PropStream &propStream) {
...
void ConditionSpeed::addCondition(Creature *creature,
                                  const Condition *addCondition) {
  if (conditionType != addCondition->getType()) {
    return;
  }
  ...
  const ConditionSpeed &conditionSpeed =
      static_cast<const ConditionSpeed &>(*addCondition);
...
bool ConditionOutfit::unserializeProp(ConditionAttr_t attr,
                                      PropStream &propStream) {
...
bool ConditionOutfit::startCondition(Creature *creature) {
  if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) &&
      outfit.lookType != 0 && !g_game().isLookTypeRegistered(outfit.lookType)) {
    SPDLOG_WARN(
        "[ConditionOutfit::startCondition] An unregistered creature looktype "
        "type with id '{}' was blocked to prevent client crash.",
        outfit.lookType);
    return false;
  }

  if ((outfit.lookType == 0 && outfit.lookTypeEx == 0) &&
      !monsterName.empty()) {
...
void ConditionOutfit::addCondition(Creature *creature,
                                   const Condition *addCondition) {
  if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) &&
      outfit.lookType != 0 && !g_game().isLookTypeRegistered(outfit.lookType)) {
    SPDLOG_WARN(
        "[ConditionOutfit::addCondition] An unregistered creature looktype "
        "type with id '{}' was blocked to prevent client crash.",
        outfit.lookType);
    return;
  }
  ...
  if (updateCondition(addCondition)) {
    ...
    const ConditionOutfit &conditionOutfit =
        static_cast<const ConditionOutfit &>(*addCondition);
    if (!conditionOutfit.monsterName.empty() &&
        conditionOutfit.monsterName.compare(monsterName) != 0) {
      const MonsterType *monsterType =
          g_monsters().getMonsterType(conditionOutfit.monsterName);
      if (monsterType) {
        setOutfit(monsterType->info.outfit);
      } else {
        SPDLOG_ERROR(
            "[ConditionOutfit::addCondition] - Monster {} does not exist",
            monsterName);
        return;
      }
    } else if (conditionOutfit.outfit.lookType != 0 ||
               conditionOutfit.outfit.lookTypeEx != 0) {
...
void ConditionLight::addCondition(Creature *creature,
                                  const Condition *condition) {
  if (updateCondition(condition)) {
    setTicks(condition->getTicks());

    const ConditionLight &conditionLight =
        static_cast<const ConditionLight &>(*condition);
...
bool ConditionLight::unserializeProp(ConditionAttr_t attr,
                                     PropStream &propStream) {
...
void ConditionLight::serialize(PropWriteStream &propWriteStream) {
  ...
  // TODO: color and level could be serialized as 8-bit if we can retain
  // backwards compatibility, but perhaps we should keep it like this in case
  // they increase in the future...
...
void ConditionSpellCooldown::addCondition(Creature *creature,
                                          const Condition *addCondition) {
  if (updateCondition(addCondition)) {
    setTicks(addCondition->getTicks());
...
void ConditionSpellGroupCooldown::addCondition(Creature *creature,
                                               const Condition *addCondition) {
  if (updateCondition(addCondition)) {
    setTicks(addCondition->getTicks());
