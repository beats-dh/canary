...
CombatDamage Combat::applyImbuementElementalDamage(Item* item,
	...
	for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); slotid++) {
		...
		if (imbuementInfo.imbuement->combatType == COMBAT_NONE || damage.primary.type == COMBAT_HEALING || damage.secondary.type == COMBAT_HEALING) {
			continue;
		}
...
void ValueCallback::getMinMaxValues(Player* player, CombatDamage& damage,
	...
	if (lua_pcall(L, parameters, 2, 0) != 0) {
		...
		int32_t defaultDmg = normal_random(LuaScriptInterface::getNumber<int32_t>(L, -2),
