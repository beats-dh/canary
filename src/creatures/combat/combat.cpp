...
CombatDamage Combat::getCombatDamage(Creature *creature,
  ...
  if (formulaType == COMBAT_FORMULA_DAMAGE) {
    damage.primary.value =
        normal_random(static_cast<int32_t>(mina), static_cast<int32_t>(maxa));
  } else if (creature) {
    int32_t min, max;
    if (creature->getCombatValues(min, max)) {
      ...
      if (params.valueCallback) {
        ...
        int32_t levelFormula =
            player->getLevel() * 2 + player->getMagicLevel() * 3;
        damage.primary.value =
            normal_random(static_cast<int32_t>(levelFormula * mina + minb),
        ...
        if (weapon) {
          damage.primary.value = normal_random(
              ...
              static_cast<int32_t>(
                  weapon->getWeaponDamage(player, target, tool, true) * maxa +
                  maxb));

          damage.secondary.type = weapon->getElementType();
          damage.secondary.value =
              weapon->getElementDamage(player, target, tool);
          ...
          damage.primary.value = normal_random(static_cast<int32_t>(minb),
...
bool Combat::isProtected(const Player *attacker, const Player *target) {
  ...
  if (target->getLevel() < protectionLevel ||
      attacker->getLevel() < protectionLevel) {
    return true;
  }

  if (attacker->getVocationId() == VOCATION_NONE ||
      target->getVocationId() == VOCATION_NONE) {
    return true;
  }

  if (attacker->getSkull() == SKULL_BLACK &&
      attacker->getSkullClient(target) == SKULL_NONE) {
    return true;
  }
  ...
  if (attacker) {
    ...
    if (const Player *targetPlayer = target->getPlayer()) {
      ...
      if (attacker->getMonster() &&
        ...
        if (attacker->getFaction() != FACTION_DEFAULT &&
            !attacker->getMonster()->isEnemyFaction(
                targetPlayer->getFaction())) {
          return RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER;
        }
      }
      ...
      if (attacker->getFaction() != FACTION_DEFAULT &&
          attacker->getFaction() != FACTION_PLAYER && attacker->getMonster() &&
          !attacker->getMonster()->isEnemyFaction(target->getFaction())) {
        return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
      }
    ...
    if (g_game().getWorldType() == WORLD_TYPE_NO_PVP) {
      if (attacker->getPlayer() ||
          (attackerMaster && attackerMaster->getPlayer())) {
        if (target->getPlayer()) {
...
void Combat::setPlayerCombatValues(formulaType_t newFormulaType, double newMina,
                                   double newMinb, double newMaxa,
                                   double newMaxb) {
...
void Combat::CombatHealthFunc(Creature *caster, Creature *target,
  ...
  if (g_game().combatBlockHit(damage, caster, target, params.blockedByShield,
...
CombatDamage Combat::applyImbuementElementalDamage(Item *item,
                                                   CombatDamage damage) {
  if (!item) {
    return damage;
  }
...
void Combat::CombatManaFunc(Creature *caster, Creature *target,
                            const CombatParams &params, CombatDamage *data) {
  assert(data);
  CombatDamage damage = *data;
  ...
  for (const auto &condition : params.conditionList) {
    ...
    if (player) {
      if (player->isImmuneCleanse(condition->getType())) {
        ...
        uint16_t playerCharmRaceid =
            player->parseRacebyCharm(CHARM_CLEANSE, false, 0);
        if (playerCharmRaceid != 0) {
          const MonsterType *mType =
              g_monsters().getMonsterType(caster->getName());
...
void Combat::CombatDispelFunc(Creature *, Creature *target,
                              const CombatParams &params, CombatDamage *) {
  if (target) {
    target->removeCombatCondition(params.dispelType);
  }
}

void Combat::CombatNullFunc(Creature *caster, Creature *target,
                            const CombatParams &params, CombatDamage *) {
  CombatConditionFunc(caster, target, params, nullptr);
  CombatDispelFunc(caster, target, params, nullptr);
}

void Combat::combatTileEffects(const SpectatorHashSet &spectators,
                               Creature *caster, Tile *tile,
                               const CombatParams &params) {
  if (params.itemId != 0) {
    ...
    if (caster) {
      ...
      if (casterPlayer) {
        if (g_game().getWorldType() == WORLD_TYPE_NO_PVP ||
            tile->hasFlag(TILESTATE_NOPVPZONE)) {
        ...
        } else if (itemId == ITEM_FIREFIELD_PVP_FULL ||
                   itemId == ITEM_POISONFIELD_PVP ||
                   itemId == ITEM_ENERGYFIELD_PVP || itemId == ITEM_MAGICWALL ||
                   itemId == ITEM_WILDGROWTH) {
...
void Combat::postCombatEffects(Creature *caster, const Position &pos,
                               const CombatParams &params) {
  if (caster && params.distanceEffect != CONST_ANI_NONE) {
    addDistanceEffect(caster, caster->getPosition(), pos,
...
void Combat::CombatFunc(Creature *caster, const Position &pos,
                        ...
                        CombatFunction func, CombatDamage *data) {
  ...
  g_game().map.getSpectators(spectators, pos, true, true, rangeX, rangeX,
                             rangeY, rangeY);
...
void Combat::doCombatHealth(Creature *caster, Creature *target,
  ...
  if ((caster && target) && (caster == target || canCombat) &&
  ...
  if (caster && caster->getPlayer()) {
    // Critical damage
    uint16_t chance =
        caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_CHANCE);
    // Charm low blow rune)
    if (target && target->getMonster() &&
        damage.primary.type != COMBAT_HEALING) {
      uint16_t playerCharmRaceid =
          caster->getPlayer()->parseRacebyCharm(CHARM_LOW, false, 0);
      if (playerCharmRaceid != 0) {
        const MonsterType *mType =
            g_monsters().getMonsterType(target->getName());
        if (mType && playerCharmRaceid == mType->info.raceid) {
    ...
    if (chance != 0 && uniform_random(1, 100) <= chance) {
      ...
      damage.primary.value +=
          (damage.primary.value *
           caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE)) /
          100;
      damage.secondary.value +=
          (damage.secondary.value *
           caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE)) /
...
void Combat::doCombatHealth(Creature *caster, const Position &position,
                            const AreaCombat *area, CombatDamage &damage,
                            const CombatParams &params) {
  if (caster && caster->getPlayer()) {
    // Critical damage
    uint16_t chance =
        caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_CHANCE);
    if (damage.primary.type != COMBAT_HEALING && chance != 0 &&
...
void Combat::doCombatMana(Creature *caster, const Position &position,
                          const AreaCombat *area, CombatDamage &damage,
                          const CombatParams &params) {
  if (caster && caster->getPlayer()) {
    // Critical damage
...
void Combat::doCombatCondition(Creature *caster, const Position &position,
                               const AreaCombat *area,
                               const CombatParams &params) {
  CombatFunc(caster, position, area, params, CombatConditionFunc, nullptr);
}

void Combat::doCombatCondition(Creature *caster, Creature *target,
                               const CombatParams &params) {
  bool canCombat = !params.aggressive ||
                   (caster != target &&
                    Combat::canDoCombat(caster, target) == RETURNVALUE_NOERROR);
...
void Combat::doCombatDispel(Creature *caster, const Position &position,
                            const AreaCombat *area,
                            const CombatParams &params) {
  CombatFunc(caster, position, area, params, CombatDispelFunc, nullptr);
}

void Combat::doCombatDispel(Creature *caster, Creature *target,
                            const CombatParams &params) {
  bool canCombat = !params.aggressive ||
                   (caster != target &&
                    Combat::canDoCombat(caster, target) == RETURNVALUE_NOERROR);
...
void Combat::doCombatDefault(Creature *caster, Creature *target,
                             const CombatParams &params) {
  if (!params.aggressive ||
      (caster != target &&
       Combat::canDoCombat(caster, target) == RETURNVALUE_NOERROR)) {
...
void ValueCallback::getMinMaxValues(Player *player, CombatDamage &damage,
  ...
  if (lua_pcall(L, parameters, 2, 0) != 0) {
    ...
    int32_t defaultDmg =
        normal_random(LuaScriptInterface::getNumber<int32_t>(L, -2),
                      LuaScriptInterface::getNumber<int32_t>(L, -1));

    if (shouldCalculateSecondaryDamage) {
      double factor =
          (double)elementAttack /
          (double)attackValue; // attack value here is phys dmg + element dmg
...
void TileCallback::onTileCombat(Creature *creature, Tile *tile) const {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    SPDLOG_ERROR("[TileCallback::onTileCombat - Creature {} type {} on tile x: "
                 "{} y: {} z: {}] "
                 "Call stack overflow. Too many lua script calls being nested.",
                 creature->getName(), type, (tile->getPosition()).getX(),
                 (tile->getPosition()).getY(), (tile->getPosition()).getZ());
    return;
  }
...
void TargetCallback::onTargetCombat(Creature *creature,
                                    Creature *target) const {
...
void AreaCombat::copyArea(const MatrixArea *input, MatrixArea *output,
                          MatrixOperation_t op) const {
  uint32_t centerY, centerX;
  ...
  if (op == MATRIXOPERATION_COPY) {
    ...
    for (uint32_t x = 0, cols = input->getCols(); x < cols; ++x) {
      for (uint32_t y = 0; y < rows; ++y) {
        ...
        (*output)[rotatedY + rotateCenterY][rotatedX + rotateCenterX] =
            (*input)[y][x];
      }
    }
...
MatrixArea *AreaCombat::createArea(const std::list<uint32_t> &list,
                                   uint32_t rows) {
  uint32_t cols;
  if (rows == 0) {
    cols = 0;
...
void MagicField::onStepInField(Creature &creature) {
  ...
  if (!isBlocking() && g_game().getWorldType() == WORLD_TYPE_NO_PVP &&
  ...
  if (it.conditionDamage) {
    ...
    if (ownerId) {
      ...
      if (g_game().getWorldType() == WORLD_TYPE_NO_PVP ||
          getTile()->hasFlag(TILESTATE_NOPVPZONE)) {
        Creature *owner = g_game().getCreatureByID(ownerId);
        if (owner) {
          if (owner->getPlayer() ||
              (owner->isSummon() && owner->getMaster()->getPlayer())) {
            harmfulField = false;
          }
        }
      }
