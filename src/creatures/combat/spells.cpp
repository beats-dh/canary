...
bool Spells::hasInstantSpell(const std::string &word) const {
  if (auto iterate = instants.find(word); iterate != instants.end()) {
    return true;
  }
  return false;
}
...
bool Spells::registerInstantLuaEvent(InstantSpell *event) {
  ...
  if (instant) {
    ...
    if (instant->getWordsMap().empty()) {
      SPDLOG_ERROR("[Spells::registerInstantLuaEvent] - Missing register words "
                   "for spell with name {}",
                   instantName);
      return false;
    }
    ...
    if (hasInstantSpell(words)) {
      SPDLOG_WARN("[Spells::registerInstantLuaEvent] - "
                  "Duplicate registered instant spell with words: {}, on spell "
                  "with name: {}",
                  words, instantName);
      return false;
    }
...
std::list<uint16_t> Spells::getSpellsByVocation(uint16_t vocationId) {
  ...
  for (const auto &it : instants) {
    ...
    if (vocSpellsIt != vocSpells.end() && vocSpellsIt->second) {
...
CombatSpell::CombatSpell(Combat *initCombat, bool initNeedTarget,
                         bool initNeedDirection)
    : Event(&g_spells().getScriptInterface()), combat(initCombat),
      needDirection(initNeedDirection), needTarget(initNeedTarget) {}
...
bool Spell::configureSpell(const pugi::xml_node &node) {
  ...
  if ((attr = node.attribute("magiclevel")) ||
      (attr = node.attribute("maglv"))) {
  ...
  if ((attr = node.attribute("cooldown")) ||
      (attr = node.attribute("exhaustion"))) {
...
bool Spell::playerSpellCheck(Player *player) const {
  ...
  if (aggressive &&
      (range < 1 || (range > 0 && !player->getAttackedCreature())) &&
  ...
  if (aggressive && !player->hasFlag(PlayerFlag_IgnoreProtectionZone) &&
...
bool Spell::playerRuneSpellCheck(Player *player, const Position &toPos) {
  ...
  if (range != -1 &&
      !g_game().canThrowObjectTo(playerPos, toPos, true, range, range)) {
...
void Spell::applyCooldownConditions(Player *player) const {
  if (cooldown > 0) {
    Condition *condition = Condition::createCondition(
        ...
        cooldown / g_configManager().getFloat(RATE_SPELL_COOLDOWN), 0, false,
        spellId);
  ...
  if (groupCooldown > 0) {
    Condition *condition = Condition::createCondition(
        ...
        groupCooldown / g_configManager().getFloat(RATE_SPELL_COOLDOWN), 0,
        false, group);
    player->addCondition(condition);
  }
  ...
  if (secondaryGroupCooldown > 0) {
    Condition *condition = Condition::createCondition(
        ...
        secondaryGroupCooldown /
            g_configManager().getFloat(RATE_SPELL_COOLDOWN),
...
void Spell::postCastSpell(Player *player, uint32_t manaCost,
                          uint32_t soulCost) {
  if (manaCost > 0) {
...
bool InstantSpell::playerCastInstant(Player *player, std::string &param) {
  ...
  if (selfTarget) {
    ...
    if (hasParam) {
      ...
      if (playerTarget && playerTarget->isAccessPlayer() &&
    ...
    if (getHasPlayerNameParam()) {
      ...
      if (playerTarget &&
          (!playerTarget->isAccessPlayer() || player->isAccessPlayer())) {
        param = playerTarget->getName();
      }
    }
  ...
  if (!allowOnSelf && playerTarget &&
      playerTarget->getName() == player->getName()) {
    player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return false;
  }
  ...
  if (pzLocked &&
      (worldType == WORLD_TYPE_PVP || worldType == WORLD_TYPE_PVP_ENFORCED)) {
...
bool InstantSpell::canThrowSpell(const Creature *creature,
                                 const Creature *target) const {
  const Position &fromPos = creature->getPosition();
  const Position &toPos = target->getPosition();
  if (fromPos.z != toPos.z ||
      (range == -1 &&
       !g_game().canThrowObjectTo(fromPos, toPos, checkLineOfSight)) ||
      (range != -1 && !g_game().canThrowObjectTo(
                          fromPos, toPos, checkLineOfSight, range, range))) {
    return false;
  }
  return true;
}
...
ReturnValue RuneSpell::canExecuteAction(const Player *player,
...
bool RuneSpell::executeUse(Player *player, Item *item, const Position &,
    ...
    if (target == nullptr) {
      ...
      if (toTile) {
        const Creature *visibleCreature =
            toTile->getBottomVisibleCreature(player);
        if (visibleCreature) {
