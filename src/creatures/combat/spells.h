...
using RuneSpellFunction = std::function<bool(
	const RuneSpell* spell, Player* player, const Position& posTo)>;
...
public:
	...
	void postCastSpell(Player* player, bool finishedCast = true,
		bool payCost = true) const;
	static void postCastSpell(Player* player, uint32_t manaCost,
		uint32_t soulCost);
	...
	SpellGroup_t getGroup() { return group; }
	void setGroup(SpellGroup_t g) { group = g; }
	SpellGroup_t getSecondaryGroup() { return secondaryGroup; }
	...
	void setAllowOnSelf(bool s) { allowOnSelf = s; }
	bool getLockedPZ() const { return pzLocked; }
	void setLockedPZ(bool b) { pzLocked = b; }
...
public:
	...
	ReturnValue canExecuteAction(const Player* player,
		const Position& toPos) override;
	bool hasOwnErrorHandler() override { return true; }
	Thing* getTarget(Player*, Creature* targetCreature, const Position&,
		uint8_t) const override {
		return targetCreature;
	}

	bool executeUse(Player* player, Item* item, const Position& fromPosition,
