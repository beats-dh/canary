...
void PrivateChatChannel::invitePlayer(const Player &player,
                                      Player &invitePlayer) {
  ...
  for (const auto &it : users) {
    it.second->sendChannelEvent(id, invitePlayer.getName(),
                                CHANNELEVENT_INVITE);
  }
}

void PrivateChatChannel::excludePlayer(const Player &player,
  ...
  for (const auto &it : users) {
    it.second->sendChannelEvent(id, excludePlayer.getName(),
  ...
  if (id == CHANNEL_GUILD) {
    ...
    if (guild && !guild->getMotd().empty()) {
      g_scheduler().addEvent(createSchedulerTask(
          150, std::bind(&Game::sendGuildMotd, &g_game(), player.getID())));
    }
  }
...
void ChatChannel::sendToAll(const std::string &message,
...
bool ChatChannel::talk(const Player &fromPlayer, SpeakClasses type,
...
bool Chat::load() {
  ...
  pugi::xml_parse_result result =
      doc.load_file("data/chatchannels/chatchannels.xml");
  if (!result) {
  ...
  for (auto channelNode : doc.child("channels").children()) {
    uint16_t channelId =
        pugi::cast<uint16_t>(channelNode.attribute("id").value());
    ...
    if (scriptAttribute) {
      if (scriptInterface.loadFile("data/chatchannels/scripts/" +
                                   std::string(scriptAttribute.as_string())) ==
          0) {
        channel.onSpeakEvent = scriptInterface.getEvent("onSpeak");
        channel.canJoinEvent = scriptInterface.getEvent("canJoin");
        channel.onJoinEvent = scriptInterface.getEvent("onJoin");
...
ChatChannel *Chat::createChannel(const Player &player, uint16_t channelId) {
  ...
  case CHANNEL_GUILD: {
    Guild *guild = player.getGuild();
    if (guild != nullptr) {
      auto ret = guildChannels.emplace(std::make_pair(
          guild->getId(), ChatChannel(channelId, guild->getName())));
  ...
  case CHANNEL_PARTY: {
    ...
    if (party != nullptr) {
      auto ret = partyChannels.emplace(
          std::make_pair(party, ChatChannel(channelId, "Party")));
      return &ret.first->second;
    }
    break;
  }
...
bool Chat::talkToChannel(const Player &player, SpeakClasses type,
  ...
  if (channelId == CHANNEL_GUILD) {
    ...
    if (rank && rank->level > 1) {
      ...
      type = TALKTYPE_CHANNEL_Y;
             ...
             (channelId == CHANNEL_PRIVATE || channelId == CHANNEL_PARTY)) {
    type = TALKTYPE_CHANNEL_Y;
  }
...
ChatChannel *Chat::getChannel(const Player &player, uint16_t channelId) {
  ...
  default: {
    ...
    if (it2 != privateChannels.end() &&
        it2->second.isInvited(player.getGUID())) {
      return &it2->second;
    }
    break;
  }
