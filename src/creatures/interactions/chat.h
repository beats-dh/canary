...
public:
  ...
  ChatChannel(uint16_t channelId, std::string channelName)
      : name(std::move(channelName)), id(channelId) {}
  ...
  bool talk(const Player &fromPlayer, SpeakClasses type,
  ...
  bool executeOnSpeakEvent(const Player &player, SpeakClasses &type,
...
public:
  ...
  void setOwner(uint32_t newOwner) { this->owner = newOwner; }
...
public:
  ...
  bool talkToChannel(const Player &player, SpeakClasses type,
