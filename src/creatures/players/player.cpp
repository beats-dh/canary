...
Player::Player(ProtocolGame_ptr p)
    : Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing),
      inbox(new Inbox(ITEM_INBOX)), client(std::move(p)) {
  ...
  for (int items = CONST_SLOT_FIRST; items <= CONST_SLOT_LAST; ++items) {
    ...
    for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); slotid++) {
      ...
      const CategoryImbuement *categoryImbuement =
          g_imbuements().getCategoryByID(
              imbuementInfo.imbuement->getCategory());
      if (categoryImbuement->agressive && playerTile &&
      ...
      int32_t duration = std::max<int32_t>(
          0, imbuementInfo.duration - EVENT_IMBUEMENT_INTERVAL / 1000);
      item->decayImbuementTime(slotid, imbuementInfo.imbuement->getID(),
                               duration);
      if (duration == 0) {
  ...
  for (const auto &[key, player] : g_game().getPlayers()) {
    if (!this->isInGhostMode() || player->isAccessPlayer()) {
      player->notifyStatusChange(
          this, value ? VIPSTATUS_TRAINING : VIPSTATUS_ONLINE, false);
    }
  }
...
void Player::addStorageValue(const uint32_t key, const int32_t value,
  ...
  if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
    if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
      ...
      familiars.emplace_back(value >> 16);
      return;
    } else {
  ...
  if (player) {
    ...
    if (!playerTile ||
        (!playerTile->hasFlag(TILESTATE_NOPVPZONE) &&
         !playerTile->hasFlag(TILESTATE_PROTECTIONZONE) &&
         player->getLevel() > static_cast<uint32_t>(g_configManager().getNumber(
                                  PROTECTION_LEVEL)) &&
...
bool Player::canWalkthroughEx(const Creature *creature) const {
  ...
  if (player) {
    const Tile *playerTile = player->getTile();
    return playerTile && (playerTile->hasFlag(TILESTATE_NOPVPZONE) ||
                          playerTile->hasFlag(TILESTATE_PROTECTIONZONE) ||
                          player->getLevel() <=
                              static_cast<uint32_t>(g_configManager().getNumber(
                                  PROTECTION_LEVEL)) ||
                          g_game().getWorldType() == WORLD_TYPE_NO_PVP);
  } else if (npc) {
    const Tile *tile = npc->getTile();
...
bool Player::isNearDepotBox() const {
  ...
  for (int32_t cx = -1; cx <= 1; ++cx) {
    for (int32_t cy = -1; cy <= 1; ++cy) {
      const Tile *posTile =
          g_game().map.getTile(static_cast<uint16_t>(pos.x + cx),
                               static_cast<uint16_t>(pos.y + cy), pos.z);
      if (!posTile) {
        continue;
      }
...
DepotLocker *Player::getDepotLocker(uint32_t depotId) {
  ...
  Container *depotChest = Item::CreateItemAsContainer(
      ITEM_DEPOT,
      static_cast<uint16_t>(g_configManager().getNumber(DEPOT_BOXES)));
  for (uint32_t i = g_configManager().getNumber(DEPOT_BOXES); i > 0; i--) {
...
Item *Player::getWriteItem(uint32_t &retWindowTextId,
                           uint16_t &retMaxWriteLen) {
...
void Player::onApplyImbuement(Imbuement *imbuement, Item *item, uint8_t slot,
  ...
  if (item->getImbuementInfo(slot, &imbuementInfo)) {
    SPDLOG_ERROR(
        "[Player::onApplyImbuement] - An error occurred while player with name "
        "{} try to apply imbuement, item already contains imbuement",
        this->getName());
    this->sendImbuementResult(
        "An error ocurred, please reopen imbuement window.");
    return;
  }
  ...
  for (auto &[key, value] : items) {
    ...
    if (this->getItemTypeCount(key) + this->getStashItemCount(itemType.id) <
        value) {
  ...
  if (!g_game().removeMoney(this, price, 0, true)) {
    std::string message =
        "You don't have " + std::to_string(price) + " gold coins.";

    SPDLOG_ERROR(
        "[Player::onApplyImbuement] - An error occurred while player with name "
        "{} try to apply imbuement, player do not have money",
        this->getName());
  ...
  for (auto &[key, value] : items) {
    ...
    if (inventoryItemCount > 0 &&
        removeItemOfType(key, inventoryItemCount, -1, false)) {
      mathItemCount = mathItemCount - inventoryItemCount;
    }

    const ItemType &itemType = Item::items[key];

    withdrawItemMessage << "Using " << mathItemCount << "x " << itemType.name
                        << " from your supply stash. ";
  ...
  if (!protectionCharm && uniform_random(1, 100) > baseImbuement->percent) {
    ...
    sendImbuementResult(
        "Oh no!\n\nThe imbuement has failed. You have lost the astral sources "
        "and gold you needed for the imbuement.\n\nNext time use a protection "
        "charm to better your chances.");
    openImbuementWindow(item);
    return;
  }
...
void Player::onClearImbuement(Item *item, uint8_t slot) {
  ...
  if (!item->getImbuementInfo(slot, &imbuementInfo)) {
    SPDLOG_ERROR(
        "[Player::onClearImbuement] - An error occurred while player with name "
        "{} try to apply imbuement, item not contains imbuement",
        this->getName());
    this->sendImbuementResult(
        "An error ocurred, please reopen imbuement window.");
    return;
  ...
  const BaseImbuement *baseImbuement =
      g_imbuements().getBaseByID(imbuementInfo.imbuement->getBaseID());
  if (!baseImbuement) {
    return;
  }

  if (!g_game().removeMoney(this, baseImbuement->removeCost, 0, true)) {
    std::string message = "You don't have " +
                          std::to_string(baseImbuement->removeCost) +
                          " gold coins.";
...
void Player::onUpdateTileItem(const Tile *updateTile, const Position &pos,
                              const Item *oldItem, const ItemType &oldType,
                              const Item *newItem, const ItemType &newType) {
  Creature::onUpdateTileItem(updateTile, pos, oldItem, oldType, newItem,
                             newType);
...
void Player::onRemoveTileItem(const Tile *fromTile, const Position &pos,
...
void Player::onCreatureAppear(Creature *creature, bool isLogin) {
  ...
  if (isLogin && creature == this) {
    ...
    if (getLastLogout() != 0) {
      ...
      offlineTime =
          std::min<int32_t>(time(nullptr) - getLastLogout(), 86400 * 21);
    } else {
      offlineTime = 0;
    }
...
void Player::onThink(uint32_t interval) {
  ...
  if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer() &&
    ...
    const int32_t kickAfterMinutes =
        g_configManager().getNumber(KICK_AFTER_MINUTES);
    if (idleTime > (kickAfterMinutes * 60000) + 60000) {
      removePlayer(true);
    } else if (client && idleTime == 60000 * kickAfterMinutes) {
      std::ostringstream ss;
      ss << "There was no variation in your behaviour for " << kickAfterMinutes
         << " minutes. You will be disconnected in one minute if there is no "
            "change in your actions until then.";
...
void Player::addMessageBuffer() {
  if (MessageBufferCount > 0 &&
      g_configManager().getNumber(MAX_MESSAGEBUFFER) != 0 &&
...
void Player::removeMessageBuffer() {
  ...
  const int32_t maxMessageBuffer =
      g_configManager().getNumber(MAX_MESSAGEBUFFER);
...
void Player::addExperience(Creature *target, uint64_t exp,
                           bool sendText /* = false*/) {
...
BlockType_t Player::blockHit(Creature *attacker, CombatType_t combatType,
  ...
  if (damage > 0) {
    for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
      ...
      const ItemType &it = Item::items[item->getID()];
      if (it.abilities) {
        const int16_t &absorbPercent =
            it.abilities->absorbPercent[combatTypeToIndex(combatType)];
        if (absorbPercent != 0) {
      ...
      for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); slotid++) {
        ...
        const int16_t &imbuementAbsorbPercent =
            imbuementInfo.imbuement
                ->absorbPercent[combatTypeToIndex(combatType)];
    ...
    if (lastHitCreature && lastHitCreature->getMonster()) {
      if (charmRuneBless != 0) {
        const MonsterType *mType =
            g_monsters().getMonsterType(lastHitCreature->getName());
  ...
  for (Creature *spectator : spectators) {
    ...
    if (const Player *player = spectator->getPlayer()) {
      oldStackPosVector.push_back(
          player->canSeeCreature(this)
              ? tile->getStackposOfCreature(player, this)
              : -1);
    }
...
void Player::addInFightTicks(bool pzlock /*= false*/) {
  ...
  Condition *condition =
      Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT,
                                 g_configManager().getNumber(PZ_LOCKED), 0);
  addCondition(condition);
}
  ...
  if (message) {
    if (status == VIPSTATUS_ONLINE) {
      client->sendTextMessage(TextMessage(
          MESSAGE_FAILURE, loginPlayer->getName() + " has logged in."));
    } else if (status == VIPSTATUS_OFFLINE) {
      client->sendTextMessage(TextMessage(
          MESSAGE_FAILURE, loginPlayer->getName() + " has logged out."));
    }
  }
}
...
ReturnValue Player::queryAdd(int32_t index, const Thing &thing, uint32_t count,
      ...
      if (!g_configManager().getBoolean(CLASSIC_EQUIPMENT_SLOTS)) {
        ...
        if (type == WEAPON_NONE || type == WEAPON_SHIELD ||
          ...
          if (type == WEAPON_DISTANCE &&
              inventory[CONST_SLOT_RIGHT]->isQuiver()) {
            ret = RETURNVALUE_NOERROR;
          } else {
            ret = RETURNVALUE_BOTHHANDSNEEDTOBEFREE;
          }
  ...
  case CONST_SLOT_AMMO: {
    if ((slotPosition & SLOTP_AMMO) ||
        g_configManager().getBoolean(CLASSIC_EQUIPMENT_SLOTS)) {
      ret = RETURNVALUE_NOERROR;
    }
    break;
  }
  ...
  if (ret == RETURNVALUE_NOERROR || ret == RETURNVALUE_NOTENOUGHROOM) {
    ...
    if (!g_moveEvents().onPlayerEquip(const_cast<Player &>(*this),
...
void Player::stashContainer(StashContainerList itemDict) {
  ...
  if (getStashSize(stashItemDict) > g_configManager().getNumber(STASH_ITEMS)) {
    sendCancelMessage(
        "You don't have capacity in the Supply Stash to stow all this item.");
    return;
  }
  ...
  for (auto stashIterator : itemDict) {
    ...
    if (g_game().internalRemoveItem(
            stashIterator.first, stashIterator.second) == RETURNVALUE_NOERROR) {
  ...
  retString << "Stowed " << totalStowed << " object"
            << (totalStowed > 1 ? "s." : ".");
  if (moved) {
    retString << " Moved " << movedItems << " object"
              << (movedItems > 1 ? "s." : ".");
    movedItems = 0;
  }
    ...
    if (!ignoreEquipped && item->getID() == itemId) {
      ...
      for (ContainerIterator it = container->iterator(); it.hasNext();
        ...
        if (containerItem->getID() == itemId) {
          ...
          if (player &&
              !player->canSellImbuedItem(containerItem, ignoreImbued)) {
            continue;
          }
...
std::map<uint16_t, uint16_t> Player::getInventoryItemsId() const {
  ...
  for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
    ...
    if (rootSearch != itemMap.end()) {
      itemMap[item->getID()] =
          itemMap[item->getID()] +
          static_cast<uint16_t>(Item::countByType(item, -1));
    } else {
      itemMap.emplace(item->getID(),
                      static_cast<uint16_t>(Item::countByType(item, -1)));
    }
    ...
    if (Container *container = item->getContainer()) {
      for (ContainerIterator it = container->iterator(); it.hasNext();
        ...
        if (containerSearch != itemMap.end()) {
          itemMap[(*it)->getID()] =
              itemMap[(*it)->getID()] +
              static_cast<uint16_t>(Item::countByType(*it, -1));
        } else {
...
void Player::getAllItemTypeCountAndSubtype(
  ...
  for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
    ...
    if (Item::items[itemId].isFluidContainer()) {
      countMap[static_cast<uint32_t>(itemId) |
               (static_cast<uint32_t>(item->getFluidType()) << 16)] +=
          item->getItemCount();
    } else {
...
bool Player::updateSaleShopList(const Item *item) {
  ...
  g_dispatcher().addTask(
      createTask(std::bind(&Game::updatePlayerSaleItems, &g_game(), getID())));
...
bool Player::hasShopItemForSale(uint16_t itemId, uint8_t subType) const {
  ...
  return std::any_of(
      shoplist.begin(), shoplist.end(), [&](const ShopBlock &shopBlock) {
        return shopBlock.itemId == itemId && shopBlock.itemBuyPrice != 0 &&
               (!itemType.isFluidContainer() ||
                shopBlock.itemSubType == subType);
      });
}
  ...
  if (g_configManager().getBoolean(EXPERIENCE_FROM_PLAYERS)) {
    ...
    if (attackerPlayer && attackerPlayer != this && skillLoss &&
        std::abs(static_cast<int32_t>(attackerPlayer->getLevel() - level)) <=
            g_configManager().getNumber(EXP_FROM_PLAYERS_LEVEL_RANGE)) {
  ...
  if (Player *targetPlayer = target->getPlayer()) {
    if (targetPlayer && targetPlayer->getZone() == ZONE_PVP) {
      ...
      if (!Combat::isInPvpZone(this, targetPlayer) &&
          hasAttacked(targetPlayer) && !targetPlayer->hasAttacked(this) &&
        ...
        if (lastHit && hasCondition(CONDITION_INFIGHT)) {
          ...
          Condition *condition = Condition::createCondition(
              ...
              g_configManager().getNumber(WHITE_SKULL_TIME), 0);
          addCondition(condition);
        }
      }
    }
             ...
             TaskHuntingSlot *taskSlot =
                 getTaskHuntingWithCreature(monster->getRaceId())) {
    if (const TaskHuntingOption *option =
            g_ioprey().GetTaskRewardOption(taskSlot)) {
      taskSlot->currentKills += 1;
      if ((taskSlot->upgrade &&
           taskSlot->currentKills >= option->secondKills) ||
          (!taskSlot->upgrade &&
           taskSlot->currentKills >= option->firstKills)) {
        taskSlot->state = PreyTaskDataState_Completed;
        sendTextMessage(MESSAGE_STATUS,
                        "You succesfully finished your hunting task. Your "
...
void Player::changeSoul(int32_t soulChange) {
  if (soulChange > 0) {
    soul += std::min<int32_t>(soulChange *
                                  g_configManager().getFloat(RATE_SOUL_REGEN),
...
bool Player::canWear(uint16_t lookType, uint8_t addons) const {
  if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && lookType != 0 &&
    ...
    SPDLOG_WARN("[Player::canWear] An unregistered creature looktype type with "
                "id '{}' was blocked to prevent client crash.",
                lookType);
    return false;
  }
  ...
  if (player && player->getSkull() == SKULL_NONE) {
    if (player == this) {
      for (const auto &kill : unjustifiedKills) {
        if (kill.unavenged &&
            (time(nullptr) - kill.time) <
                g_configManager().getNumber(ORANGE_SKULL_DURATION) * 24 * 60 *
                    60) {
          return SKULL_ORANGE;
        }
      }
...
bool Player::hasKilled(const Player *player) const {
  for (const auto &kill : unjustifiedKills) {
    if (kill.target == player->getGUID() &&
        (time(nullptr) - kill.time) <
            g_configManager().getNumber(ORANGE_SKULL_DURATION) * 24 * 60 * 60 &&
        kill.unavenged) {
      return true;
    }
  }
...
void Player::addUnjustifiedDead(const Player *attacked) {
  ...
  if (getSkull() != SKULL_BLACK) {
    if (dayKills >= 2 * g_configManager().getNumber(DAY_KILLS_TO_RED) ||
        weekKills >= 2 * g_configManager().getNumber(WEEK_KILLS_TO_RED) ||
        monthKills >= 2 * g_configManager().getNumber(MONTH_KILLS_TO_RED)) {
      setSkull(SKULL_BLACK);
      // start black skull time
      skullTicks = static_cast<int64_t>(
                       g_configManager().getNumber(BLACK_SKULL_DURATION)) *
                   24 * 60 * 60 * 1000;
    } else if (dayKills >= g_configManager().getNumber(DAY_KILLS_TO_RED) ||
               weekKills >= g_configManager().getNumber(WEEK_KILLS_TO_RED) ||
               monthKills >= g_configManager().getNumber(MONTH_KILLS_TO_RED)) {
      setSkull(SKULL_RED);
      // reset red skull time
      skullTicks = static_cast<int64_t>(
                       g_configManager().getNumber(RED_SKULL_DURATION)) *
                   24 * 60 * 60 * 1000;
    }
  }
...
double Player::getLostPercent() const {
  ...
  uint8_t maxBlessing = (operatingSystem == CLIENTOS_NEW_WINDOWS ||
                         operatingSystem == CLIENTOS_NEW_MAC)
                            ? 8
                            : 6;
  for (int i = 2; i <= maxBlessing; i++) {
    if (hasBlessing(i)) {
  ...
  if (level >= 24) {
    ...
    lossPercent =
        ((tmpLevel + 50) * 50 * ((tmpLevel * tmpLevel) - (5 * tmpLevel) + 8)) /
        experience;
  } else {
    lossPercent = 5;
  }
...
void Player::setTibiaCoins(int32_t v) { coinBalance = v; }
...
void Player::sendUnjustifiedPoints() {
  if (client) {
    ...
    auto dayMax =
        ((isRed ? 2 : 1) * g_configManager().getNumber(DAY_KILLS_TO_RED));
    auto weekMax =
        ((isRed ? 2 : 1) * g_configManager().getNumber(WEEK_KILLS_TO_RED));
    auto monthMax =
        ((isRed ? 2 : 1) * g_configManager().getNumber(MONTH_KILLS_TO_RED));
...
bool Player::addOfflineTrainingTries(skills_t skill, uint64_t tries) {
  ...
  if (skill == SKILL_MAGLEVEL) {
    ...
    oldPercentToNextLevel =
        static_cast<long double>(skills[skill].tries * 100) / nextReqTries;
  ...
  std::ostringstream ss;
  ss << std::fixed << std::setprecision(2) << "Your "
     << ucwords(getSkillName(skill)) << " skill changed from level "
     << oldSkillValue << " (with " << oldPercentToNextLevel
     << "% progress towards level " << (oldSkillValue + 1) << ") to level "
     << newSkillValue << " (with " << newPercentToNextLevel
     << "% progress towards level " << (newSkillValue + 1) << ')';
  sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
  return sendUpdate;
}
...
void Player::updateNpcExhausted() { lastNpcInteraction = OTSYS_TIME(); }

uint64_t Player::getItemCustomPrice(uint16_t itemId,
                                    bool buyPrice /* = false*/) const {
...
bool Player::addItemFromStash(uint16_t itemId, uint32_t itemCount) {
  ...
  while (itemCount > 0) {
    ...
    if (g_game().internalQuickLootItem(this, newItem,
  ...
  // This check is necessary because we need to block it when we retrieve an
  // item from depot search.
  ...
  if (allItems) {
    for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
      ...
      if (inventoryItem->getID() == item->getID()) {
        itemDict.push_back(std::pair<Item *, uint32_t>(
            inventoryItem, inventoryItem->getItemCount()));
      }
    ...
    for (Item *containerItem : item->getContainer()->getItems(true)) {
      ...
      if (g_configManager().getBoolean(STASH_MOVING) && containerItem &&
          !containerItem->isStackable() && validDepot) {
        g_game().internalMoveItem(containerItem->getParent(),
                                  getDepotChest(depotChest, true),
                                  INDEX_WHEREEVER, containerItem,
                                  containerItem->getItemCount(), nullptr);
        movedItems++;
        moved = true;
      }
    }
...
void Player::openPlayerContainers() {
  ...
  for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
    ...
    if (itemContainer) {
      ...
      for (ContainerIterator it = itemContainer->iterator(); it.hasNext();
           it.advance()) {
        Container *subContainer = (*it)->getContainer();
        if (subContainer) {
          uint8_t subcid = (*it)->getIntAttr(ITEM_ATTRIBUTE_OPENCONTAINER);
          if (subcid > 0) {
            openContainersList.emplace_back(
                std::make_pair(subcid, subContainer));
          }
        }
      }
    }
...
void Player::initializePrey() {
  if (preys.empty()) {
    for (uint8_t slotId = PreySlot_First; slotId <= PreySlot_Last; slotId++) {
      ...
      } else if (slot->id == PreySlot_Three &&
                 !g_configManager().getBoolean(PREY_FREE_THIRD_SLOT)) {
    ...
    for (uint8_t slotId = PreySlot_First; slotId <= PreySlot_Last; slotId++) {
      ...
      if (!g_configManager().getBoolean(TASK_HUNTING_ENABLED)) {
        slot->state = PreyTaskDataState_Inactive;
                 ...
                 !g_configManager().getBoolean(TASK_HUNTING_FREE_THIRD_SLOT)) {
...
bool Player::isCreatureUnlockedOnTaskHunting(const MonsterType *mtype) const {
  ...
  return getBestiaryKillCount(mtype->info.raceid) >=
         mtype->info.bestiaryToUnlock;
}
...
void Player::requestDepotItems() {
  ...
  for (Item *locker : depotLocker->getItemList()) {
    ...
    for (ContainerIterator it = c->iterator(); it.hasNext(); it.advance()) {
      ...
      uint8_t itemTier =
          Item::items[(*it)->getID()].upgradeClassification > 0 ? 1 : 0;
      ...
      if (itemMap_it == itemMap.end()) {
        ...
        count++;
      } else if (auto itemTier_it = itemMap[(*it)->getID()].find(itemTier);
                 itemTier_it == itemMap[(*it)->getID()].end()) {
  ...
  for (const auto &[itemId, itemCount] : getStashItems()) {
    ...
    if (itemMap_it == itemMap.end()) {
      ...
      count++;
    } else if (auto itemTier_it = itemMap[itemId].find(itemTier);
               itemTier_it == itemMap[itemId].end()) {
...
void Player::requestDepotSearchItem(uint16_t itemId, uint8_t tier) {
  ...
  for (Item *locker : depotLocker->getItemList()) {
    ...
    for (ContainerIterator it = c->iterator(); it.hasNext(); it.advance()) {
      Item *item = *it;
      // To-Do: When forge is complete check for item tier here using
      // 'depotSearchOnItem.second'.
  ...
  sendDepotSearchResultDetail(itemId, tier, depotCount, depotItems, inboxCount,
                              inboxItems, stashCount);
}

void Player::retrieveAllItemsFromDepotSearch(uint16_t itemId, uint8_t tier,
                                             bool isDepot) {
  const DepotLocker *depotLocker = getDepotLocker(getLastDepotId());
  if (!depotLocker) {
    return;
  ...
  for (Item *locker : depotLocker->getItemList()) {
    const Container *c = locker->getContainer();
    if (!c || c->empty() || (c->isInbox() && isDepot) || // Retrieve from inbox.
  ...
  for (Item *item : itemsVector) {
    ...
    if (ret = g_game().internalQuickLootItem(this, item,
                                             OBJECTCATEGORY_STASHRETRIEVE);
        ret == RETURNVALUE_NOERROR) {
      continue;
    }

    // If the retrieve fails to move the item to the stash retrieve container,
    // let's add the item anywhere.
    if (ret =
            g_game().internalMoveItem(item->getParent(), this, INDEX_WHEREEVER,
...
void Player::openContainerFromDepotSearch(const Position &pos) {
  ...
  Container *container =
      item->getParent() ? item->getParent()->getContainer() : nullptr;
  if (!container) {
    sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
    return;
  }
