...
void Player::updateInventoryImbuement(bool init /* = false */) {
	...
	for (int items = CONST_SLOT_FIRST; items <= CONST_SLOT_LAST; ++items) {
		...
		for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); slotid++) {
			...
			const CategoryImbuement* categoryImbuement = g_imbuements().getCategoryByID(
...
bool Player::isNearDepotBox() const {
	...
	for (int32_t cx = -1; cx <= 1; ++cx) {
		for (int32_t cy = -1; cy <= 1; ++cy) {
			const Tile* posTile = g_game().map.getTile(static_cast<uint16_t>(pos.x + cx),
	...
	if (damage > 0) {
		for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
			...
			for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); slotid++) {
				...
				const int16_t& imbuementAbsorbPercent = imbuementInfo.imbuement
...
void Player::stashContainer(StashContainerList itemDict) {
	...
	for (auto stashIterator : itemDict) {
		...
		if (g_game().internalRemoveItem(
				stashIterator.first, stashIterator.second)
			== RETURNVALUE_NOERROR) {
	...
	if (const Item* item = thing->getItem()) {
		if (const Container* container = item->getContainer()) {
			...
			if (container->isRemoved() || !Position::areInRange<1, 1, 0>(getPosition(), container->getPosition())) {
				...
				onSendContainer(container);
			} else if (const Container* topContainer = dynamic_cast<const Container*>(
...
void Player::changeSoul(int32_t soulChange) {
	if (soulChange > 0) {
		soul += std::min<int32_t>(soulChange * g_configManager().getFloat(RATE_SOUL_REGEN),
...
double Player::getLostPercent() const {
	...
	uint8_t maxBlessing = (operatingSystem == CLIENTOS_NEW_WINDOWS || operatingSystem == CLIENTOS_NEW_MAC)
		? 8
		: 6;
	for (int i = 2; i <= maxBlessing; i++) {
		if (hasBlessing(i)) {
...
void Player::retrieveAllItemsFromDepotSearch(uint16_t itemId, uint8_t tier,
	...
	for (Item* item : itemsVector) {
		...
		if (ret = g_game().internalMoveItem(item->getParent(), this, INDEX_WHEREEVER,
