      ...
      for (auto childNode : baseNode.children()) {
        ...
        if (strcasecmp(type.c_str(), "item") == 0) {
          ...
          if (strcasecmp(effecttype.c_str(), "skill") == 0) {
            ...
            uint8_t usenormalskill =
                1; // 1 = skill normal, 2 = magiclevel, 3 = leechs/crit
            ...
            if (usenormalskill == 1) {
              ...
              if ((attr = childNode.attribute("chance")))
                chance =
                    std::min<uint32_t>(100, pugi::cast<int32_t>(attr.value()));
            ...
            uint32_t percent =
                std::min<uint32_t>(100, pugi::cast<uint32_t>(attr.value()));
...
BaseImbuement *Imbuements::getBaseByID(uint16_t id) {
  auto baseImbuements =
      std::find_if(basesImbuement.begin(), basesImbuement.end(),
...
CategoryImbuement *Imbuements::getCategoryByID(uint16_t id) {
  auto categoryImbuements =
      std::find_if(categoriesImbuement.begin(), categoriesImbuement.end(),
...
std::vector<Imbuement *> Imbuements::getImbuements(const Player *player,
  ...
  for (auto &[key, value] : imbuementMap) {
    ...
    const CategoryImbuement *categoryImbuement =
        getCategoryByID(imbuement->getCategory());
