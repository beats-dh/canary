...
bool Imbuements::loadFromXml(bool /* reloading */) {
	...
	if (!result) {
		printXMLError("Error - Imbuements::loadFromXml", "data/XML/imbuements.xml",
			result);
		return false;
	}
	...
	for (auto baseNode : doc.child("imbuements").children()) {
		...
		if (strcasecmp(baseNode.name(), "base") == 0) {
			...
			auto imbuements = imbuementMap.emplace(
				std::piecewise_construct, std::forward_as_tuple(runningid),
			...
			for (auto childNode : baseNode.children()) {
				...
				if (strcasecmp(type.c_str(), "item") == 0) {
					if (!(attr = childNode.attribute("value"))) {
						SPDLOG_WARN("Missing item ID for imbuement name '{}'",
							imbuement.name);
						continue;
					}
					...
					auto it2 = std::find_if(
						imbuement.items.begin(), imbuement.items.end(),
						[sourceId](const std::pair<uint16_t, uint16_t>& source) -> bool {
					...
					if (!(attr = childNode.attribute("type"))) {
						SPDLOG_WARN("Missing effect type for imbuement name: {}",
							imbuement.name);
						continue;
					}
					...
					if (strcasecmp(effecttype.c_str(), "skill") == 0) {
						if (!(attr = childNode.attribute("value"))) {
							SPDLOG_WARN("Missing effect value for imbuement name {}",
								imbuement.name);
							continue;
						}
						...
						if (!(attr = childNode.attribute("combat"))) {
							SPDLOG_WARN("Missing combat for imbuement name {}",
								imbuement.name);
							continue;
						}
						...
						if (combatType == COMBAT_NONE) {
							SPDLOG_WARN("Unknown combat type for element {}",
								attr.as_string());
							continue;
						}
						...
						if (!(attr = childNode.attribute("value"))) {
							SPDLOG_WARN(
								"Missing damage reduction percentage for imbuement name {}",
								imbuement.name);
							continue;
						}
						...
						if (!(attr = childNode.attribute("value"))) {
							SPDLOG_WARN("Missing speed value for imbuement name {}",
								imbuement.name);
							continue;
						}
						...
						if (!(attr = childNode.attribute("value"))) {
							SPDLOG_WARN("Missing cap value for imbuement name {}",
								imbuement.name);
							continue;
						}
...
BaseImbuement* Imbuements::getBaseByID(uint16_t id) {
	auto baseImbuements = std::find_if(basesImbuement.begin(), basesImbuement.end(),
		[id](const BaseImbuement& groupImbuement) {
...
CategoryImbuement* Imbuements::getCategoryByID(uint16_t id) {
	auto categoryImbuements = std::find_if(categoriesImbuement.begin(), categoriesImbuement.end(),
		[id](const CategoryImbuement& categoryImbuement) {
			return categoryImbuement.id == id;
		});

	return categoryImbuements != categoriesImbuement.end() ? &*categoryImbuements
														   : nullptr;
}

std::vector<Imbuement*> Imbuements::getImbuements(const Player* player,
	Item* item) {
	...
	for (auto& [key, value] : imbuementMap) {
		...
		// Parse the storages for each imbuement in imbuements.xml and config.lua
		// (enable/disable storage)
		int32_t storageValue;
		if (g_configManager().getBoolean(TOGGLE_IMBUEMENT_SHRINE_STORAGE) && imbuement->getStorage() != 0 && !player->getStorageValue(imbuement->getStorage(), storageValue) && imbuement->getBaseID() >= 1 && imbuement->getBaseID() <= 3) {
			continue;
		}

		// Send only the imbuements registered on item (in items.xml) to the
		// imbuement window
		const CategoryImbuement* categoryImbuement = getCategoryByID(imbuement->getCategory());
		if (!item->hasImbuementType(
				static_cast<ImbuementTypes_t>(categoryImbuement->id),
				imbuement->getBaseID())) {
			continue;
		}

		// If the item is already imbued with an imbuement, remove the imbuement
