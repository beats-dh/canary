...
struct BaseImbuement {
  BaseImbuement(uint16_t initId, std::string initName,
                uint32_t initProtectionPrice, uint32_t initPrice,
                uint32_t initRemoveCost, int32_t initDuration,
                uint8_t initPercent)
      : id(initId), name(std::move(initName)),
        protectionPrice(initProtectionPrice), price(initPrice),
        removeCost(initRemoveCost), duration(initDuration),
        percent(initPercent) {}
...
public:
  ...
  uint16_t getBaseID() const { return baseid; }
  ...
  uint16_t getIconID() { return icon + (baseid - 1); }
