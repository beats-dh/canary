...
public:
  ...
  uint8_t getBaseId() const { return baseId; }
  ...
  uint32_t getHealthGainTicks() const {
    return gainHealthTicks /
           g_configManager().getFloat(RATE_HEALTH_REGEN_SPEED);
  }
  ...
  uint32_t getBaseAttackSpeed() const { return attackSpeed; }
  ...
  bool getMagicShield() const { return magicShield; }
...
private:
  ...
  float skillMultipliers[SKILL_LAST + 1] = {1.5f, 2.0f, 2.0f, 2.0f,
                                            2.0f, 1.5f, 1.1f};
