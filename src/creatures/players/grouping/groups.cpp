...
const phmap::flat_hash_map<std::string, PlayerCustomFlags>
    ParsePlayerCustomFlagMap = {
...
bool Groups::load() {
  ...
  for (auto groupNode : doc.child("groups").children()) {
    ...
    group.maxDepotItems =
        pugi::cast<uint32_t>(groupNode.attribute("maxdepotitems").value());
    group.maxVipEntries =
        pugi::cast<uint32_t>(groupNode.attribute("maxvipentries").value());
    ...
    group.customflags =
        pugi::cast<uint64_t>(groupNode.attribute("customflags").value());
