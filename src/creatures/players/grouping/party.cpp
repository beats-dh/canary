...
void Party::disband() {
  ...
  currentLeader->sendTextMessage(MESSAGE_PARTY_MANAGEMENT,
  ...
  for (Player *member : memberList) {
    ...
    member->sendTextMessage(MESSAGE_PARTY_MANAGEMENT,
                            "Your party has been disbanded.");
  }
...
bool Party::isPlayerInvited(const Player *player) const {
  return std::find(inviteList.begin(), inviteList.end(), player) !=
         inviteList.end();
}
...
void Party::broadcastPartyMessage(MessageClasses msgClass,
...
bool Party::setSharedExperience(Player *player, bool newSharedExpActive) {
  ...
  if (newSharedExpActive) {
    ...
    if (this->sharedExpEnabled) {
      leader->sendTextMessage(MESSAGE_PARTY_MANAGEMENT,
                              "Shared Experience is now active.");
    } else {
      leader->sendTextMessage(MESSAGE_PARTY_MANAGEMENT,
                              "Shared Experience has been activated, but some "
                              "members of your party are inactive.");
    }
  } else {
    leader->sendTextMessage(MESSAGE_PARTY_MANAGEMENT,
                            "Shared Experience has been deactivated.");
  }
...
bool Party::canUseSharedExperience(const Player *player) const {
  ...
  if (!player->hasFlag(PlayerFlag_NotGainInFight)) {
    ...
    if (timeDiff >
        static_cast<uint64_t>(g_configManager().getNumber(PZ_LOCKED))) {
      return false;
    }
  }
  return true;
...
void Party::showPlayerStatus(Player *player, Player *member, bool showStatus) {
  ...
  if (showStatus) {
    for (Creature *summon : member->getSummons()) {
      ...
      player->sendPartyCreatureHealth(
          summon, std::ceil((static_cast<double>(summon->getHealth()) /
                             std::max<int32_t>(summon->getMaxHealth(), 1)) *
                            100));
    }
    for (Creature *summon : player->getSummons()) {
      member->sendPartyCreatureShowStatus(summon, showStatus);
      member->sendPartyCreatureHealth(
          summon, std::ceil((static_cast<double>(summon->getHealth()) /
                             std::max<int32_t>(summon->getMaxHealth(), 1)) *
                            100));
    }
    player->sendPartyCreatureHealth(
        member, std::ceil((static_cast<double>(member->getHealth()) /
                           std::max<int32_t>(member->getMaxHealth(), 1)) *
                          100));
    member->sendPartyCreatureHealth(
        player, std::ceil((static_cast<double>(player->getHealth()) /
                           std::max<int32_t>(player->getMaxHealth(), 1)) *
                          100));
    player->sendPartyPlayerMana(
        member, std::ceil((static_cast<double>(member->getMana()) /
                           std::max<int32_t>(member->getMaxMana(), 1)) *
                          100));
    member->sendPartyPlayerMana(
        player, std::ceil((static_cast<double>(player->getMana()) /
                           std::max<int32_t>(player->getMaxMana(), 1)) *
                          100));
  } else {
    for (Creature *summon : player->getSummons()) {
      member->sendPartyCreatureShowStatus(summon, showStatus);
...
void Party::updatePlayerStatus(Player *player) {
  ...
  for (Player *member : memberList) {
    bool condition =
        (maxDistance == 0 ||
         (Position::getDistanceX(player->getPosition(),
                                 member->getPosition()) <= maxDistance &&
          Position::getDistanceY(player->getPosition(),
                                 member->getPosition()) <= maxDistance));
    if (condition) {
  ...
  bool condition =
      (maxDistance == 0 ||
       (Position::getDistanceX(player->getPosition(), leader->getPosition()) <=
            maxDistance &&
        Position::getDistanceY(player->getPosition(), leader->getPosition()) <=
            maxDistance));
  if (condition) {
...
void Party::updatePlayerStatus(Player *player, const Position &oldPos,
                               const Position &newPos) {
  int32_t maxDistance = g_configManager().getNumber(PARTY_LIST_MAX_DISTANCE);
  if (maxDistance != 0) {
    for (Player *member : memberList) {
      bool condition1 =
          (Position::getDistanceX(oldPos, member->getPosition()) <=
               maxDistance &&
           Position::getDistanceY(oldPos, member->getPosition()) <=
               maxDistance);
      bool condition2 =
          (Position::getDistanceX(newPos, member->getPosition()) <=
               maxDistance &&
           Position::getDistanceY(newPos, member->getPosition()) <=
               maxDistance);
    ...
    bool condition1 =
        (Position::getDistanceX(oldPos, leader->getPosition()) <= maxDistance &&
         Position::getDistanceY(oldPos, leader->getPosition()) <= maxDistance);
    bool condition2 =
        (Position::getDistanceX(newPos, leader->getPosition()) <= maxDistance &&
         Position::getDistanceY(newPos, leader->getPosition()) <= maxDistance);
    if (condition1 && !condition2) {
      showPlayerStatus(player, leader, false);
    } else if (!condition1 && condition2) {
      showPlayerStatus(player, leader, true);
...
void Party::updatePlayerHealth(const Player *player, const Creature *target,
...
void Party::addPlayerLoot(const Player *player, const Item *item) {
  ...
  if (auto it = playerAnalyzer->lootMap.find(item->getID());
      it != playerAnalyzer->lootMap.end()) {
    (*it).second += count;
  } else {
  ...
  if (priceType == LEADER_PRICE) {
    playerAnalyzer->lootPrice +=
        leader->getItemCustomPrice(item->getID()) * count;
  } else {
...
void Party::addPlayerSupply(const Player *player, const Item *item) {
  ...
  if (auto it = playerAnalyzer->supplyMap.find(item->getID());
      it != playerAnalyzer->supplyMap.end()) {
    (*it).second += 1;
  } else {
  ...
  if (priceType == LEADER_PRICE) {
    playerAnalyzer->supplyPrice +=
        leader->getItemCustomPrice(item->getID(), true);
  } else {
...
void Party::reloadPrices() {
  for (PartyAnalyzer *analyzer : membersData) {
    if (priceType == MARKET_PRICE) {
      analyzer->lootPrice =
          g_game().getItemMarketPrice(analyzer->lootMap, false);
      analyzer->supplyPrice =
          g_game().getItemMarketPrice(analyzer->supplyMap, true);
      continue;
    }
    ...
    for (const auto it : analyzer->supplyMap) {
      analyzer->supplyPrice +=
          leader->getItemCustomPrice(it.first, true) * it.second;
    }
  }
}
