...
bool Familiars::loadFromXml() {
	...
	if (!result) {
		...
		printXMLError("[Familiars::loadFromXml] - ", "data/XML/familiars.xml",
			result);
		return false;
	}
	...
	for (auto familiarsNode : doc.child("familiars").children()) {
		...
		if (vocation > VOCATION_LAST) {
			SPDLOG_WARN("[Familiars::loadFromXml] - Invalid familiar vocation {}",
				vocation);
			continue;
		}
	...
	for (uint16_t vocation = VOCATION_NONE; vocation <= VOCATION_LAST;
		 ++vocation) {
...
const Familiar* Familiars::getFamiliarByLookType(uint16_t vocation,
