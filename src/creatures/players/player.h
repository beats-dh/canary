...
public:
  ...
  void setGUID(uint32_t newGuid) { this->guid = newGuid; }
  uint32_t getGUID() const { return guid; }
  bool canSeeInvisibility() const override {
    return hasFlag(PlayerFlag_CanSenseInvisibility) || group->access;
  }

  void setDailyReward(uint8_t reward) { this->isDailyReward = reward; }
  ...
  void addOfflineTrainingTime(int32_t addTime) {
    offlineTrainingTime =
        std::min<int32_t>(12 * 3600 * 1000, offlineTrainingTime + addTime);
  }
  void removeOfflineTrainingTime(int32_t removeTime) {
    offlineTrainingTime =
        std::max<int32_t>(0, offlineTrainingTime - removeTime);
  }
  ...
  void addBestiaryTrackerList(MonsterType *mtype) {
    if (client) {
      auto it =
          std::find(BestiaryTracker.begin(), BestiaryTracker.end(), mtype);
  ...
  void setParty(Party *newParty) { this->party = newParty; }
  ...
  bool hasImbuingItem() { return imbuingItem != nullptr; }
  ...
  bool hasBlessing(uint8_t index) const { return blessings[index - 1] != 0; }
  uint8_t getBlessingCount(uint8_t index) const { return blessings[index - 1]; }
  ...
  void setSpecialMenuAvailable(bool supplyStashBool, bool marketMenuBool,
                               bool depotSearchBool) {

    // Closing depot search when player have special container disabled and it's
    // still open.
  ...
  bool isDepotSearchOpen() const { return depotSearchOnItem.first != 0; }
  ...
  bool isDepotSearchAvailable() const { return depotSearch; }
  bool isSupplyStashMenuAvailable() { return supplyStash; }
  bool isMarketMenuAvailable() { return marketMenu; }
  bool isExerciseTraining() { return exerciseTraining; }
  void setExerciseTraining(bool isTraining) { exerciseTraining = isTraining; }
  ...
  double_t getMagicLevelPercent() const { return magLevelPercent; }
  ...
  void setTown(Town *newTown) { this->town = newTown; }
  ...
  StashItemList getStashItems() const { return stashItems; }
  ...
  Container *setLootContainer(ObjectCategory_t category, Container *container,
                              bool loading = false);
  ...
  // shop functions
  void setShopOwner(Npc *owner) { shopOwner = owner; }
  ...
  bool editVIP(uint32_t vipGuid, const std::string &description, uint32_t icon,
               bool notify);
  ...
  void changeHealth(int32_t healthChange,
                    bool sendHealthChange = true) override;
  void changeMana(int32_t manaChange) override;
  void changeSoul(int32_t soulChange);

  bool isPzLocked() const { return pzLocked; }
  BlockType_t blockHit(Creature *attacker, CombatType_t combatType,
                       int32_t &damage, bool checkDefense = false,
                       bool checkArmor = false, bool field = false) override;
  ...
  uint16_t getSkillLevel(uint8_t skill) const {
    uint16_t skillLevel =
        std::max<uint16_t>(0, skills[skill].level + varSkills[skill]);
  ...
  // tile
  // send methods
  void sendAddTileItem(const Tile *itemTile, const Position &pos,
                       const Item *item) {
    if (client) {
  ...
  void sendUpdateTileItem(const Tile *updateTile, const Position &pos,
                          const Item *item) {
    if (client) {
  ...
  void sendChannelMessage(const std::string &author, const std::string &text,
  ...
  void sendCreatureMove(const Creature *creature, const Position &newPos,
                        int32_t newStackPos, const Position &oldPos,
                        int32_t oldStackPos, bool teleport) {
    if (client) {
      client->sendMoveCreature(creature, newPos, newStackPos, oldPos,
                               oldStackPos, teleport);
    }
  }
  void sendCreatureTurn(const Creature *creature) {
    if (client && canSeeCreature(creature)) {
      int32_t stackpos =
          creature->getTile()->getStackposOfCreature(this, creature);
      if (stackpos != -1) {
  ...
  void sendUpdateContainerItem(const Container *container, uint16_t slot,
                               const Item *newItem);
  ...
  void sendDepotSearchResultDetail(uint16_t itemId, uint8_t tier,
                                   uint32_t depotCount,
    ...
    if (client) {
      client->sendDepotSearchResultDetail(itemId, tier, depotCount, depotItems,
                                          inboxCount, inboxItems, stashCount);
    }
  }
  ...
  void sendStorePurchaseSuccessful(const std::string &message,
                                   const uint32_t newCoinBalance) {
    if (client) {
      client->sendStorePurchaseSuccessful(message, newCoinBalance);
    }
  }

  void sendStoreRequestAdditionalInfo(uint32_t offerId,
                                      ClientOffer_t clientOfferType) {
    if (client) {
      client->sendStoreRequestAdditionalInfo(offerId, clientOfferType);
    }
  }

  void sendStoreTrasactionHistory(HistoryStoreOfferList &list, uint32_t page,
  ...
  // event methods
  void onUpdateTileItem(const Tile *tile, const Position &pos,
  ...
  void onCreatureMove(Creature *creature, const Tile *newTile,
  ...
  void onUpdateContainerItem(const Container *container, const Item *oldItem,
                             const Item *newItem);
  ...
  void sendPartyCreatureHealth(const Creature *creature,
  ...
  void sendPartyCreatureShowStatus(const Creature *creature,
  ...
  void
  sendSaleItemList(const std::map<uint32_t, uint32_t> &inventoryMap) const {
    if (client && shopOwner) {
  ...
  void
  sendMarketBrowseOwnHistory(const HistoryMarketOfferList &buyOffers,
  ...
  void sendTradeItemRequest(const std::string &traderName, const Item *item,
                            bool ack) const {
    if (client) {
  ...
  // Imbuements
  void onApplyImbuement(Imbuement *imbuement, Item *item, uint8_t slot,
  ...
  void sendItemInspection(uint16_t itemId, uint8_t itemCount, const Item *item,
                          bool cyclopedia) {
    if (client) {
  ...
  void
  sendCyclopediaCharacterNoData(CyclopediaCharacterInfoType_t characterInfoType,
                                uint8_t errorCode) {
    if (client) {
  ...
  void sendHighscores(const std::vector<HighscoreCharacter> &characters,
                      uint8_t categoryId, uint32_t vocationId, uint16_t page,
                      uint16_t pages) {
    if (client) {
      client->sendHighscores(characters, categoryId, vocationId, page, pages);
    }
  }
  void addAsyncOngoingTask(uint64_t flags) { asyncOngoingTasks |= flags; }
  bool hasAsyncOngoingTask(uint64_t flags) const {
    return (asyncOngoingTasks & flags);
  }
  void resetAsyncOngoingTask(uint64_t flags) { asyncOngoingTasks &= ~(flags); }
  ...
  void updateStashExhausted() { lastStashInteraction = OTSYS_TIME(); }
  ...
  bool canDoPotionAction() const { return nextPotionAction <= OTSYS_TIME(); }
  ...
  bool getScheduledSaleUpdate() { return scheduledSaleUpdate; }

  bool inPushEvent() { return inEventMovePush; }

  void pushEvent(bool b) { inEventMovePush = b; }
  ...
  void setWalkExhaust(int64_t value) { lastWalking = OTSYS_TIME() + value; }
  ...
  void setBaseXpGain(uint16_t value) {
    baseXpGain =
        std::min<uint16_t>(std::numeric_limits<uint16_t>::max(), value);
  }
  uint16_t getVoucherXpBoost() const { return voucherXpBoost; }
  ...
  uint16_t getStoreXpBoost() const { return storeXpBoost; }
  void setStoreXpBoost(uint16_t exp) { storeXpBoost = exp; }
  ...
  void setExpBoostStamina(uint16_t stamina) { expBoostStamina = stamina; }

  uint16_t getExpBoostStamina() { return expBoostStamina; }
  ...
  void updateMarketExhausted() { lastMarketInteraction = OTSYS_TIME(); }
  ...
  bool updateKillTracker(Container *corpse, const std::string &playerName,
  ...
  void updateInputAnalyzer(CombatType_t type, int32_t amount,
                           std::string target) {
    if (client) {
  ...
  uint32_t getCharmPoints() { return charmPoints; }
  void setCharmPoints(uint32_t points) { charmPoints = points; }
  bool hasCharmExpansion() { return charmExpansion; }
  void setCharmExpansion(bool onOff) { charmExpansion = onOff; }
  void setUsedRunesBit(int32_t bit) { UsedRunesBit = bit; }
  int32_t getUsedRunesBit() { return UsedRunesBit; }
  void setUnlockedRunesBit(int32_t bit) { UnlockedRunesBit = bit; }
  int32_t getUnlockedRunesBit() { return UnlockedRunesBit; }
  ...
  void sendPreyData() const {
    if (client) {
      ...
      client->sendResourcesBalance(getMoney(), getBankBalance(),
                                   getPreyCards());
    }
  }
  ...
  PreySlot *getPreySlotById(PreySlot_t slotid) {
    if (auto it = std::find_if(
            preys.begin(), preys.end(),
            [slotid](const PreySlot *preyIt) { return preyIt->id == slotid; });
        it != preys.end()) {
      return *it;
    }
  ...
  uint64_t getPreyCards() const { return preyCards; }
  ...
  PreySlot *getPreyWithMonster(uint16_t raceId) const {
    ...
    if (auto it = std::find_if(preys.begin(), preys.end(),
                               [raceId](const PreySlot *it) {
  ...
  TaskHuntingSlot *getTaskHuntingSlotById(PreySlot_t slotid) {
    if (auto it = std::find_if(taskHunting.begin(), taskHunting.end(),
                               [slotid](const TaskHuntingSlot *itTask) {
  ...
  std::vector<uint16_t> getTaskHuntingBlackList() const {
    ...
    std::for_each(taskHunting.begin(), taskHunting.end(),
                  [&rt](const TaskHuntingSlot *slot) {
                    if (slot->isOccupied()) {
                      rt.push_back(slot->selectedRaceId);
                    } else {
                      std::for_each(
                          slot->raceIdList.begin(), slot->raceIdList.end(),
                          [&rt](uint16_t raceId) { rt.push_back(raceId); });
                    }
                  });
  ...
  uint32_t getTaskHuntingRerollPrice() const {
    return getLevel() *
           g_configManager().getNumber(TASK_HUNTING_REROLL_PRICE_LEVEL);
  }
  ...
  TaskHuntingSlot *getTaskHuntingWithCreature(uint16_t raceId) const {
    ...
    if (auto it = std::find_if(taskHunting.begin(), taskHunting.end(),
                               [raceId](const TaskHuntingSlot *itTask) {
  ...
  void retrieveAllItemsFromDepotSearch(uint16_t itemId, uint8_t tier,
                                       bool isDepot);
...
private:
  ...
  /**
   * @brief Starts checking the imbuements in the item so that the time decay is
   * performed Registers the player in an unordered_map in game.h so that the
   * function can be initialized by the task
   */
  ...
  Item *getCorpse(Creature *lastHitCreature,
                  Creature *mostDamageCreature) override;
  ...
  ReturnValue queryRemove(const Thing &thing, uint32_t count, uint32_t flags,
                          Creature *actor = nullptr) const override;
  Cylinder *queryDestination(int32_t &index, const Thing &thing,
                             Item **destItem, uint32_t &flags) override;
  ...
  uint32_t getItemTypeCount(uint16_t itemId,
                            int32_t subType = -1) const override;
  void stashContainer(StashContainerList itemDict);
  std::map<uint32_t, uint32_t> &
  getAllItemTypeCount(std::map<uint32_t, uint32_t> &countMap) const override;
  std::map<uint16_t, uint16_t> getInventoryItemsId() const;
  void
  getAllItemTypeCountAndSubtype(std::map<uint32_t, uint32_t> &countMap) const;
  ...
  // TODO: This variable is only temporarily used when logging in, get rid of it
  // somehow.
  ...
  int32_t getStepSpeed() const override {
    return std::max<int32_t>(PLAYER_MIN_SPEED,
  ...
  void getPathSearchParams(const Creature *creature,
                           FindPathParams &fpp) const override;
