...
Npc::Npc(NpcType *npcType)
    : Creature(), strDescription(npcType->nameDescription), npcType(npcType) {
...
void Npc::onCreatureAppear(Creature *creature, bool isLogin) {
  ...
  CreatureCallback callback =
      CreatureCallback(npcType->info.scriptInterface, this);
...
void Npc::onCreatureMove(Creature *creature, const Tile *newTile,
...
void Npc::onPlayerBuyItem(Player *player, uint16_t itemId, uint8_t subType,
  ...
  if (!itemType.stackable && player->getFreeBackpackSlots() < amount ||
      player->getFreeBackpackSlots() == 0) {
  ...
  if (getCurrency() == ITEM_GOLD_COIN) {
    if (!g_game().removeMoney(player, totalCost, 0, true)) {
      SPDLOG_ERROR("[Npc::onPlayerBuyItem (removeMoney)] - Player {} have a "
                   "problem for buy item {} on shop for npc {}",
                   player->getName(), itemId, getName());
      SPDLOG_DEBUG("[Information] Player {} buyed item {} on shop for npc {}, "
                   "at position {}",
                   player->getName(), itemId, getName(),
    ...
    SPDLOG_ERROR("[Npc::onPlayerBuyItem (removeItemOfType)] - Player {} have a "
                 "problem for buy item {} on shop for npc {}",
                 player->getName(), itemId, getName());
    SPDLOG_DEBUG("[Information] Player {} buyed item {} on shop for npc {}, at "
                 "position {}",
                 player->getName(), itemId, getName(),
                 player->getPosition().toString());
    return;
...
void Npc::onPlayerSellItem(Player *player, uint16_t itemId, uint8_t subType,
  ...
  if (!player->removeItemOfType(itemId, amount, -1, false, false)) {
    SPDLOG_ERROR("[Npc::onPlayerSellItem] - Player {} have a problem for sell "
                 "item {} on shop for npc {}",
                 player->getName(), itemId, getName());
    return;
  }
...
void Npc::onThinkYell(uint32_t interval) {
  ...
  if (yellTicks >= npcType->info.yellSpeedTicks) {
    ...
    if (!npcType->info.voiceVector.empty() &&
        (npcType->info.yellChance >=
...
void Npc::onPlacedCreature() { addEventWalk(); }
...
void Npc::resetPlayerInteractions() { playerInteractions.clear(); }
...
bool Npc::canWalkTo(const Position &fromPos, Direction dir) const {
  ...
  if (!floorChange &&
      (toTile->hasFlag(TILESTATE_FLOORCHANGE) || toTile->getTeleportItem())) {
    return false;
  }
