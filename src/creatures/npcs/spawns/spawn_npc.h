...
private:
	...
	bool spawnNpc(uint32_t spawnId, NpcType* npcType, const Position& pos,
		Direction dir, bool startup = false);
	void checkSpawnNpc();
	void scheduleSpawnNpc(uint32_t spawnId, spawnBlockNpc_t& sb,
		uint16_t interval);
};

class SpawnsNpc {
public:
	static bool isInZone(const Position& centerPos, int32_t radius,
		const Position& pos);
	...
	bool setStarted(bool setStarted) { return started = setStarted; }

	bool isLoaded() const { return loaded; }
	bool setLoaded(bool setLoaded) { return loaded = setLoaded; }

	std::string setFileName(std::string setName) { return fileName = setName; }

	std::forward_list<SpawnNpc>& getSpawnNpcList() { return spawnNpcList; }
