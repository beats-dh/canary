...
bool SpawnsNpc::loadFromXml(const std::string &fileNpcName) {
  ...
  for (auto spawnNode : doc.child("npcs").children()) {
    ...
    if (!spawnNode.first_child()) {
      SPDLOG_WARN("[SpawnsNpc::loadFromXml] - Empty spawn at position: {} with "
                  "radius: {}",
    ...
    for (auto childNode : spawnNode.children()) {
      if (strcasecmp(childNode.name(), "npc") == 0) {
        ...
        Position pos(centerPos.x +
                         pugi::cast<uint16_t>(childNode.attribute("x").value()),
                     centerPos.y +
                         pugi::cast<uint16_t>(childNode.attribute("y").value()),
                     centerPos.z);
        int64_t interval =
            pugi::cast<int64_t>(childNode.attribute("spawntime").value()) *
            1000;
        if (interval >= MINSPAWN_INTERVAL && interval <= MAXSPAWN_INTERVAL) {
          spawnNpc.addNpc(nameAttribute.as_string(), pos, dir,
                          static_cast<uint32_t>(interval));
        } else {
          if (interval <= MINSPAWN_INTERVAL) {
            SPDLOG_WARN("[SpawnsNpc::loadFromXml] - {} {} spawntime can not be "
                        "less than {} seconds",
...
bool SpawnsNpc::isInZone(const Position &centerPos, int32_t radius,
                         const Position &pos) {
  if (radius == -1) {
    return true;
  }
...
void SpawnNpc::startSpawnNpcCheck() {
  if (checkSpawnNpcEvent == 0) {
    checkSpawnNpcEvent = g_scheduler().addEvent(createSchedulerTask(
        getInterval(), std::bind(&SpawnNpc::checkSpawnNpc, this)));
  }
}
...
bool SpawnNpc::findPlayer(const Position &pos) {
  ...
  for (Creature *spectator : spectators) {
    if (!spectator->getPlayer()->hasCustomFlag(
            PlayerCustomFlag_IgnoredByNpcs)) {
      return true;
    }
  }
  return false;
...
bool SpawnNpc::spawnNpc(uint32_t spawnId, NpcType *npcType, const Position &pos,
...
void SpawnNpc::scheduleSpawnNpc(uint32_t spawnId, spawnBlockNpc_t &sb,
                                uint16_t interval) {
  if (interval <= 0) {
    ...
    g_scheduler().addEvent(createSchedulerTask(
        1400, std::bind(&SpawnNpc::scheduleSpawnNpc, this, spawnId, sb,
                        interval - NONBLOCKABLE_SPAWN_NPC_INTERVAL)));
  }
}
...
bool SpawnNpc::addNpc(const std::string &name, const Position &pos,
...
void SpawnNpc::removeNpc(Npc *npc) {
  for (auto it = spawnedNpcMap.begin(), end = spawnedNpcMap.end(); it != end;
       ++it) {
    if (it->second == npc) {
      npc->decrementReferenceCounter();
