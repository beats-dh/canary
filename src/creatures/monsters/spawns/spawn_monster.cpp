...
bool SpawnsMonster::loadFromXML(const std::string &filemonstername) {
  ...
  for (auto spawnMonsterNode : doc.child("monsters").children()) {
    ...
    if (!spawnMonsterNode.first_child()) {
      SPDLOG_WARN("[SpawnsMonster::loadFromXml] - Empty spawn at position: {} "
                  "with radius: {}",
                  centerPos.toString(), radius);
      continue;
    }
    ...
    for (auto childMonsterNode : spawnMonsterNode.children()) {
      if (strcasecmp(childMonsterNode.name(), "monster") == 0) {
        ...
        pugi::xml_attribute directionAttribute =
            childMonsterNode.attribute("direction");
        if (directionAttribute) {
          dir = static_cast<Direction>(
              pugi::cast<uint16_t>(directionAttribute.value()));
        } else {
          dir = DIRECTION_NORTH;
        }

        Position pos(centerPos.x + pugi::cast<uint16_t>(
                                       childMonsterNode.attribute("x").value()),
                     centerPos.y + pugi::cast<uint16_t>(
                                       childMonsterNode.attribute("y").value()),
                     centerPos.z);
        ...
        uint32_t interval =
            pugi::cast<uint32_t>(
                childMonsterNode.attribute("spawntime").value()) *
            1000 * 100 /
            std::max((uint32_t)1, (g_configManager().getNumber(RATE_SPAWN) *
                                   boostedrate * eventschedule));
        if (interval >= MONSTER_MINSPAWN_INTERVAL &&
            interval <= MONSTER_MAXSPAWN_INTERVAL) {
          spawnMonster.addMonster(nameAttribute.as_string(), pos, dir,
                                  static_cast<uint32_t>(interval));
        } else {
          if (interval <= MONSTER_MINSPAWN_INTERVAL) {
            SPDLOG_WARN("[SpawnsMonster::loadFromXml] - {} {} spawntime cannot "
                        "be less than {} seconds, set to {} by default.",
                        nameAttribute.as_string(), pos.toString(),
                        MONSTER_MINSPAWN_INTERVAL / 1000,
                        MONSTER_MINSPAWN_INTERVAL / 1000);
            spawnMonster.addMonster(nameAttribute.as_string(), pos, dir,
                                    MONSTER_MINSPAWN_INTERVAL);
          } else {
            SPDLOG_WARN("[SpawnsMonster::loadFromXml] - {} {} spawntime can "
                        "not be more than {} seconds",
                        nameAttribute.as_string(), pos.toString(),
                        MONSTER_MAXSPAWN_INTERVAL / 1000);
          }
        }
      }
    }
...
bool SpawnsMonster::isInZone(const Position &centerPos, int32_t radius,
                             const Position &pos) {
  if (radius == -1) {
    return true;
  }

  return ((pos.getX() >= centerPos.getX() - radius) &&
          (pos.getX() <= centerPos.getX() + radius) &&
...
void SpawnMonster::startSpawnMonsterCheck() {
  if (checkSpawnMonsterEvent == 0) {
    checkSpawnMonsterEvent = g_scheduler().addEvent(createSchedulerTask(
        getInterval(), std::bind(&SpawnMonster::checkSpawnMonster, this)));
  }
}
...
bool SpawnMonster::spawnMonster(uint32_t spawnMonsterId,
                                MonsterType *monsterType, const Position &pos,
                                Direction dir, bool startup /*= false*/) {
  std::unique_ptr<Monster> monster_ptr(new Monster(monsterType));
  if (startup) {
    // No need to send out events to the surrounding since there is no one out
    // there to listen!
  ...
  for (auto &it : spawnMonsterMap) {
    ...
    if (OTSYS_TIME() >= sb.lastSpawn + sb.interval) {
      ...
      if (sb.monsterType->info.isBlockable) {
        ...
        scheduleSpawn(spawnMonsterId, sb,
                      3 * NONBLOCKABLE_SPAWN_MONSTER_INTERVAL);
      }

      if (++spawnMonsterCount >=
          static_cast<uint32_t>(g_configManager().getNumber(RATE_SPAWN))) {
        break;
      }
    }
  }
...
void SpawnMonster::scheduleSpawn(uint32_t spawnMonsterId, spawnBlock_t &sb,
                                 uint16_t interval) {
  if (interval <= 0) {
    spawnMonster(spawnMonsterId, sb.monsterType, sb.pos, sb.direction);
  } else {
    ...
    g_scheduler().addEvent(createSchedulerTask(
        1400, std::bind(&SpawnMonster::scheduleSpawn, this, spawnMonsterId, sb,
                        interval - NONBLOCKABLE_SPAWN_MONSTER_INTERVAL)));
  }
}
...
bool SpawnMonster::addMonster(const std::string &name, const Position &pos,
                              Direction dir, uint32_t scheduleInterval) {
...
void SpawnMonster::removeMonster(Monster *monster) {
  for (auto it = spawnedMonsterMap.begin(), end = spawnedMonsterMap.end();
       it != end; ++it) {
