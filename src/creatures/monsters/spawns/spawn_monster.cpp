	...
	for (auto spawnMonsterNode : doc.child("monsters").children()) {
		...
		for (auto childMonsterNode : spawnMonsterNode.children()) {
			if (strcasecmp(childMonsterNode.name(), "monster") == 0) {
				...
				Position pos(centerPos.x + pugi::cast<uint16_t>(childMonsterNode.attribute("x").value()),
				...
				uint32_t interval = pugi::cast<uint32_t>(
										childMonsterNode.attribute("spawntime").value())
					* 1000 * 100 / std::max((uint32_t)1, (g_configManager().getNumber(RATE_SPAWN) * boostedrate * eventschedule));
...
void SpawnMonster::scheduleSpawn(uint32_t spawnMonsterId, spawnBlock_t& sb,
	...
	if (interval <= 0) {
		...
		g_scheduler().addEvent(createSchedulerTask(
			1400, std::bind(&SpawnMonster::scheduleSpawn, this, spawnMonsterId, sb, interval - NONBLOCKABLE_SPAWN_MONSTER_INTERVAL)));
	}
}
