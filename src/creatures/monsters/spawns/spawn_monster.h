...
private:
	...
	bool spawnMonster(uint32_t spawnMonsterId, MonsterType* monsterType,
		const Position& pos, Direction dir, bool startup = false);
	void checkSpawnMonster();
	void scheduleSpawn(uint32_t spawnMonsterId, spawnBlock_t& sb,
		uint16_t interval);
};
