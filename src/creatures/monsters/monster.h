...
public:
	...
	const std::string& getTypeName() const override { return mType->typeName; }
	...
	bool isEnemyFaction(Faction_t faction) const {
		...
		return mType->info.enemyFactions.empty()
			? false
			: mType->info.enemyFactions.find(faction) != mType->info.enemyFactions.end();
	}
	...
	bool isFamiliar() const { return mType->info.isFamiliar; }
	bool canSee(const Position& pos) const override;
	bool canSeeInvisibility() const override {
		return isImmune(CONDITION_INVISIBLE);
	}
	uint32_t getManaCost() const { return mType->info.manaCost; }
	RespawnType getRespawnType() const { return mType->info.respawnType; }
	...
	void onCreatureSay(Creature* creature, SpeakClasses type,
	...
	void setIgnoreFieldDamage(bool ignore) { ignoreFieldDamage = ignore; }
	bool getIgnoreFieldDamage() const { return ignoreFieldDamage; }
	uint16_t getRaceId() const { return mType->info.raceid; }
