...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb,
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			ConditionSpeed* condition = static_cast<ConditionSpeed*>(Condition::createCondition(
			...
			if ((attr = node.attribute("monster"))) {
				...
				if (mType) {
					ConditionOutfit* condition = static_cast<ConditionOutfit*>(Condition::createCondition(
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb,
	...
	if (spell->isScripted) {
		...
		if (spell->conditionType != CONDITION_NONE) {
			...
			Condition* condition = getDamageCondition(spell->conditionType, maxDamage, minDamage,
