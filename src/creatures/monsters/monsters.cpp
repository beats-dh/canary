...
bool Monsters::loadFromXml(bool reloading /*= false*/) {
  ...
  for (auto monsterNode : doc.child("monsters").children()) {
    std::string name =
        asLowerCaseString(monsterNode.attribute("name").as_string());
    std::string file = "data/monster/" +
                       std::string(monsterNode.attribute("file").as_string());
...
bool Monsters::deserializeSpell(const pugi::xml_node &node, spellBlock_t &sb,
  ...
  if (isScripted) {
    ...
    if (!combatSpellPtr->loadScript("data/" + g_spells().getScriptBaseName() +
                                    "/scripts/" + scriptName)) {
      return false;
    }
    ...
    combatSpell->getCombat()->setPlayerCombatValues(
        COMBAT_FORMULA_DAMAGE, sb.minCombatValue, 0, sb.maxCombatValue, 0);
  } else {
    ...
    if (tmpName == "melee") {
      ...
      if ((attackAttribute = node.attribute("attack")) &&
        ...
        sb.minCombatValue = 0;
        sb.maxCombatValue = -Weapons::getMaxMeleeDamage(
            pugi::cast<int32_t>(skillAttribute.value()),
      ...
      if (conditionType != CONDITION_NONE) {
        Condition *condition = getDamageCondition(conditionType, maxDamage,
                                                  minDamage, 0, tickInterval);
      ...
      ConditionSpeed *condition =
          static_cast<ConditionSpeed *>(Condition::createCondition(
              CONDITIONID_COMBAT, conditionType, duration, 0));
      condition->setFormulaVars(speedChange / 1000.0, 0, speedChange / 1000.0,
                                0);
      combat->addCondition(condition);
      ...
      if ((attr = node.attribute("monster"))) {
        const MonsterType *mType =
            g_monsters().getMonsterType(attr.as_string());
        if (mType) {
          ConditionOutfit *condition =
              static_cast<ConditionOutfit *>(Condition::createCondition(
                  CONDITIONID_COMBAT, CONDITION_OUTFIT, duration, 0));
      ...
      Condition *condition = Condition::createCondition(
          CONDITIONID_COMBAT, CONDITION_INVISIBLE, duration, 0);
      combat->setParam(COMBAT_PARAM_AGGRESSIVE, 0);
      combat->addCondition(condition);
      ...
      Condition *condition = Condition::createCondition(
          CONDITIONID_COMBAT, CONDITION_DRUNK, duration, 0);
      combat->addCondition(condition);
      ...
      Condition *condition = getDamageCondition(
          conditionType, maxDamage, minDamage, startDamage, tickInterval);
      combat->addCondition(condition);
    ...
    combat->setPlayerCombatValues(COMBAT_FORMULA_DAMAGE, sb.minCombatValue, 0,
                                  sb.maxCombatValue, 0);
      ...
      if ((attr = attributeNode.attribute("key"))) {
        ...
        if (strcasecmp(value, "shooteffect") == 0) {
          if ((attr = attributeNode.attribute("value"))) {
            ShootType_t shoot =
                getShootType(asLowerCaseString(attr.as_string()));
          ...
          if ((attr = attributeNode.attribute("value"))) {
            MagicEffectClasses effect =
                getMagicEffect(asLowerCaseString(attr.as_string()));
  ...
  if (spell->isScripted) {
    ...
    if (tmpName == "melee") {
      ...
      if (spell->attack > 0 && spell->skill > 0) {
        sb.minCombatValue = 0;
        sb.maxCombatValue =
            -Weapons::getMaxMeleeDamage(spell->skill, spell->attack);
      }
    ...
    // If a spell has a condition, it always applies, no matter what kind of
    // spell it is
    if (spell->conditionType != CONDITION_NONE) {
      ...
      Condition *condition =
          getDamageCondition(spell->conditionType, maxDamage, minDamage,
    ...
    combat->setPlayerCombatValues(COMBAT_FORMULA_DAMAGE, sb.minCombatValue, 0,
                                  sb.maxCombatValue, 0);
    combatSpell = new CombatSpell(combat.release(), spell->needTarget,
                                  spell->needDirection);
  }
...
MonsterType *Monsters::loadMonster(const std::string &file,
  ...
  if (!monsterNode) {
    SPDLOG_ERROR("[Monsters::loadMonster] - Missing monster node in: {}", file);
    return nullptr;
  }
  ...
  if ((attr = monsterNode.attribute("script"))) {
    ...
    if (scriptInterface->loadFile("data/monster/scripts/" + script) == 0) {
      ...
      mType->info.creatureAppearEvent =
          scriptInterface->getEvent("onCreatureAppear");
      mType->info.creatureDisappearEvent =
          scriptInterface->getEvent("onCreatureDisappear");
      mType->info.creatureMoveEvent =
          scriptInterface->getEvent("onCreatureMove");
    ...
    for (auto flagNode : node.children()) {
      ...
      if (strcasecmp(attrName, "summonable") == 0) {
        ...
        mType->info.targetDistance =
            std::max<int32_t>(1, pugi::cast<int32_t>(attr.value()));
        ...
        SpawnType_t spawnType =
            getSpawnType(asLowerCaseString(attr.as_string()));
  ...
  if ((node = monsterNode.child("summons"))) {
    if ((attr = node.attribute("maxSummons"))) {
      mType->info.maxSummons =
          std::min<uint32_t>(pugi::cast<uint32_t>(attr.value()), 100);
    } else {
      SPDLOG_WARN("[Monsters::loadMonster] - "
    ...
    for (auto summonNode : node.children()) {
      ...
      if ((attr = summonNode.attribute("speed")) ||
          (attr = summonNode.attribute("interval"))) {
...
bool Monsters::loadLootItem(const pugi::xml_node &node, LootBlock &lootBlock) {
  ...
  if ((attr = node.attribute("chance")) || (attr = node.attribute("chance1"))) {
    lootBlock.chance =
        std::min<int32_t>(MAX_LOOTCHANCE, pugi::cast<int32_t>(attr.value()));
  } else {
...
void Monsters::loadLootContainer(const pugi::xml_node &node,
                                 LootBlock &lBlock) {
...
MonsterType *Monsters::getMonsterType(const std::string &name) {
  ...
  if (auto it = monsters.find(lowerCaseName);
      it != monsters.end()
      // We will only return the MonsterType if it match the exact name of the
      // monster
      && it->first.find(lowerCaseName) != it->first.npos) {
    return it->second;
  }
  SPDLOG_ERROR("[Monsters::getMonsterType] - Monster with name {} not exist",
               lowerCaseName);
  return nullptr;
}
