		...
		case TARGETSEARCH_NEAREST: {
			...
			if (!resultList.empty()) {
				...
				if (++it != resultList.end()) {
					...
					int32_t minRange = std::max<int32_t>(Position::getDistanceX(myPos, targetPosition),
						Position::getDistanceY(myPos, targetPosition));
					do {
						const Position& pos = (*it)->getPosition();

						int32_t distance = std::max<int32_t>(Position::getDistanceX(myPos, pos),
...
void Monster::pushItems(Tile* tile) {
	...
	if (TileItemVector* items = tile->getItemList()) {
		...
		for (int32_t i = downItemSize; --i >= 0;) {
			...
			if (item && item->hasProperty(CONST_PROP_MOVEABLE) && (item->hasProperty(CONST_PROP_BLOCKPATH) || item->hasProperty(CONST_PROP_BLOCKSOLID)) && item->getActionId() != 100 /* non-moveable action*/) {
...
bool Monster::getDistanceStep(const Position& targetPos,
	...
	if (int32_t distance = std::max<int32_t>(static_cast<int32_t>(dx), static_cast<int32_t>(dy));
		!flee && (distance > targetDistance || !g_game().isSightClear(creaturePos, targetPos, true))) {
