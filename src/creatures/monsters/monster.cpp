...
Monster::Monster(MonsterType *mType)
    : Creature(), strDescription(asLowerCaseString(mType->nameDescription)),
      mType(mType) {
...
bool Monster::canSee(const Position &pos) const {
  return Creature::canSee(
      getPosition(), pos, 10,
      10); // jlcvp FIX - range 10 Avoids killing monster without reaction
}
...
void Monster::onCreatureAppear(Creature *creature, bool isLogin) {
  ...
  if (mType->info.creatureAppearEvent != -1) {
    ...
    if (!scriptInterface->reserveScriptEnv()) {
      SPDLOG_ERROR(
          "[Monster::onCreatureAppear - Monster {} creature {}] "
...
void Monster::onRemoveCreature(Creature *creature, bool isLogout) {
  ...
  if (mType->info.creatureDisappearEvent != -1) {
    ...
    if (!scriptInterface->reserveScriptEnv()) {
      SPDLOG_ERROR(
          "[Monster::onCreatureDisappear - Monster {} creature {}] "
          "Call stack overflow. Too many lua script calls being nested.",
          getName(), creature->getName());
      return;
    }
...
void Monster::onCreatureMove(Creature *creature, const Tile *newTile,
  ...
  if (mType->info.creatureMoveEvent != -1) {
    ...
    if (!scriptInterface->reserveScriptEnv()) {
      SPDLOG_ERROR(
          "[Monster::onCreatureMove - Monster {} creature {}] "
          "Call stack overflow. Too many lua script calls being nested.",
          getName(), creature->getName());
      return;
    }
    ...
    if (!isSummon()) {
      if (followCreature) {
        ...
        if ((offset_x > 1 || offset_y > 1) &&
            mType->info.changeTargetChance > 0) {
          ...
          if (nextTile) {
            ...
            if (topCreature && followCreature != topCreature &&
                isOpponent(topCreature)) {
  ...
  if (isSummon() && getMaster()->getPlayer()) {
    ...
    if (getFaction() != FACTION_DEFAULT) {
      return isEnemyFaction(creature->getFaction()) ||
             creature->getFaction() == FACTION_PLAYER;
    }
    if ((creature->getPlayer()) ||
...
void Monster::onCreatureLeave(Creature *creature) {
  if (getMaster() == creature) {
    // Take random steps and only use defense abilities (e.g. heal) until its
    // master comes back
  ...
  case TARGETSEARCH_NEAREST: {
    ...
    if (!resultList.empty()) {
      ...
      if (++it != resultList.end()) {
        ...
        int32_t minRange =
            std::max<int32_t>(Position::getDistanceX(myPos, targetPosition),
                              Position::getDistanceY(myPos, targetPosition));
        do {
          const Position &pos = (*it)->getPosition();

          int32_t distance =
              std::max<int32_t>(Position::getDistanceX(myPos, pos),
                                Position::getDistanceY(myPos, pos));
...
void Monster::updateIdleStatus() {
  ...
  if (conditions.empty()) {
    if (!isSummon() && targetList.empty()) {
      idle = true;
    } else if ((!master || master->getMonster()) &&
               getFaction() != FACTION_DEFAULT &&
               (totalPlayersOnScreen == 0 &&
                (!master || master->getMonster()->totalPlayersOnScreen == 0))) {
      idle = true;
    }
  }
      ...
      if (isSummon()) {
        if (!attackedCreature) {
          if (getMaster() && getMaster()->getAttackedCreature()) {
            ...
            // Our master has not ordered us to attack anything, lets follow him
            // around instead.
...
void Monster::onThinkYell(uint32_t interval) {
  ...
  if (yellTicks >= mType->info.yellSpeedTicks) {
    ...
    if (!mType->info.voiceVector.empty() &&
      ...
      if (vb.yellText) {
        g_game().internalCreatureSay(this, TALKTYPE_MONSTER_YELL, vb.text,
                                     false);
      } else {
        g_game().internalCreatureSay(this, TALKTYPE_MONSTER_SAY, vb.text,
                                     false);
      }
    }
  }
...
bool Monster::pushItem(Item *item) {
  ...
  for (const auto &it : relList) {
    ...
    if (tile && g_game().canThrowObjectTo(centerPos, tryPos) &&
        g_game().internalMoveItem(item->getParent(), tile, INDEX_WHEREEVER,
  ...
  if (TileItemVector *items = tile->getItemList()) {
    ...
    for (int32_t i = downItemSize; --i >= 0;) {
      ...
      if (item && item->hasProperty(CONST_PROP_MOVEABLE) &&
          ...
          item->getActionId() != 100 /* non-moveable action*/) {
        if (moveCount < 20 && Monster::pushItem(item)) {
          ++moveCount;
        } else if (!item->isCorpse() &&
                   g_game().internalRemoveItem(item) == RETURNVALUE_NOERROR) {
          ++removeCount;
        }
      }
    }
...
bool Monster::getDanceStep(const Position &creaturePos,
                           Direction &moveDirection, bool keepAttack /*= true*/,
...
bool Monster::getDistanceStep(const Position &targetPos,
  ...
  if (int32_t distance =
          std::max<int32_t>(static_cast<int32_t>(dx), static_cast<int32_t>(dy));
      !flee && (distance > targetDistance ||
