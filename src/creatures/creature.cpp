...
bool Creature::canSee(const Position &myPos, const Position &pos,
                      int32_t viewRangeX, int32_t viewRangeY) {
  if (myPos.z <= 7) {
...
void Creature::onThink(uint32_t interval) {
  ...
  if (followCreature && master != followCreature &&
      !canSeeCreature(followCreature)) {
    onCreatureDisappear(followCreature, false);
  }

  if (attackedCreature && master != attackedCreature &&
      !canSeeCreature(attackedCreature)) {
  ...
  const CreatureEventList &thinkEvents =
...
void Creature::onAttacking(uint32_t interval) {
  ...
  if (g_game().isSightClear(getPosition(), attackedCreature->getPosition(),
                            true)) {
...
void Creature::addEventWalk(bool firstStep) {
  ...
  eventWalk = g_scheduler().addEvent(createSchedulerTask(
      ...
      std::bind(&Game::checkCreatureWalk, &g_game(), getID())));
}
...
void Creature::updateTileCache(const Tile *newTile, int32_t dx, int32_t dy) {
  if (std::abs(dx) <= maxWalkCacheWidth && std::abs(dy) <= maxWalkCacheHeight) {
    localMapCache[maxWalkCacheHeight + dy][maxWalkCacheWidth + dx] =
        newTile &&
        newTile->queryAdd(0, *this, 1,
                          FLAG_PATHFINDING | FLAG_IGNOREFIELDDAMAGE) ==
            RETURNVALUE_NOERROR;
  }
}
...
void Creature::onUpdateTileItem(const Tile *updateTile, const Position &pos,
                                const Item *, const ItemType &oldType,
                                const Item *, const ItemType &newType) {
  if (!isMapLoaded) {
    return;
  }

  if (oldType.blockSolid || oldType.blockPathFind || newType.blockPathFind ||
      newType.blockSolid) {
...
void Creature::onRemoveTileItem(const Tile *updateTile, const Position &pos,
                                const ItemType &iType, const Item *) {
  if (!isMapLoaded) {
    return;
  }
...
void Creature::checkSummonMove(const Position &newPos,
                               bool teleportSummon) const {
  if (hasSummons()) {
    ...
    for (Creature *creature : getSummons()) {
      ...
      bool protectionZoneCheck =
          this->getTile()->hasFlag(TILESTATE_PROTECTIONZONE);
      // Check if any of our summons is out of range (+/- 0 floors or 15 tiles
      // away)
      bool checkSummonDist =
          Position::getDistanceZ(newPos, pos) > 0 ||
          (std::max<int32_t>(Position::getDistanceX(newPos, pos),
                             Position::getDistanceY(newPos, pos)) > 15);
      // Check if any of our summons is out of range (+/- 2 floors or 30 tiles
      // away)
      bool checkRemoveDist =
          Position::getDistanceZ(newPos, pos) > 2 ||
          (std::max<int32_t>(Position::getDistanceX(newPos, pos),
                             Position::getDistanceY(newPos, pos)) > 30);

      if (monster->isFamiliar() && checkSummonDist ||
          teleportSummon && !protectionZoneCheck && checkSummonDist) {
        g_game().internalTeleport(creature,
                                  creature->getMaster()->getPosition(), true);
        continue;
      }

      if (monster->isSummon() && !monster->isFamiliar() && !teleportSummon &&
          checkRemoveDist) {
...
void Creature::onCreatureMove(Creature *creature, const Tile *newTile,
      ...
      if (teleport || oldPos.z != newPos.z) {
        ...
        if (oldPos.y > newPos.y) { // north
          ...
          for (int32_t y = mapWalkHeight - 1; --y >= 0;) {
            memcpy(localMapCache[y + 1], localMapCache[y],
                   sizeof(localMapCache[y]));
          }

          // update 0
          for (int32_t x = -maxWalkCacheWidth; x <= maxWalkCacheWidth; ++x) {
            const Tile *cacheTile = g_game().map.getTile(
                static_cast<uint16_t>(myPos.getX() + x),
                static_cast<uint16_t>(myPos.getY() - maxWalkCacheHeight),
                myPos.z);
          ...
          for (int32_t y = 0; y <= mapWalkHeight - 2; ++y) {
            memcpy(localMapCache[y], localMapCache[y + 1],
                   sizeof(localMapCache[y]));
          }
        ...
        if (oldPos.x < newPos.x) { // east
          ...
          for (int32_t y = -maxWalkCacheHeight; y <= maxWalkCacheHeight; ++y) {
            const Tile *cacheTile = g_game().map.getTile(
                myPos.x + maxWalkCacheWidth, static_cast<uint16_t>(myPos.y + y),
                myPos.z);
...
void Creature::onDeath() {
  ...
  if (mostDamageCreature) {
    if (mostDamageCreature != lastHitCreature &&
        mostDamageCreature != lastHitCreatureMaster) {
...
bool Creature::dropCorpse(Creature *lastHitCreature,
                          Creature *mostDamageCreature, bool lastHitUnjustified,
                          bool mostDamageUnjustified) {
  if (!lootDrop && getMonster()) {
    if (master) {
      // Scripting event onDeath
      const CreatureEventList &deathEvents =
          getCreatureEvents(CREATURE_EVENT_DEATH);
      for (CreatureEvent *deathEvent : deathEvents) {
        deathEvent->executeOnDeath(this, nullptr, lastHitCreature,
                                   mostDamageCreature, lastHitUnjustified,
                                   mostDamageUnjustified);
      }
    }
    ...
    if (tile && corpse) {
      ...
      bool corpses =
          corpse->isRewardCorpse() || (corpse->getID() == ITEM_MALE_CORPSE ||
                                       corpse->getID() == ITEM_FEMALE_CORPSE);
      if (mostDamageCreature && mostDamageCreature->getPlayer() && !corpses) {
        ...
        if (g_configManager().getBoolean(AUTOLOOT)) {
          ...
          g_dispatcher().addTask(createTask(
              std::bind(&Game::playerQuickLoot, &g_game(),
                        mostDamageCreature->getID(), this->getPosition(),
                        corpse->getID(), pos - 1, nullptr, false, true)));
        }
      }
    }
    ...
    for (CreatureEvent *deathEvent : getCreatureEvents(CREATURE_EVENT_DEATH)) {
      if (deathEvent) {
        deathEvent->executeOnDeath(this, corpse, lastHitCreature,
                                   mostDamageCreature, lastHitUnjustified,
                                   mostDamageUnjustified);
      }
    }
...
bool Creature::hasBeenAttacked(uint32_t attackerId) {
  ...
  return (OTSYS_TIME() - it->second.ticks) <=
         g_configManager().getNumber(PZ_LOCKED);
}
...
void Creature::changeHealth(int32_t healthChange,
  ...
  if (health <= 0) {
    g_dispatcher().addTask(
        createTask(std::bind(&Game::executeDeath, &g_game(), getID())));
  }
}
...
BlockType_t Creature::blockHit(Creature *attacker, CombatType_t combatType,
...
bool Creature::setAttackedCreature(Creature *creature) {
  if (creature) {
    if (this->getMonster() && this->getMonster()->isFamiliar() &&
        this->getTile() && this->getTile()->hasFlag(TILESTATE_PROTECTIONZONE)) {
      return false;
    }
...
void Creature::getPathSearchParams(const Creature *,
...
void Creature::goToFollowCreature() {
  if (followCreature) {
    ...
    if (monster && !monster->getMaster() &&
        (monster->isFleeing() || fpp.maxTargetDist > 1)) {
...
bool Creature::setMaster(Creature *newMaster,
  ...
  // Reloading summon icon/knownCreature and reset informations
  // (follow/dropLoot/skillLoss)
  if (reloadCreature) {
    setFollowCreature(nullptr);
    setDropLoot(false);
    setSkillLoss(false);
  ...
  if (oldMaster) {
    auto summon =
        std::find(oldMaster->summons.begin(), oldMaster->summons.end(), this);
    ...
    if (!force && conditionType == CONDITION_PARALYZE) {
      ...
      if (walkDelay > 0) {
        g_scheduler().addEvent(createSchedulerTask(
            walkDelay, std::bind(&Game::forceRemoveCondition, &g_game(),
                                 getID(), conditionType, conditionId)));
        return;
      }
    }
...
int64_t Creature::getStepDuration() const {
  ...
  uint32_t calculatedStepSpeed = std::max<uint32_t>(
      floor((Creature::speedA * log((stepSpeed / 2) + Creature::speedB) +
             Creature::speedC) +
            0.5),
      1);
  calculatedStepSpeed =
      (stepSpeed > -Creature::speedB) ? calculatedStepSpeed : 1;
  ...
  if (monster && monster->isTargetNearby() && !monster->isFleeing() &&
      !monster->getMaster()) {
    stepDuration *= 2;
  }
...
bool FrozenPathingConditionCall::isInRange(const Position &startPos,
...
bool FrozenPathingConditionCall::operator()(const Position &startPos,
                                            const Position &testPos,
  ...
  int32_t testDist =
      std::max<int32_t>(Position::getDistanceX(targetPos, testPos),
...
bool Creature::isInvisible() const {
  return std::find_if(conditions.begin(), conditions.end(),
                      [](const Condition *condition) {
                        return condition->getType() == CONDITION_INVISIBLE;
                      }) != conditions.end();
}

bool Creature::getPathTo(const Position &targetPos,
                         std::forward_list<Direction> &dirList,
                         const FindPathParams &fpp) const {
  return g_game().map.getPathMatching(
      *this, dirList, FrozenPathingConditionCall(targetPos), fpp);
}
