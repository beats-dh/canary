...
bool Creature::canSee(const Position& myPos, const Position& pos,
	...
	return (pos.getX() >= myPos.getX() - viewRangeX + offsetz) && (pos.getX() <= myPos.getX() + viewRangeX + offsetz) && (pos.getY() >= myPos.getY() - viewRangeY + offsetz) && (pos.getY() <= myPos.getY() + viewRangeY + offsetz);
}
...
void Creature::onGainExperience(uint64_t gainExp, Creature* target) {
	...
	if (!m->isFamiliar()) {
		...
		TextMessage message(
			...
			ucfirst(getNameDescription()) + " gained " + std::to_string(gainExp) + (gainExp != 1 ? " experience points." : " experience point."));
		...
		if (!force && conditionType == CONDITION_PARALYZE) {
			...
			if (walkDelay > 0) {
				g_scheduler().addEvent(createSchedulerTask(
					walkDelay, std::bind(&Game::forceRemoveCondition, &g_game(), getID(), conditionType, conditionId)));
				return;
			}
		}
