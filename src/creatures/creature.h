...
public:
	...
	// Real creature name, set on creature creation "createNpcType(typeName) and
	// createMonsterType(typeName)"
	...
	bool isMoveLocked() const { return moveLocked; }
	void setMoveLocked(bool locked) { moveLocked = locked; }
	...
	virtual uint32_t getMaxMana() const { return mana; }

	uint16_t getManaShield() const { return manaShield; }

	void setManaShield(uint16_t value) { manaShield = value; }

	uint16_t getMaxManaShield() const { return maxManaShield; }

	void setMaxManaShield(uint16_t value) { maxManaShield = value; }

	int32_t getBuff(int32_t buff) { return varBuffs[buff]; }

	void setBuff(buffs_t buff, int32_t modifier) { varBuffs[buff] += modifier; }

	virtual CreatureIcon_t getIcon() const { return CREATUREICON_NONE; }
	...
	virtual BlockType_t blockHit(Creature* attacker, CombatType_t combatType,
		int32_t& damage, bool checkDefense = false,
		bool checkArmor = false, bool field = false);
	 ...
	 */
	bool hasBeenSummoned() const { return summoned; }
	...
	void removeCondition(ConditionType_t conditionType, ConditionId_t conditionId,
		bool force = false);
	...
	/**
	 * @brief Check if the summon can move/spawn and if the familiar can teleport
	 * to the master
	 *
	 ...
	 * @return false
	 */
	void checkSummonMove(const Position& newPos,
		bool teleportSummon = false) const;
	virtual void onCreatureMove(Creature* creature, const Tile* newTile,
	...
	void setDropLoot(bool newLootDrop) { this->lootDrop = newLootDrop; }
	void setSkillLoss(bool newSkillLoss) { this->skillLoss = newSkillLoss; }
	...
	bool getPathTo(const Position& targetPos,
		std::forward_list<Direction>& dirList,
		const FindPathParams& fpp) const;
	bool getPathTo(const Position& targetPos,
		std::forward_list<Direction>& dirList, int32_t minTargetDist,
		int32_t maxTargetDist, bool fullPathSearch = true,
...
protected:
	...
	virtual void getPathSearchParams(const Creature* creature,
		FindPathParams& fpp) const;
	virtual void death(Creature*) { }
	virtual bool dropCorpse(Creature* lastHitCreature,
		Creature* mostDamageCreature, bool lastHitUnjustified,
		bool mostDamageUnjustified);
	virtual Item* getCorpse(Creature* lastHitCreature,
