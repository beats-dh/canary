...
bool Outfits::loadFromXml() {
	...
	for (auto outfitNode : doc.child("outfits").children()) {
		...
		if (uint16_t lookType = pugi::cast<uint16_t>(lookTypeAttribute.value());
			g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && lookType != 0 && !g_game().isLookTypeRegistered(lookType)) {
			SPDLOG_WARN("[Outfits::loadFromXml] An unregistered creature looktype "
						"type with id '{}' was blocked to prevent client crash.",
				lookType);
			return false;
		}

		outfits[type].emplace_back(outfitNode.attribute("name").as_string(),
...
const Outfit* Outfits::getOutfitByLookType(PlayerSex_t sex,
	uint16_t lookType) const {
 ...
 * @return <b>const</b> pointer to the outfit or <b>nullptr</b> if it could not
 * be found.
 */

const Outfit* Outfits::getOpositeSexOutfitByLookType(PlayerSex_t sex,
	uint16_t lookType) {
