...
Item *Item::CreateItem(const uint16_t type, uint16_t count /*= 0*/) {
  ...
  if (it.id != 0) {
    ...
    SPDLOG_WARN("[Item::CreateItem] Item with id '{}' is not registered and "
                "cannot be created.",
                type);
  }
...
bool Item::getImbuementInfo(uint8_t slot, ImbuementInfo *imbuementInfo) {
  const ItemAttributes::CustomAttribute *attribute =
      getCustomAttribute(IMBUEMENT_SLOT + slot);
...
void Item::addImbuement(uint8_t slot, uint16_t imbuementId, int32_t duration) {
  ...
  if (!hasImbuementType(static_cast<ImbuementTypes_t>(categoryImbuement->id),
                        imbuement->getBaseID())) {
    return;
  }
  ...
  if (hasImbuementCategoryId(categoryImbuement->id)) {
    SPDLOG_ERROR("[Item::setImbuement] - An error occurred while player with "
                 "name {} try to apply imbuement, item already contains "
                 "imbuement of the same type: {}",
...
Container *Item::CreateItemAsContainer(const uint16_t type, uint16_t size) {
  if (const ItemType &it = Item::items[type];
      it.id == 0 || it.stackable || it.multiUse || it.moveable ||
...
Item::Item(const uint16_t itemId, uint16_t itemCount /*= 0*/) : id(itemId) {
...
void Item::setID(uint16_t newid) {
  ...
  if (newDuration == 0 && !it.stopTime && it.decayTo < 0) {
    // We'll get called startDecay anyway so let's schedule it - actually not in
    // all casses
...
bool Item::isItemStorable() const {
  auto isContainerAndHasSomethingInside =
      (getContainer() != NULL) && (getContainer()->getItemList().size() > 0);
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream &propStream) {
  ...
  // If these are called then something has changed in the items.xml since the
  // map was saved just read the values
...
bool Item::unserializeAttr(PropStream &propStream) {
  ...
  while (propStream.read<uint8_t>(attr_type) && attr_type != 0) {
    Attr_ReadValue ret =
        readAttr(static_cast<AttrTypes_t>(attr_type), propStream);
...
bool Item::unserializeItemNode(OTB::Loader &, const OTB::Node &,
...
void Item::serializeAttr(PropWriteStream &propWriteStream) const {
  ...
  if (hasAttribute(ITEM_ATTRIBUTE_CUSTOM)) {
    const ItemAttributes::CustomAttributeMap *customAttrMap =
        attributes->getCustomAttributeMap();
  ...
  if (item) {
    ...
    if (attack != 0) {
      if (it.abilities && it.abilities->elementType != COMBAT_NONE &&
        ...
        ss.str("");
        ss << attack << " physical +" << it.abilities->elementDamage << ' '
    ...
    if (it.abilities) {
      ...
      if (it.abilities->stats[STAT_MAGICPOINTS]) {
        ss.str("");
        ss << std::showpos << it.abilities->stats[STAT_MAGICPOINTS]
           << std::noshowpos;
    ...
    if (item->getContainer()) {
      descriptions.emplace_back(
          "Capacity", std::to_string(item->getContainer()->capacity()));
    }
    ...
    if (it.minReqLevel != 0) {
      descriptions.emplace_back("Required Level",
                                std::to_string(it.minReqLevel));
    }

    if (it.minReqMagicLevel != 0) {
      descriptions.emplace_back("Required Magic Level",
                                std::to_string(it.minReqMagicLevel));
    }
    ...
    if (it.slotPosition & SLOTP_BACKPACK) {
      ...
      descriptions.emplace_back("Body Position", "Extra Slot");
               ...
               it.slotPosition & SLOTP_LEFT || it.slotPosition & SLOTP_RIGHT) {
...
std::string Item::parseImbuementDescription(const Item *item) {
  ...
  if (item && item->getImbuementSlot() >= 1) {
    ...
    for (uint8_t slotid = 0; slotid < item->getImbuementSlot(); slotid++) {
      ...
      s << baseImbuement->name << " " << imbuementInfo.imbuement->getName()
...
std::string Item::parseShowAttributesDescription(const Item *item,
                                                 const uint16_t itemId) {
  std::ostringstream itemDescription;
  const ItemType &itemType = Item::items[itemId];
  if (itemType.armor != 0 || (item && item->getArmor() != 0) ||
      itemType.showAttributes) {
    bool begin = true;
    ...
    if (itemType.abilities) {
      for (uint8_t i = SKILL_FIRST; i <= SKILL_FISHING; i++) {
        ...
        itemDescription << getSkillName(i) << ' ' << std::showpos
                        << itemType.abilities->skills[i] << std::noshowpos;
      }
      ...
      if (itemType.abilities->stats[STAT_MAGICPOINTS]) {
        ...
        itemDescription << "magic level " << std::showpos
                        << itemType.abilities->stats[STAT_MAGICPOINTS]
                        << std::noshowpos;
      }
      ...
      if (!show) {
        ...
        for (size_t i = 0; i < COMBAT_COUNT; ++i) {
          ...
          itemDescription << getCombatName(indexToCombatType(i)) << ' '
                          << std::showpos
                          << itemType.abilities->absorbPercent[i]
                          << std::noshowpos << '%';
        }
      } else {
        if (begin) {
        ...
        itemDescription << "protection all " << std::showpos << show
                        << std::noshowpos << '%';
      }
      ...
      if (!show) {
        ...
        for (size_t i = 0; i < COMBAT_COUNT; ++i) {
          ...
          itemDescription << getCombatName(indexToCombatType(i)) << " field "
                          << std::showpos
                          << itemType.abilities->fieldAbsorbPercent[i]
                          << std::noshowpos << '%';
        }
      } else {
        if (begin) {
        ...
        itemDescription << "protection all fields " << std::showpos << show
                        << std::noshowpos << '%';
      }
      ...
      if (itemType.abilities->speed) {
        ...
        itemDescription << "speed " << std::showpos
                        << (itemType.abilities->speed >> 1) << std::noshowpos;
      }
    }
...
std::string Item::getDescription(const ItemType &it, int32_t lookDistance,
  ...
  if (it.isRune()) {
    if (it.runeLevel > 0 || it.runeMagLevel > 0) {
      if (const RuneSpell *rune = g_spells().getRuneSpell(it.id)) {
        ...
        s << ". " << (it.stackable && tmpSubType > 1 ? "They" : "It")
          << " can only be used by ";

        const VocSpellMap &vocMap = rune->getVocMap();
        std::vector<Vocation *> showVocMap;

        // vocations are usually listed with the unpromoted and promoted
        // version, the latter being hidden from description, so `total / 2` is
        // most likely the amount of vocations to be shown.
    ...
    if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
      ...
      begin = false;
      s << " (Range: "
        << static_cast<uint16_t>(item ? item->getShootRange() : it.shootRange);
      ...
      if (hitChance != 0) {
        s << ", Hit% " << std::showpos << static_cast<int16_t>(hitChance)
          << std::noshowpos;
      }
      ...
      if (it.abilities) {
        for (uint8_t i = SKILL_FIRST; i <= SKILL_FISHING; i++) {
          ...
          s << getSkillName(i) << ' ' << std::showpos << it.abilities->skills[i]
            << std::noshowpos;
        }
        ...
        if (show == 0) {
          ...
          for (size_t i = 0; i < COMBAT_COUNT; ++i) {
            ...
            s << getCombatName(indexToCombatType(i)) << " field "
              << std::showpos << it.abilities->fieldAbsorbPercent[i]
              << std::noshowpos << '%';
          }
        } else {
          if (begin) {
          ...
          s << "protection all fields " << std::showpos << show
            << std::noshowpos << '%';
        }
        ...
        if (it.abilities->speed) {
          ...
          s << "speed " << std::showpos << (it.abilities->speed >> 1)
            << std::noshowpos;
        }
      }
    ...
    if (it.abilities) {
      if (it.abilities->speed > 0) {
        s << " (speed " << std::showpos << (it.abilities->speed / 2)
          << std::noshowpos << ')';
      } else if (hasBitSet(CONDITION_DRUNK,
                           it.abilities->conditionSuppressions)) {
        s << " (hard drinking)";
    ...
    if (!found) {
      if (it.isKey()) {
        s << " (Key:" << std::setfill('0') << std::setw(4)
          << (item ? item->getActionId() : 0) << ')';
  ...
  if (it.showCharges) {
    if (subType == 0) {
      s << " that has " << it.charges << " charge" << (subType != 1 ? "s" : "")
        << " left";
    } else {
      s << " that has " << subType << " charge" << (subType != 1 ? "s" : "")
        << " left";
    }
  }
...
std::string Item::getNameDescription(const ItemType &it,
                                     const Item *item /*= nullptr*/,
                                     int32_t subType /*= -1*/,
                                     bool addArticle /*= true*/) {
  if (item) {
...
const ItemAttributes::Attribute *
ItemAttributes::getExistingAttr(ItemAttrTypes type) const {
  if (hasAttribute(type)) {
...
void Item::startDecaying() { g_decay().startDecay(this); }

void Item::stopDecaying() { g_decay().stopDecay(this); }
...
bool Item::hasMarketAttributes() {
  ...
  for (const auto &attribute : attributes->getList()) {
    if (attribute.type == ITEM_ATTRIBUTE_CHARGES &&
        static_cast<uint16_t>(attribute.value.integer) != items[id].charges) {
      return false;
    }

    if (attribute.type == ITEM_ATTRIBUTE_DURATION &&
        static_cast<uint32_t>(attribute.value.integer) !=
            getDefaultDuration()) {
      return false;
    }
...
bool Item::isInsideDepot(bool includeInbox /* = false*/) const {
  if (const Container *thisContainer = getContainer();
      thisContainer &&
      (thisContainer->getDepotLocker() || thisContainer->isDepotChest() ||
  ...
  while (container) {
    if (container->getDepotLocker() || container->isDepotChest() ||
        (includeInbox && container->isInbox())) {
      return true;
    }

    container = container->getParent() ? container->getParent()->getContainer()
                                       : nullptr;
  }
