...
void Decay::startDecay(Item *item) {
  ...
  if (decayState == DECAYING_STOPPING ||
      (!item->canDecay() && decayState == DECAYING_TRUE)) {
    stopDecay(item);
    return;
  }
  ...
  if (duration > 0) {
    ...
    if (decayMap.empty()) {
      eventId = g_scheduler().addEvent(
          createSchedulerTask(std::max<int32_t>(SCHEDULER_MINTICKS, duration),
                              std::bind(&Decay::checkDecay, this)));
    } else {
...
void Decay::checkDecay() {
  ...
  if (it != end) {
    eventId = g_scheduler().addEvent(createSchedulerTask(
        std::max<int32_t>(SCHEDULER_MINTICKS,
    ...
    if (player) {
      ...
      for (int32_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) {
        if (it.abilities && it.abilities->skills[i] != 0) {
          ...
          player->setVarSkill(static_cast<skills_t>(i),
                              -it.abilities->skills[i]);
        }
      }
