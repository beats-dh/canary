...
bool Weapons::registerEvent(Event_ptr event, const pugi::xml_node &) {
  Weapon *weapon = static_cast<Weapon *>(
      event.release()); // event is guaranteed to be a Weapon
...
int32_t Weapons::getMaxWeaponDamage(uint32_t level, int32_t attackSkill,
  ...
  if (isMelee) {
    return static_cast<int32_t>(std::round(
        (0.085 * attackFactor * attackValue * attackSkill) + (level / 5)));
  } else {
    return static_cast<int32_t>(std::round(
        (0.09 * attackFactor * attackValue * attackSkill) + (level / 5)));
  }
}
...
bool Weapon::configureEvent(const pugi::xml_node &node) {
  ...
  if ((attr = node.attribute("breakchance")) &&
      g_configManager().getBoolean(REMOVE_WEAPON_CHARGES)) {
...
int32_t Weapon::playerWeaponCheck(Player *player, Creature *target,
...
bool Weapon::useWeapon(Player *player, Item *item, Creature *target) const {
  int32_t damageModifier =
      playerWeaponCheck(player, target, item->getShootRange());
  if (damageModifier == 0) {
    return false;
  }
...
CombatDamage Weapon::getCombatDamage(CombatDamage combat, Player *player,
                                     Item *item, int32_t damageModifier) const {
  // Local variables
  ...
  float attackFactor =
      player->getAttackFactor(); // full atk, balanced or full defense
  ...
  int32_t maxDamage = static_cast<int32_t>(
      Weapons::getMaxWeaponDamage(level, playerSkill, totalAttack, attackFactor,
                                  true) *
      player->getVocation()->meleeDamageMultiplier * damageModifier / 100);
  ...
  if (scripted) {
    ...
    if (damage.secondary.type == COMBAT_NONE) {
      damage.primary.value =
          (getWeaponDamage(player, target, item) * damageModifier) / 100;
      damage.secondary.value = 0;
    } else {
      damage.primary.value =
          (getWeaponDamage(player, target, item) * damageModifier) / 100;
      damage.secondary.value =
          (getElementDamage(player, target, item) * damageModifier) / 100;
    }
...
void Weapon::onUsedWeapon(Player *player, Item *item, Tile *destTile) const {
  ...
  case WEAPONACTION_MOVE:
    g_game().internalMoveItem(item->getParent(), destTile, INDEX_WHEREEVER,
                              item, 1, nullptr, FLAG_NOLIMIT);
    break;
...
bool WeaponMelee::useWeapon(Player *player, Item *item,
...
bool WeaponMelee::getSkillType(const Player *player, const Item *item,
  ...
  if (player->getAddAttackSkill() &&
      player->getLastAttackBlockType() != BLOCK_IMMUNITY) {
    skillpoint = 1;
  } else {
    skillpoint = 0;
  }
...
int32_t WeaponMelee::getElementDamage(const Player *player, const Creature *,
  ...
  int32_t maxValue = Weapons::getMaxWeaponDamage(
      level, attackSkill, attackValue, attackFactor, true);
  return -normal_random(
      minValue, static_cast<int32_t>(
                    maxValue * player->getVocation()->meleeDamageMultiplier));
}

int16_t WeaponMelee::getElementDamageValue() const { return elementDamage; }

int32_t WeaponMelee::getWeaponDamage(const Player *player, const Creature *,
                                     const Item *item,
  ...
  int32_t maxValue = static_cast<int32_t>(
      Weapons::getMaxWeaponDamage(level, attackSkill, attackValue, attackFactor,
                                  true) *
...
bool WeaponDistance::useWeapon(Player *player, Item *item,
                               Creature *target) const {
  int32_t damageModifier;
  ...
  if (it.weaponType == WEAPON_AMMO) {
    ...
    if (mainWeapon) {
      damageModifier = mainWeapon->playerWeaponCheck(
          player, target, mainWeaponItem->getShootRange());
    } else {
      damageModifier =
          playerWeaponCheck(player, target, mainWeaponItem->getShootRange());
    }
  } else {
  ...
  if (it.hitChance == 0) {
    ...
    const Position &targetPos = target->getPosition();
    uint32_t distance =
        std::max<uint32_t>(Position::getDistanceX(playerPos, targetPos),
                           Position::getDistanceY(playerPos, targetPos));
    ...
    if (maxHitChance == 75) {
      ...
      case 2:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 28) * 2.40f) + 8;
        break;
      case 3:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 45) * 1.55f) + 6;
        break;
      case 4:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 58) * 1.25f) + 3;
        break;
      case 6:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 90) * 0.80f) + 3;
        break;
      case 7:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 104) * 0.70f) + 2;
        break;
      default:
        chance = it.hitChance;
        break;
      ...
      case 5:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 74) * 1.20f) + 1;
        break;
      case 2:
      ...
      case 5:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 73) * 1.35f) + 1;
        break;
      case 2:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 30) * 3.20f) + 4;
        break;
      case 3:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 48) * 2.05f) + 2;
        break;
      case 4:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 65) * 1.50f) + 2;
        break;
      case 6:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 87) * 1.20f) - 4;
        break;
      case 7:
        chance =
            static_cast<uint32_t>(std::min<uint32_t>(skill, 90) * 1.10f) + 1;
        break;
      default:
        chance = it.hitChance;
        break;
  ...
  if (chance >= uniform_random(1, 100)) {
    ...
    if (!Position::areInRange<1, 1, 0>(player->getPosition(),
                                       target->getPosition())) {
      ...
      for (const auto &dir : destList) {
        ...
        Tile *tmpTile = g_game().map.getTile(
            static_cast<uint16_t>(destPos.x + dir.first),
            static_cast<uint16_t>(destPos.y + dir.second), destPos.z);
...
int32_t WeaponDistance::getElementDamage(const Player *player,
  ...
  int32_t maxValue =
      std::round((0.09f * attackFactor) * attackSkill * attackValue +
                 minValue) /
      2;
...
int16_t WeaponDistance::getElementDamageValue() const { return elementDamage; }

int32_t WeaponDistance::getWeaponDamage(const Player *player,
                                        const Creature *target,
                                        const Item *item,
                                        bool maxDamage /*= false*/) const {
  ...
  int32_t maxValue =
      std::round((0.09f * attackFactor) * attackSkill * attackValue + minValue);
  if (maxDamage) {
    return -maxValue;
  }
...
int32_t WeaponWand::getWeaponDamage(const Player *, const Creature *,
                                    const Item *,
                                    bool maxDamage /*= false*/) const {
  if (maxDamage) {
    return -maxChange;
...
int16_t WeaponWand::getElementDamageValue() const { return 0; }
