...
const Creature *Tile::getBottomVisibleCreature(const Creature *creature) const {
  if (const CreatureVector *creatures = getCreatures()) {
    if (creature) {
      ...
      for (auto it = creatures->rbegin(), end = creatures->rend(); it != end;
           ++it) {
...
Item *Tile::getItemByTopOrder(int32_t topOrder) {
  ...
  if (TileItemVector *items = getItemList()) {
    for (auto
             it = ItemVector::const_reverse_iterator(items->getEndTopItem()),
...
void Tile::onAddTileItem(Item *item) {
  if ((item->hasProperty(CONST_PROP_MOVEABLE) || item->getContainer()) ||
      (item->isWrapable() && !item->hasProperty(CONST_PROP_MOVEABLE) &&
  ...
  if ((!hasFlag(TILESTATE_PROTECTIONZONE) ||
       g_configManager().getBoolean(CLEAN_PROTECTION_ZONES)) &&
      item->isCleanable()) {
...
void Tile::onUpdateTileItem(Item *oldItem, const ItemType &oldType,
                            Item *newItem, const ItemType &newType) {
  if ((newItem->hasProperty(CONST_PROP_MOVEABLE) || newItem->getContainer()) ||
      (newItem->isWrapable() && newItem->hasProperty(CONST_PROP_MOVEABLE) &&
       !oldItem->hasProperty(CONST_PROP_BLOCKPATH))) {
    auto it = g_game().browseFields.find(this);
    if (it != g_game().browseFields.end()) {
  ...
  } else if ((oldItem->hasProperty(CONST_PROP_MOVEABLE) ||
              oldItem->getContainer()) ||
             (oldItem->isWrapable() &&
              !oldItem->hasProperty(CONST_PROP_MOVEABLE) &&
              !oldItem->hasProperty(CONST_PROP_BLOCKPATH))) {
    auto it = g_game().browseFields.find(this);
    if (it != g_game().browseFields.end()) {
  ...
  // event methods
  for (Creature *spectator : spectators) {
    spectator->onUpdateTileItem(this, cylinderMapPos, oldItem, oldType, newItem,
                                newType);
  }
}

void Tile::onRemoveTileItem(const SpectatorHashSet &spectators,
                            const std::vector<int32_t> &oldStackPosVector,
                            Item *item) {
  if ((item->hasProperty(CONST_PROP_MOVEABLE) || item->getContainer()) ||
      (item->isWrapable() && !item->hasProperty(CONST_PROP_MOVEABLE) &&
       !item->hasProperty(CONST_PROP_BLOCKPATH))) {
...
ReturnValue Tile::queryAdd(int32_t, const Thing &thing, uint32_t,
                           uint32_t tileFlags, Creature *) const {
  if (const Creature *creature = thing.getCreature()) {
    ...
    if (const Monster *monster = creature->getMonster()) {
      if (hasFlag(TILESTATE_PROTECTIONZONE | TILESTATE_FLOORCHANGE |
                  TILESTATE_TELEPORT) &&
          (!monster->isFamiliar() ||
           (monster->isFamiliar() && monster->getMaster() &&
            monster->getMaster()->getAttackedCreature()))) {
        return RETURNVALUE_NOTPOSSIBLE;
      }

      if (monster->isSummon()) {
        if (ground->getID() >= ITEM_WALKABLE_SEA_START &&
            ground->getID() <= ITEM_WALKABLE_SEA_END) {
          return RETURNVALUE_NOTPOSSIBLE;
        }
      }
        ...
        if (creatures) {
          for (Creature *tileCreature : *creatures) {
            if (tileCreature->getPlayer() &&
                tileCreature->getPlayer()->isInGhostMode()) {
              continue;
            }

            const Monster *creatureMonster = tileCreature->getMonster();
            if (!creatureMonster || !tileCreature->isPushable() ||
                (creatureMonster->isSummon() &&
                 creatureMonster->getMaster()->getPlayer())) {
              return RETURNVALUE_NOTPOSSIBLE;
            }
          }
        }
      ...
      if (hasBitSet(FLAG_PATHFINDING, tileFlags) &&
          hasFlag(TILESTATE_IMMOVABLENOFIELDBLOCKPATH)) {
        return RETURNVALUE_NOTPOSSIBLE;
      }

      if (hasFlag(TILESTATE_BLOCKSOLID) ||
          (hasBitSet(FLAG_PATHFINDING, tileFlags) &&
           hasFlag(TILESTATE_NOFIELDBLOCKPATH))) {
        if (!(monster->canPushItems() ||
              hasBitSet(FLAG_IGNOREBLOCKITEM, tileFlags))) {
          return RETURNVALUE_NOTPOSSIBLE;
        }
      }
      ...
      if (field && !field->isBlocking() && field->getDamage() != 0) {
        ...
        if (!monster->isImmune(combatType)) {
          ...
          // 2) Being attacked while random stepping will make it ignore field
          // damages
          if (hasBitSet(FLAG_IGNOREFIELDDAMAGE, tileFlags)) {
            if (!(monster->getIgnoreFieldDamage() ||
                  monster->canWalkOnFieldType(combatType))) {
              return RETURNVALUE_NOTPOSSIBLE;
            }
          } else {
            return RETURNVALUE_NOTPOSSIBLE;
    ...
    if (const Player *player = creature->getPlayer()) {
      if (creatures && !creatures->empty() &&
          !hasBitSet(FLAG_IGNOREBLOCKCREATURE, tileFlags) &&
      ...
      if (playerTile && player->isPzLocked()) {
        ...
        if ((!playerTile->hasFlag(TILESTATE_NOPVPZONE) &&
             hasFlag(TILESTATE_NOPVPZONE)) ||
            (!playerTile->hasFlag(TILESTATE_PROTECTIONZONE) &&
             hasFlag(TILESTATE_PROTECTIONZONE))) {
          // player is trying to enter a non-pvp/protection zone while being
          // pz-locked
    ...
    if (!hasBitSet(FLAG_IGNOREBLOCKITEM, tileFlags)) {
      // If the FLAG_IGNOREBLOCKITEM bit isn't set we dont have to iterate every
      // single item
      if (hasFlag(TILESTATE_BLOCKSOLID)) {
      ...
      if (ground) {
        ...
        if (iiType.blockSolid &&
            (!iiType.moveable ||
             ground->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID))) {
          return RETURNVALUE_NOTPOSSIBLE;
        }
      }
    ...
    if (itemIsHangable && hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
      ...
      if (ground) {
        const ItemType &iiType = Item::items[ground->getID()];
        if (iiType.blockSolid) {
          if (!iiType.allowPickupable || item->isMagicField() ||
              item->isBlocking()) {
...
ReturnValue Tile::queryMaxCount(int32_t, const Thing &, uint32_t count,
                                uint32_t &maxQueryCount, uint32_t) const {
  maxQueryCount = std::max<uint32_t>(1, count);
  return RETURNVALUE_NOERROR;
}

ReturnValue Tile::queryRemove(const Thing &thing, uint32_t count,
                              uint32_t tileFlags,
...
Tile *Tile::queryDestination(int32_t &, const Thing &, Item **destItem,
                             uint32_t &tileFlags) {
  ...
  *destItem = nullptr;
  ...
  if (hasFlag(TILESTATE_FLOORCHANGE_DOWN)) {
    ...
    if (southDownTile &&
      ...
      if (eastDownTile &&
          eastDownTile->hasFlag(TILESTATE_FLOORCHANGE_EAST_ALT)) {
        dx -= 2;
        destTile = g_game().map.getTile(dx, dy, dz);
      } else {
  ...
  if (destTile == nullptr) {
    ...
    tileFlags |=
        FLAG_NOLIMIT; // Will ignore that there is blocking items/creatures
  }
  ...
  if (creature) {
    ...
    if (itemType.isGroundTile()) {
      ...
      if (itemType.isSplash() && items) {
        ...
        for (ItemVector::const_iterator it = items->getBeginTopItem(),
                                        end = items->getEndTopItem();
             it != end; ++it) {
          Item *oldSplash = *it;
      ...
      if (items) {
        for (auto it = items->getBeginTopItem(), end = items->getEndTopItem();
          ...
          if (itemType.alwaysOnTopOrder <=
              Item::items[(*it)->getID()].alwaysOnTopOrder) {
...
void Tile::removeThing(Thing *thing, uint32_t count) {
  ...
  if (item == ground) {
    ...
    onRemoveTileItem(spectators, std::vector<int32_t>(spectators.size(), 0),
                     item);
    return;
  }
  ...
  if (item->isAlwaysOnTop()) {
    ...
    auto it =
        std::find(items->getBeginDownItem(), items->getEndDownItem(), item);
    ...
    if (itemType.stackable && count != item->getItemCount()) {
      uint8_t newCount = static_cast<uint8_t>(std::max<int32_t>(
          0, static_cast<int32_t>(item->getItemCount() - count)));
...
int32_t Tile::getClientIndexOfCreature(const Player *player,
...
int32_t Tile::getStackposOfCreature(const Player *player,
                                    const Creature *creature) const {
  int32_t n;
  if (ground) {
    n = 1;
...
uint32_t Tile::getItemTypeCount(uint16_t itemId,
...
void Tile::postAddNotification(Thing *thing, const Cylinder *oldParent,
                               int32_t index,
                               CylinderLink_t link /*= LINK_OWNER*/) {
  SpectatorHashSet spectators;
  g_game().map.getSpectators(spectators, getPosition(), true, true);
  for (Creature *spectator : spectators) {
    spectator->getPlayer()->postAddNotification(thing, oldParent, index,
                                                LINK_NEAR);
  }

  // add a reference to this item, it may be deleted after being added (mailbox
  // for example)
  Creature *creature = thing->getCreature();
  Item *item;
  if (creature) {
...
void Tile::postRemoveNotification(Thing *thing, const Cylinder *newParent,
  ...
  for (Creature *spectator : spectators) {
    spectator->getPlayer()->postRemoveNotification(thing, newParent, index,
                                                   LINK_NEAR);
  }
...
void Tile::internalAddThing(uint32_t, Thing *thing) {
  ...
  if (creature) {
    ...
    if (item->isAlwaysOnTop()) {
      ...
      for (auto it = items->getBeginTopItem(), end = items->getEndTopItem();
           it != end; ++it) {
        if (Item::items[(*it)->getID()].alwaysOnTopOrder >
            itemType.alwaysOnTopOrder) {
          items->insert(it, item);
          isInserted = true;
          break;
        }
...
void Tile::resetTileFlags(const Item *item) {
  ...
  if (item->hasProperty(CONST_PROP_BLOCKSOLID) &&
      !hasProperty(item, CONST_PROP_BLOCKSOLID)) {
  ...
  if (item->hasProperty(CONST_PROP_BLOCKPATH) &&
      !hasProperty(item, CONST_PROP_BLOCKPATH)) {
