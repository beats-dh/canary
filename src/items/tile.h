...
public:
	...
	void increaseDownItemCount() { downItemCount += 1; }
	void decreaseDownItemCount() { downItemCount -= 1; }
...
public:
	...
	int32_t getClientIndexOfCreature(const Player* player,
		const Creature* creature) const;
	int32_t getStackposOfCreature(const Player* player,
		const Creature* creature) const;
	...
	ReturnValue queryRemove(const Thing& thing, uint32_t count,
		uint32_t tileFlags,
		Creature* actor = nullptr) const override;
...
private:
	...
	void onRemoveTileItem(const SpectatorHashSet& spectators,
