...
Container::Container(uint16_t initType, uint16_t initSize,
                     bool initUnlocked /*= true*/,
                     bool initPagination /*= false*/)
    : Item(initType), maxSize(initSize), unlocked(initUnlocked),
...
bool Container::hasParent() const {
  return getID() != ITEM_BROWSEFIELD &&
         dynamic_cast<const Player *>(getParent()) == nullptr;
}
  ...
  for (auto item : itemlist) {
    if (item->getContainer() != NULL) {
      ...
      for (auto subContItem : subContainer) {
        ...
        toReturnList.push_back(std::pair<Item *, uint32_t>(
            containerItem,
            static_cast<uint32_t>(containerItem->getItemCount())));
      }
    } else if (item->isItemStorable()) {
      toReturnList.push_back(std::pair<Item *, uint32_t>(
          item, static_cast<uint32_t>(item->getItemCount())));
    }
  }
...
std::ostringstream &
Container::getContentDescription(std::ostringstream &os) const {
  bool firstitem = true;
...
void Container::onAddContainerItem(Item *item) {
  ...
  g_game().map.getSpectators(spectators, getPosition(), false, true, 2, 2, 2,
                             2);
...
void Container::onUpdateContainerItem(uint32_t index, Item *oldItem,
                                      Item *newItem) {
  SpectatorHashSet spectators;
  g_game().map.getSpectators(spectators, getPosition(), false, true, 2, 2, 2,
                             2);
...
ReturnValue Container::queryAdd(int32_t addIndex, const Thing &addThing,
                                uint32_t addCount, uint32_t flags,
                                Creature *actor /* = nullptr*/) const {
  bool childIsOwner = hasBitSet(FLAG_CHILDISOWNER, flags);
  if (childIsOwner) {
    // a child container is querying, since we are the top container (not
    // carried by a player) just return with no error.
  ...
  if (!hasBitSet(FLAG_NOLIMIT, flags)) {
    ...
    if (addIndex == INDEX_WHEREEVER && size() >= capacity() &&
        !hasPagination()) {
      return RETURNVALUE_CONTAINERNOTENOUGHROOM;
    }
  } else {
  ...
  if (const Container *topParentContainer = getTopParentContainer()) {
    uint32_t maxItem =
        static_cast<uint32_t>(g_configManager().getNumber(MAX_ITEM));
    if (const Container *addContainer = item->getContainer()) {
      uint32_t addContainerCount = addContainer->getContainerHoldingCount() + 1;
      uint32_t maxContainer =
          static_cast<uint32_t>(g_configManager().getNumber(MAX_CONTAINER));
      if (addContainerCount + topParentContainer->getContainerHoldingCount() >
          maxContainer) {
        return RETURNVALUE_NOTPOSSIBLE;
      }
  ...
  if (topParent != this) {
    return topParent->queryAdd(INDEX_WHEREEVER, *item, addCount,
                               flags | FLAG_CHILDISOWNER, actor);
  } else {
    return RETURNVALUE_NOERROR;
  }
}

ReturnValue Container::queryMaxCount(int32_t index, const Thing &thing,
  ...
  if (item->isStackable()) {
    ...
    if (index == INDEX_WHEREEVER) {
      // Iterate through every item and check how much free stackable slots
      // there is.
...
ReturnValue Container::queryRemove(const Thing &thing, uint32_t count,
                                   uint32_t flags,
                                   Creature *actor /*= nullptr */) const {
...
Cylinder *Container::queryDestination(int32_t &index, const Thing &thing,
  ...
  if (index == 255 /*add wherever*/) {
    ...
    *destItem = nullptr;
    ...
    then you open a bag that is inside the container you will have a bag with 8
    slots and a "grey" area where the other 12 slots where from the container if
    you drop the item on that grey area the client calculates the slot position
  ...
  if (autoStack && item->isStackable() && item->getParent() != this) {
    if (*destItem && (*destItem)->equals(item) &&
    ...
    for (Item *listItem : itemlist) {
      if (listItem != item && listItem->equals(item) &&
          listItem->getItemCount() < 100) {
        *destItem = listItem;
        index = n;
        return this;
      }
...
void Container::replaceThing(uint32_t index, Thing *thing) {
  ...
  itemlist[index] = item;
  item->setParent(this);
  updateItemWeight(-static_cast<int32_t>(replacedItem->getWeight()) +
                   item->getWeight());
...
uint32_t Container::getItemTypeCount(uint16_t itemId,
...
Container::getAllItemTypeCount(std::map<uint32_t, uint32_t> &countMap) const {
  for (Item *item : itemlist) {
...
void Container::postAddNotification(Thing *thing, const Cylinder *oldParent,
  ...
  if (topParent->getCreature()) {
    ...
    if (topParent->getParent()) {
      topParent->getParent()->postAddNotification(thing, oldParent, index,
                                                  LINK_NEAR);
    }
  } else {
...
void Container::postRemoveNotification(Thing *thing, const Cylinder *newParent,
                                       int32_t index, CylinderLink_t) {
  Cylinder *topParent = getTopParent();
  if (topParent->getCreature()) {
    ...
    // let the tile class notify surrounding players
    if (topParent->getParent()) {
      topParent->getParent()->postRemoveNotification(thing, newParent, index,
                                                     LINK_NEAR);
    }
  } else {
