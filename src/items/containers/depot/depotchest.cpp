...
ReturnValue DepotChest::queryAdd(int32_t index, const Thing &thing,
  ...
  if (!skipLimit) {
    ...
    if (Cylinder *localParent = getRealParent()) {
      if (localParent->getContainer()->getItemHoldingCount() + addCount >
          maxDepotItems) {
...
void DepotChest::postAddNotification(Thing *thing, const Cylinder *oldParent,
...
void DepotChest::postRemoveNotification(Thing *thing, const Cylinder *newParent,
                                        int32_t index, CylinderLink_t) {
  Cylinder *localParent = getParent();
  if (localParent != nullptr) {
