...
public:
  ...
  time_t getDate() const { return getIntAttr(ITEM_ATTRIBUTE_DATE); }
    ...
    if (decayState == DECAYING_TRUE || decayState == DECAYING_STOPPING) {
      return std::max<int32_t>(
          0, static_cast<int32_t>(
                 getIntAttr(ITEM_ATTRIBUTE_DURATION_TIMESTAMP) - OTSYS_TIME()));
    } else {
  ...
  struct CustomAttribute {
    typedef boost::variant<boost::blank, std::string, int64_t, double, bool>
        VariantAttribute;
    VariantAttribute value;

    CustomAttribute() : value(boost::blank()) {}

    template <typename T> explicit CustomAttribute(const T &v) : value(v) {}

    template <typename T> void set(const T &v) { value = v; }
    ...
    bool unserialize(PropStream &propStream) {
      // This is hard coded so it's not general, depends on the position of the
      // variants.
      uint8_t pos;
...
private:
  ...
  template <typename R> void setCustomAttribute(int64_t key, R value) {
  ...
  template <typename R> void setCustomAttribute(std::string &key, R value) {
...
public:
  ...
  void setIsLootTrackeable(bool value) { isLootTrackeable = value; }

  bool getIsLootTrackeable() { return isLootTrackeable; }
  ...
  void setCustomAttribute(std::string &key,
  ...
  const ItemAttributes::CustomAttribute *
  getCustomAttribute(const std::string &key) {
    return getAttributes()->getCustomAttribute(key);
  }
  const ItemAttributes::CustomAttribute *
  getCustomAttribute(const std::string &key) const {
    if (!attributes) {
      return nullptr;
    }
    ...
    ItemAttributes::CustomAttributeMap *customAttrMap =
        attributes->getAttr(ITEM_ATTRIBUTE_CUSTOM).value.custom;
    if (!customAttrMap) {
      return nullptr;
    }
  ...
  static std::string parseShowAttributesDescription(const Item *item,
                                                    const uint16_t itemId);

  static std::vector<std::pair<std::string, std::string>>
  getDescriptions(const ItemType &it, const Item *item = nullptr);
  static std::string getDescription(const ItemType &it, int32_t lookDistance,
                                    const Item *item = nullptr,
                                    int32_t subType = -1,
                                    bool addArticle = true);
  static std::string getNameDescription(const ItemType &it,
                                        const Item *item = nullptr,
                                        int32_t subType = -1,
                                        bool addArticle = true);
  static std::string getWeightDescription(const ItemType &it, uint32_t weight,
                                          uint32_t count = 1);
  ...
  virtual bool unserializeItemNode(OTB::Loader &, const OTB::Node &,
  ...
  bool isWrapContainer() const { return items[id].wrapContainer; }
  ...
  bool isRotatable() const { return items[id].rotatable && items[id].rotateTo; }
  bool isPodium() const { return items[id].isPodium; }
  bool isWrapable() const { return items[id].wrapable && items[id].wrapableTo; }
  bool hasWalkStack() const { return items[id].walkStack; }
  bool isQuiver() const { return items[id].isQuiver(); }
  ...
  bool isCleanable() const {
    return !loadedFromMap && canRemove() && isPickupable() &&
           !hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) &&
  ...
  /**
   * @brief Decay imbuement time duration, only use this for decay the imbuement
   * time
   *
   ...
   */
  void decayImbuementTime(uint8_t slot, uint16_t imbuementId,
                          int32_t duration) {
