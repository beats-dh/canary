...
public:
  ...
  bool isQuiver() const { return (type == ITEM_TYPE_QUIVER); }
  ...
  void setImbuementType(ImbuementTypes_t imbuementType, uint16_t slotMaxTier) {
    imbuementTypes[imbuementType] =
        std::min<uint16_t>(IMBUEMENT_MAX_TIER, slotMaxTier);
  }
...
public:
  ...
  /**
   * @brief Check if the itemid "hasId" is stored on "items", if not, return
   * false
   *
