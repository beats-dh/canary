...
public:
   ...
   * position) -1 is a internal value and means add to a empty position, with no
   * destItem \param thing the object to move/add \param count is the amount
   * that we want to move/add \param flags if FLAG_CHILDISOWNER if set the query
   * is from a child-cylinder (check cap etc.) if FLAG_NOLIMIT is set blocking
   * items/container limits is ignored \param actor the creature trying to add
   * the thing \returns ReturnValue holds the return value
   */
  virtual ReturnValue queryAdd(int32_t index, const Thing &thing,
   ...
   * position) -1 is a internal value and means add to a empty position, with no
   * destItem \param thing the object to move/add \param count is the amount
   * that we want to move/add \param maxQueryCount is the max amount that the
   * cylinder can accept \param flags optional flags to modify the default
   * behaviour \returns ReturnValue holds the return value
   */
  virtual ReturnValue queryMaxCount(int32_t index, const Thing &thing,
   ...
   */
  virtual ReturnValue queryRemove(const Thing &thing, uint32_t count,
                                  uint32_t flags,
   ...
   * index \param destItem is the destination object \param flags optional flags
   * to modify the default behaviour this method can modify the flags \returns
   * Cylinder returns the destination cylinder
   */
  virtual Cylinder *queryDestination(int32_t &index, const Thing &thing,
                                     Item **destItem, uint32_t &flags) = 0;
   ...
   */
  virtual void postAddNotification(Thing *thing, const Cylinder *oldParent,
                                   int32_t index,
   ...
   * \param link holds the relation the object has to the cylinder
   */
  virtual void postRemoveNotification(Thing *thing, const Cylinder *newParent,
                                      int32_t index,
                                      CylinderLink_t link = LINK_OWNER) = 0;
   ...
   * asked item type
   */
  virtual uint32_t getItemTypeCount(uint16_t itemId,
  ...
  getAllItemTypeCount(std::map<uint32_t, uint32_t> &countMap) const;
...
public:
  ...
  virtual ReturnValue queryMaxCount(int32_t, const Thing &, uint32_t,
  ...
  virtual Cylinder *queryDestination(int32_t &, const Thing &, Item **,
                                     uint32_t &) override {
    return nullptr;
  }
