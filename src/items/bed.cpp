...
void BedItem::serializeAttr(PropWriteStream &propWriteStream) const {
  ...
  if (sleepStart != 0) {
    ...
    // FIXME: should be stored as 64-bit, but we need to retain backwards
    // compatibility
...
bool BedItem::canUse(Player *player) {
  ...
  if (house->getHouseAccessLevel(&sleeper) >
      house->getHouseAccessLevel(player)) {
    return false;
  }
  return true;
}
...
bool BedItem::trySleep(Player *player) {
  ...
  if (sleeperGUID != 0) {
    if (Item::items[id].transformToFree != 0 &&
        house->getOwner() == player->getGUID()) {
      wakeUp(nullptr);
    }
...
bool BedItem::sleep(Player *player) {
  ...
  g_scheduler().addEvent(createSchedulerTask(
      SCHEDULER_MINTICKS,
      std::bind(&ProtocolGame::logout, player->client, false, false)));
...
void BedItem::regeneratePlayer(Player *player) const {
  ...
  if (condition != nullptr) {
    ...
    if (condition->getTicks() != -1) {
      regen = std::min<int32_t>((condition->getTicks() / 1000), sleptTime) /
              30; // RATE_HEALTH_REGEN_SPEED and RATE_MANA_REGEN_SPEED?
    ...
    player->changeHealth(regen * g_configManager().getFloat(RATE_HEALTH_REGEN),
                         false);
  ...
  if (it.type == ITEM_TYPE_BED) {
    if ((player != nullptr) && it.transformToOnUse[player->getSex()] != 0) {
      const ItemType &newType =
          Item::items[it.transformToOnUse[player->getSex()]];
