...
bool DatabaseManager::optimizeTables() {
  ...
  query << "SELECT `TABLE_NAME` FROM `information_schema`.`TABLES` WHERE "
           "`TABLE_SCHEMA` = "
        << db.escapeString(g_configManager().getString(MYSQL_DB))
        << " AND `DATA_FREE` > 0";
...
int32_t DatabaseManager::getDatabaseVersion() {
  if (!tableExists("server_config")) {
    Database &db = Database::getInstance();
    db.executeQuery(
        "CREATE TABLE `server_config` (`config` VARCHAR(50) NOT NULL, `value` "
        "VARCHAR(256) NOT NULL DEFAULT '', UNIQUE(`config`)) ENGINE = InnoDB");
...
void DatabaseManager::updateDatabase() {
  ...
  do {
    ...
    if (lua_pcall(L, 0, 1, 0) != 0) {
      ...
      SPDLOG_WARN("[DatabaseManager::updateDatabase - Version: {}] {}", version,
                  lua_tostring(L, -1));
      break;
    }
...
bool DatabaseManager::getDatabaseConfig(const std::string &config,
                                        int32_t &value) {
  Database &db = Database::getInstance();
  std::ostringstream query;
  query << "SELECT `value` FROM `server_config` WHERE `config` = "
...
void DatabaseManager::registerDatabaseConfig(const std::string &config,
                                             int32_t value) {
  Database &db = Database::getInstance();
  std::ostringstream query;
  ...
  if (!getDatabaseConfig(config, tmp)) {
    query << "INSERT INTO `server_config` VALUES (" << db.escapeString(config)
          << ", '" << value << "')";
  } else {
