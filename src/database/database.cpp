...
bool Database::connect() {
	...
	if (!mysql_real_connect(handle,
			g_configManager().getString(MYSQL_HOST).c_str(),
			g_configManager().getString(MYSQL_USER).c_str(),
			g_configManager().getString(MYSQL_PASS).c_str(),
			g_configManager().getString(MYSQL_DB).c_str(),
			g_configManager().getNumber(SQL_PORT),
			g_configManager().getString(MYSQL_SOCK).c_str(), 0)) {
...
retry:
	...
	// we should call that every time as someone would call
	// executeQuery('SELECT...') as it is described in MySQL manual: "it doesn't
	// hurt" :P
...
const char* DBResult::getStream(const std::string& s,
