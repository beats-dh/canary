...
std::string RSA::base64Decrypt(const std::string &input) const {
  ...
  while (pos < length) {
    ...
    output.push_back(static_cast<std::string::value_type>(
        ((posOfCharacter(input[pos])) << 2) + ((pos1 & 0x30) >> 4)));
    if (input[pos + 2] != '=' && input[pos + 2] != '.') {
      ...
      if (input[pos + 3] != '=' && input[pos + 3] != '.') {
        output.push_back(static_cast<std::string::value_type>(
            ((pos2 & 0x03) << 6) + posOfCharacter(input[pos + 3])));
      }
    }
...
void RSA::readHexString(char *&pos, uint16_t length,
...
bool RSA::loadPEM(const std::string &filename) {
  ...
  if (key.compare(0, header_old.size(), header_old) == 0) {
    if (key.compare(key.size() - footer_old.size(), footer_old.size(),
                    footer_old) != 0) {
      SPDLOG_ERROR("[RSA::loadPEM] - Missing RSA private key footer");
      return false;
    }

    key = base64Decrypt(key.substr(
        header_old.size(), key.size() - header_old.size() - footer_old.size()));
  } else if (key.compare(0, header_new.size(), header_new) == 0) {
    if (key.compare(key.size() - footer_new.size(), footer_new.size(),
                    footer_new) != 0) {
      SPDLOG_ERROR("[RSA::loadPEM] - Missing RSA private key footer");
      return false;
    }
    ...
    key = base64Decrypt(key.substr(
        header_new.size(), key.size() - header_new.size() - footer_new.size()));
  } else {
  ...
  if (tag == CRYPT_RSA_ASN1_INTEGER &&
      static_cast<uint8_t>(*(pos + 0)) == 0x01 &&
      static_cast<uint8_t>(*(pos + 1)) == 0x00 &&
      static_cast<uint8_t>(*(pos + 2)) == 0x30) {
    pos += 3;
