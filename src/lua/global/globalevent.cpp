...
bool GlobalEvents::registerEvent(Event_ptr event, const pugi::xml_node &) {
  GlobalEvent_ptr globalEvent{static_cast<GlobalEvent *>(
      event.release())}; // event is guaranteed to be a GlobalEvent
  if (globalEvent->getEventType() == GLOBALEVENT_TIMER) {
    auto result =
        timerMap.emplace(globalEvent->getName(), std::move(*globalEvent));
    if (result.second) {
      if (timerEventId == 0) {
        timerEventId = g_scheduler().addEvent(createSchedulerTask(
            SCHEDULER_MINTICKS, std::bind(&GlobalEvents::timer, this)));
      }
      return true;
    }
  } else if (globalEvent->getEventType() != GLOBALEVENT_NONE) {
    auto result =
        serverMap.emplace(globalEvent->getName(), std::move(*globalEvent));
    if (result.second) {
      return true;
    }
  } else { // think event
    auto result =
        thinkMap.emplace(globalEvent->getName(), std::move(*globalEvent));
    if (result.second) {
      if (thinkEventId == 0) {
        thinkEventId = g_scheduler().addEvent(createSchedulerTask(
            SCHEDULER_MINTICKS, std::bind(&GlobalEvents::think, this)));
      }
      return true;
    }
  }
...
bool GlobalEvents::registerLuaEvent(GlobalEvent *event) {
  ...
  SPDLOG_WARN("Duplicate registered globalevent with name: {}",
              globalEvent->getName());
  return false;
}
...
void GlobalEvents::timer() {
  ...
  while (it != timerMap.end()) {
    ...
    globalEvent.setNextExecution(globalEvent.getNextExecution() +
                                 nextExecutionTime);
  ...
  if (nextScheduledTime != std::numeric_limits<int64_t>::max()) {
    timerEventId = g_scheduler().addEvent(
        createSchedulerTask(std::max<int64_t>(1000, nextScheduledTime * 1000),
...
void GlobalEvents::think() {
  ...
  if (nextScheduledTime != std::numeric_limits<int64_t>::max()) {
    thinkEventId = g_scheduler().addEvent(createSchedulerTask(
        nextScheduledTime, std::bind(&GlobalEvents::think, this)));
  }
}
...
bool GlobalEvent::configureEvent(const pugi::xml_node &node) {
  ...
  if (!nameAttribute) {
    SPDLOG_ERROR(
        "[GlobalEvent::configureEvent] - Missing name for a globalevent");
    return false;
  }
  ...
  if ((attr = node.attribute("time"))) {
    std::vector<int32_t> params =
        vectorAtoi(explodeString(attr.as_string(), ":"));
    ...
    time_t difference =
        static_cast<time_t>(difftime(mktime(timeinfo), current_time));
    if (difference < 0) {
      difference += 86400;
    }
    ...
    interval = std::max<int32_t>(SCHEDULER_MINTICKS,
...
bool GlobalEvent::executePeriodChange(LightState_t lightState,
                                      LightInfo lightInfo) {
