...
Modules::Modules() : scriptInterface("Modules Interface") {
...
LuaScriptInterface &Modules::getScriptInterface() { return scriptInterface; }

std::string Modules::getScriptBaseName() const { return "modules"; }
...
bool Modules::registerEvent(Event_ptr event, const pugi::xml_node &) {
  ...
  if (oldModule) {
    if (!oldModule->isLoaded() &&
        oldModule->getEventType() == module->getEventType()) {
...
void Modules::executeOnRecvbyte(uint32_t playerId, NetworkMessage &msg,
                                uint8_t byte) const {
  ...
  for (auto &it : recvbyteList) {
    ...
    if (module.getEventType() == MODULE_TYPE_RECVBYTE &&
        module.getRecvbyte() == byte &&
        player->canRunModule(module.getRecvbyte())) {
