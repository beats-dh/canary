...
int GlobalFunctions::luaDoPlayerAddItem(lua_State *L) {
  // doPlayerAddItem(cid, itemid, <optional: default: 1> count/subtype,
  // <optional: default: 1> canDropOnMap) doPlayerAddItem(cid, itemid,
  // <optional: default: 1> count, <optional: default: 1> canDropOnMap,
  ...
  if (parameters > 4) {
    ...
    if (it.stackable) {
      itemCount =
          static_cast<int32_t>(std::ceil(static_cast<float>(count) / 100));
    } else {
      itemCount = 1;
    }
    subType = count;
  ...
  while (itemCount > 0) {
    ...
    ReturnValue ret =
        g_game().internalPlayerAddItem(player, newItem, canDropOnMap);
...
int GlobalFunctions::luaDoSetCreatureLight(lua_State *L) {
  ...
  Condition *condition = Condition::createCondition(
      CONDITIONID_COMBAT, CONDITION_LIGHT, time, level | (color << 8));
...
int GlobalFunctions::luaIsDepot(lua_State *L) {
  // isDepot(uid)
  Container *container =
      getScriptEnv()->getContainerByUID(getNumber<uint32_t>(L, -1));
...
int GlobalFunctions::luaCreateCombatArea(lua_State *L) {
  ...
  uint32_t areaId =
      g_luaEnvironment.createAreaObject(env->getScriptInterface());
...
int GlobalFunctions::luaDoAreaCombatHealth(lua_State *L) {
  // doAreaCombatHealth(cid, type, pos, area, min, max, effect[, origin =
  // ORIGIN_SPELL])
  Creature *creature = getCreature(L, 1);
  ...
  if (area || areaId == 0) {
    ...
    damage.primary.value =
        normal_random(getNumber<int32_t>(L, 6), getNumber<int32_t>(L, 5));
...
int GlobalFunctions::luaDoTargetCombatHealth(lua_State *L) {
  ...
  damage.primary.type = combatType;
  damage.primary.value =
      normal_random(getNumber<int32_t>(L, 4), getNumber<int32_t>(L, 5));
...
int GlobalFunctions::luaDoTargetCombatMana(lua_State *L) {
  ...
  damage.primary.type = COMBAT_MANADRAIN;
  damage.primary.value =
      normal_random(getNumber<int32_t>(L, 3), getNumber<int32_t>(L, 4));
...
int GlobalFunctions::luaAddEvent(lua_State *L) {
  ...
  if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) ||
      g_configManager().getBoolean(CONVERT_UNSAFE_SCRIPTS)) {
  ...
  for (int i = 0; i < parameters - 2;
       ++i) { // -2 because addEvent needs at least two parameters
  ...
  eventDesc.eventId = g_scheduler().addEvent(createSchedulerTask(
      delay, std::bind(&LuaEnvironment::executeTimerEvent, &g_luaEnvironment,
                       lastTimerEventId)));
...
bool GlobalFunctions::getArea(lua_State *L, std::list<uint32_t> &list,
                              uint32_t &rows) {
  lua_pushnil(L);
