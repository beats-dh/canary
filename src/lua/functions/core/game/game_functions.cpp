...
int GameFunctions::luaGameCreateNpcType(lua_State *L) {
  ...
  if (getScriptEnv()->getScriptInterface() !=
...
int GameFunctions::luaGameGetSpectators(lua_State *L) {
  // Game.getSpectators(position[, multifloor = false[, onlyPlayer = false[,
  // minRangeX = 0[, maxRangeX = 0[, minRangeY = 0[, maxRangeY = 0]]]]]])
  ...
  g_game().map.getSpectators(spectators, position, multifloor, onlyPlayers,
...
int GameFunctions::luaGameGetBestiaryList(lua_State *L) {
  ...
  if (lua_gettop(L) <= 2) {
    ...
    if (isNumber(L, 2)) {
      std::map<uint16_t, std::string> tmplist = g_iobestiary().findRaceByName(
          "CANARY", false, getNumber<BestiaryType_t>(L, 2));
      ...
      std::map<uint16_t, std::string> tmplist =
          g_iobestiary().findRaceByName(getString(L, 2));
...
int GameFunctions::luaGameCreateMonster(lua_State *L) {
  // Game.createMonster(monsterName, position[, extended = false[, force =
  // false[, master = nil]]])
...
int GameFunctions::luaGameCreateBestiaryCharm(lua_State *L) {
  ...
  if (getScriptEnv()->getScriptInterface() !=
      &g_scripts().getScriptInterface()) {
    reportErrorFunc(
        "Charm bestiary can only be registered in the Scripts interface.");
    lua_pushnil(L);
    return 1;
  }

  if (Charm *charm = g_iobestiary().getBestiaryCharm(
          static_cast<charmRune_t>(getNumber<int8_t>(L, 1, 0)), true)) {
...
int GameFunctions::luaGameCreateItemClassification(lua_State *L) {
  ...
  if (getScriptEnv()->getScriptInterface() !=
      &g_scripts().getScriptInterface()) {
    reportErrorFunc(
        "Item classification can only be registered in the Scripts interface.");
    lua_pushnil(L);
    return 1;
  }

  ItemClassification *itemClassification =
      g_game().getItemsClassification(getNumber<uint8_t>(L, 1), true);
