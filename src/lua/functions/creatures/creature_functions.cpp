...
int CreatureFunctions::luaCreatureCreate(lua_State *L) {
  ...
  if (isNumber(L, 2)) {
    ...
    if (type != LuaData_Player && type != LuaData_Monster &&
        type != LuaData_Npc) {
      lua_pushnil(L);
      return 1;
    }
...
int CreatureFunctions::luaCreatureSetDirection(lua_State *L) {
  ...
  if (creature) {
    pushBoolean(
        L, g_game().internalCreatureTurn(creature, getNumber<Direction>(L, 2)));
  } else {
    lua_pushnil(L);
  }
  return 1;
...
int CreatureFunctions::luaCreatureSetHealth(lua_State *L) {
  ...
  creature->health =
      std::min<int32_t>(getNumber<uint32_t>(L, 2), creature->healthMax);
...
int CreatureFunctions::luaCreatureSetOutfit(lua_State *L) {
  ...
  if (creature) {
    ...
    if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) &&
      ...
      SPDLOG_WARN(
          "[CreatureFunctions::luaCreatureSetOutfit] An unregistered creature "
          "looktype type with id '{}' was blocked to prevent client crash.",
          outfit.lookType);
      return 1;
    }
...
int CreatureFunctions::luaCreatureGetCondition(lua_State *L) {
  // creature:getCondition(conditionType[, conditionId = CONDITIONID_COMBAT[,
  // subId = 0]])
  Creature *creature = getUserdata<Creature>(L, 1);
  if (!creature) {
    lua_pushnil(L);
  ...
  ConditionId_t conditionId =
      getNumber<ConditionId_t>(L, 3, CONDITIONID_COMBAT);
  uint32_t subId = getNumber<uint32_t>(L, 4, 0);

  const Condition *condition =
      creature->getCondition(conditionType, conditionId, subId);
  if (condition) {
...
int CreatureFunctions::luaCreatureRemoveCondition(lua_State *L) {
  // creature:removeCondition(conditionType[, conditionId = CONDITIONID_COMBAT[,
  // subId = 0[, force = false]]])
  Creature *creature = getUserdata<Creature>(L, 1);
  if (!creature) {
    lua_pushnil(L);
...
int CreatureFunctions::luaCreatureTeleportTo(lua_State *L) {
  ...
  if (g_game().internalTeleport(creature, position, pushMovement) !=
      RETURNVALUE_NOERROR) {
    SPDLOG_WARN("[{}] - Cannot teleport creature with name: {}, fromPosition "
                "{}, toPosition {}",
                __FUNCTION__, creature->getName(), oldPosition.toString(),
                position.toString());
    pushBoolean(L, false);
    return 1;
  }
...
int CreatureFunctions::luaCreatureSay(lua_State *L) {
  // creature:say(text[, type = TALKTYPE_MONSTER_SAY[, ghost = false[, target =
  // nullptr[, position]]]])
  ...
  if (position.x != 0) {
    pushBoolean(L, g_game().internalCreatureSay(creature, type, text, ghost,
                                                &spectators, &position));
  } else {
    pushBoolean(L, g_game().internalCreatureSay(creature, type, text, ghost,
                                                &spectators));
...
int CreatureFunctions::luaCreatureGetPathTo(lua_State *L) {
  // creature:getPathTo(pos[, minTargetDist = 0[, maxTargetDist = 1[,
  // fullPathSearch = true[, clearSight = true[, maxSearchDist = 0]]]]])
  Creature *creature = getUserdata<Creature>(L, 1);
  if (!creature) {
    lua_pushnil(L);
    return 1;
...
int CreatureFunctions::luaCreatureMove(lua_State *L) {
  ...
  if (isNumber(L, 2)) {
    ...
    lua_pushnumber(
        L, g_game().internalMoveCreature(creature, direction, FLAG_NOLIMIT));
  } else {
    ...
    lua_pushnumber(L, g_game().internalMoveCreature(*creature, *tile,
