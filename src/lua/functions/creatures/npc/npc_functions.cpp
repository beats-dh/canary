...
int NpcFunctions::luaNpcSay(lua_State *L) {
  // npc:say(text[, type = TALKTYPE_PRIVATE_NP[, ghost = false[, target =
  // nullptr[, position]]]])
  ...
  if (position.x != 0) {
    pushBoolean(L, g_game().internalCreatureSay(npc, type, text, ghost,
                                                &spectators, &position));
  } else {
    pushBoolean(
        L, g_game().internalCreatureSay(npc, type, text, ghost, &spectators));
  }
  return 1;
}
 ...
 * @param true, If true, force stop walk, if @param false, do not force stop
 * walk
 */
...
int NpcFunctions::luaNpcSellItem(lua_State *L) {
  // npc:sellItem(player, itemid, amount, <optional> subtype, <optional>
  // actionid, <optional: default: 1> canDropOnMap)
  Npc *npc = getUserdata<Npc>(L, 1);
  if (!npc) {
    reportErrorFunc(getErrorDesc(LUA_ERROR_NPC_NOT_FOUND));
    pushBoolean(L, false);
  ...
  if (it.stackable) {
    while (amount > 0) {
      ...
      if (g_game().internalPlayerAddItem(player, item, canDropOnMap) !=
          RETURNVALUE_NOERROR) {
        delete item;
