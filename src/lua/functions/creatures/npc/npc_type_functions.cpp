...
void NpcTypeFunctions::createNpcTypeShopLuaTable(
    lua_State *L, const std::vector<ShopBlock> &shopVector) {
  ...
  if (npcType) {
    if (lua_gettop(L) == 1) {
      ...
      if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) &&
        ...
        SPDLOG_WARN(
            "[NpcTypeFunctions::luaNpcTypeOutfit] An unregistered creature "
