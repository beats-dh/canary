...
int CombatFunctions::luaCombatCreate(lua_State *L) {
  // Combat()
  pushUserdata<Combat>(L, g_luaEnvironment.createCombatObject(
                              getScriptEnv()->getScriptInterface()));
...
int CombatFunctions::luaCombatSetArea(lua_State *L) {
  ...
  const AreaCombat *area =
      g_luaEnvironment.getAreaObject(getNumber<uint32_t>(L, 2));
  if (!area) {
...
int CombatFunctions::luaCombatSetCallback(lua_State *L) {
  ...
  pushBoolean(L, callback->loadCallBack(getScriptEnv()->getScriptInterface(),
                                        function));
  return 1;
}
