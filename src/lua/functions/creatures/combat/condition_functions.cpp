...
int ConditionFunctions::luaConditionCreate(lua_State *L) {
  ...
  Condition *condition =
      Condition::createCondition(conditionId, conditionType, 0, 0);
  if (condition) {
...
int ConditionFunctions::luaConditionSetFormula(lua_State *L) {
  ...
  ConditionSpeed *condition =
      dynamic_cast<ConditionSpeed *>(getUserdata<Condition>(L, 1));
  if (condition) {
...
int ConditionFunctions::luaConditionSetOutfit(lua_State *L) {
  ...
  // condition:setOutfit(lookTypeEx, lookType, lookHead, lookBody, lookLegs,
  // lookFeet[, lookAddons[, lookMount[, lookMountHead[, lookMountBody[,
  // lookMountLegs[, lookMountFeet[, lookFamiliarsType]]]]]]])
  Outfit_t outfit;
  if (isTable(L, 2)) {
    outfit = getOutfit(L, 2);
  } else {
    outfit.lookFamiliarsType =
        getNumber<uint16_t>(L, 14, outfit.lookFamiliarsType);
  ...
  ConditionOutfit *condition =
      dynamic_cast<ConditionOutfit *>(getUserdata<Condition>(L, 1));
  if (condition) {
    condition->setOutfit(outfit);
    pushBoolean(L, true);
  } else {
...
int ConditionFunctions::luaConditionAddDamage(lua_State *L) {
  ...
  ConditionDamage *condition =
      dynamic_cast<ConditionDamage *>(getUserdata<Condition>(L, 1));
  if (condition) {
