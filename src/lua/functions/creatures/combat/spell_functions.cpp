...
int SpellFunctions::luaSpellCreate(lua_State *L) {
  ...
  // Spell(type) ex: Spell(SPELL_INSTANT) or Spell(SPELL_RUNE) to create a new
  // spell
  if (lua_gettop(L) == 1) {
  ...
  if (spellType == SPELL_INSTANT) {
    InstantSpell *spell =
        new InstantSpell(getScriptEnv()->getScriptInterface());
    spell->fromLua = true;
  ...
  if (spell) {
    if (spell->spellType == SPELL_INSTANT) {
      InstantSpell *instant =
          dynamic_cast<InstantSpell *>(getUserdata<Spell>(L, 1));
...
int SpellFunctions::luaSpellRegister(lua_State *L) {
  ...
  if (spell->spellType == SPELL_INSTANT) {
    ...
    if (rune->getMagicLevel() != 0 || rune->getLevel() != 0) {
      ...
      // If the item is not registered in items.xml then we will register it by
      // rune name
      if (iType.name.empty()) {
...
int SpellFunctions::luaSpellId(lua_State *L) {
  ...
  if (spell) {
    if (spell->spellType != SPELL_INSTANT) {
      reportErrorFunc(
          "The method: 'spell:id(id)' is only for use of instant spells");
      pushBoolean(L, false);
      return 1;
    }
    ...
    if (lua_gettop(L) == 1) {
      ...
      for (int i = 0; i < parameters; ++i) {
        if (getString(L, 2 + i).find(";") != std::string::npos) {
          std::vector<std::string> vocList =
              explodeString(getString(L, 2 + i), ";");
...
int SpellFunctions::luaSpellWords(lua_State *L) {
  ...
  if (spell) {
    // if spell != SPELL_INSTANT, it means that this actually is no
    // InstantSpell, so we return nil
    if (spell->spellType != SPELL_INSTANT) {
      lua_pushnil(L);
      return 1;
    }
...
int SpellFunctions::luaSpellRuneId(lua_State *L) {
  ...
  if (spell) {
    // if spell != SPELL_RUNE, it means that this actually is no RuneSpell, so
    // we return nil
