...
void MonsterTypeFunctions::createMonsterTypeLootLuaTable(
    lua_State *L, const std::vector<LootBlock> &lootList) {
...
int MonsterTypeFunctions::luaMonsterTypeCreate(lua_State *L) {
  ...
  if (isNumber(L, 2)) {
    monsterType =
        g_monsters().getMonsterTypeByRaceId(getNumber<uint16_t>(L, 2));
  } else {
...
int MonsterTypeFunctions::luaMonsterTypeEnemyFactions(lua_State *L) {
  // get: monsterType:enemyFactions() set:
  // monsterType:enemyFactions(enemyFaction)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeTargetPreferPlayer(lua_State *L) {
  // get: monsterType:targetPreferPlayer() set:
  // monsterType:targetPreferPlayer(bool)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeTargetPreferMaster(lua_State *L) {
  // get: monsterType:targetPreferMaster() set:
  // monsterType:targetPreferMaster(bool)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
      lua_pushnumber(L, monsterType->info.faction);
...
int MonsterTypeFunctions::luaMonsterTypeBestiaryFirstUnlock(lua_State *L) {
  // get: monsterType:BestiaryFirstUnlock() set:
  // monsterType:BestiaryFirstUnlock(value)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeBestiarySecondUnlock(lua_State *L) {
  // get: monsterType:BestiarySecondUnlock() set:
  // monsterType:BestiarySecondUnlock(value)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeBestiaryCharmsPoints(lua_State *L) {
  // get: monsterType:BestiaryCharmsPoints() set:
  // monsterType:BestiaryCharmsPoints(value)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeBestiaryOccurrence(lua_State *L) {
  // get: monsterType:BestiaryOccurrence() set:
  // monsterType:BestiaryOccurrence(value)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeBestiaryLocations(lua_State *L) {
  // get: monsterType:BestiaryLocations() set:
  // monsterType:BestiaryLocations(string)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeCombatImmunities(lua_State *L) {
  // get: monsterType:combatImmunities() set:
  // monsterType:combatImmunities(immunity)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeConditionImmunities(lua_State *L) {
  // get: monsterType:conditionImmunities() set:
  // monsterType:conditionImmunities(immunity)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
      ...
      if (immunity == "physical") {
        ...
        SPDLOG_WARN(
            "[MonsterTypeFunctions::luaMonsterTypeConditionImmunities] - "
            "Unknown immunity name: {} for monster: {}",
            immunity, monsterType->name);
        lua_pushnil(L);
      }
  ...
  if (monsterType) {
    if (lua_gettop(L) == 1) {
      ...
      if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) &&
          outfit.lookType != 0 &&
          !g_game().isLookTypeRegistered(outfit.lookType)) {
        SPDLOG_WARN("[MonsterTypeFunctions::luaMonsterTypeOutfit] An "
                    "unregistered creature looktype type with id '{}' was "
                    "blocked to prevent client crash.",
                    outfit.lookType);
        lua_pushnil(L);
      } else {
...
int MonsterTypeFunctions::luaMonsterTypeStaticAttackChance(lua_State *L) {
  // get: monsterType:staticAttackChance() set:
  // monsterType:staticAttackChance(chance)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeChangeTargetSpeed(lua_State *L) {
  // get: monsterType:changeTargetSpeed() set:
  // monsterType:changeTargetSpeed(speed)
  MonsterType *monsterType = getUserdata<MonsterType>(L, 1);
  if (monsterType) {
    if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeRespawnTypeIsUnderground(lua_State *L) {
  ...
  if (monsterType) {
    if (lua_gettop(L) == 1) {
      ...
      monsterType->info.respawnType.underground =
          getNumber<RespawnPeriod_t>(L, 2);
      pushBoolean(L, true);
    }
  } else {
    lua_pushnil(L);
