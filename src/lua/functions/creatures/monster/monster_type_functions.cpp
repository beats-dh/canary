...
int MonsterTypeFunctions::luaMonsterTypeIsAttackable(lua_State* L) {
	...
	std::shared_ptr<MonsterType> monsterType = getUserdataShared<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
...
int MonsterTypeFunctions::luaMonsterTypeGetTypeName(lua_State* L) {
	...
	const std::shared_ptr<MonsterType> monsterType = getUserdataShared<MonsterType>(L, 1);
	if (!monsterType) {
		return 1;
	}
