  ...
  if (monster) {
    ...
    for (const std::string &scriptName : monster->mType->info.scripts) {
      if (!monster->unregisterCreatureEvent(scriptName)) {
        SPDLOG_WARN("[Warning - MonsterFunctions::luaMonsterSetType] Unknown "
                    "event name: {}",
                    scriptName);
      }
    }
...
int MonsterFunctions::luaMonsterIsInSpawnRange(lua_State *L) {
  ...
  Monster *monster = getUserdata<Monster>(L, 1);
  if (monster) {
    pushBoolean(L, monster->isInSpawnRange(lua_gettop(L) >= 2
                                               ? getPosition(L, 2)
                                               : monster->getPosition()));
  } else {
    lua_pushnil(L);
...
int MonsterFunctions::luaMonsterSearchTarget(lua_State *L) {
  ...
  if (monster) {
    TargetSearchType_t searchType =
        getNumber<TargetSearchType_t>(L, 2, TARGETSEARCH_DEFAULT);
...
int MonsterFunctions::luaMonsterSetSpawnPosition(lua_State *L) {
  ...
  SpawnMonster &spawnMonster =
      g_game().map.spawnsMonster.getspawnMonsterList().front();
