...
int PlayerFunctions::luaPlayerUpdateKillTracker(lua_State *L) {
  ...
  player->updateKillTracker(corpse, monster->getName(),
                            monster->getCurrentOutfit());
  pushBoolean(L, true);
...
int PlayerFunctions::luaPlayerUnlockAllCharmRunes(lua_State *L) {
  ...
  if (player) {
    for (int8_t i = CHARM_WOUND; i <= CHARM_LAST; i++) {
      Charm *charm =
          g_iobestiary().getBestiaryCharm(static_cast<charmRune_t>(i));
      if (charm) {
        int32_t value = g_iobestiary().bitToggle(player->getUnlockedRunesBit(),
                                                 charm, true);
...
int PlayerFunctions::luaPlayeraddBestiaryKill(lua_State *L) {
  ...
  if (player) {
    ...
    if (mtype) {
      g_iobestiary().addBestiaryKill(player, mtype,
                                     getNumber<uint32_t>(L, 3, 1));
      pushBoolean(L, true);
    } else {
      lua_pushnil(L);
    }
...
int PlayerFunctions::luaPlayerGetPreyExperiencePercentage(lua_State *L) {
  ...
  if (const Player *player = getUserdata<Player>(L, 1)) {
    if (const PreySlot *slot =
            player->getPreyWithMonster(getNumber<uint16_t>(L, 2, 0));
        slot && slot->isOccupied() && slot->bonus == PreyBonus_Experience &&
...
int PlayerFunctions::luaPlayerSetKills(lua_State *L) {
  ...
  while (lua_next(L, 2) != 0) {
    ...
    newKills.emplace_back(luaL_checknumber(L, -3), luaL_checknumber(L, -2),
                          getBoolean(L, -1));
    lua_pop(L, 4);
  }
  ...
  if (player) {
    ...
    if (getNumber<uint64_t>(L, 4, 0) > 0) {
      ...
      uint64_t nextReqTries =
          player->vocation->getReqSkillTries(skillType, level + 1);
      player->skills[skillType].tries = tries;
      player->skills[skillType].percent =
          Player::getPercentLevel(tries, nextReqTries);
    } else {
...
int PlayerFunctions::luaPlayerSetVocation(lua_State *L) {
  ...
  if (isNumber(L, 2)) {
    ...
    vocation =
        g_vocations().getVocation(g_vocations().getVocationId(getString(L, 2)));
  } else if (isUserdata(L, 2)) {
...
int PlayerFunctions::luaPlayerAddItem(lua_State *L) {
  // player:addItem(itemId[, count = 1[, canDropOnMap = true[, subType = 1[,
  ...
  for (int32_t i = 1; i <= itemCount; ++i) {
    ...
    ReturnValue ret =
        g_game().internalPlayerAddItem(player, item, canDropOnMap, slot);
    if (ret != RETURNVALUE_NOERROR) {
      delete item;
      if (!hasTable) {
        lua_pushnil(L);
...
int PlayerFunctions::luaPlayerAddItemEx(lua_State *L) {
  // player:addItemEx(item[, canDropOnMap = false[, index = INDEX_WHEREEVER[,
  // flags = 0]]]) player:addItemEx(item[, canDropOnMap = true[, slot =
...
int PlayerFunctions::luaPlayerRemoveItem(lua_State *L) {
  ...
  pushBoolean(L,
              player->removeItemOfType(itemId, count, subType, ignoreEquipped));
  return 1;
}
...
int PlayerFunctions::luaPlayerSendContainer(lua_State *L) {
  ...
  player->sendContainer(static_cast<uint8_t>(container->getID()), container,
                        container->hasParent(),
                        static_cast<uint8_t>(container->getFirstIndex()));
  pushBoolean(L, true);
  return 1;
}
...
int PlayerFunctions::luaPlayerSendTextMessage(lua_State *L) {
  // player:sendTextMessage(type, text[, position, primaryValue = 0,
  // primaryColor = TEXTCOLOR_NONE[, secondaryValue = 0, secondaryColor =
  // TEXTCOLOR_NONE]]) player:sendTextMessage(type, text, channelId)
...
int PlayerFunctions::luaPlayerSetGhostMode(lua_State *L) {
  ...
  for (Creature *spectator : spectators) {
    ...
    if (tmpPlayer != player && !tmpPlayer->isAccessPlayer()) {
      if (enabled) {
        tmpPlayer->sendRemoveTileThing(
            position, tile->getStackposOfCreature(tmpPlayer, player));
      } else {
