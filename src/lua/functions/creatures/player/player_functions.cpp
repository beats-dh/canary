...
int PlayerFunctions::luaPlayerUnlockAllCharmRunes(lua_State* L) {
	...
	if (player) {
		for (int8_t i = CHARM_WOUND; i <= CHARM_LAST; i++) {
			const std::shared_ptr<Charm> &charm = g_iobestiary().getBestiaryCharm(static_cast<charmRune_t>(i));
			if (charm) {
...
int PlayerFunctions::luaPlayerAddBestiaryKill(lua_State* L) {
	...
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		std::shared_ptr<MonsterType> mtype = g_monsters().getMonsterType(getString(L, 2));
		if (mtype) {
...
int PlayerFunctions::luaPlayergetCharmMonsterType(lua_State* L) {
	...
	if (player) {
		...
		if (raceid > 0) {
			std::shared_ptr<MonsterType> mtype = g_monsters().getMonsterTypeByRaceId(raceid);
			if (mtype) {
...
int PlayerFunctions::luaPlayerAddBosstiaryKill(lua_State* L) {
	...
	if (Player* player = getUserdata<Player>(L, 1);
		player) {
		const std::shared_ptr<MonsterType> mtype = g_monsters().getMonsterType(getString(L, 2));
		if (mtype) {
