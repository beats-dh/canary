...
int GuildFunctions::luaGuildGetId(lua_State* L) {
	const std::shared_ptr<Guild> &guild = getUserdataShared<Guild>(L, 1);
	if (guild) {
...
int GuildFunctions::luaGuildSetBankBalance(lua_State* L) {
	...
	std::shared_ptr<Guild> guild = getUserdataShared<Guild>(L, 1);
	if (!guild) {
		lua_pushnil(L);
		return 1;
	}
...
int GuildFunctions::luaGuildSetMotd(lua_State* L) {
	...
	guild->setMotd(motd);
	pushBoolean(L, true); // exemplo de retorno de sucesso
	return 1;
}
