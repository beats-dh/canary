...
int ActionFunctions::luaActionItemId(lua_State *L) {
  ...
  if (action) {
    int parameters =
        lua_gettop(L) -
        1; // - 1 because self is a parameter aswell, which we want to skip ofc
    if (parameters > 1) {
...
int ActionFunctions::luaActionPosition(lua_State *L) {
   ...
   * @param itemId or @param itemName = if item id or string name is set, the
   * item is created on position (if not exists), this variable is nil by
   * default action:position(positions, itemId or name)
   */
  Action *action = getUserdata<Action>(L, 1);
  if (!action) {
    reportErrorFunc(getErrorDesc(LUA_ERROR_ACTION_NOT_FOUND));
  ...
  // The parameter "- 1" because self is a parameter aswell, which we want to
  // skip L 1 (UserData) isNumber(L, 2) is for skip the itemId
  if (int parameters = lua_gettop(L) - 1; parameters > 1 && isNumber(L, 2)) {
    for (int i = 0; i < parameters; ++i) {
