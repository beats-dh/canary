...
int PositionFunctions::luaPositionGetDistance(lua_State* L) {
	...
	const Position& position = getPosition(L, 1);
	lua_pushnumber(
		L, std::max<int32_t>(std::max<int32_t>(std::abs(Position::getDistanceX(position, positionEx)), std::abs(Position::getDistanceY(position, positionEx))), std::abs(Position::getDistanceZ(position, positionEx))));
	return 1;
}

int PositionFunctions::luaPositionGetPathTo(lua_State* L) {
	// position:getPathTo(pos[, minTargetDist = 0[, maxTargetDist = 1[,
	...
	if (g_game().map.getPathMatching(pos, dirList,
			FrozenPathingConditionCall(position), fpp)) {
		lua_newtable(L);
...
int PositionFunctions::luaPositionSendMagicEffect(lua_State* L) {
	...
	if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && !g_game().isMagicEffectRegistered(magicEffect)) {
		SPDLOG_WARN(
			"[PositionFunctions::luaPositionSendMagicEffect] An unregistered magic "
			"effect type with id '{}' was blocked to prevent client crash.",
			magicEffect);
		pushBoolean(L, false);
		return 1;
	}
...
int PositionFunctions::luaPositionSendDistanceEffect(lua_State* L) {
	...
	if (g_configManager().getBoolean(WARN_UNSAFE_SCRIPTS) && !g_game().isDistanceEffectRegistered(distanceEffect)) {
		SPDLOG_WARN("[PositionFunctions::luaPositionSendDistanceEffect] An "
					"unregistered distance effect type with id '{}' was blocked to "
