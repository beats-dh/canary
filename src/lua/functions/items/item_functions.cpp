...
int ItemFunctions::luaItemSplit(lua_State *L) {
  ...
  uint16_t count =
      std::min<uint16_t>(getNumber<uint16_t>(L, 2, 1), item->getItemCount());
...
int ItemFunctions::luaItemRemove(lua_State *L) {
  ...
  if (item) {
    ...
    pushBoolean(L, g_game().internalRemoveItem(item, count) ==
                       RETURNVALUE_NOERROR);
  } else {
    lua_pushnil(L);
  }
...
int ItemFunctions::luaItemMoveTo(lua_State *L) {
  ...
  uint32_t flags = getNumber<uint32_t>(L, 3,
                                       FLAG_NOLIMIT | FLAG_IGNOREBLOCKITEM |
                                           FLAG_IGNOREBLOCKCREATURE |
                                           FLAG_IGNORENOTMOVEABLE);

  if (item->getParent() == VirtualCylinder::virtualCylinder) {
    pushBoolean(L, g_game().internalAddItem(toCylinder, item, INDEX_WHEREEVER,
...
int ItemFunctions::luaItemMoveToSlot(lua_State *L) {
  ...
  Item *moveItem = nullptr;
  ReturnValue ret = g_game().internalMoveItem(
      item->getParent(), player, slot, item, item->getItemCount(), nullptr);
  if (moveItem) {
    item = moveItem;
  }
...
int ItemFunctions::luaItemSetDuration(lua_State *L) {
  // item:setDuration(minDuration, maxDuration = 0, decayTo = 0, showDuration =
  // true) Example: item:setDuration(10000, 20000, 2129, false) = random
  // duration from range 10000/20000
  Item *item = getUserdata<Item>(L, 1);
  if (!item) {
    reportErrorFunc(getErrorDesc(LUA_ERROR_ITEM_NOT_FOUND));
    pushBoolean(L, false);
