...
int ItemClassificationFunctions::luaItemClassificationCreate(lua_State *L) {
  ...
  if (isNumber(L, 2)) {
    ItemClassification *itemClassification =
        g_game().getItemsClassification(getNumber<uint8_t>(L, 2), false);
...
int ItemClassificationFunctions::luaItemClassificationAddTier(lua_State *L) {
  ...
  ItemClassification *itemClassification =
      getUserdata<ItemClassification>(L, 1);
  if (itemClassification) {
    itemClassification->addTier(getNumber<uint8_t>(L, 2),
                                getNumber<uint64_t>(L, 3));
    pushBoolean(L, true);
  } else {
    lua_pushnil(L);
  }
