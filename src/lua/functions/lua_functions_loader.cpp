...
void LuaFunctionsLoader::reportError(const char* function,
	...
	getScriptEnv()->getEventInfo(scriptId, scriptInterface, callbackId,
		timerEvent);
	...
	SPDLOG_ERROR(
		...
		(stack_trace && scriptInterface)
			? scriptInterface->getStackTrace(error_desc)
			: error_desc);
}
...
// Metatables
void LuaFunctionsLoader::setMetatable(lua_State* L, int32_t index,
	const std::string& name) {
	luaL_getmetatable(L, name.c_str());
	lua_setmetatable(L, index - 1);
}

void LuaFunctionsLoader::setWeakMetatable(lua_State* L, int32_t index,
	const std::string& name) {
	...
	if (result.second) {
		...
		static const std::vector<std::string> methodKeys = { "__index",
			"__metatable", "__eq" };
...
void LuaFunctionsLoader::setItemMetatable(lua_State* L, int32_t index,
	const Item* item) {
...
void LuaFunctionsLoader::setCreatureMetatable(lua_State* L, int32_t index,
...
std::string LuaFunctionsLoader::getFieldString(lua_State* L, int32_t arg,
...
void LuaFunctionsLoader::pushCombatDamage(lua_State* L,
...
void LuaFunctionsLoader::pushInstantSpell(lua_State* L,
	const InstantSpell& spell) {
...
void LuaFunctionsLoader::pushPosition(lua_State* L, const Position& position,
	int32_t stackpos /* = 0*/) {
	lua_createtable(L, 0, 4);
...
void LuaFunctionsLoader::registerClass(
	lua_State* L, const std::string& className, const std::string& baseClass,
...
void LuaFunctionsLoader::registerMethod(lua_State* L,
...
void LuaFunctionsLoader::registerTable(lua_State* L,
...
void LuaFunctionsLoader::registerMetaMethod(lua_State* L,
...
void LuaFunctionsLoader::registerVariable(lua_State* L,
	const std::string& tableName,
	const std::string& name,
	lua_Number value) {
...
void LuaFunctionsLoader::registerGlobalBoolean(lua_State* L,
	const std::string& name,
	bool value) {
	// _G[name] = value
	pushBoolean(L, value);
...
void LuaFunctionsLoader::registerGlobalMethod(lua_State* L,
...
void LuaFunctionsLoader::registerGlobalVariable(lua_State* L,
	const std::string& name,
	lua_Number value) {
	// _G[name] = value
