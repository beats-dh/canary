...
bool Actions::registerEvent(Event_ptr event, const pugi::xml_node &node) {
  ...
  if ((attr = node.attribute("itemid"))) {
    ...
    if (!result.second) {
      SPDLOG_WARN(
          "[Actions::registerEvent] - Duplicate "
...
bool Actions::registerLuaItemEvent(Action *action) {
  ...
  std::for_each(
      itemIdVector.begin(), itemIdVector.end(),
      [this, &action, &itemIdVector](uint16_t &itemId) {
        // Check if the item is already registered and prevent it from being
        // registered again
        if (hasItemId(itemId)) {
          SPDLOG_WARN(
              "[Actions::registerLuaItemEvent] - Duplicate "
              "registered item with id: {} in range from id: {}, to id: {}",
              itemId, itemIdVector.at(0),
...
bool Actions::registerLuaUniqueEvent(Action *action) {
  ...
  std::for_each(
      uniqueIdVector.begin(), uniqueIdVector.end(),
      [this, &action, &uniqueIdVector](uint16_t &uniqueId) {
        // Check if the unique is already registered and prevent it from being
        // registered again
        if (hasUniqueId(uniqueId)) {
          SPDLOG_WARN(
              "[Actions::registerLuaUniqueEvent] - Duplicate "
              "registered item with uid: {} in range from uid: {}, to uid: {}",
              uniqueId, uniqueIdVector.at(0),
...
bool Actions::registerLuaActionEvent(Action *action) {
  ...
  std::for_each(
      actionIdVector.begin(), actionIdVector.end(),
      [this, &action, &actionIdVector](uint16_t &actionId) {
        // Check if the unique is already registered and prevent it from being
        // registered again
        if (hasActionId(actionId)) {
          SPDLOG_WARN(
              "[Actions::registerLuaActionEvent] - Duplicate "
              "registered item with aid: {} in range from aid: {}, to aid: {}",
              actionId, actionIdVector.at(0),
...
bool Actions::registerLuaPositionEvent(Action *action) {
  ...
  for (Position position : positionVector) {
    // Check if the position is already registered and prevent it from being
    // registered again
...
bool Actions::registerLuaEvent(Action *event) {
  ...
  if (registerLuaItemEvent(event) || registerLuaUniqueEvent(event) ||
      registerLuaActionEvent(event) || registerLuaPositionEvent(event)) {
    return true;
  } else {
  ...
  SPDLOG_DEBUG(
      "[Actions::registerLuaEvent] - Missing or incorrect script event");
  return false;
}
...
ReturnValue Actions::canUse(const Player *player, const Position &pos) {
  if (pos.x != 0xFFFF) {
    ...
    if (playerPos.z != pos.z) {
      return playerPos.z > pos.z ? RETURNVALUE_FIRSTGOUPSTAIRS
...
ReturnValue Actions::canUseFar(const Creature *creature, const Position &toPos,
  ...
  if (checkFloor && creaturePos.z != toPos.z) {
    return creaturePos.z > toPos.z ? RETURNVALUE_FIRSTGOUPSTAIRS
                                   : RETURNVALUE_FIRSTGODOWNSTAIRS;
  }
...
Action *Actions::getAction(const Item *item) {
  ...
  if (auto iteratePositions = actionPositionMap.find(item->getPosition());
    ...
    if (const Tile *tile = item->getTile(); tile) {
      if (const Player *player = item->getHoldingPlayer();
          player && item->getTopParent() == player) {
        SPDLOG_DEBUG("[Actions::getAction] - The position only is valid for "
                     "use item in the map, player name {}",
                     player->getName());
        return nullptr;
      }
  ...
  if (Container *container = item->getContainer()) {
    ...
    if (container->isRewardCorpse()) {
      ...
      if (player->getGroup()->id >= account::GROUP_TYPE_GAMEMASTER ||
          player->getAccountType() >= account::ACCOUNT_TYPE_SENIORTUTOR) {
        return RETURNVALUE_YOUCANTOPENCORPSEADM;
      }
      if (!player->getReward(container->getIntAttr(ITEM_ATTRIBUTE_DATE),
                             false)) {
...
bool Actions::useItem(Player *player, const Position &pos, uint8_t index,
  ...
  if (it.isRune() || it.type == ITEM_TYPE_POTION) {
    ...
    player->setNextPotionAction(
        OTSYS_TIME() + g_configManager().getNumber(ACTIONS_DELAY_INTERVAL));
  } else {
    player->setNextAction(OTSYS_TIME() +
                          g_configManager().getNumber(ACTIONS_DELAY_INTERVAL));
  }
  ...
  if (isHotkey) {
    ...
    showUseHotkeyMessage(
        player, item,
        player->getItemTypeCount(
            item->getID(), subType != item->getItemCount() ? subType : -1));
  }
...
bool Actions::useItemEx(Player *player, const Position &fromPos,
  ...
  if (it.isRune() || it.type == ITEM_TYPE_POTION) {
    ...
    player->setNextPotionAction(
        OTSYS_TIME() + g_configManager().getNumber(EX_ACTIONS_DELAY_INTERVAL));
  } else {
    player->setNextAction(
        OTSYS_TIME() + g_configManager().getNumber(EX_ACTIONS_DELAY_INTERVAL));
  }
  ...
  if (action->function) {
    if (action->function(player, item, fromPos,
                         action->getTarget(player, creature, toPos, toStackPos),
                         toPos, isHotkey)) {
      return true;
    }
    return false;
  ...
  if (it.isMultiUse()) {
    player->sendUseItemCooldown(
        g_configManager().getNumber(EX_ACTIONS_DELAY_INTERVAL));
  }
  return true;
}
