...
bool Raids::loadFromXml() {
  ...
  for (auto raidNode : doc.child("raids").children()) {
    ...
    interval =
        pugi::cast<uint32_t>(raidNode.attribute("interval2").value()) * 60;
    if (interval == 0) {
...
bool Raid::loadFromXml(const std::string &filename) {
  ...
  std::sort(raidEvents.begin(), raidEvents.end(),
            [](const RaidEvent *lhs, const RaidEvent *rhs) {
...
void Raid::executeRaidEvent(RaidEvent *raidEvent) {
  if (raidEvent->executeEvent()) {
    ...
    if (newRaidEvent) {
      uint32_t ticks = static_cast<uint32_t>(std::max<int32_t>(
          RAID_MINTICKS, newRaidEvent->getDelay() - raidEvent->getDelay()));
      nextEventEvent = g_scheduler().addEvent(createSchedulerTask(
          ticks, std::bind(&Raid::executeRaidEvent, this, newRaidEvent)));
    } else {
      resetRaid();
    }
  } else {
...
bool RaidEvent::configureRaidEvent(const pugi::xml_node &eventNode) {
  ...
  delay = std::max<uint32_t>(RAID_MINTICKS,
                             pugi::cast<uint32_t>(delayAttribute.value()));
  return true;
}
    ...
    for (uint32_t i = 0; i < amount; ++i) {
      ...
      for (int32_t tries = 0; tries < MAXIMUM_TRIES_PER_MONSTER; tries++) {
        const Tile *tile = g_game().map.getTile(
            static_cast<uint16_t>(uniform_random(fromPos.x, toPos.x)),
            static_cast<uint16_t>(uniform_random(fromPos.y, toPos.y)),
            static_cast<uint8_t>(uniform_random(fromPos.z, toPos.z)));
