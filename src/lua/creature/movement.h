...
public:
	...
	uint32_t onCreatureMove(Creature& creature, Tile& tile,
	...
	void clear(bool fromLua) override { fromLua = false; }
	...
	bool hasPosition(Position position) const {
		if (auto it = positionsMap.find(position); it != positionsMap.end()) {
			return true;
		}
		return false;
	}
	...
	std::map<int32_t, MoveEventList> getItemIdMap() const { return itemIdMap; }

	bool hasItemId(int32_t itemId) const {
		if (auto it = itemIdMap.find(itemId); it != itemIdMap.end()) {
			return true;
		}
		return false;
	}
	...
	bool hasUniqueId(int32_t uniqueId) const {
		if (auto it = uniqueIdMap.find(uniqueId); it != uniqueIdMap.end()) {
			return true;
		}
		return false;
	}
	...
	bool hasActionId(int32_t actionId) const {
		if (auto it = actionIdMap.find(actionId); it != actionIdMap.end()) {
			return true;
		}
		return false;
	}
...
private:
	...
	std::string getScriptBaseName() const override { return ""; }
	Event_ptr getEvent(const std::string& nodeName) override;
	bool registerEvent([[maybe_unused]] Event_ptr event,
		[[maybe_unused]] const pugi::xml_node& node) override {
		return false;
	}

	void registerEvent(MoveEvent& moveEvent, int32_t id,
		std::map<int32_t, MoveEventList>& moveListMap) const;
	void registerEvent(MoveEvent& moveEvent, const Position& position,
...
public:
	...
	const std::map<uint16_t, bool>& getVocEquipMap() const { return vocEquipMap; }
	...
	std::vector<uint32_t> getItemIdsVector() const { return itemIdVector; }
	void setItemId(uint32_t id) { itemIdVector.emplace_back(id); }
	std::vector<uint32_t> getActionIdsVector() const { return actionIdVector; }
	void setActionId(uint32_t id) { actionIdVector.emplace_back(id); }
	std::vector<uint32_t> getUniqueIdsVector() const { return uniqueIdVector; }
	void setUniqueId(uint32_t id) { uniqueIdVector.emplace_back(id); }
	std::vector<Position> getPositionsVector() const { return positionVector; }
	void setPosition(Position pos) { positionVector.emplace_back(pos); }
	...
	static uint32_t StepInField(Creature* creature, Item* item,
		const Position& pos);
	static uint32_t StepOutField(Creature* creature, Item* item,
		const Position& pos);

	static uint32_t AddItemField(Item* item, Item* tileItem, const Position& pos);
	static uint32_t RemoveItemField(Item* item, Item* tileItem,
		const Position& pos);

	static uint32_t EquipItem(MoveEvent* moveEvent, Player* player, Item* item,
		Slots_t slot, bool boolean);
	static uint32_t DeEquipItem(MoveEvent* moveEvent, Player* player, Item* item,
		Slots_t slot, bool boolean);

	MoveEvent_t eventType = MOVE_EVENT_NONE;
	// Step function structure
	std::function<uint32_t(Creature* creature, Item* item, const Position& pos)>
		stepFunction;
	// Move function structure
	std::function<uint32_t(Item* item, Item* tileItem, const Position& pos)>
		moveFunction;
	// Equip function structure
	std::function<uint32_t(MoveEvent* moveEvent, Player* player, Item* item,
		Slots_t slot, bool boolean)>
		equipFunction;

private:
	std::string getScriptEventName() const override { return ""; }
