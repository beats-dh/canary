...
bool TalkActions::registerEvent(Event_ptr event, const pugi::xml_node &) {
  TalkAction_ptr talkAction{static_cast<TalkAction *>(
      event.release())}; // event is guaranteed to be a TalkAction
...
TalkActionResult_t TalkActions::playerSaySpell(Player *player,
                                               SpeakClasses type,
  ...
  for (auto it = talkActions.begin(); it != talkActions.end();) {
    ...
    if (wordsLength < talkactionLength ||
        strncasecmp(words.c_str(), talkactionWords.c_str(), talkactionLength) !=
            0) {
      ++it;
      continue;
    }
...
bool TalkAction::executeSay(Player *player, const std::string &words,
                            const std::string &param, SpeakClasses type) const {
