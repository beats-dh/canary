...
bool MoveEvents::registerLuaItemEvent(MoveEvent &moveEvent) {
  ...
  std::for_each(itemIdVector.begin(), itemIdVector.end(),
                [this, &moveEvent](const uint32_t &itemId) {
...
bool MoveEvents::registerLuaActionEvent(MoveEvent &moveEvent) {
  ...
  std::for_each(actionIdVector.begin(), actionIdVector.end(),
                [this, &moveEvent](const uint32_t &actionId) {
...
bool MoveEvents::registerLuaUniqueEvent(MoveEvent &moveEvent) {
  ...
  std::for_each(uniqueIdVector.begin(), uniqueIdVector.end(),
                [this, &moveEvent](const uint32_t &uniqueId) {
...
bool MoveEvents::registerLuaPositionEvent(MoveEvent &moveEvent) {
  ...
  std::for_each(positionVector.begin(), positionVector.end(),
                [this, &moveEvent](const Position &position) {
...
bool MoveEvents::registerLuaEvent(MoveEvent &moveEvent) {
  ...
  if (registerLuaItemEvent(moveEvent) || registerLuaUniqueEvent(moveEvent) ||
  ...
  SPDLOG_DEBUG("[MoveEvents::registerLuaEvent] - Missing or incorrect event "
               "for some script");
  return false;
}

void MoveEvents::registerEvent(
    MoveEvent &moveEvent, int32_t id,
    std::map<int32_t, MoveEventList> &moveListMap) const {
  auto it = moveListMap.find(id);
  if (it == moveListMap.end()) {
    MoveEventList moveEventList;
    moveEventList.moveEvent[moveEvent.getEventType()].push_back(
        std::move(moveEvent));
    moveListMap[id] = moveEventList;
  } else {
    std::list<MoveEvent> &moveEventList =
        it->second.moveEvent[moveEvent.getEventType()];
...
MoveEvent *MoveEvents::getEvent(Item &item, MoveEvent_t eventType,
  ...
  if (item.hasAttribute(ITEM_ATTRIBUTE_ACTIONID)) {
    std::map<int32_t, MoveEventList>::iterator it =
        actionIdMap.find(item.getActionId());
...
void MoveEvents::registerEvent(
    ...
    std::map<Position, MoveEventList> &moveListMap) const {
...
uint32_t MoveEvents::onCreatureMove(Creature &creature, Tile &tile,
...
uint32_t MoveEvent::EquipItem(MoveEvent *moveEvent, Player *player, Item *item,
  ...
  if (!player->hasFlag(PlayerFlag_IgnoreWeaponCheck) &&
    ...
    if (!vocEquipMap.empty() &&
        vocEquipMap.find(player->getVocationId()) == vocEquipMap.end()) {
      return 0;
    }
  }
  ...
  if (it.abilities->invisible) {
    Condition *condition = Condition::createCondition(
        static_cast<ConditionId_t>(slot), CONDITION_INVISIBLE, -1, 0);
    player->addCondition(condition);
  }

  if (it.abilities->manaShield) {
    Condition *condition = Condition::createCondition(
        static_cast<ConditionId_t>(slot), CONDITION_MANASHIELD, -1, 0);
    player->addCondition(condition);
  }
  ...
  for (int32_t s = STAT_FIRST; s <= STAT_LAST; ++s) {
    ...
    if (it.abilities->statsPercent[s]) {
      ...
      player->setVarStats(static_cast<stats_t>(s),
                          static_cast<int32_t>(
                              player->getDefaultStats(static_cast<stats_t>(s)) *
...
uint32_t MoveEvent::DeEquipItem(MoveEvent *, Player *player, Item *item,
                                Slots_t slot, bool) {
  if (player == nullptr) {
    SPDLOG_ERROR("[MoveEvent::EquipItem] - Player is nullptr");
    return 0;
  ...
  if (it.abilities->invisible) {
    player->removeCondition(CONDITION_INVISIBLE,
                            static_cast<ConditionId_t>(slot));
  }
...
bool MoveEvent::executeStep(Creature &creature, Item *item,
                            const Position &pos) {
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    if (item != nullptr) {
      SPDLOG_ERROR(
          "[MoveEvent::executeStep - Creature {} item {}, position {}] "
          "Call stack overflow. Too many lua script calls being nested.",
          creature.getName(), item->getName(), pos.toString());
    } else {
      SPDLOG_ERROR(
          "[MoveEvent::executeStep - Creature {}, position {}] "
          "Call stack overflow. Too many lua script calls being nested.",
...
uint32_t MoveEvent::fireEquip(Player &player, Item &item, Slots_t toSlot,
                              bool isCheck) {
  if (isScripted()) {
    if (!equipFunction ||
        equipFunction(this, &player, &item, toSlot, isCheck) == 1) {
...
bool MoveEvent::executeEquip(Player &player, Item &item, Slots_t onSlot,
                             bool isCheck) {
...
uint32_t MoveEvent::fireAddRemItem(Item &item, Item &fromTile,
                                   const Position &pos) {
  if (isScripted()) {
...
bool MoveEvent::executeAddRemItem(Item &item, Item &fromTile,
                                  const Position &pos) {
