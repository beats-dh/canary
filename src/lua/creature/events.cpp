...
bool Events::loadFromXml() {
  ...
  for (auto eventNode : doc.child("events").children()) {
    ...
    if (res.second) {
      ...
      if (scriptInterface.loadFile("data/events/scripts/" + lowercase +
                                   ".lua") != 0) {
...
// Creature
bool Events::eventCreatureOnChangeOutfit(Creature *creature,
...
ReturnValue Events::eventCreatureOnAreaCombat(Creature *creature, Tile *tile,
                                              bool aggressive) {
  // Creature:onAreaCombat(tile, aggressive) or Creature.onAreaCombat(self,
  // tile, aggressive)
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    SPDLOG_ERROR("[Events::eventCreatureOnAreaCombat - "
                 ...
                 "Call stack overflow. Too many lua script calls being nested.",
                 creature->getName(), (tile->getPosition()).getX(),
...
ReturnValue Events::eventCreatureOnTargetCombat(Creature *creature,
                                                Creature *target) {
...
void Events::eventCreatureOnHear(Creature *creature, Creature *speaker,
...
void Events::eventCreatureOnDrainHealth(Creature *creature, Creature *attacker,
                                        CombatType_t &typePrimary,
                                        int32_t &damagePrimary,
                                        CombatType_t &typeSecondary,
                                        int32_t &damageSecondary,
                                        TextColor_t &colorPrimary,
                                        TextColor_t &colorSecondary) {
...
void Events::eventPartyOnShareExperience(Party *party, uint64_t &exp) {
  ...
  if (!scriptInterface.reserveScriptEnv()) {
    SPDLOG_ERROR("Party leader {}. Call stack overflow. Too many lua script "
                 "calls being nested.",
                 party->getLeader()->getName());
    return;
  }
...
// Player
bool Events::eventPlayerOnBrowseField(Player *player,
...
void Events::eventPlayerOnLook(Player *player, const Position &position,
                               Thing *thing, uint8_t stackpos,
                               int32_t lookDistance) {
  // Player:onLook(thing, position, distance) or Player.onLook(self, thing,
  // position, distance)
...
void Events::eventPlayerOnLookInBattleList(Player *player, Creature *creature,
                                           int32_t lookDistance) {
  // Player:onLookInBattleList(creature, position, distance) or
  // Player.onLookInBattleList(self, creature, position, distance)
...
void Events::eventPlayerOnLookInTrade(Player *player, Player *partner,
                                      Item *item, int32_t lookDistance) {
  // Player:onLookInTrade(partner, item, distance) or Player.onLookInTrade(self,
...
bool Events::eventPlayerOnLookInShop(Player *player, const ItemType *itemType,
                                     uint8_t count) {
  // Player:onLookInShop(itemType, count) or Player.onLookInShop(self, itemType,
  // count)
...
bool Events::eventPlayerOnMoveItem(Player *player, Item *item, uint16_t count,
                                   ...
                                   Cylinder *toCylinder) {
  // Player:onMoveItem(item, count, fromPosition, toPosition) or
  // Player.onMoveItem(self, item, count, fromPosition, toPosition,
...
void Events::eventPlayerOnItemMoved(Player *player, Item *item, uint16_t count,
                                    ...
                                    Cylinder *fromCylinder,
                                    Cylinder *toCylinder) {
  // Player:onItemMoved(item, count, fromPosition, toPosition) or
  // Player.onItemMoved(self, item, count, fromPosition, toPosition,
  // fromCylinder, toCylinder)
...
bool Events::eventPlayerOnMoveCreature(Player *player, Creature *creature,
                                       const Position &fromPosition,
                                       const Position &toPosition) {
  // Player:onMoveCreature(creature, fromPosition, toPosition) or
  // Player.onMoveCreature(self, creature, fromPosition, toPosition)
...
void Events::eventPlayerOnReportRuleViolation(Player *player,
                                              const std::string &targetName,
                                              uint8_t reportType,
                                              uint8_t reportReason,
                                              const std::string &comment,
                                              const std::string &translation) {
  // Player:onReportRuleViolation(targetName, reportType, reportReason, comment,
  // translation)
...
bool Events::eventPlayerOnTradeRequest(Player *player, Player *target,
                                       Item *item) {
...
bool Events::eventPlayerOnTradeAccept(Player *player, Player *target,
...
void Events::eventPlayerOnGainExperience(Player *player, Creature *target,
                                         uint64_t &exp, uint64_t rawExp) {
...
void Events::eventPlayerOnGainSkillTries(Player *player, skills_t skill,
                                         uint64_t &tries) {
...
void Events::eventPlayerOnCombat(Player *player, Creature *target, Item *item,
                                 CombatDamage &damage) {
  // Player:onCombat(target, item, primaryDamage, primaryType, secondaryDamage,
  // secondaryType)
  ...
  if (scriptInterface.protectedCall(L, 8, 4) != 0) {
    ...
    damage.primary.value =
        std::abs(LuaScriptInterface::getNumber<int32_t>(L, -4));
    damage.primary.type = LuaScriptInterface::getNumber<CombatType_t>(L, -3);
    damage.secondary.value =
        std::abs(LuaScriptInterface::getNumber<int32_t>(L, -2));
    ...
    /*
            Only EK with dealing physical damage will get elemental damage on
       skill
    */
...
void Events::eventOnStorageUpdate(Player *player, const uint32_t key,
                                  const int32_t value, int32_t oldValue,
