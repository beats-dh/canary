...
bool CreatureEvents::registerEvent(Event_ptr event, const pugi::xml_node &) {
  CreatureEvent_ptr creatureEvent{static_cast<CreatureEvent *>(
      event.release())}; // event is guaranteed to be a CreatureEvent
  if (creatureEvent->getEventType() == CREATURE_EVENT_NONE) {
    SPDLOG_ERROR("[CreatureEvents::registerEvent] - Trying to register event "
                 "without type");
    return false;
  }
...
bool CreatureEvents::registerLuaEvent(CreatureEvent *event) {
  ...
  if (creatureEvent->getEventType() == CREATURE_EVENT_NONE) {
    SPDLOG_ERROR("[CreatureEvents::registerLuaEvent] - Trying to register "
                 "event without type");
    return false;
  }
...
bool CreatureEvents::playerAdvance(Player *player, skills_t skill,
...
bool CreatureEvent::configureEvent(const pugi::xml_node &node) {
  ...
  if (!nameAttribute) {
    SPDLOG_ERROR(
        "[CreatureEvent::configureEvent] - Missing name for creature event");
    return false;
  }
  ...
  if (!typeAttribute) {
    SPDLOG_ERROR(
        "[CreatureEvent::configureEvent] - Missing type for creature event: {}",
        eventName);
    return false;
  }
  ...
  if (tmpStr == "login") {
    ...
    SPDLOG_ERROR(
        "[CreatureEvent::configureEvent] - Invalid type for creature event: {}",
        eventName);
    return false;
  }
...
bool CreatureEvent::executeOnPrepareDeath(Creature *creature,
                                          Creature *killer) {
  // onPrepareDeath(creature, killer)
  if (!scriptInterface->reserveScriptEnv()) {
    SPDLOG_ERROR("[CreatureEvent::executeOnPrepareDeath - Creature {} killer {}"
                 " event {}] Call stack overflow. Too many lua script calls "
                 "being nested.",
...
bool CreatureEvent::executeOnDeath(Creature *creature, Item *corpse,
                                   Creature *killer, Creature *mostDamageKiller,
                                   bool lastHitUnjustified,
                                   bool mostDamageUnjustified) {
  // onDeath(creature, corpse, lasthitkiller, mostdamagekiller,
  // lasthitunjustified, mostdamageunjustified)
  if (!scriptInterface->reserveScriptEnv()) {
    SPDLOG_ERROR(
        "[CreatureEvent::executeOnDeath - Creature {} killer {} event {}] "
        "Call stack overflow. Too many lua script calls being nested.",
        creature->getName(), killer->getName(), getName());
    return false;
  }
...
bool CreatureEvent::executeAdvance(Player *player, skills_t skill,
                                   uint32_t oldLevel, uint32_t newLevel) {
...
void CreatureEvent::executeOnKill(Creature *creature, Creature *target,
  ...
  if (!scriptInterface->reserveScriptEnv()) {
    SPDLOG_ERROR(
        "[CreatureEvent::executeOnKill - Creature {} target {} event {}] "
        "Call stack overflow. Too many lua script calls being nested.",
...
void CreatureEvent::executeModalWindow(Player *player, uint32_t modalWindowId,
...
bool CreatureEvent::executeTextEdit(Player *player, Item *item,
...
void CreatureEvent::executeHealthChange(Creature *creature, Creature *attacker,
                                        CombatDamage &damage) {
  // onHealthChange(creature, attacker, primaryDamage, primaryType,
...
void CreatureEvent::executeManaChange(Creature *creature, Creature *attacker,
                                      CombatDamage &damage) {
  // onManaChange(creature, attacker, primaryDamage, primaryType,
  // secondaryDamage, secondaryType, origin)
  if (!scriptInterface->reserveScriptEnv()) {
