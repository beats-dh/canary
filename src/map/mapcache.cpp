...
std::shared_ptr<Tile> MapCache::getOrCreateTileFromCache(const std::shared_ptr<Floor> &floor, uint16_t x, uint16_t y) {
    ...
    if (cachedTile->isHouse()) {
        if (const auto &house = map->houses.getHouse(cachedTile->houseId)) {
            ...
            g_logger().error("[{}] house not found for houseId {}",
...
void MapCache::setBasicTile(uint16_t x, uint16_t y, uint8_t z, const std::shared_ptr<BasicTile> &newTile) {
    if (z >= MAP_MAX_LAYERS) {
        g_logger().error("[{}] Attempt to set tile on invalid coordinate: {}",
                         std::source_location::current().function_name(),
...
void BasicTile::hash(size_t &h) const {
	...
	if (flags > 0) stdext::hash_combine(h, flags);
	if (houseId > 0) stdext::hash_combine(h, houseId);
	if (type > 0) stdext::hash_combine(h, type);
	if (isStatic) stdext::hash_combine(h, 1); // isStatic é bool
...
void BasicItem::hash(size_t &h) const {
	// Processar valores diretamente
	if (id > 0) stdext::hash_combine(h, id);
	if (charges > 0) stdext::hash_combine(h, charges);
	if (actionId > 0) stdext::hash_combine(h, actionId);
	if (uniqueId > 0) stdext::hash_combine(h, uniqueId);
	if (destX > 0) stdext::hash_combine(h, destX);
	if (destY > 0) stdext::hash_combine(h, destY);
	if (destZ > 0) stdext::hash_combine(h, destZ);
	if (doorOrDepotId > 0) stdext::hash_combine(h, doorOrDepotId);
...
std::expected<bool, std::string> BasicItem::unserializeItemNode(FileStream &stream, uint16_t x, uint16_t y, uint8_t z) {
    ...
    while (stream.startNode()) {
        ...
        if (!result) {
            return std::unexpected(fmt::format("[x:{}, y:{}, z:{}] Failed to load item: {}",
                                            x, y, z, result.error()));
        }
