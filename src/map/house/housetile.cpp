...
HouseTile::HouseTile(int32_t initX, int32_t initY, int32_t initZ,
                     House *initHouse)
...
ReturnValue HouseTile::queryAdd(int32_t index, const Thing &thing,
                                uint32_t count, uint32_t tileFlags,
  ...
  if (const Creature *creature = thing.getCreature()) {
    if (const Player *player = creature->getPlayer()) {
      ...
      if (monster->isSummon()) {
        ...
        if (house->isInvited(monster->getMaster()->getPlayer()) &&
            (hasFlag(TILESTATE_BLOCKSOLID) ||
             (hasBitSet(FLAG_PATHFINDING, flags) &&
...
Tile *HouseTile::queryDestination(int32_t &index, const Thing &thing,
                                  Item **destItem, uint32_t &tileFlags) {
  if (const Creature *creature = thing.getCreature()) {
    if (const Player *player = creature->getPlayer()) {
      if (!house->isInvited(player)) {
...
ReturnValue HouseTile::queryRemove(const Thing &thing, uint32_t count,
                                   uint32_t flags,
                                   Creature *actor /*= nullptr*/) const {
