...
void House::setOwner(uint32_t guid, bool updateDatabase /* = true*/,
  ...
  if (updateDatabase && owner != guid) {
    ...
    query << "UPDATE `houses` SET `owner` = " << guid
          << ", `bid` = 0, `bid_end` = 0, `last_bid` = 0, `highest_bidder` = 0 "
             " WHERE `id` = "
          << id;
...
bool House::kickPlayer(Player *player, Player *target) {
  ...
  if (g_game().internalTeleport(target, getEntryPosition()) ==
      RETURNVALUE_NOERROR) {
...
bool House::transferToDepot(Player *player) const {
  ...
  for (Item *item : moveItemList) {
    g_game().internalMoveItem(item->getParent(), player->getInbox(),
...
void AccessList::addGuildRank(const std::string &name,
...
void AccessList::getList(std::string &retList) const { retList = this->list; }
...
bool Houses::loadHousesXML(const std::string &filename) {
  ...
  for (auto houseNode : doc.child("houses").children()) {
    ...
    house->setTownId(
        pugi::cast<uint32_t>(houseNode.attribute("townid").value()));
    ...
    if (player.getBankBalance() >= rent) {
      ...
      if (house->getPayRentWarnings() < 7) {
        ...
        ss << "Warning! \nThe " << period << " rent of " << house->getRent()
           << " gold for your house \"" << house->getName()
           << "\" is payable. Have it within " << daysLeft
           << " days or you will lose this house.";
        letter->setText(ss.str());
        g_game().internalAddItem(player.getInbox(), letter, INDEX_WHEREEVER,
                                 FLAG_NOLIMIT);
