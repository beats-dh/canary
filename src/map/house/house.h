...
public:
  ...
  void setName(std::string newHouseName) { this->houseName = newHouseName; }
  const std::string &getName() const { return houseName; }

  void setOwner(uint32_t guid, bool updateDatabase = true,
  ...
  void setRent(uint32_t newRent) { this->rent = newRent; }
  ...
  void setTownId(uint32_t newTownId) { this->townId = newTownId; }
  ...
  const std::list<Door *> &getDoors() const { return doorList; }
  ...
  uint32_t getBedCount() {
    return static_cast<uint32_t>(std::ceil(
        bedsList.size() /
        2.)); // each bed takes 2 sqms of space, ceil is just for bad maps
  }
