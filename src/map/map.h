...
public:
  ...
  AStarNode *createOpenNode(AStarNode *parent, uint32_t x, uint32_t y,
                            int_fast32_t f);
  ...
  static int_fast32_t getMapWalkCost(AStarNode *node,
                                     const Position &neighborPos,
                                     bool preferDiagonal = false);
  static int_fast32_t getTileWalkCost(const Creature &creature,
                                      const Tile *tile);
...
class Map {
public:
  static constexpr int32_t maxViewportX =
      11; // min value: maxClientViewportX + 1
  static constexpr int32_t maxViewportY =
      11; // min value: maxClientViewportY + 1
   ...
   */
  bool loadMap(const std::string &identifier, bool mainMap = false,
   ...
   */
  bool loadMapCustom(const std::string &identifier, bool loadHouses,
   ...
   * becase of obstacles (creatures/chests)
   */
  bool placeCreature(const Position &centerPos, Creature *creature,
                     bool extendedPos = false, bool forceLogin = false);

  void moveCreature(Creature &creature, Tile &newTile,
                    bool forceTeleport = false);

  void getSpectators(SpectatorHashSet &spectators, const Position &centerPos,
   ...
   *way \returns The result if you can throw there or not
   */
   ...
   *Destination point \param floorCheck if true then view is not clear if
   *fromPos.z is not the same as toPos.z \returns The result if there is no
   *obstacles
   */
  ...
  bool getPathMatching(const Position &startPos,
                       std::forward_list<Direction> &dirList,
                       const FrozenPathingConditionCall &pathCondition,
                       const FindPathParams &fpp) const;
...
private:
  ...
  void getSpectatorsInternal(SpectatorHashSet &spectators,
