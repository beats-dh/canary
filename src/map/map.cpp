...
Tile* Map::getTile(uint16_t x, uint16_t y, uint8_t z) const {
	...
	const QTreeLeafNode* leaf = QTreeNode::getLeafStatic<const QTreeLeafNode*, const QTreeNode*>(&root,
		x, y);
	if (!leaf) {
		return nullptr;
	}
...
void Map::getSpectatorsInternal(SpectatorHashSet& spectators,
	...
	const QTreeLeafNode* startLeaf = QTreeNode::getLeafStatic<const QTreeLeafNode*, const QTreeNode*>(
	...
	for (int_fast32_t ny = starty1; ny <= endy2; ny += FLOOR_SIZE) {
		...
		if (leafS) {
			...
			leafS = QTreeNode::getLeafStatic<const QTreeLeafNode*, const QTreeNode*>(
