...
bool Map::load(const std::string &identifier) {
  try {
    ...
    SPDLOG_ERROR("[Map::load] - The map in folder {} is missing or corrupted",
                 identifier);
    return false;
  }
  return true;
...
bool Map::loadMap(const std::string &identifier, bool mainMap /*= false*/,
                  bool loadHouses /*= false*/, bool loadMonsters /*= false*/,
                  bool loadNpcs /*= false*/) {
  // Only download map if is loading the main map and it is not already
  // downloaded
  if (mainMap && g_configManager().getBoolean(TOGGLE_DOWNLOAD_MAP) &&
      !boost::filesystem::exists(identifier)) {
    ...
    if (CURL *curl = curl_easy_init(); curl && !mapDownloadUrl.empty()) {
      SPDLOG_INFO("Downloading " + g_configManager().getString(MAP_NAME) +
                  ".otbm to world folder");
  ...
  // It needs to be after the load map to ensure the map already exists before
  // creating the items
  if (mainMap) {
  ...
  // Files need to be cleaned up if custom map is enabled to open, or will try
  // to load main map files
  if (g_configManager().getBoolean(TOGGLE_MAP_CUSTOM)) {
...
bool Map::loadMapCustom(const std::string &identifier, bool loadHouses,
                        bool loadMonsters, bool loadNpcs) {
  // Load the map
  this->load(identifier);
...
Tile *Map::getTile(uint16_t x, uint16_t y, uint8_t z) const {
  ...
  const QTreeLeafNode *leaf =
      QTreeNode::getLeafStatic<const QTreeLeafNode *, const QTreeNode *>(&root,
                                                                         x, y);
  if (!leaf) {
    return nullptr;
  }
...
bool Map::placeCreature(const Position &centerPos, Creature *creature,
  ...
  if (tile) {
    ...
    ReturnValue ret = tile->queryAdd(
        0, *creature, 1, FLAG_IGNOREBLOCKITEM | FLAG_IGNOREFIELDDAMAGE);
    foundTile = forceLogin || ret == RETURNVALUE_NOERROR ||
                ret == RETURNVALUE_PLAYERISNOTINVITED;
    if (monster) {
  ...
  if (!foundTile) {
    ...
    for (const auto &it : relList) {
      ...
      if (tile->queryAdd(0, *creature, 1,
                         FLAG_IGNOREBLOCKITEM | FLAG_IGNOREFIELDDAMAGE) ==
          RETURNVALUE_NOERROR) {
  ...
  Cylinder *toCylinder =
      tile->queryDestination(index, *creature, &toItem, flags);
...
void Map::moveCreature(Creature &creature, Tile &newTile,
    ...
    if (Player *tmpPlayer = spectator->getPlayer()) {
      if (tmpPlayer->canSeeCreature(&creature)) {
        oldStackPosVector.push_back(
            oldTile.getClientIndexOfCreature(tmpPlayer, &creature));
      } else {
  ...
  // event method
  for (Creature *spectator : spectators) {
    spectator->onCreatureMove(&creature, &newTile, newPos, &oldTile, oldPos,
                              teleport);
  }
...
void Map::getSpectatorsInternal(SpectatorHashSet &spectators,
                                const Position &centerPos, int32_t minRangeX,
                                int32_t maxRangeX, int32_t minRangeY,
                                int32_t maxRangeY, int32_t minRangeZ,
  ...
  uint16_t x1 =
      std::min<uint32_t>(0xFFFF, std::max<int32_t>(0, (min_x + minoffset)));
  uint16_t y1 =
      std::min<uint32_t>(0xFFFF, std::max<int32_t>(0, (min_y + minoffset)));

  int32_t maxoffset = centerPos.getZ() - minRangeZ;
  uint16_t x2 =
      std::min<uint32_t>(0xFFFF, std::max<int32_t>(0, (max_x + maxoffset)));
  uint16_t y2 =
      std::min<uint32_t>(0xFFFF, std::max<int32_t>(0, (max_y + maxoffset)));
  ...
  const QTreeLeafNode *startLeaf =
      QTreeNode::getLeafStatic<const QTreeLeafNode *, const QTreeNode *>(
          &root, startx1, starty1);
  ...
  for (int_fast32_t ny = starty1; ny <= endy2; ny += FLOOR_SIZE) {
    ...
    for (int_fast32_t nx = startx1; nx <= endx2; nx += FLOOR_SIZE) {
      if (leafE) {
        const CreatureVector &node_list =
            (onlyPlayers ? leafE->player_list : leafE->creature_list);
    ...
    if (leafS) {
      ...
      leafS =
          QTreeNode::getLeafStatic<const QTreeLeafNode *, const QTreeNode *>(
              &root, startx1, ny + FLOOR_SIZE);
    }
  }
}

void Map::getSpectators(SpectatorHashSet &spectators, const Position &centerPos,
                        bool multifloor /*= false*/,
                        bool onlyPlayers /*= false*/, int32_t minRangeX /*= 0*/,
                        int32_t maxRangeX /*= 0*/, int32_t minRangeY /*= 0*/,
                        int32_t maxRangeY /*= 0*/) {
  ...
  if (!foundCache) {
    ...
    getSpectatorsInternal(spectators, centerPos, minRangeX, maxRangeX,
                          minRangeY, maxRangeY, minRangeZ, maxRangeZ,
                          onlyPlayers);
...
bool Map::checkSightLine(const Position &fromPos, const Position &toPos) const {
  ...
  // now we need to perform a jump between floors to see if everything is clear
  // (literally)
...
const Tile *Map::canWalkTo(const Creature &creature,
  ...
  if (creature.getTile() != tile) {
    if (!tile || tile->queryAdd(0, creature, 1,
...
bool Map::getPathMatching(const Creature &creature,
  ...
  while (fpp.maxSearchDist != 0 || nodes.getClosedNodes() < 100) {
    ...
    for (uint_fast32_t i = 0; i < dirCount; ++i) {
      ...
      if (fpp.maxSearchDist != 0 &&
          (Position::getDistanceX(startPos, pos) > fpp.maxSearchDist ||
...
bool Map::getPathMatching(const Position &start,
                          std::forward_list<Direction> &dirList,
                          const FrozenPathingConditionCall &pathCondition,
                          const FindPathParams &fpp) const {
  Position pos = start;
...
AStarNode *AStarNodes::createOpenNode(AStarNode *parent, uint32_t x, uint32_t y,
                                      int_fast32_t f) {
...
int_fast32_t AStarNodes::getMapWalkCost(AStarNode *node,
                                        const Position &neighborPos,
                                        bool preferDiagonal) {
...
int_fast32_t AStarNodes::getTileWalkCost(const Creature &creature,
                                         const Tile *tile) {
  int_fast32_t cost = 0;
...
uint32_t Map::clean() const {
  ...
  SPDLOG_INFO("CLEAN: Removed {} item{} from {} tile{} in {} seconds", count,
              (count != 1 ? "s" : ""), tiles, (tiles != 1 ? "s" : ""),
