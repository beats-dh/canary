...
std::string getGlobalString(lua_State *L, const char *identifier,
...
float getGlobalFloat(lua_State *L, const char *identifier,
...
bool ConfigManager::load() {
  ...
  if (!loaded) {
    boolean[BIND_ONLY_GLOBAL_ADDRESS] =
        getGlobalBoolean(L, "bindOnlyGlobalAddress", false);
    boolean[OPTIMIZE_DATABASE] =
        getGlobalBoolean(L, "startupDatabaseOptimization", true);
    ...
    string[MAP_CUSTOM_AUTHOR] =
        getGlobalString(L, "mapCustomAuthor", "OTServBR");
    ...
    integer[MARKET_OFFER_DURATION] =
        getGlobalNumber(L, "marketOfferDuration", 30 * 24 * 60 * 60);

    integer[FREE_DEPOT_LIMIT] = getGlobalNumber(L, "freeDepotLimit", 2000);
    integer[PREMIUM_DEPOT_LIMIT] =
        getGlobalNumber(L, "premiumDepotLimit", 8000);
  ...
  boolean[ONE_PLAYER_ON_ACCOUNT] =
      getGlobalBoolean(L, "onePlayerOnlinePerAccount", true);
  boolean[AIMBOT_HOTKEY_ENABLED] =
      getGlobalBoolean(L, "hotkeyAimbotEnabled", true);
  boolean[REMOVE_RUNE_CHARGES] =
      getGlobalBoolean(L, "removeChargesFromRunes", true);
  boolean[EXPERIENCE_FROM_PLAYERS] =
      getGlobalBoolean(L, "experienceByKillingPlayers", false);
  boolean[FREE_PREMIUM] = getGlobalBoolean(L, "freePremium", false);
  boolean[REPLACE_KICK_ON_LOGIN] =
      getGlobalBoolean(L, "replaceKickOnLogin", true);
  boolean[ALLOW_CLONES] = getGlobalBoolean(L, "allowClones", false);
  boolean[MARKET_PREMIUM] =
      getGlobalBoolean(L, "premiumToCreateMarketOffer", true);
  ...
  boolean[CONVERT_UNSAFE_SCRIPTS] =
      getGlobalBoolean(L, "convertUnsafeScripts", true);
  boolean[CLASSIC_EQUIPMENT_SLOTS] =
      getGlobalBoolean(L, "classicEquipmentSlots", false);
  boolean[CLASSIC_ATTACK_SPEED] =
      getGlobalBoolean(L, "classicAttackSpeed", false);
  boolean[SCRIPTS_CONSOLE_LOGS] =
  ...
  boolean[REMOVE_WEAPON_AMMO] =
      getGlobalBoolean(L, "removeWeaponAmmunition", true);
  boolean[REMOVE_WEAPON_CHARGES] =
      getGlobalBoolean(L, "removeWeaponCharges", true);
  boolean[REMOVE_POTION_CHARGES] =
      getGlobalBoolean(L, "removeChargesFromPotions", true);
  boolean[GLOBAL_SERVER_SAVE_NOTIFY_MESSAGE] =
      getGlobalBoolean(L, "globalServerSaveNotifyMessage", true);
  boolean[GLOBAL_SERVER_SAVE_CLEAN_MAP] =
      getGlobalBoolean(L, "globalServerSaveCleanMap", false);
  boolean[GLOBAL_SERVER_SAVE_CLOSE] =
  ...
  boolean[HOUSE_OWNED_BY_ACCOUNT] =
      getGlobalBoolean(L, "houseOwnedByAccount", false);
  boolean[CLEAN_PROTECTION_ZONES] =
      getGlobalBoolean(L, "cleanProtectionZones", false);
  boolean[GLOBAL_SERVER_SAVE_SHUTDOWN] =
      getGlobalBoolean(L, "globalServerSaveShutdown", true);
  ...
  boolean[PUSH_WHEN_ATTACKING] =
      getGlobalBoolean(L, "pushWhenAttacking", false);
  ...
  boolean[TOGGLE_SAVE_INTERVAL] =
      getGlobalBoolean(L, "toggleSaveInterval", false);
  boolean[TOGGLE_SAVE_INTERVAL_CLEAN_MAP] =
      getGlobalBoolean(L, "toggleSaveIntervalCleanMap", false);
  boolean[TELEPORT_SUMMONS] = getGlobalBoolean(L, "teleportSummons", false);

  boolean[ONLY_PREMIUM_ACCOUNT] =
      getGlobalBoolean(L, "onlyPremiumAccount", false);
  boolean[RATE_USE_STAGES] = getGlobalBoolean(L, "rateUseStages", false);
  boolean[TOGGLE_IMBUEMENT_SHRINE_STORAGE] =
      getGlobalBoolean(L, "toggleImbuementShrineStorage", true);

  boolean[TOGGLE_DOWNLOAD_MAP] =
      getGlobalBoolean(L, "toggleDownloadMap", false);
  ...
  string[GLOBAL_SERVER_SAVE_TIME] =
      getGlobalString(L, "globalServerSaveTime", "06:00");
  ...
  integer[ACTIONS_DELAY_INTERVAL] =
      getGlobalNumber(L, "timeBetweenActions", 200);
  integer[EX_ACTIONS_DELAY_INTERVAL] =
      getGlobalNumber(L, "timeBetweenExActions", 1000);
  integer[MAX_MESSAGEBUFFER] = getGlobalNumber(L, "maxMessageBuffer", 4);
  integer[KICK_AFTER_MINUTES] =
      getGlobalNumber(L, "kickIdlePlayerAfterMinutes", 15);
  ...
  integer[FRAG_TIME] =
      getGlobalNumber(L, "timeToDecreaseFrags", 24 * 60 * 60 * 1000);
  integer[WHITE_SKULL_TIME] =
      getGlobalNumber(L, "whiteSkullTime", 15 * 60 * 1000);
  integer[STAIRHOP_DELAY] = getGlobalNumber(L, "stairJumpExhaustion", 2000);
  integer[MAX_CONTAINER] = getGlobalNumber(L, "maxContainer", 500);
  integer[MAX_ITEM] = getGlobalNumber(L, "maxItem", 10000);
  integer[EXP_FROM_PLAYERS_LEVEL_RANGE] =
      getGlobalNumber(L, "expFromPlayersLevelRange", 75);
  ...
  integer[MAX_PACKETS_PER_SECOND] =
      getGlobalNumber(L, "maxPacketsPerSecond", 25);
  ...
  integer[GLOBAL_SERVER_SAVE_NOTIFY_DURATION] =
      getGlobalNumber(L, "globalServerSaveNotifyDuration", 5);

  integer[PARTY_LIST_MAX_DISTANCE] =
      getGlobalNumber(L, "partyListMaxDistance", 0);
  ...
  boolean[INVENTORY_GLOW] =
      getGlobalBoolean(L, "inventoryGlowOnFiveBless", false);
  integer[ADVENTURERSBLESSING_LEVEL] =
      getGlobalNumber(L, "adventurersBlessingLevel", 21);

  floating[RATE_HEALTH_REGEN] = getGlobalFloat(L, "rateHealthRegen", 1.0);
  floating[RATE_HEALTH_REGEN_SPEED] =
      getGlobalFloat(L, "rateHealthRegenSpeed", 1.0);
  floating[RATE_MANA_REGEN] = getGlobalFloat(L, "rateManaRegen", 1.0);
  floating[RATE_MANA_REGEN_SPEED] =
      getGlobalFloat(L, "rateManaRegenSpeed", 1.0);
  floating[RATE_SOUL_REGEN] = getGlobalFloat(L, "rateSoulRegen", 1.0);
  floating[RATE_SOUL_REGEN_SPEED] =
      getGlobalFloat(L, "rateSoulRegenSpeed", 1.0);
  ...
  floating[RATE_OFFLINE_TRAINING_SPEED] =
      getGlobalFloat(L, "rateOfflineTrainingSpeed", 1.0);
  floating[RATE_EXERCISE_TRAINING_SPEED] =
      getGlobalFloat(L, "rateExerciseTrainingSpeed", 1.0);
  ...
  boolean[PREY_FREE_THIRD_SLOT] =
      getGlobalBoolean(L, "preyFreeThirdSlot", false);
  integer[PREY_REROLL_PRICE_LEVEL] =
      getGlobalNumber(L, "preyRerollPricePerLevel", 200);
  integer[PREY_SELECTION_LIST_PRICE] =
      getGlobalNumber(L, "preySelectListPrice", 1);
  integer[PREY_BONUS_PERCENT_MIN] =
      getGlobalNumber(L, "preyBonusPercentMin", 5);
  integer[PREY_BONUS_PERCENT_MAX] =
      getGlobalNumber(L, "preyBonusPercentMax", 40);
  integer[PREY_BONUS_TIME] = getGlobalNumber(L, "preyBonusTime", 7200);
  integer[PREY_BONUS_REROLL_PRICE] =
      getGlobalNumber(L, "preyBonusRerollPrice", 2);
  integer[PREY_FREE_REROLL_TIME] =
      getGlobalNumber(L, "preyFreeRerollTime", 72000);

  boolean[TASK_HUNTING_ENABLED] =
      getGlobalBoolean(L, "taskHuntingSystemEnabled", true);
  boolean[TASK_HUNTING_FREE_THIRD_SLOT] =
      getGlobalBoolean(L, "taskHuntingFreeThirdSlot", false);
  integer[TASK_HUNTING_LIMIT_EXHAUST] =
      getGlobalNumber(L, "taskHuntingLimitedTasksExhaust", 72000);
  integer[TASK_HUNTING_REROLL_PRICE_LEVEL] =
      getGlobalNumber(L, "taskHuntingRerollPricePerLevel", 200);
  integer[TASK_HUNTING_SELECTION_LIST_PRICE] =
      getGlobalNumber(L, "taskHuntingSelectListPrice", 1);
  integer[TASK_HUNTING_BONUS_REROLL_PRICE] =
      getGlobalNumber(L, "taskHuntingBonusRerollPrice", 2);
  integer[TASK_HUNTING_FREE_REROLL_TIME] =
      getGlobalNumber(L, "taskHuntingFreeRerollTime", 72000);
...
const std::string &ConfigManager::getString(stringConfig_t what) const {
  if (what >= LAST_STRING_CONFIG) {
    SPDLOG_WARN("[ConfigManager::getString] - Accessing invalid index: {}",
                what);
    return dummyStr;
  }
  return string[what];
...
int32_t ConfigManager::getNumber(integerConfig_t what) const {
  if (what >= LAST_INTEGER_CONFIG) {
    SPDLOG_WARN("[ConfigManager::getNumber] - Accessing invalid index: {}",
                what);
    return 0;
  }
  return integer[what];
...
int16_t ConfigManager::getShortNumber(integerConfig_t what) const {
  if (what >= LAST_INTEGER_CONFIG) {
    SPDLOG_WARN("[ConfigManager::getShortNumber] - Accessing invalid index: {}",
                what);
    return 0;
  }
  return integer[what];
...
bool ConfigManager::getBoolean(booleanConfig_t what) const {
  if (what >= LAST_BOOLEAN_CONFIG) {
    SPDLOG_WARN("[ConfigManager::getBoolean] - Accessing invalid index: {}",
                what);
    return false;
  }
  return boolean[what];
...
float ConfigManager::getFloat(floatingConfig_t what) const {
  if (what >= LAST_FLOATING_CONFIG) {
    SPDLOG_WARN("[ConfigManager::getFLoat] - Accessing invalid index: {}",
                what);
    return 0;
  }
