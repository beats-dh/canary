...
/**
 *It is preferable to keep the close button off as it closes the server without
 *saving (this can cause the player to lose items from houses and others
 *informations, since windows automatically closes the process in five seconds,
 *when forcing the close) Choose to use "CTROL + C" or "CTROL + BREAK" for
 *security close To activate/desactivate window; \param MF_GRAYED Disable the
 *"x" (force close) button \param MF_ENABLED Enable the "x" (force close) button
 */
...
void loadModules() {
  modulesLoadHelper(g_configManager().load(), "config.lua");

  SPDLOG_INFO("Server protocol: {}.{}", CLIENT_VERSION_UPPER,
              CLIENT_VERSION_LOWER);

  const char *p("14299623962416399520070177382898895550795403345466153217470516"
                "08293473758277603888296721338620460067414539284585385921799062"
                "6450972452084065728686565928113");
  const char *q("76309791959704047218912018477920021255354012927791239372074475"
  ...
  try {
    if (!g_RSA().loadPEM("key.pem")) {
      ...
      SPDLOG_ERROR("File key.pem not found or have problem on loading... "
  ...
  modulesLoadHelper((g_game().loadAppearanceProtobuf(
                         "data/items/appearances.dat") == ERROR_NONE),
                    "appearances.dat");
  ...
  modulesLoadHelper(
      (g_luaEnvironment.loadFile("data/startup/startup.lua") == 0),
  ...
  modulesLoadHelper(g_scripts().loadScripts("scripts/lib", true, false),
                    "data/scripts/libs");
  modulesLoadHelper(g_vocations().loadFromXml(), "data/XML/vocations.xml");
  ...
  modulesLoadHelper(g_imbuements().loadFromXml(), "data/XML/imbuements.xml");
  modulesLoadHelper(g_modules().loadFromXml(), "data/modules/modules.xml");
  modulesLoadHelper(g_events().loadFromXml(), "data/events/events.xml");
  modulesLoadHelper(g_scripts().loadScripts("scripts", false, false),
                    "data/scripts");
  modulesLoadHelper(g_scripts().loadScripts("monster", false, false),
                    "data/monster");
  modulesLoadHelper(g_scripts().loadScripts("npc", false, false), "data/npc");
...
int main(int argc, char *argv[]) {
  ...
  spdlog::set_pattern(
      "[%Y-%d-%m %H:%M:%S.%e] [file %@] [func %!] [thread %t] [%^%l%$] %v ");
#else
  ...
  g_dispatcher().addTask(
      createTask(std::bind(mainLoader, argc, argv, &serviceManager)));
...
void mainLoader(int, char *[], ServiceManager *services) {
  ...
  SPDLOG_INFO("{} - Version [{}] dated [{}]", STATUS_SERVER_NAME,
              STATUS_SERVER_VERSION, GIT_COMMIT_DATE_ISO8601);
#if GIT_IS_DIRTY
...
#ifdef _WIN32
  const std::string &defaultPriority =
      g_configManager().getString(DEFAULT_PRIORITY);
  ...
  std::string worldType =
      asLowerCaseString(g_configManager().getString(WORLD_TYPE));
  if (worldType == "pvp") {
  ...
  services->add<ProtocolGame>(
      static_cast<uint16_t>(g_configManager().getNumber(GAME_PORT)));
  services->add<ProtocolLogin>(
      static_cast<uint16_t>(g_configManager().getNumber(LOGIN_PORT)));
  // OT protocols
  services->add<ProtocolStatus>(
      static_cast<uint16_t>(g_configManager().getNumber(STATUS_PORT)));

  RentPeriod_t rentPeriod;
  std::string strRentPeriod =
      asLowerCaseString(g_configManager().getString(HOUSE_RENT_PERIOD));
  ...
  webhook_send_message("Server is now online",
                       "Server has successfully started.", WEBHOOK_COLOR_ONLINE,
                       url);
