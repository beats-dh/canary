...
WildcardTreeNode *WildcardTreeNode::addChild(char ch, bool breakp) {
  ...
  if (child) {
    ...
    auto pair =
        children.emplace(std::piecewise_construct, std::forward_as_tuple(ch),
                         std::forward_as_tuple(breakp));
...
ReturnValue WildcardTreeNode::findOne(const std::string &query,
