...
void printXMLError(const std::string &where, const std::string &fileName,
  ...
  SPDLOG_ERROR("[{}] Failed to load {}: {}", where, fileName,
               result.description());
...
static void processSHA1MessageBlock(const uint8_t *messageBlock, uint32_t *H) {
  ...
  for (int i = 0; i < 20; ++i) {
    const uint32_t tmp =
        circularShift(5, A) + ((B & C) | ((~B) & D)) + E + W[i] + 0x5A827999;
    E = D;
    D = C;
  ...
  for (int i = 40; i < 60; ++i) {
    const uint32_t tmp = circularShift(5, A) + ((B & C) | (B & D) | (C & D)) +
                         E + W[i] + 0x8F1BBCDC;
    E = D;
    D = C;
    C = circularShift(30, B);
...
std::string generateToken(const std::string &key, uint32_t ticks) {
  ...
  for (uint8_t i = 0; i < message.length(); i += 2) {
    oKeyPad.push_back(
        static_cast<char>(std::stol(message.substr(i, 2), nullptr, 16)));
  }
  ...
  uint32_t offset = static_cast<uint32_t>(
      std::stol(message.substr(39, 1), nullptr, 16) & 0xF);

  // get truncated hash
  uint32_t truncHash =
      std::stol(message.substr(2 * offset, 8), nullptr, 16) & 0x7FFFFFFF;
  message.assign(std::to_string(truncHash));

  // return only last AUTHENTICATOR_DIGITS (default 6) digits, also asserts
  // exactly 6 digits
  uint32_t hashLen = message.length();
  message.assign(
      message.substr(hashLen - std::min(hashLen, AUTHENTICATOR_DIGITS)));
  message.insert(
      0, AUTHENTICATOR_DIGITS - std::min(hashLen, AUTHENTICATOR_DIGITS), '0');
  return message;
}

void replaceString(std::string &str, const std::string &sought,
...
StringVector explodeString(const std::string &inString,
  ...
  while (--limit != -1 &&
         (end = inString.find(separator, start)) != std::string::npos) {
...
bool boolean_random(double probability /* = 0.5*/) {
  ...
  return booleanRand(getRandomGenerator(),
                     std::bernoulli_distribution::param_type(probability));
}
...
std::string convertIPToString(uint32_t ip) {
  ...
  int res = sprintf(buffer, "%u.%u.%u.%u", ip & 0xFF, (ip >> 8) & 0xFF,
                    (ip >> 16) & 0xFF, (ip >> 24));
  if (res < 0) {
    return {};
  }
...
std::string formatDate(time_t time) {
  ...
  char buffer[20];
  int res = sprintf(buffer, "%02d/%02d/%04d %02d:%02d:%02d", tms->tm_mday,
...
Direction getDirection(const std::string &string) {
  ...
  } else if (string == "southwest" || string == "south west" ||
             string == "south-west" || string == "sw" || string == "4") {
    direction = DIRECTION_SOUTHWEST;
  } else if (string == "southeast" || string == "south east" ||
             string == "south-east" || string == "se" || string == "5") {
    direction = DIRECTION_SOUTHEAST;
  } else if (string == "northwest" || string == "north west" ||
             string == "north-west" || string == "nw" || string == "6") {
    direction = DIRECTION_NORTHWEST;
  } else if (string == "northeast" || string == "north east" ||
             string == "north-east" || string == "ne" || string == "7") {
...
using CombatTypeNames =
    phmap::flat_hash_map<CombatType_t, std::string, std::hash<int32_t>>;
...
CombatTypeNames combatTypeNames = {
    {COMBAT_DROWNDAMAGE, "drown"},         {COMBAT_DEATHDAMAGE, "death"},
    {COMBAT_ENERGYDAMAGE, "energy"},       {COMBAT_EARTHDAMAGE, "earth"},
    {COMBAT_FIREDAMAGE, "fire"},           {COMBAT_HEALING, "healing"},
    {COMBAT_HOLYDAMAGE, "holy"},           {COMBAT_ICEDAMAGE, "ice"},
    {COMBAT_UNDEFINEDDAMAGE, "undefined"}, {COMBAT_LIFEDRAIN, "lifedrain"},
    {COMBAT_MANADRAIN, "manadrain"},       {COMBAT_PHYSICALDAMAGE, "physical"},
};
...
SkullNames skullNames = {
    {"black", SKULL_BLACK},   {"green", SKULL_GREEN}, {"none", SKULL_NONE},
    {"orange", SKULL_ORANGE}, {"red", SKULL_RED},     {"yellow", SKULL_YELLOW},
    {"white", SKULL_WHITE},
};
...
CombatType_t getCombatType(const std::string &combatname) {
  auto it = std::find_if(
      combatTypeNames.begin(), combatTypeNames.end(),
      [combatname](std::pair<CombatType_t, std::string> const &pair) {
...
const char *getReturnMessage(ReturnValue value) {
  ...
  case RETURNVALUE_REWARDCHESTISEMPTY:
    return "The chest is currently empty. You did not take part in any battles "
           "in the last seven days or already claimed your reward.";
  ...
  case RETURNVALUE_PLAYERISPZLOCKED:
    return "You can not enter a protection zone after attacking another "
           "player.";
  ...
  case RETURNVALUE_TURNSECUREMODETOATTACKUNMARKEDPLAYERS:
    return "Turn secure mode off if you really want to attack unmarked "
           "players.";
  ...
  case RETURNVALUE_TRADEPLAYERHIGHESTBIDDER:
    return "Trade player is currently the highest bidder of an auctioned "
           "house.";
...
/**
 * @details
 * Prevents the console from closing so there is time to read the error
 * information Then can press any key to close
 */
...
NameEval_t validateName(const std::string &name) {
  ...
  StringVector prohibitedWords = {
      ...
      "account", "god",        "anal",   "ass",   "fuck",  "sex",
      "hitler",  "pussy",      "dick",   "rape",  "cm",    "gm",
      "tutor",   "counsellor", "god"};
  StringVector toks;
  ...
  for (std::string str : toks) {
    ...
    else if (std::find(prohibitedWords.begin(), prohibitedWords.end(), str) !=
             prohibitedWords.end()) { // searching for prohibited words
